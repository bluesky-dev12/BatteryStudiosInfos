        ...
        if((bActioned == false) && AmmoCharge_CurLoaded > 0)
        {
            ...
            if(FireMode[0].NextFireTime <= Level.TimeSeconds)
            {
                ...
                if(HasAnim(RechamberAnim))
                {
                    ...
                    Instigator.PlayRechamber();
                    Log((string(self) $ " CheckActioned() HasAnim(RechamberAnim) / currentAnim :") $ string(Anim));
                    StateRechamber = 2;
                    return false;
                }
            }
...
function ServerReload()
{
    ...
    Level.Game.TeamReceiveLocalizedMessageEx(byte(iTeamNum), Class'Engine_Decompressed.wMessage_Game', iCodeID, Pawn(Owner).PlayerReplicationInfo,,, vLocation, fMessageRange, true);
    //return;    
}
...
simulated function ApplyOffset_Rot()
{
    ...
    local Rotator rLerpRot, rLerpRot_Aim;
...
simulated function float CalcDestPivot()
{
    ...
    if(int(AimState) != int(0))
    {
        GetAnimParams(0, szAnim, fFrame, fRate);
        // End:0x59
        if((szAnim == 'Ads_Down') || VSize(Instigator.Velocity) > float(0))
        {
