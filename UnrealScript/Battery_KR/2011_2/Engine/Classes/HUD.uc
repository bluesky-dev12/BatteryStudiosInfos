...
function DrawCustomBeacon(Canvas C, Pawn P, float ScreenLocX, float ScreenLocY)
{
    ...
    {
        C.DrawColor = Class'Engine_Decompressed.PlayerController'.default.TeamBeaconTeamColors[P.PlayerReplicationInfo.Team.TeamIndex];        
    }
    else
    {
        C.DrawColor = Class'Engine_Decompressed.PlayerController'.default.TeamBeaconTeamColors[0];
    }
...
simulated event PostBeginPlay()
{
    ...
    CreateKeyMenus();
    // End:0x26
    foreach AllActors(Class'Engine_Decompressed.SceneSubtitles', SubTitles)
    {
        // End:0x26
        break;        
    }    
...
{
    PlayerOwner.ReceiveLocalizedMessage(Class'Engine_Decompressed.FailedConnect', Class'Engine_Decompressed.FailedConnect'.static.GetFailSwitch(FailCode));
    //return;    
}
...
function DrawDebug_Helicopter(Canvas C, int X, out int Y, int dy)
{
    ...
    Y += dy;
    // End:0x5BC
    foreach DynamicActors(Class'Engine_Decompressed.AIController', H)
    {
        // End:0x5BB
...
function DrawDebug_AllPlayerStates(Canvas C, int X, out int Y, int dy)
{
    ...
    // End:0x131
    foreach DynamicActors(Class'Engine_Decompressed.Controller', PC)
    {
...
function DrawDebug3(Canvas C, int X, out int Y, int dy)
{
    ...
    Y += dy;
    pco = Level.GetLocalPlayerController();
    // End:0x2E6
    if(pco != none)
    {
        C.BtrDrawTextJustifiedWithVolumeLineNoCache((pco.PlayerReplicationInfo.PlayerName $ ", state=") $ string(pco.GetStateName()), 0, float(X), float(Y), 640.0000000, float(Y + dy), 9);
        Y += dy;
        // End:0x1B1
        if(pco.PlayerReplicationInfo.PlayerName == kMM.kUserName)
        {
            C.BtrDrawTextJustifiedWithVolumeLineNoCache("CurrentNetSpeed = " $ string(pco.Player.CurrentNetSpeed), 0, float(X), float(Y), 640.0000000, float(Y + dy), 9);
            Y += dy;
        }
        C.BtrDrawTextJustifiedWithVolumeLineNoCache(((((((((((((((("Acc(KBtyes) = " $ string(pco.InKBytesAcc)) $ "/") $ string(pco.OutKBytesAcc)) $ ", Rate(Bytes) = ") $ string(pco.InRate)) $ "/") $ string(pco.OutRate)) $ ", Loss(%) = ") $ string(pco.InLoss)) $ "/") $ string(pco.OutLoss)) $ "") $ ", Best/Avg Lag(ms) = ") $ string(pco.BestLag)) $ "/") $ string(pco.AvgLag), 0, float(X), float(Y), 640.0000000, float(Y + dy), 9);
        Y += dy;
    }
    co = Level.ControllerList;
    J0x2FA:
    ...
    if(co != none)
    {
        pco = PlayerController(co);
        // End:0x57C
        if((pco != none) && NetConnection(pco.Player) != none)
        {
            C.BtrDrawTextJustifiedWithVolumeLineNoCache((pco.PlayerReplicationInfo.PlayerName $ ", state=") $ string(pco.GetStateName()), 0, float(X), float(Y), 640.0000000, float(Y + dy), 9);
            Y += dy;
            // End:0x447
...
simulated function DrawRoute()
{
    ...
    if(C.bAdjusting)
    {
        Draw3DLine(C.Pawn.Location, C.AdjustLoc, Class'Engine_Decompressed.Canvas'.static.MakeColor(byte(255), 0, byte(255)));
        Start = C.AdjustLoc;
    }
    // End:0x2DC
    if((C == PlayerOwner) || (C.MoveTarget == C.RouteCache[0]) && C.MoveTarget != none)
    {
        ...
        if((C == PlayerOwner) && C.Destination != vect(0.0000000, 0.0000000, 0.0000000))
        {
            ...
            {
                Draw3DLine(C.Pawn.Location, C.Destination, Class'Engine_Decompressed.Canvas'.static.MakeColor(byte(255), byte(255), byte(255)));
                return;
            }
        ...
        if(i < 16)
        {
            ...
            bPath = true;
            Draw3DLine(Start, C.RouteCache[i].Location, Class'Engine_Decompressed.Canvas'.static.MakeColor(0, byte(255), 0));
        ...
        if(bPath)
        {
            Draw3DLine(RealStart, C.Destination, Class'Engine_Decompressed.Canvas'.static.MakeColor(byte(255), byte(255), byte(255)));
        }        
    }
    else
    {
    ...
    Draw3DLine(PlayerOwner.ViewTarget.Location + (Pawn(PlayerOwner.ViewTarget).BaseEyeHeight * vect(0.0000000, 0.0000000, 1.0000000)), End, Class'Engine_Decompressed.Canvas'.static.MakeColor(byte(255), 0, 0));
    //return;    
}
...
function CallSkill(optional int iKey, optional bool bRequire, optional bool bSucess)
{
    //return;    
}
...
simulated function Message(PlayerReplicationInfo PRI, coerce string Msg, name MsgType)
{
    ...
    AddTextMessage(Msg, Class'Engine_Decompressed.LocalMessage', PRI);
    //return;    
}
...
simulated function Font LoadProgressFont()
{
    ...
    if(ProgressFontFont == none)
    {
        ...
        if(ProgressFontFont == none)
        {
            ...
            ProgressFontFont = Font'Engine_Decompressed.DefaultFont';
        }
    }
...
function OnClientReceiveDeathMessage(PlayerController P, PlayerReplicationInfo RelatedPRI_1, PlayerReplicationInfo RelatedPRI_2, optional Object OptionalObject)
{
    //return;    
}
