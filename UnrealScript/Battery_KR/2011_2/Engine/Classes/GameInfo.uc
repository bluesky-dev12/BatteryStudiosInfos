...
{
    Level.WeaponMgr.PrecacheMaterials(Level);
...
function ProcessKillMessage(Controller Killer, Controller Killed, optional int CollisionPart, optional int WeaponType, optional out int iConKillorHeadShot)
{
    local Controller C;
    local PlayerController PC;
    local float fDist;
    ...
    byKillSuccession = ++Killer.PlayerReplicationInfo.KillSuccession;
    C = Level.ControllerList;
    J0x74:
    ...
    if(C != none)
    {
        ...
        if((((PC != none) && !bGameEnded) && !IsInState('MatchOver')) && kMM.InGamePlaying)
        {
            ...
            if((Killer.PlayerReplicationInfo == C.PlayerReplicationInfo) || Killer.Pawn == Pawn(PC.ViewTarget))
            {
                ...
                if(-1 != iArray)
                {
                    PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.wMultiKillMessage', iArray);
                    iConKillorHeadShot = 1;                    
                }
                else
                {
                    // End:0x4F6
                    ...
                    {
                        PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.wSpecialKillMessage');
                        iConKillorHeadShot = 1;                        
                    }
                    else
                    {
                    ...
                    // End:0x5F6
                    if((((((Killer != none) && Killer.PlayerReplicationInfo != none) && PC.ViewTarget != none) && Killer.PlayerReplicationInfo != PC.PlayerReplicationInfo) && Killer.Pawn == Pawn(PC.ViewTarget)) && iConKillorHeadShot != 1)
                    {
...
function int SendMessageKillSuccssion(Controller Killer, PlayerController PC, byte byKillSuccession)
{
    ...
    switch(byKillSuccession)
    {
        // End:0x51
        case 3:
            PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.wMessage_Game', Class'Engine_Decompressed.wMessage_Game'.default.Code_3Kill, Killer.PlayerReplicationInfo);
            iArray = 0;
            // End:0x156
            break;
        // End:0x90
        case 5:
            PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.wMessage_Game', Class'Engine_Decompressed.wMessage_Game'.default.Code_5Kill, Killer.PlayerReplicationInfo);
            iArray = 1;
            // End:0x156
            break;
        // End:0xD0
        case 7:
            PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.wMessage_Game', Class'Engine_Decompressed.wMessage_Game'.default.Code_7Kill, Killer.PlayerReplicationInfo);
            iArray = 2;
            // End:0x156
            break;
        // End:0x110
        case 10:
            PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.wMessage_Game', Class'Engine_Decompressed.wMessage_Game'.default.Code_10Kill, Killer.PlayerReplicationInfo);
            iArray = 3;
            // End:0x156
            break;
        // End:0x150
        case 15:
            PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.wMessage_Game', Class'Engine_Decompressed.wMessage_Game'.default.Code_15Kill, Killer.PlayerReplicationInfo);
            iArray = 4;
            // End:0x156
            break;
        // End:0xFFFF
...
function KDLOG_Finalize()
{
    ...
    if(((SC.KDLogSaved && SC.KDLog.CharKillUID != -1) && SC.KDLog.CharDeathUID != -1) && Level.GetMatchMaker().bHasChannelTCP)
    {
        ...
        BTSend.sfReqLogGetScore(SC.KDLog, SC.AssistUIDs);
        i = 0;
        J0xDE:
        ...
        if(i < SC.AssistUIDs.Length)
        {
            ...
            Log((((("[GameInfo::KDLOG_Finalize::AssistList]: count:" $ string(i + 1)) $ "/AssistUIDs:") $ string(iPlayerID)) $ "/PlayerName:") $ UserInfo.UserName);
            i++;
            // [Loop Continue]
            goto J0xDE;
        }
        Log((((((((((((((((((((((((((((((((((((((("[GameInfo::KDLOG_Finalize] CharKillUID=" $ string(SC.KDLog.CharKillUID)) $ " KillXCoord=") $ string(SC.KDLog.KillXCoord)) $ " KillYCoord=") $ string(SC.KDLog.KillYCoord)) $ " KillZCoord=") $ string(SC.KDLog.KillZCoord)) $ " KillPos=") $ string(SC.KDLog.KillPos)) $ " KillWeapon=") $ string(SC.KDLog.KillWeapon)) $ " KillWeaponPart=") $ string(SC.KDLog.KillWeaponPart)) $ " KillRange=") $ string(SC.KDLog.KillRange)) $ " KillDamage=") $ string(SC.KDLog.KillDamage)) $ " KillBodyPart=") $ string(SC.KDLog.KillBodyPart)) $ " IsUAV=") $ string(SC.KDLog.IsUAV)) $ " CharDeathUID=") $ string(SC.KDLog.CharDeathUID)) $ " DeathXCoord=") $ string(SC.KDLog.DeathXCoord)) $ " DeathYCoord=") $ string(SC.KDLog.DeathYCoord)) $ " DeathZCoord=") $ string(SC.KDLog.DeathZCoord)) $ " DeathPos=") $ string(SC.KDLog.DeathPos)) $ " DeathWeapon=") $ string(SC.KDLog.DeathWeapon)) $ " DeathWeaponPart=") $ string(SC.KDLog.DeathWeaponPart)) $ " IsDropItem=") $ string(SC.KDLog.IsDropItem)) $ " DropItemID=") $ string(SC.KDLog.DropItemID));
    }
    SC.KDLogSaved = false;
    //return;    
}
...
function Killed(Controller Killer, Controller Killed, Pawn KilledPawn, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType)
{
    ...
    if(((((Killer != none) && Killer.PlayerReplicationInfo != none) && Killed != none) && Killed.PlayerReplicationInfo != none) && !DamageType.default.SkipDeathIncrement)
    {
        ...
        // End:0x310
        if(((DamageType == Class'Engine_Decompressed.wDamageAirStrike') || strDamageType ~= "WWeapons_Res.wWeaponRes_Artillery") || strDamageType ~= "WWeapons_Res.wWeaponRes_HellFire")
        {
            // End:0x2F4
            if(strDamageType ~= "WWeapons_Res.wWeaponRes_HellFire")
            {
        ...
        {
            // End:0x364
            if((DamageType == Class'Engine_Decompressed.wDamageFlameThrower') || strDamageType ~= "WWeapons_Res.wWeaponRes_M2B")
            {
    ...
    {
        ProcessKillMessage(Killer, Killed, CollisionPart, WeaponType, iConKillorHeadShot);
    }
    // End:0x1139
...
function Class<DamageType> GetWDamageType(Class<DamageType> DamageType, int CollisionPart, int WeaponType)
{
    local Actor.CollisionPartsType MyCollisionPart;
    local Actor.eWeaponType MyWeaponType;
    ...
    if((int(MyCollisionPart) == int(1)) && int(MyWeaponType) != int(13))
    {
        return Class'Engine_Decompressed.wDamageHeadshot';        
    }
    else
    {
        switch(MyWeaponType)
...
function BroadcastDeathMessage(Controller Killer, Controller Other, Class<DamageType> DamageType, int CollisionPart, int WeaponType)
{
    ...
    if(Killer != none)
    {
        ...
        {
            BroadcastLocalized(self, DeathMessageClass, 0, Killer.PlayerReplicationInfo, Other.PlayerReplicationInfo, GetWDamageType(DamageType, CollisionPart, WeaponType));            
        }
        else
        {
            BroadcastLocalized(self, DeathMessageClass, 0, Killer.GetPRI(), Other.PlayerReplicationInfo, GetWDamageType(DamageType, CollisionPart, WeaponType));
        }
    }
    //return;    
}
...
function ScoreKill(Controller Killer, Controller Other)
{
    ...
    if((Killer == Other) || Killer == none)
    {
        ...
        if((Other != none) && Other.PlayerReplicationInfo != none)
        {
            ...
            Other.PlayerReplicationInfo.Exp = FMax(0.0000000, Other.PlayerReplicationInfo.Exp - GameDecExp);
    ...
    else
    {
        ...
        if(Killer.PlayerReplicationInfo != none)
        {
            ...
            Killer.PlayerReplicationInfo.Exp += (GameAddExp + float(AddExp));
            ...
            if(Other.IsA('wHelicopterController'))
            {
                Killer.PlayerReplicationInfo.Exp += GameAddHeliExp;
...
function SendSeverScoreAndAssist(Controller Killer, Controller Other)
{
    ...
    if(iTemp < AssistList.Length)
    {
        ...
        if((PC != none) && PC.PlayerReplicationInfo != none)
        {
            ...
            if(iPlayerID != -1)
            {
                SC.AssistUIDs[Level.Game.SC.AssistUIDs.Length] = iPlayerID;
            }
        }
        iTemp++;
        // [Loop Continue]
...
function SetAssistPoint(Controller Killer, Controller Other)
{
    ...
    if(iTemp < AssistList.Length)
    {
        ...
        AssistList[iTemp].PlayerReplicationInfo.Exp += GameAssistExp;
    ...
    else
    {
        ...
        if(Killer.IsA('PlayerController') && addScore != 0)
        {
            switch(KillWeapon)
            {
                ...
                case 50000:
                    // End:0xA4
                    if(iConKillorHeadShot != 1)
                    {
                        iCode_Message = Class'Engine_Decompressed.wMessage_Score'.default.Code_MessageScore_Artillery;
                        PlayerController(ReceiveController).ReceiveLocalizedMessage(Class'Engine_Decompressed.wMessage_Score', iCode_Message);
                    }
                    // End:0x177
                    break;
                // End:0xAF
                ...
                case 50002:
                    ...
                    if(iConKillorHeadShot != 1)
                    {
                        iCode_Message = Class'Engine_Decompressed.wMessage_Score'.default.Code_MessageScore_Heli_Killer;
                        PlayerController(ReceiveController).ReceiveLocalizedMessage(Class'Engine_Decompressed.wMessage_Score', iCode_Message);
                    }
                    // End:0x177
                    break;
...
{
    return ((!C.PlayerReplicationInfo.bOutOfLives && !C.PlayerReplicationInfo.bOnlySpectator) && !C.IsInState('Spectating')) && !C.IsInState('PlayerWaiting') || bHasRespawnRestriction && !IsRespawnRestrictionTime();
    //return;    
}
...
function SendRawMatchResult()
{
    ...
    foreach kMM.playingLevelInfo.DynamicActors(Class'Engine_Decompressed.PlayerReplicationInfo', PRI)
    {
        ...
        if(!PRI.IsConnected)
        {
            Log((((((((((((("[SendRawMatchResult][!IsConnected] Name=" $ PRI.PlayerName) $ " UID=") $ string(UID)) $ " Kills=") $ string(PRI.Kills)) $ " Deaths=") $ string(PRI.Deaths)) $ " EXP=") $ string(PRI.Exp)) $ " Point=") $ string(PRI.Point)) $ " Score=") $ string(PRI.Score));
            continue;            
        }
        // End:0x1B7
        ...
        if(true)
        {
            ...
            {
                EXPs[EXPs.Length] = int(float(Max(0, int(PRI.Exp))) * UserInfo.GetBonusExP(Level.GameMgr));
            ...
            Log((((((((((((("[GameInfo::SendRawMatchResult] Name=" $ PRI.PlayerName) $ " UID=") $ string(UID)) $ " Kills=") $ string(PRI.Kills)) $ " Deaths=") $ string(PRI.Deaths)) $ " EXP=") $ string(PRI.Exp)) $ " Point=") $ string(PRI.Point)) $ " Score=") $ string(PRI.Score));
...
defaultproperties
{
    ...
    GIPropsDisplayText[0]="???? ??"
    GIPropsDisplayText[1]="?? ??? ??"
    GIPropsDisplayText[2]="??? ??"
    GIPropsDisplayText[3]="?? ??"
    GIPropsDisplayText[4]="?? ??? ?"
    GIPropsDisplayText[5]="?? ???? ?"
    GIPropsDisplayText[6]="?? ???"
    GIPropsDisplayText[7]="?? ?? ?"
    GIPropsDisplayText[8]="?? ??"
    GIPropsDisplayText[9]="??? ?? ???"
    GIPropsDisplayText[10]="?? ??? ??"
    GIPropsDisplayText[11]="3?? ??"
    GIPropsDisplayText[12]="?? ???? ?? ?? ???? ?? ??"
    GIPropsDisplayText[13]="???? ??"
    GIPropsDisplayText[14]="?? ??? ??"
    GIPropDescText[0]="??? ???? ???? ??? ?????."
    GIPropDescText[1]="??? ??? ??? ? ??? ??? ?? ?? ????."
    GIPropDescText[2]="?? ??? ?? ?? ??? ??? ?????."
    GIPropDescText[3]="?? ? ????? ???? ?????."
    ...
    GIPropDescText[7]="????? ?? ?? ???? ? ?? ??? ?????."
    GIPropDescText[8]="? ???? ?? ??? ?????."
    GIPropDescText[9]="?? ??? UT2004 ??? ?? ??? ????."
    GIPropDescText[10]="????? ??? ??? ?? ? ????."
    GIPropDescText[11]="????? 3?? ???? ?? ? ? ????."
    GIPropDescText[12]="?? ???? ??? ????? ?? ? ????."
    GIPropDescText[13]="??? ???? ????? ???? ??? ??? ?????."
    GIPropDescText[14]="?? ??? ???? ??? ?????."
    GIPropsExtras="0.000000;??;1.000000;??;2.000000;???;3.000000;???;4.000000;???;5.000000;???;6.000000;????;7.000000;??"
    CallSigns[0]="ALPHA"
    CallSigns[1]="BRAVO"
    CallSigns[2]="CHARLIE"
    CallSigns[3]="DELTA"
