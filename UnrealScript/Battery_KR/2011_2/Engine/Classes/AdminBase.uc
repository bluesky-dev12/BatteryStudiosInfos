...
exec function PlayerList()
{
    ...
    if(CanPerform("Xp"))
    {
        Log(Msg_PlayerList);
        // End:0x5F
        foreach Outer.DynamicActors(Class'Engine_Decompressed.PlayerReplicationInfo', PRI)
        {
...
protected function string FindGameType(string GameType)
{
    ...
    Class'Engine_Decompressed.CacheManager'.static.GetGameTypeList(Records);
    i = 0;
    J0x18:
...
defaultproperties
{
    ...
    Msg_NextMapNotFound="???? ?? ? ????. ?? ??? ?? ?? ???."
    Msg_ChangingMapTo="%NextMap%(?)? ?? ?????"
    Msg_NoMapInRotation="%maplist% ????? ??? ?? ????."
    Msg_NoMapsFound="%maplist% ????? ?? ???? ?? ?? ? ????."
    Msg_MapIsInRotation="Matching %maplist% ????? ??? ?????."
