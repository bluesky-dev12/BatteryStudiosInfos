...
function CheckBob(float DeltaTime, Vector Y)
{
    ...
    local Vector AxisX, AxisY, AxisZ, vTemp;
...
function CheckEntryVehicle()
{
    ...
    EntryVehicle = none;
    // End:0xDE
    if((int(Physics) == int(1)) && bIsProned == false)
    {
        lp1 = 0;
        J0x2C:
...
function bool AddInventory(Inventory NewItem)
{
    ...
    if(wWeapon(NewItem) != none)
    {
        ...
        {
            // End:0x10A
            if((int(Inv.InventoryGroup) == int(NewItem.InventoryGroup)) && wWeapon(Inv) != none)
            {
                // End:0x107
                if(int(wWeapon(Inv).Priority) < int(wWeapon(NewItem).Priority))
                {
                    bAddedInOrder = true;
                    // [Explicit Break]
                    goto J0x17E;
            ...
            {
                // End:0x151
                if((wWeapon(Prev) != none) && int(wWeapon(Prev).InventoryGroup) == int(NewItem.InventoryGroup))
                {
                    bAddedInOrder = true;
                    // [Explicit Break]
                    goto J0x17E;
...
singular event BaseChange()
{
    ...
    else
    {
        ...
        if((Pawn(Base) != none) && Base != DrivenVehicle)
        {
            ...
            {
                Base.TakeDamage(5, self, Location, 0.5000000 * Velocity, Class'Engine_Decompressed.wDamageStepOn');
                JumpOffPawn();
            }            
        }
        else
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType)
{
    ...
    AddDamageInfo(instigatedBy, ActualDamage);
    // End:0x361
    if(ActualDamage > 0)
    {
...
function AddDamageInfo(Pawn instigatedBy, int Damage)
{
...
function array<Controller> GetAssistList(Pawn Killer)
{
    ...
    if(i < DamageInfo.Length)
    {
        ...
        if(j < ADmInfo.Length)
        {
            ...
            {
                // End:0xF7
                if((tmSrcInfo.DamageTime - ADmInfo[j].DamageTime) <= float(8))
                {
    ...
    {
        // End:0x231
        if((((ADmInfo[i].Attacker.Controller != none) && ADmInfo[i].Attacker.Controller.bIsPlayer) && ADmInfo[i].DamageAmount > float(30)) && (Level.TimeSeconds - ADmInfo[i].DamageTime) <= float(8))
        {
...
function Died(Controller Killer, Class<DamageType> DamageType, Vector HitLocation, optional int CollisionPart, optional int WeaponType)
{
    ...
    if(Controller != none)
    {
        ...
        Level.Game.Killed(Killer, Controller, self, DamageType, CollisionPart, WeaponType);        
    }
    else
    {
