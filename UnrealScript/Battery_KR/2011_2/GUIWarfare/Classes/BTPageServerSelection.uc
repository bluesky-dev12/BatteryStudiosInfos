...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    LabelBackground = NewLabelComponent(fbLabelBackground, Class'GUIWarfareControls.BTUIResourcePoolHK'.default.img_back0, 0.0001000);
    ...
    TPMessenger.HideTabButton(0);
    TPMessenger.TabControl.SetVisiblePanel(1);
...
function CMDHK(int Index, optional string Param1, optional string Param2, optional string param3)
{
    ...
    switch(Index)
    {
        ...
        // End:0x6EE
        case 23:
            MM.kIsRegisteredPCBang = int(!bool(MM.kIsRegisteredPCBang));
            PlayerOwner().dblog("MM.kIsRegisteredPCBang = " $ string(MM.kIsRegisteredPCBang));
            // End:0xD6B
            break;
        // End:0x74D
        case 24:
            MM.kIsPaidPCBang = int(!bool(MM.kIsPaidPCBang));
            PlayerOwner().dblog("MM.kIsPaidPCBang = " $ string(MM.kIsPaidPCBang));
            // End:0xD6B
            break;
        // End:0x881
        case 25:
            PlayerOwner().dblog("MM.kIsRegisteredPCBang = " $ string(MM.kIsRegisteredPCBang));
            PlayerOwner().dblog("MM.kIsPaidPCBang = " $ string(MM.kIsPaidPCBang));
            PlayerOwner().dblog("MM.kIsMainPCBang = " $ string(MM.kIsMainPCBang));
            PlayerOwner().dblog("MM.kPCBangName = " $ MM.kPCBangName);
...
function AutoMoveTochannel(int ServerID, int ChannelID)
{
    ...
    MM.PI_EnterChannel(page_Main.SCIList[i].ServerGroupID, page_Main.SCIList[i].ServerGroupName, page_Main.SCIList[i].ServerID, page_Main.SCIList[i].ServerName, page_Main.SCIList[i].ChannelID, page_Main.SCIList[i].ChannelName, page_Main.SCIList[i].IsClanChannel > 0, page_Main.SCIList[i].IsPracticeChannel);
...
function bool rfAckServerListFromLoginServer(array<string> CServername, array<string> CServerShortName, array<string> serverIP, array<int> serverPort, array<string> ChannelName, array<string> ChannelNickName, array<int> ChannelNum, array<int> chMaxUser, array<int> chCurUser, array<string> ServerGroupName, array<int> ServerGroupID, array<int> ServerDispOrder, array<int> DataPort, array<byte> OptionCount_byte, array<byte> OptionKey_byte, array<byte> OptionValue_byte, array<byte> OptionCount_int, array<byte> OptionKey_int, array<int> OptionValue_int, array<byte> OptionCount_float, array<byte> OptionKey_float, array<float> OptionValue_float, array<byte> IsClanChannel)
{
    ...
    if(i < distinctiveServerNames.Length)
    {
        ...
        serverACL.__OnEnterChannel__Delegate = MoveToChannel;
    ...
    if(i < CServername.Length)
    {
        ...
        page_Main.AddServerChannelList(ServerDispOrder[i], ChannelNum[i], serverIP[i], serverPort[i], DataPort[i], CServername[i], CServerShortName[i], ChannelName[i], ServerGroupID[i], ServerGroupName[i], int(IsClanChannel[i]), ChannelNickName[i], IsPracticeChannel);
        j = 0;
        J0xCE6:
...
function bool rfAckEnterChannel(int id, int UID, string UserID, int Level, string Clanname, int ClanMarkPattern, int ClanMarkBG, int ClanMarkBL, int ClanGrade, int ClanWin, int ClanLose, byte IsLookFor, int Result, string ErrMsg, int ClanLevel, int LevelMarkID, int TitleMarkID, int Reserved1, int Reserved2, int Reserved3)
{
    ...
    if(Result == 0)
    {
        ...
        // End:0x121
        if(MM.kRecvPaidPCBangInfo && MM.kIsRegisteredPCBang > 0)
        {
            // End:0x121
...
function bool rfAckRepairCost(int Result, string ErrMsg, array<BtrDouble> UniqueItemID, array<int> RepairCost)
{
    ...
    if(page_Main.DurWarningItems.bDurWarningItem && page_Main.DurWarningItems.bReqRepairCost)
    {
        ...
        {
            page_Main.AddDurabilityWarningItemRepairCost(UniqueItemID[i], RepairCost[i]);
            i++;
            // [Loop Continue]
            goto J0x1C6;
        }
        ...
        {
            BTWindow.AddItem(page_Main.DurWarningItems.items[i].UniqueID, page_Main.DurWarningItems.items[i].ItemID, page_Main.DurWarningItems.items[i].PartID, page_Main.DurWarningItems.items[i].Durability, RepairCost[i]);
            i++;
            // [Loop Continue]
            goto J0x20C;
        }
...
function bool rfReqRecvChatWispher(string CharnameFrom, string Message)
{
    ...
    RecvMsg = ((("[" $ CharnameFrom) $ "]") $ strFrom) @ Message;
