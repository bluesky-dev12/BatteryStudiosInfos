...
var private export editinline BTEditBoxHK chatEditBox;
var private Material ChatEditBoxBG;
var private float ChatEditBoxX;
var private float ChatEditBoxYFromBottom;
var private float ChatEditBoxWidth;
var private float ChatEditBoxHeight;
var private Material ChatEditBoxCursor;
...
function bool KeyEvent(Interactions.EInputKey key, Interactions.EInputAction Action, float Delta)
{
    ...
    if(bChatEnabled && !IsInState('Typing'))
    {
        ...
        if(ViewportOwner.Actor.bCanChangeQS)
        {
            ...
            {
                // End:0x450
                if(((((ViewportOwner.Actor.IsInState('WaitingForPawn') == false) && ViewportOwner.Actor.IsInState('Dead') == false) && ViewportOwner.Actor.IsInState('RoundEnded') == false) && ViewportOwner.Actor.IsInState('Spectating') == false) && ViewportOwner.Actor.IsInState('WaitingForStart') == false)
                {
                    // End:0x40E
...
function DrawChatEditBox(Canvas C)
{
    ...
    local FloatBox fbCursorBox;
    local float ratioX, ratioY, XL, YL;
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, Class'XInterface.BTHUDResourcePoolHK'.default.InGameChatCursor, fbCursorBox.X2, fbCursorBox.Y1, fbCursorBox.X2 + (float(4) * ratioX), fbCursorBox.Y2);
...
function SendMessage(MessageItem msgItem)
{
    ...
    local Object.MessageType mType;
...
simulated event Tick(float Delta)
{
    ...
    {
        // End:0xF7
        if(MainMenu.rMM.kGame_bOwner && IsInRoomLobby())
        {
            ProcessOutofSeat_InRoomLobby();
        }
    }
    // End:0x13B
...
function ProcessOutofSeat_InRoomLobby()
{
    local float CurrKeySleepTime;
    local int iLKST, iCKST;
    local PlayerController PC;
    ...
    {
        // End:0xED
        if(ViewportOwner.Actor.Level.GetIsServiceBuild() == false)
        {
            MainMenu.AddChatLog("OoS Time = " $ string(iCKST), 5, true);
            PC.dblog("OoS Time = " $ string(iCKST));
        }
        // End:0x505
        if(iCKST >= 5)
        {
            ...
            {
                // End:0x2C4
                if(MainMenu.page_RoomLobby.IsUserAllReady() && MainMenu.page_RoomLobby.IsUserCountFull())
                {
                    // End:0x18A
                    if((OwnerOutofSeatTime - iCKST) == 10)
                    {
                        MainMenu.AddChatLog(Class'GUIWarfareControls.BTWindowDefineInfoHK'.static.GetFormatString(165, string(OwnerOutofSeatTime - iCKST)), 5, true);                        
                    }
                    else
                    {
                        // End:0x2C1
                        if((OwnerOutofSeatTime - iCKST) <= 5)
                        {
                            // End:0x290
                            if((OwnerOutofSeatTime - iCKST) <= 0)
                            {
                                PC.dblog("RoomOwner UAR&&UCF = " $ string(iCKST));
                                MainMenu.pwm.CreateAndPush(7, "GUIWarfareControls.BTWindowDefineInfoHK", true, 0);
                                BTWindowDefineInfoHK(MainMenu.pwm.Last(7)).SetContentText_Instance(167);
                ...
                {
                    // End:0x505
                    if(((OutofSeatTime - iCKST) <= 30) && MM.UserInfos.Length != 1)
                    {
                        // End:0x335
                        if((OutofSeatTime - iCKST) == 30)
                        {
                            MainMenu.AddChatLog(Class'GUIWarfareControls.BTWindowDefineInfoHK'.static.GetFormatString(166, string(OutofSeatTime - iCKST)), 5, true);                            
                        }
                        else
                        {
                            // End:0x37C
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if((OutofSeatTime - iCKST) <= 5)
                                    {
                                        ...
                                        if((OutofSeatTime - iCKST) <= 0)
                                        {
                                            PC.dblog("RoomOwner bReadyToGame==false = " $ string(iCKST));
                                            MainMenu.pwm.CreateAndPush(7, "GUIWarfareControls.BTWindowDefineInfoHK", true, 0);
                                            BTWindowDefineInfoHK(MainMenu.pwm.Last(7)).SetContentText_Instance(168);
                                            PC.ResetKeySleepTime();
                                            MainMenu.StartAutoMove(MM.kServerID, MM.kChannelID, -1);                                            
                    ...
                    switch(ci.ChatType)
                    {
                        ...
                        case 3:
                            ...
                            else
                            {
                                ...
                                {
                                    // End:0x40A
                                    if(ViewportOwner.Actor.Level.GameMgr.ContainsBadNicks(ci.ChatTo, filter))
                                    {
                                        chatEditBox.SetText("/w ");
                                        MainMenu.PrevChatMode.ChatTo = "";
                                        return true;                                        
                                    }
                                    else
                                    {
...
defaultproperties
{
    ...
    MessageType_Color[4]=(RemoveAll[623240703]=none,rfAckPCBangUserList=658.0000000,X2=55.0000000,Y2=678.0000000)
    MessageType_Color[5]=(fbChatEditBox=(X1=59.0000000,Y1=661.0000000,X2=387.0000000,Y2=675.0000000),fbLabelEditBox=(X1=0.0000000,Y1=655.0000000,X2=395.0000000,Y2=681.0000000),strGhostChatModeTeam="[??] ",strGhostChatModeAll="[??] ",MessageType_PostString=/* Array type was not detected. */,MessageType_Color=/* Array type was not detected. */,chatEditBox=// Reference: BTEditBoxHK'GUIWarfare_Decompressed.BTConsole.mChatEditBox'
    begin object name="mChatEditBox" class=GUIWarfareControls.BTEditBoxHK
        bHudMode=true
        bLimitDrawSize=true
    ...
    chatEditBox=mChatEditBox,ChatEditBoxBG=Texture'Warfare_UI.ServerSelection.NewChannelBar_1',ChatEditBoxX=0.0050000,ChatEditBoxYFromBottom=0.1400000,ChatEditBoxWidth=0.3500000,ChatEditBoxHeight=0.0350000,ChatEditBoxCursor=FadeColor'Warfare_UI.HUD.InGameChatCursor',CharEditBoxCursorHeightRatio=0.9000000,MessageItemsX=0.0050000,MessageItemsYFromBottom=0.1500000,MessageItemsLineSpacing=3.0000000,MaxMessageItems=7,ToggleIme=true,ImeNative=true,EventTimerNextStep=60.0000000,ShadowColor=(R=10,G=10,B=10,A=255),BGMFadeInTime=1.0000000,BGMFadeOutTime=1.0000000,BGMTermBetween=2,bVisible=true)
    MessageType_Color[6]=none
    MessageType_Color[7]=none
    MessageType_Color[8]=none
    chatEditBox=mChatEditBox
    ChatEditBoxBG=Texture'Warfare_UI.ServerSelection.NewChannelBar_1'
    ChatEditBoxX=0.0050000
    ChatEditBoxYFromBottom=0.1400000
    ChatEditBoxWidth=0.3500000
    ChatEditBoxHeight=0.0350000
    ChatEditBoxCursor=FadeColor'Warfare_UI.HUD.InGameChatCursor'
    CharEditBoxCursorHeightRatio=0.9000000
