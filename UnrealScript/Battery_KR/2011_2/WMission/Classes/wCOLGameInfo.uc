...
var int FlagScore;
var array<COLFlagSpawnPoint> FlagSpawnPoints;
...
function SpawnFlag()
{
    local COLFlagSpawnPoint S;

    S = GetBestFlagSpawnPoint();
    CurrentFlag = Spawn(Class'WMission_Decompressed.wCOLFlagObjective',,, S.Location);
    Log("[SpawnFlag] CurrentFlag=" $ string(CurrentFlag));
    //return;    
}

function COLFlagSpawnPoint GetBestFlagSpawnPoint()
{
    local int lp1;
    local COLFlagSpawnPoint S, bestSpawnPoint;
    local Pawn P;
    local int Team;
