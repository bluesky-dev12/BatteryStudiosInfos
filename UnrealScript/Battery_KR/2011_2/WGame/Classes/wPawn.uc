...
function PlayDyingSound()
{
    ...
    if(bGibbed)
    {
        PlaySound(GibGroupClass.static.GibSound(), 2, 3.5000000 * TransientSoundVolume, true, 500.0000000);
        return;
    }
    // End:0x6F
    if(HeadVolume.bWaterVolume)
    {
        PlaySound(GetSound(6), 2, 2.5000000 * TransientSoundVolume, true, 500.0000000);
        return;
    }
    //return;    
}
        ...
        else
        {
            ...
            if(!bSkeletized)
            {
                ...
                if(HitFx[SimHitFxTicker].iCollisionPart == 1)
                {
                    ...
                    {
                        AttachEffect(GibGroupClass.default.Teen_BloodHitHeadshotClass, HitFx[SimHitFxTicker].Bone, boneCoords.Origin, HitFx[SimHitFxTicker].rotDir);                        
                    }
                    else
                    {
                        AttachEffect(GibGroupClass.default.BloodHitHeadshotClass, HitFx[SimHitFxTicker].Bone, boneCoords.Origin, HitFx[SimHitFxTicker].rotDir);
                    }                    
                }
                else
                {
                    ...
                    if(Level.GetIsTeenVersion())
                    {
                        AttachEffect(GibGroupClass.default.Teen_BloodHitClass, HitFx[SimHitFxTicker].Bone, boneCoords.Origin, HitFx[SimHitFxTicker].rotDir);                        
                    }
                    else
                    {
                        AttachEffect(GibGroupClass.default.BloodHitClass, HitFx[SimHitFxTicker].Bone, boneCoords.Origin, HitFx[SimHitFxTicker].rotDir);
                    }
                }
                // End:0x28B
                if(Level.GetIsTeenVersion())
                {
                    AttachEffect(GibGroupClass.default.Teen_BloodHitClass_01, HitFx[SimHitFxTicker].Bone, boneCoords.Origin, HitFx[SimHitFxTicker].rotDir);                    
                }
                else
                {
                    AttachEffect(GibGroupClass.default.BloodHitClass_01, HitFx[SimHitFxTicker].Bone, boneCoords.Origin, HitFx[SimHitFxTicker].rotDir);
                }
            }
            // End:0x2EE
...
function DoDamageFX(name BoneName, int Damage, Class<DamageType> DamageType, Rotator R, optional int CollisionPart)
{
    // End:0x142
...
function PlayHit(float Damage, Pawn instigatedBy, Vector HitLocation, Class<DamageType> DamageType, Vector Momentum, optional int CollisionPart)
{
    ...
    DoDamageFX(HitBone, int(Damage), DamageType, Rotator(HitNormal), CollisionPart);
    //return;    
}
...
event PlayLandedSound(int SurfType)
{
    ...
    {
        PlayOwnedSound(SoundGroupClass.static.GetLanedSound(SurfType), 0, FMin(1.0000000, (-0.6000000 * Velocity.Z) / JumpZ),, 95.0000000);
    }
    //return;    
}
...
simulated event SetAnimAction(name NewAction)
{
    ...
    // End:0x6E7
    if(!bWaitForAnim && int(bySpecialState) == 0)
    {
        ...
        else
        {
            ...
            if(InStr(strAction, "_Reload") != -1)
            {
                ...
                // End:0x155
                if((GetTeamNum()) == Level.GetLocalPlayerController().GetTeamNum())
                {
                    PlayOwnedSound(Sound(DynamicLoadObject(Class'Engine.wMessage_Game'.default.SoundAllyReload, Class'Engine.Sound')), 0, 1.0000000);
                }
                // End:0x177
                if(FPWeaponAttachment != none)
                {
...
simulated function StartFiring(bool bHeavy, bool bRapid)
{
    ...
    // End:0x4D
    if((int(TypeOfWeapon) == int(5)) || int(TypeOfWeapon) == int(16))
    {
        bFrag = true;        
    }
    else
    ...
    // End:0x83
    if(((bFrag == false) && bIsIdle == false) && int(TypeOfWeapon) != int(10))
    {
        return;
    }
    // End:0xA5
    ...
    {
        PlayOwnedSound(Sound(DynamicLoadObject(Class'Engine.wGameManager'.default.VoicePackageName $ WeaponAttachment.ResParams.default.str_sound_Grenade, Class'Engine.Sound')), 0);
    }
...
simulated function ScriptPlayIdle(int Channel)
{
    ...
    LoopAnim(IdleWeaponAnim,, 0.2500000, Channel);
    //return;    
}
    ...
    if(bIsProned)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // End:0xC9
                    if((int(TypeOfWeapon) == int(14)) || int(TypeOfWeapon) == int(15))
                    {
...
function PlayTakeHit(Vector HitLocation, int Damage, Class<DamageType> DamageType)
{
    ...
    if(HeadVolume.bWaterVolume)
    {
        ...
        {
            PlaySound(GetSound(6), 2, 1.5000000 * TransientSoundVolume);            
        }
        else
        {
            PlaySound(GetSound(2), 2, 1.5000000 * TransientSoundVolume);
        }
        return;
    }
    //return;    
...
simulated function PlayDirectionalDeath(Vector HitLoc)
{
    ...
    if(((Dir Dot X) > 0.7000000) || Dir == vect(0.0000000, 0.0000000, 0.0000000))
    {
        ...
        if(bTmCrouched == true)
        {
            PlayAnim(Death_Cr_B[Rand(2)],, 0.2000000);            
        }
        else
        {
            PlayAnim(Death_B[Rand(3)],, 0.2000000);
        }        
    }
    else
    {
        ...
        if((Dir Dot X) < -0.7000000)
        {
            ...
            if(bTmCrouched == true)
            {
                PlayAnim(Death_Cr_F[Rand(2)],, 0.2000000);                
            }
            else
            {
                PlayAnim(Death_F[Rand(3)],, 0.2000000);
            }            
        }
        else
        {
            ...
            if((Dir Dot Y) > float(0))
            {
                ...
                if(bTmCrouched == true)
                {
                    PlayAnim(Death_Cr_L[Rand(2)],, 0.2000000);                    
                }
                else
                {
                    PlayAnim(Death_L[Rand(3)],, 0.2000000);
                }                
            }
            else
            {
                ...
                if(HasAnim('DeathR'))
                {
                    ...
                    if(bTmCrouched == true)
                    {
                        PlayAnim(Death_Cr_R[Rand(2)],, 0.2000000);                        
                    }
                    else
                    {
                        PlayAnim(Death_R[Rand(3)],, 0.2000000);
                    }                    
                }
                else
                {
    ...
    AnimBlendToAlpha(2, 0.0000000, 0.2000000);
    //return;    
}
...
simulated function FootStepping(int side)
{
    ...
    PlaySound(SoundFootsteps[SurfaceNum], 0, FootstepVolume,, 105.0000000,, true,,, false);
    //return;    
}
...
{
    local Sound sndScream, sndDie;

    sndScream = Sound(DynamicLoadObject("Warfare_Sound_Char.breath.char_breath_pain", Class'Engine.Sound'));
    sndDie = Sound(DynamicLoadObject("Warfare_Sound_Char.etc.char_die", Class'Engine.Sound'));
    // End:0x86
    if(ActualDamage <= 0)
    {
        return;
    ...
    if(LeftHealth <= 0)
    {
        ...
        if(Killer != none)
        {
            ...
            {
                PlayerController(Killer).ClientPlaySoundHK(sndDie, 0, 1.0000000, false, 3000.0000000, 1.0000000, true, false, -1, false, Location);                
            }
            else
            {
                PlayerController(Killer).ClientPlaySoundHK(sndDie, 0, 1.0000000, false, 3000.0000000, 1.0000000, true, false, -1, false, Location);
        ...
        if(int(byte(CollisionPart)) == int(1))
        {
            PlaySound(SoundGroup'Warfare_Sound_Impact.char.impact_char_headshot', 0, 1.0000000);            
        }
        else
        {
            PlaySound(SoundGroup'Warfare_Sound_Impact.char.impact_char_nomal', 0, 1.0000000);
            PlaySound(sndScream, 0, 1.0000000);
        }        
    }
    else
    {
        ...
        if(DamageType != Class'Engine.fell')
        {
            ...
            if(Killer != none)
            {
                ...
                if(int(byte(CollisionPart)) == int(1))
                {
                    PlayerController(Killer).ClientPlaySoundHK(sndScream, 0, 1.0000000, false, 3000.0000000, 1.0000000, true, false, -1, false, Location);
                    PlayerController(Killer).ClientPlaySoundHK(SoundGroup'Warfare_Sound_Impact.char.impact_char_headshot', 0, 0.7000000, false, 3000.0000000, 1.0000000, true, false, -1, false, Location);                    
                }
                else
                {
                    PlayerController(Killer).ClientPlaySoundHK(sndScream, 0, 1.0000000, false, 3000.0000000, 1.0000000, true, false, -1, false, Location);
                    PlayerController(Killer).ClientPlaySoundHK(SoundGroup'Warfare_Sound_Impact.char.impact_char_nomal', 0, 0.7000000, false, 3000.0000000, 1.0000000, true, false, -1, false, Location);
                }
            }
            // End:0x2A8
            if(int(byte(CollisionPart)) == int(1))
...
simulated function Setup(PlayerRecord Rec, optional bool bLoadNow)
{
    ...
    ResetAllParts();
    // End:0x179
    if(!Class'WGame_Decompressed.wPlayerInfo'.static.SetupPawn(self, BodyID, FaceID, HeadID, AcceID, iSideIndex))
    {
...
function Died(Controller Killer, Class<DamageType> DamageType, Vector HitLocation, optional int CollisionPart, optional int WeaponType)
{
    // End:0x38
    if(PlayerReplicationInfo != none)
    {
    ...
    super(Pawn).Died(Killer, DamageType, HitLocation, CollisionPart, WeaponType);
    //return;    
}
...
function AddSkillByInventory(wMatchUserInfo kUser)
{
    ...
    {
        // End:0x120
        if(kUser.EquippedItems[i].SlotPosition == int(BTNetEnums.13))
        {
...
function SetSelectedQuickSlotSkills(wMatchUserInfo kUser)
{
    ...
    if(PlayerController(Controller).iSelectedQSlotIndex > 0)
    {
        ...
        if(i < 4)
        {
            ...
            {
                // End:0x11A
                if((int(kMM.1) == int(kMM.eWeaponLimit)) && (GetSlotPosbySkillID(kUser, curQSlot.SkillID[i])) == int(BTNetEnums.14))
                {                    
                }
                else
                {
            ...
            AddedSkillList[i] = GetInvenSkillIDbyPos(kUser, int(BTNetEnums.14) + i);
            J0x164:
    ...
    if(i < arStartWeaponInfo.Length)
    {
        PParam = none;
        // End:0x223
        if(arStartWeaponInfo[i].iPartsID > 0)
        {
            PParam = Level.GameMgr.GetAddPartsGroupParam(arStartWeaponInfo[i].iPartsID);
        }
        // End:0x284
        if((((PParam != none) && PParam.iAddPartID > 0) && PParam.iSkillID > 0) && j < 6)
        {
            AddedSkillList[j++] = PParam.iSkillID;
        }
        i++;
        // [Loop Continue]
        goto J0x1C8;
...
function int GetInvenSkillIDbyPos(wMatchUserInfo kUser, int iPos)
{
    ...
    if(i < kUser.EquippedSkills.Length)
    {
        ...
        {
            // End:0xB1
            if((int(kMM.1) == int(kMM.eWeaponLimit)) && kUser.EquippedSkills[i].SlotPosition == int(BTNetEnums.14))
            {
                iResult = 0;                
            }
            else
...
function ChangeInventoryByQSlot()
{
    ...
    {
        // End:0x8C
        if(((int(tmInv.InventoryGroup) == 6) || int(tmInv.InventoryGroup) == 7) || int(tmInv.InventoryGroup) == 10)
        {
            // End:0x75
            if(tmStart == none)
            {
        ...
        if(kUser.EquippedItems.Length > 0)
        {
            ...
            else
            {
                ...
                if(i < kUser.EquippedItems.Length)
                {
                    ...
                    {
                        // End:0x1E4
                        if(kUser.EquippedItems[i].SlotPosition == int(BTNetEnums.0))
                        {
                        ...
                        {
                            AddBaseInvenWeapon(kUser, kUser.EquippedItems[i].SlotPosition);
                        }
                        // [Explicit Continue]
                        goto J0x4AC;
                    }
                    ...
                    {
                        // End:0x388
                        if((kUser.EquippedItems[i].SlotPosition == int(BTNetEnums.2)) || kUser.EquippedItems[i].SlotPosition == int(BTNetEnums.3))
                        {                            
                        }
                        else
                        {
...
function SetSelectedQuickSlotInventory(wMatchUserInfo kUser, bool bQuickslotChange)
{
    ...
    if(i >= 0)
    {
        ...
        else
        {
            ...
            {
                // End:0x196
                if(kUser.EquippedItems[j].SlotPosition == (int(BTNetEnums.0) + i))
                {
        ...
        // End:0x3C5
        if((curQSlot.ItemID[i] != 0) && bPermitWeaponChange)
        {
        ...
        if(int(kMM.eWeaponLimit) != int(kMM.0))
        {
            iSlotPos = int(BTNetEnums.0) + i;
            // End:0x4B1
            if(int(kMM.4) == int(kMM.eWeaponLimit))
            {
                // End:0x48F
                if(iSlotPos == int(BTNetEnums.0))
                {
                    AddWeapon(EmptyBtrDouble(), 5012, 0, false);
                    sInfo.iWeaponID = 5012;
                    sInfo.iPartsID = 0;
                ...
                {
                    AddBaseInvenWeapon(kUser, int(BTNetEnums.0) + i);
                }                
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        // End:0x5D8
                        if((iSlotPos == int(BTNetEnums.2)) || iSlotPos == int(BTNetEnums.3))
                        {                            
                        }
                        else
                        {
...
function int SpawnSupplyItem()
{
    ...
    else
    {
        ...
        if((spawnID == 0) && fRandom < float(0))
        {
            spawnedActor = Spawn(Class'WGame_Decompressed.wArtilleryPack',,, Location - (CollisionHeight * vect(0.0000000, 0.0000000, 1.0000000)));
...
simulated function PlayMelee(bool bMeleeKnifeOrButt)
{
    ...
    if(bIsProned == true)
    {
        ...
        {
            // End:0x53
            if(int(WeaponAttachment.BaseParams.eType) == int(16))
            {
...
function HandlePickup(Pickup pick)
{
    ...
    if(pick.IsA('wWeaponPickup'))
    {
        ...
        // End:0x1DD
        if(((WPick.BaseParams.iInventoryGroup == 1) || WPick.BaseParams.iInventoryGroup == 2) || WPick.BaseParams.iInventoryGroup == 5)
        {
            J0x81:
        ...
        if(WBP.iInventoryGroup == int(bySlot))
        {
            ...
            if(resParam.iItemResourceID != -1)
            {
                switch(resParam.iItemResourceID)
                {
                    ...
                    // End:0xEF
                    case 6003:
                        PlayerController(Controller).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_NoThrowingGreanade);
                        // End:0x1AE
                        break;
                    // End:0x121
                    case 6001:
                        PlayerController(Controller).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_NoThrowingSmoke);
                        // End:0x1AE
                        break;
                    // End:0x153
                    case 6002:
                        PlayerController(Controller).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_NoThrowingStun);
                        // End:0x1AE
                        break;
                    // End:0x182
                    case 7001:
                        PlayerController(Controller).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_NoThrowingC4);
                    // End:0xFFFF
                    default:
...
simulated function RenderFPWeaponAttachment(Canvas C)
{
    ...
    if(FPWeaponAttachment == none)
    {
        FPWeaponAttachment = Spawn(Class'Engine.wFPWeaponAttachment');
...
function SetAddWeaponSkill(wWeapon W, int iaWeaponID, int iPartsID)
{
    ...
    if(iPartsID > 0)
    {
        PParam = Level.GameMgr.GetAddPartsGroupParam(iPartsID);
    }
    // End:0x21B
    if(((PParam != none) && PParam.iAddPartID > 0) && PParam.iSkillID > 0)
    {
        iCheck = 0;
        i = 4;
        J0x120:
        ...
        {
            // End:0x158
            if(AddedSkillList[i] == PParam.iSkillID)
            {
                iCheck = i;
                // [Explicit Break]
                goto J0x162;
        ...
        {
            // End:0x195
            if(AddedSkillList[4] == 0)
            {
                AddedSkillList[4] = PParam.iSkillID;                
            }
            else
            {
                // End:0x1BD
                if(AddedSkillList[5] == 0)
                {
                    AddedSkillList[5] = PParam.iSkillID;                    
                }
                else
                {
                    iCheck = -1;
            ...
            if(iCheck != -1)
            {
                wPlayer(Controller).AddSkill(PParam.iSkillID);
                wPlayer(Controller).ApplySkill_Client(PParam.iSkillID);
            }
        }
    }
    //return;    
...
function SetRemoveWeaponSkill(wWeapon W)
{
    ...
    // End:0x40
    if(W.InventoryClassAddPartsID > 0)
    {
        PParam = Level.GameMgr.GetAddPartsGroupParam(W.InventoryClassAddPartsID);
    }
    // End:0x145
    if(((PParam != none) && PParam.iAddPartID > 0) && PParam.iSkillID > 0)
    {
        ...
        if(iCount < 2)
        {
            wPlayer(Controller).RemoveSkill(PParam.iSkillID);
        }
    }
    //return;    
}
...
state Dying
{
    ...
    simulated function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType)
    {
...
defaultproperties
{
    ...
    SoundFootsteps[0]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[1]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[2]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[3]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[4]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[5]=SoundGroup'Warfare_Sound_FootStep.Dirt.foot_dirt_nomal'
    SoundFootsteps[6]=SoundGroup'Warfare_Sound_FootStep.Metal.foot_metal_nomal'
    SoundFootsteps[7]=SoundGroup'Warfare_Sound_FootStep.Metal.foot_metal_nomal'
    SoundFootsteps[8]=SoundGroup'Warfare_Sound_FootStep.wood.foot_wood_nomal'
    SoundFootsteps[9]=SoundGroup'Warfare_Sound_FootStep.grass.foot_grass_nomal'
    SoundFootsteps[10]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[11]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[12]=SoundGroup'Warfare_Sound_FootStep.snow.foot_snow_nomal'
    SoundFootsteps[13]=SoundGroup'Warfare_Sound_FootStep.waterwaist.foot_waterwaist_nomal'
    SoundFootsteps[14]=SoundGroup'Warfare_Sound_FootStep.Glass.foot_glass_nomal'
    SoundFootsteps[15]=SoundGroup'Warfare_Sound_FootStep.carpet.foot_carpet_nomal'
    SoundFootsteps[16]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[17]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[20]=SoundGroup'Warfare_Sound_FootStep.woodcreak.foot_woodcreak_nomal'
    SoundFootsteps[21]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[22]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[23]=SoundGroup'Warfare_Sound_FootStep.stone.foot_stone_nomal'
    SoundFootsteps[24]=SoundGroup'Warfare_Sound_FootStep.carpet.foot_carpet_nomal'
    SoundFootsteps[25]=SoundGroup'Warfare_Sound_FootStep.Leaves.foot_leaves_nomal'
    SoundFootsteps[26]=SoundGroup'Warfare_Sound_FootStep.carpet.foot_carpet_nomal'
    SoundFootsteps[27]=SoundGroup'Warfare_Sound_FootStep.sand.foot_sand_nomal'
    SoundFootsteps[28]=SoundGroup'Warfare_Sound_FootStep.mud.foot_mud_nomal'
    SoundFootsteps[29]=SoundGroup'Warfare_Sound_FootStep.waterfoot.foot_waterfoot_nomal'
    ...
    PlacedCharacterName="Jakob"
    PlacedFemaleCharacterName="Tamika"
    BreathVolume=0.1750000
    EquipHitVolume=0.8500000
    iWarpCount=2
    iStartQSlotIndex=-1
    Hit_Stun="Hit_Stun"
