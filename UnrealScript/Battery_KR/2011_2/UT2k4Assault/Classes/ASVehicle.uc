...
simulated function bool DrawCrosshair(Canvas C, out Vector ScreenPos)
{
    ...
    Class'UT2k4Assault_Decompressed.HUD_Assault'.static.ClipScreenCoords(C, ScreenPos.X, ScreenPos.Y, tileX * 0.5000000, tileY * 0.5000000);
    C.Style = 5;
    ...
    C.DrawTile(DefaultCrosshair, tileX, tileY, 0.0000000, 0.0000000, SizeX, SizeY);
    return true;
    //return;    
}
...
simulated function DrawEnemyName(Canvas C, HudCDeathmatch H)
{
    ...
    // End:0x147
    if((((Pawn(HitActor) != none) && HitActor != self) && Pawn(HitActor).PlayerReplicationInfo != none) && int(Team) != Pawn(HitActor).GetTeamNum())
    {
        // End:0x125
        if((H.NamedPlayer != Pawn(HitActor).PlayerReplicationInfo) || (Level.TimeSeconds - H.NameTime) > 0.5000000)
        {
