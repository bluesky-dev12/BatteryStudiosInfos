...
function PlayObjectiveSetVociceBGM(Pawn TP)
{
    ...
    TeamQueueAnnouncement(byte(int(1) - int(CurrentAttackingTeam)), (Class'Engine.wGameManager'.default.VoicePackageName $ ".") $ VoiceEnemyPlantedBomb, 1);
    //return;    
}
...
function SetObjectiveSetted(Pawn TP, name ObjectTag, Vector FlagLocation)
{
    ...
    TP.Controller.PlayerReplicationInfo.Exp += AddExp_Plant;
...
static function PrecacheGameAnnouncements(AnnouncerVoice V, bool bRewardSounds)
{
    ...
    DynamicLoadObject("" $ Class'Engine.wMessage_Game'.default.AnnounceSDAllyGotTheBomb, Class'Engine.Sound');
    DynamicLoadObject("" $ Class'Engine.wMessage_Game'.default.AnnounceSDAllyLostTheBomb, Class'Engine.Sound');
    DynamicLoadObject("" $ Class'Engine.wMessage_Game'.default.AnnounceSDIGotTheBomb, Class'Engine.Sound');
    DynamicLoadObject("" $ Class'Engine.wMessage_Game'.default.AnnounceSDAllyBombDefused, Class'Engine.Sound');
    DynamicLoadObject("" $ Class'Engine.wMessage_Game'.default.AnnounceSDEnemyBombDefused, Class'Engine.Sound');
    //return;    
}
...
function AnnounceSwitchTeam()
{
    ...
    if(C != none)
    {
        ...
        if(C.IsA('PlayerController'))
        {
            ...
            {
                PlayerController(C).QueueAnnouncement((Class'Engine.wGameManager'.default.VoicePackageName $ ".") $ sndChangeAttackTeam, 1);                
            }
            else
            {
...
function AnnounceMatchInfo_SD()
{
    ...
    if(C != none)
    {
        ...
        if(C.IsA('PlayerController'))
        {
            ...
            {
                PlayerController(C).QueueAnnouncement((Class'Engine.wGameManager'.default.VoicePackageName $ ".") $ sndModeExplain_Attack, 1);                
            }
            else
            {
                PlayerController(C).QueueAnnouncement((Class'Engine.wGameManager'.default.VoicePackageName $ ".") $ sndModeExplain_Defend, 1);
            }
        }
        C = C.nextController;
        // [Loop Continue]
...
function CalcResultPoints()
{
    ...
    if(C != none)
    {
        ...
        PRI.Exp += (((float(CurrentRound) * 7.0000000) * (GetResultPointRatio_Intervention(PRI))) * numPlayersFactor);
        ...
        {
            PRI.Exp += (((PRI.Team.Score * 8.0000000) * (GetResultPointRatio_Intervention(PRI))) * numPlayersFactor);
        }
        C = C.nextController;
        // [Loop Continue]
        goto J0x31;
...
function CalcRoundResultPoints(int ScoringTeam)
{
    ...
    if(C != none)
    {
        ...
        {
            PRI.Exp += ((30.0000000 * float(PlayerCount)) / 16.0000000);
            PRI.Point += float(10);
        }
        C = C.nextController;
        // [Loop Continue]
...
defaultproperties
{
    ...
    sndModeExplain="None"
    deleAnnounceMatchInfo=wSDGameInfo.AnnounceMatchInfo_SD
