...
function CalcResultPoints()
{
    ...
    if(C != none)
    {
        ...
        PRI.Exp += ((float(GameReplicationInfo.ElapsedTime) / 15.0000000) * (GetResultPointRatio_Intervention(PRI)));
        ...
        {
            PRI.Exp += ((float(GameReplicationInfo.ElapsedTime) / 20.0000000) * (GetResultPointRatio_Intervention(PRI)));
        }
        C = C.nextController;
        // [Loop Continue]
        goto J0x14;
