class BTRoomInfoToolTipHK extends Object;
...
var localized string strHeader[5];
var string strTitle;
var string strGameState;
...
function SetData(string RoomName, string GameState, string MapName, Material MapImg, string roommaster, string GameMode, string Weapon, string HardCore, string gamerunning)
{
    strTitle = RoomName;
    strGameState = GameState;
    strMapName = MapName;
...
function Render(Canvas C)
{
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawString(C, strGameState, 0, float(FontTitleSize), X + fbGameState.X1, Y + fbGameState.Y1, X + fbGameState.X2, Y + fbGameState.Y2, FontShadowColor);
