...
function InitializeContent(byte RankType, int nRankingCount)
{
    ...
    if(Content[0] == none)
    {
        Content[0] = NewComponent(new Class'GUIWarfareControls_Decompressed.BTTPTwoComboBoxHK', Class'Engine.BTCustomDrawHK'.static.MakeEmptyBox(), RenderWeight);
    }
    ...
    if(i < nRankingCount)
    {
        ...
        if(int(RankType) < 8)
        {
            Content[i] = NewComponent(new Class'GUIWarfareControls_Decompressed.BTOwnerDrawRankInfoHK', Class'Engine.BTCustomDrawHK'.static.MakeEmptyBox(), RenderWeight);            
        }
        else
        {
            Content[i] = NewComponent(new Class'GUIWarfareControls_Decompressed.BTOwnerDrawClanRankInfoHK', Class'Engine.BTCustomDrawHK'.static.MakeEmptyBox(), RenderWeight);
        }
