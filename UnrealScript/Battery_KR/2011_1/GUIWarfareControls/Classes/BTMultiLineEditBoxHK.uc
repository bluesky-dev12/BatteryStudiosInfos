...
function bool OwnerDraw(Canvas C)
{
    ...
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        if(tempString != "")
        {
            C.BtrTextSizeLH(tempString, self.FontSize[int(self.MenuState)], W, HT);            
        }
        else
        {
            C.BtrTextSizeLH("|", self.FontSize[int(self.MenuState)], W, HT);
            W = 0.0000000;
        }
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, cursorImg, ClientBounds[0] + W, ClientBounds[1] + float(sumLineHT[CaretLinePos]), (ClientBounds[0] + W) + float(3), (ClientBounds[1] + float(sumLineHT[CaretLinePos])) + HT);
    }
