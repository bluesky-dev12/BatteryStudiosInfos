...
static function bool SetAdvanceSettings(PlayerController PC, OptionGraphicAdvanceInfo CurrentInfo, OptionGraphicAdvanceInfo InitInfo, optional bool bIsOwner, optional bool bForceSettings)
{
    ...
    if(bForceSettings || InitInfo.Antialiasing != CurrentInfo.Antialiasing)
    {
        ...
        if(!bIsOwner)
        {
            PC.ConsoleCommand("setdata D3D9Drv.D3D9RenderDevice Antialiasing" @ string(CurrentInfo.Antialiasing));
        }
        ...
        {
            CurrentRes = PC.ConsoleCommand("GETCURRENTRES");
            PC.ConsoleCommand(("setres" @ CurrentRes) $ "x32f");
        }
        bChange = true;
    }
    // End:0xBE5
...
function LoadData()
{
    ...
    switch(CurrentInfo.Antialiasing)
    {
        // End:0x46E
        case 2:
            BTComboBoxHK(Content[4]).ComboBox.SetIndex(1);
            // End:0x513
            break;
        // End:0x498
        case 4:
            BTComboBoxHK(Content[4]).ComboBox.SetIndex(2);
            // End:0x513
            break;
        // End:0x4C2
        case 8:
            BTComboBoxHK(Content[4]).ComboBox.SetIndex(3);
            // End:0x513
            break;
        // End:0x4EC
        case 16:
            BTComboBoxHK(Content[4]).ComboBox.SetIndex(4);
            // End:0x513
            break;
        // End:0xFFFF
        default:
    ...
    // End:0x5EF
    if(BTComboBoxHK(Content[4]).ComboBox.GetIndex() > 0)
    {
        CurrentInfo.bDOF = false;
        CurrentInfo.bGlow = false;
        BTTPOptionRadioButtonHK(Content[9]).ButtonRadio.SetData(CurrentInfo.bDOF);
...
function Content4_OnChangeValue(GUIComponent Sender)
{
    ...
    switch(massIndex)
    {
        // End:0x49
        case 0:
            CurrentInfo.Antialiasing = 0;
            // End:0x9F
            break;
        // End:0x5D
        case 1:
            CurrentInfo.Antialiasing = 2;
            // End:0x9F
            break;
        // End:0x72
        case 2:
            CurrentInfo.Antialiasing = 4;
            // End:0x9F
            break;
        // End:0x87
        case 3:
            CurrentInfo.Antialiasing = 8;
            // End:0x9F
            break;
        // End:0x9C
        case 4:
            CurrentInfo.Antialiasing = 16;
            // End:0x9F
            break;
        // End:0xFFFF
        default:
    ...
    if(CurrentInfo.Antialiasing > 0)
    {
        ...
        Content[10].DisableMe();
        // End:0x194
        if(bShowAAWarning == false)
        {
            bShowAAWarning = true;
            Class'GUIWarfareControls_Decompressed.BTWindowInfoHK'.static.ShowWindow(Controller, "", strAntiAliasingOn);
        }        
    }
    else
    {
...
function InitializeContent()
{
    ...
    // End:0x525
    if(bool(PC.ConsoleCommand("SUPPORTEDMSAA SAMPLES=2")))
    {
        BTComboBoxHK(Content[4]).ComboBox.AddItem("2x");
        // End:0x525
        if(bool(PC.ConsoleCommand("SUPPORTEDMSAA SAMPLES=4")))
        {
            BTComboBoxHK(Content[4]).ComboBox.AddItem("4x");
            // End:0x525
            if(bool(PC.ConsoleCommand("SUPPORTEDMSAA SAMPLES=8")))
            {
                BTComboBoxHK(Content[4]).ComboBox.AddItem("8x");
                // End:0x525
                if(bool(PC.ConsoleCommand("SUPPORTEDMSAA SAMPLES=16")))
                {
                    BTComboBoxHK(Content[4]).ComboBox.AddItem("16x");
                }
            }
        }
    }
...
defaultproperties
{
    ...
    strLabel[16]="?? ??"
    strLabel[17]="??"
    PredefineAdvanceSettings[0]=(TextureQuality=2,BackgroundQuality=0,LOD=0,CharacterShadow=0,bVSync=false,bShader=false,bShadows=false,bReflectionMap=false,bHDR=false,bDOF=false,bGlow=false,bDynamicLighting=false,bCorona=false,bBlood=false,bDecal=false,DecalStayScale=0,PhysicEffect=0,bRagDoll=false,Antialiasing=0,LevelOfAnisotropy=0)
    ...
    strTooltip[0]="[??? ??]<P> <P>???, ?? ?? ??? ?? ??? ?????.<P> <P>? GeForce 6600GT???? ??? ??? ??? ??? ??? ?? <P>    ??? ?? ??? ??? ???? ?? ???????."
    strTooltip[1]="[?? ??]<P> <P>??? ???? ?? ???? ?? ? ?? ??? ?? ?? ??? ?????.<P> <P>? ???? ??? ?? ?? ???? ??? CPU ??(GHZ)? GPU ??? <P>    ??? ?? ??? ??? ?? ???????."
    strTooltip[2]="[LOD]<P> <P>LOD? ???? ???? ?? ???? ? ??? ?????? ????<P>?? ?? ??? ? ? ????? ??? ??????.<P> <P>? ??????? ??? ?? ? ??? ??????.<P>? ???? ??? ?? ???? ??? CPU ??(GHZ)? GPU ??? <P>    ??? ?? ??? ??? ?? ???????."
    strTooltip[3]="[?? ???]<P> <P>?? ???? ?? ??? ??? ? ??? ????? ?? ?????.<P>??? ?? ???? ? ? ??? ??? ??? ??? ???<P>??? ?? ? ????."
    strTooltip[4]="[?? ????]<P> <P>3D ???? ?? ??? ?????. ??? ???? ??? ????.<P> <P>? ?? ????? ?? ????? ?????? ??? ? ????.<P>? ???? ??? ?? ???? ??? ??? ??? ??? ?? <P>    ??? ?? ?????."
    strTooltip[5]="[???? ???]<P> <P>???? ? ? ???? ?????.<P> <P>? GeForce 6600GT???? ??? ??? ??? ??? ??? <P>    ?? ??? ?? ??? 2x ??? ???? ?? ???????."
    strTooltip[6]="[???]<P> <P>???? ??? ??? ???? ?? ??????.<P> <P>? ???? ???? ??? ??? 8600GT???? ??? ??? <P>    ???? ?? ?????."
    strTooltip[7]="[???]<P> <P>?? ?? ??? ??? ?????.<P> <P>? ??? ??? ?? ?? ?? ??? ? ?? ?????.<P>? ???? ???? ??? ??? 9600GT???? ??? ??? <P>    ???? ?? ?????."
    strTooltip[8]="[???]<P> <P>??? ???? ????? ???? ??????.<P> <P>? ??? ??? ?? ?? ?? ??? ? ?? ?????.<P>? ???? ???? ??? ??? 9600GT???? ??? ??? <P>    ???? ?? ?????."
    strTooltip[9]="[?? ??]<P> <P>?? ???? ? ?? ??? ?????.<P> <P>? ??? ??? ?? ?? ?? ??? ? ?? ?????."
    strTooltip[10]="[???]<P> <P>?? ??? ??? ???? ??????.<P> <P>? ??? ??? ?? ?? ?? ??? ? ?? ?????."
    strTooltip[11]="[???? ???]<P> <P>????? ???? ? ??? ?????. ?? ?? ?? ??? ? <P>??? ?? ??? ???.<P> <P>? ??? ??? ?? ?? ?? ??? ? ?? ?????."
    strTooltip[12]="[???]<P> <P>???? ??? ?? ??? ??? ?????.<P> <P>? GeForce 6600GT???? ??? ??? ??? ??? ??? <P>    ?? ??? ?? ??? ?? ?? ???????."
    strTooltip[14]="[??]<P> <P>?? ?? ?? ???? ??? ???? ? ??? ??? ??? ?????.<P> <P>? ???? ? ??? ?? ??? CPU ??(GHZ)? ??? ?? ??? <P>    ?? ?????."
    strTooltip[16]="[?? ??]<P> <P>?? ??? ?? ?? ?? ??? ?? ??? ??? ???? ?<P>??? ???? ???? ???.<P> <P>? ???? ??? ?? ???? CPU ??(GHZ)? ??? ?? <P>    ??? ?? ?????."
    strTooltip[17]="[??]<P> <P>??? ?? ???? ?? ? ??? ??? ??? ?? ? ?<P>????? ?? ??? ?????.<P> <P>? ???? ? ??? ?? ??? CPU ??(GHZ)? ??? ?? <P>    ??? ?? ?????."
}