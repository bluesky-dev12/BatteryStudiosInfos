...
replication
{
    ...
    reliable if(bNetDirty && int(Role) == int(ROLE_Authority))
        ...
        IsInvulnerable, KillSuccession, 
        Kills, NumLives, 
        PlayerName, PlayerVolume, 
        PlayerZone, Point, 
        RoundWhenIntervented, Score, 
        SideID, StartTime, 
        Team, TeamID, 
        UAVScanBeginTime, UAVScanEndTime, 
        VoiceID, VoiceMemberMask, 
        bAdmin, bAdminSpecator, 
        bCanSeeEnemiesOnMinimap, bCannotChangeQS, 
        bIsFemale, bIsSpectator, 
        bOnlySpectator, bOutOfLives, 
        bPrecached, bReadyToPlay, 
        bWaitingPlayer, bySupply_Helipack, 
        bySupply_UAV, fSpawnedTime, 
        iAccessoryItemID, iBodyItemID_0, 
        iBodyItemID_1, iCurrentWeaponID, 
...
simulated event PostNetBeginPlay()
{
    ...
    {
        // End:0x54
        foreach DynamicActors(Class'Engine_Decompressed.GameReplicationInfo', GRI)
        {
            GRI.AddPRI(self);
            // End:0x54
            break;            
    ...
    // End:0x74
    foreach DynamicActors(Class'Engine_Decompressed.VoiceChatReplicationInfo', VRI)
    {
        VoiceInfo = VRI;
        // End:0x74
        break;        
...
simulated function Destroyed()
{
    ...
    {
        // End:0xA7
        foreach DynamicActors(Class'Engine_Decompressed.VoiceChatReplicationInfo', VoiceInfo)
        {
            // End:0xA7
            break;            
        }        
...
simulated event LoadLevelMark()
{
    ...
    if(ClassMark == none)
    {
        ...
        if(ClassLevel >= 0)
        {
            ClassMark = Material(DynamicLoadObject(Level.GameMgr.GetLevelImageString(ClassLevel, ClassLevelMark), Class'Engine_Decompressed.Material'));
...
simulated function string GetLocationName()
{
    ...
    else
    {
        ...
        if(int(Level.NetMode) == int(NM_Client))
        {
            ...
            else
            {
                ...
                {
                    // End:0x161
                    foreach DynamicActors(Class'Engine_Decompressed.Vehicle', V)
                    {
                        // End:0x160
    ...
    // End:0x1FE
    if((PlayerVolume != none) && PlayerVolume.LocationName != Class'Engine_Decompressed.Volume'.default.LocationName)
    {
        // End:0x1E5
...
function UpdatePlayerLocation()
{
    ...
    // End:0xF7
    foreach P.TouchingActors(Class'Engine_Decompressed.Volume', V)
    {
        // End:0xA2
        if(V.LocationName == "")
        {
...
event ClientNameChange()
{
    ...
    {
        PC.ReceiveLocalizedMessage(Class'Engine_Decompressed.GameMessage', 2, self);        
    }    
    //return;    
}
...
simulated function string GetCallSign()
{
    ...
    return Class'Engine_Decompressed.GameInfo'.default.CallSigns[TeamID];
    //return;    
}
...
simulated event string GetNameCallSign()
{
    ...
    return ((PlayerName $ " [") $ Class'Engine_Decompressed.GameInfo'.default.CallSigns[TeamID]) $ "]";
    //return;    
}
...
simulated function int GetCheckFriendOrClanMember()
{
    ...
    if(IsFriendOrClanMember == -1)
    {
        ...
        {
            UID2 = Level.GetMatchMaker().UserInfos[fl1].UID;
            // End:0xEF
            if(UID == UID2)
            {
                // End:0xE5
