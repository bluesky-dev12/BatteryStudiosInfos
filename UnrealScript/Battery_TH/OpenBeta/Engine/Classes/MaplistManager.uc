...
event PreBeginPlay()
{
    ...
    Class'Engine_Decompressed.CacheManager'.static.GetGameTypeList(CachedGames);
...
protected function GenerateGroupMaplist(int GroupIndex)
{
    ...
    Class'Engine_Decompressed.CacheManager'.static.GetMapList(Records, CachedGames[i].MapPrefix);
    i = 0;
    J0x67:
    ...
    if(i < Records.Length)
    {
        Class'Engine_Decompressed.MaplistRecord'.static.CreateMapItem(Records[i].MapName, Item);
        // End:0x16D
...
protected function MaplistRecord CreateRecord(string RecordName)
{
    ...
    return new (none, Repl(RecordName, " ", Chr(27))) Class'Engine_Decompressed.MaplistRecord';
    //return;    
}
...
function array<string> GetCacheMapList(string Acronym)
{
    ...
    Class'Engine_Decompressed.CacheManager'.static.GetMapList(TempRecords, Acronym);
...
{
    DefaultListName="???"
    InvalidGameType="??? ? ????. ?? ??? ?? .int ??? ???? .u ??? ??? ??? ?????."
    ReallyInvalidGameType="???? '%gametype%' ????? ??? ? ????."
    DefaultListExists="????? ?? ???? ????!"
}