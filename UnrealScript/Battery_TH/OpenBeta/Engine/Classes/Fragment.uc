...
simulated function HitWall(Vector HitNormal, Actor HitWall, Material HitMaterial)
{
    ...
    else
    {
        ...
        if(Speed > float(80))
        {
            ...
            if(FRand() < 0.5000000)
            {
                PlaySound(ImpactSound, 0,,, 300.0000000, 0.8500000 + (FRand() * 0.3000000), true);                
            }
            else
            {
                PlaySound(AltImpactSound, 0,,, 300.0000000, 0.8500000 + (FRand() * 0.3000000), true);
            }
        }
    }
    //return;    
...
state Dying
{
    function TakeDamage(int Dam, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType)
    {
        Destroy();
        //return;        
    }
