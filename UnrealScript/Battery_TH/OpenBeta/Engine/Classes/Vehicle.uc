...
simulated function PostBeginPlay()
{
    ...
    {
        VehicleShadow = Spawn(Class'Engine_Decompressed.ShadowProjector', self, 'None', Location);
    ...
    if(int(Role) == int(ROLE_Authority))
    {
        ...
        else
        {
            ...
            if(!bRemoteControlled && !bEjectDriver)
            {
                ...
                Driver.Died(Controller, Class'Engine_Decompressed.DamRanOver', Driver.Location);
            }
        }        
    }
    else
...
function PossessedBy(Controller C)
{
    ...
    if(C.GetTeamNum() != int(Team))
    {
        ...
        if((int(Team) != 255) && PlayerController(C) != none)
        {
            ...
            if(StolenSound != none)
            {
                PlaySound(StolenSound,, 2.5000000 * TransientSoundVolume,, 400.0000000);
            }
        }
        // End:0x11D
        if(C.GetTeamNum() != 255)
...
event RanInto(Actor Other)
{
    ...
    if(Speed > MinRunOverSpeed)
    {
        ...
        {
            PlaySound(RanOverSound,, TransientSoundVolume * 2.5000000);
        }
...
{
    // End:0x73
    if((((Level.TimeSeconds - LastHornTime) > 3.0000000) && HornIndex >= 0) && HornIndex < HornSounds.Length)
    {
        PlaySound(HornSounds[HornIndex],, 3.5000000 * TransientSoundVolume,, 800.0000000);
...
function PlayTakeHit(Vector HitLocation, int Damage, Class<DamageType> DamageType)
{
    ...
    if(((IndependentVehicle()) && DamageType.default.bBulletHit) && BulletSounds.Length > 0)
    {
        ...
        {
            PlayOwnedSound(BulletSounds[SoundNum], 0, 2.0000000, false, 400.0000000);            
        }
        else
        {
            PlayOwnedSound(BulletSounds[SoundNum], 0, 2.0000000, false, 100.0000000);
        }
    }
    //return;    
}
...
defaultproperties
{
    ...
    VehicleNameString="??"
    RanOverDamageType=Class'Engine_Decompressed.DamRanOver'
    CrushedDamageType=Class'Engine_Decompressed.Crushed'
