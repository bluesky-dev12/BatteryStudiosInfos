...
function Init()
{
    ...
    if(bLocalLog)
    {
        TempLog = Spawn(Class'Engine_Decompressed.FileLog');
        // End:0x3A
        if(TempLog != none)
        {
...
function NewGame()
{
    ...
    i = 0;
    // End:0x1C4
    foreach AllActors(Class'Engine_Decompressed.Mutator', MyMutie)
    {
        // End:0x1A3
        if(tmp != "")
        {
    ...
    // End:0x223
    foreach AllActors(Class'Engine_Decompressed.GameRules', MyRules)
    {
        // End:0x202
        if(tmp != "")
        {
