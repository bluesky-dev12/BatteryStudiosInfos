...
simulated function wLoadOut(int iID, int iAddPartsID, int iPaintID, Pawn Pawn)
{
    ...
    if(iSavedWID != iID)
    {
        ...
        ResetBase();
        // End:0x1D6
        if((((((((((int(BaseParams.eType) == int(0)) || int(BaseParams.eType) == int(5)) || int(BaseParams.eType) == int(10)) || int(BaseParams.eType) == int(11)) || int(BaseParams.eType) == int(12)) || int(BaseParams.eType) == int(17)) || int(BaseParams.eType) == int(13)) || int(BaseParams.eType) == int(15)) || int(BaseParams.eType) == int(14)) || int(BaseParams.eType) == int(16))
        {
            bUseFlashEffect = false;            
        }
        else
...
simulated function SetWeaponPaint(int iPaintID)
{
    ...
    PartsParam = Level.GameMgr.GetAddPartsGroupParam(iSavedPartsID);
    // End:0x3D
    if(PartsParam.iAddPartID == -1)
    {
        return;
    ...
    if(iPaintID > 0)
    {
        Skins[0] = Level.GameMgr.GetPartsGroupPainting(iSavedWID, PartsParam.iAddPartID,, iPaintID);        
    }
    else
    {
        // End:0xDD
        ...
        {
            Skins[0] = Level.GameMgr.GetPartsGroupPainting(iSavedWID, PartsParam.iAddPartID,, PartsParam.iPaintingID);            
        }
        else
        {
            Skins.Length = 0;
...
simulated function AddParts(wWeaponAcce.EAcceType Type, string szName, string strHaveDotName, Vector AimDiff, int iItemID, int iPartsGroupID, optional bool bSecondSight)
{
    local int i;
...
{
    local float Speed2D, BobCamera, MaxSpeed;
    local Vector X, Y, Z;
...
simulated event RenderOverlays(Canvas Canvas)
{
    ...
    Canvas.DrawActor(self, false, false, 60.0000000);
    //return;    
}
