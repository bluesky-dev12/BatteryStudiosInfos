...
function RestartPlayer(Controller aPlayer)
{
    ...
    // End:0x61F
    if(float(aPlayer.Pawn.PrevTime) == 0.0000000)
    {
...
function ProcessKillMessage(Controller Killer, Controller Killed, optional int CollisionPart, optional int WeaponType, optional out int iConKillorHeadShot)
{
    ...
    // End:0x80
    if(int(Killer.PlayerReplicationInfo.KillSuccession) > 254)
    {
        Killer.PlayerReplicationInfo.KillSuccession = byte(255);        
    }
    else
    {
...
function int SendMessageKillSuccssion(Controller Killer, PlayerController PC, byte byKillSuccession)
{
    ...
    iArray = -1;
    // End:0x3E
    if((int(byKillSuccession) >= 2) && int(byKillSuccession) <= 255)
    {
...
function Class<DamageType> GetWDamageType(Class<DamageType> DamageType, int CollisionPart, int WeaponType, out int IsHeadShot)
{
    local Actor.CollisionPartsType MyCollisionPart;
    local Actor.eWeaponType MyWeaponType;
...
{
    local int TempSwitch, IsHeadShot;
    local Class<DamageType> TempDamageType;

    TempDamageType = GetWDamageType(DamageType, CollisionPart, WeaponType, IsHeadShot);
    // End:0x5A
    if(Killer.PlayerReplicationInfo != none)
    {
        TempSwitch = Min(int(Killer.PlayerReplicationInfo.KillSuccession) + 1, 16);
    }
...
{
    local array<Controller> AssistList;
    local int iTemp, iMessageCode, AssisterUID, ilength;
