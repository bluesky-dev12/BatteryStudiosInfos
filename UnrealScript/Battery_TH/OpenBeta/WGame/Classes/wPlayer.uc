...
function Vector CalcAirStrikeLocation()
{
    local Vector vMin, vMax;
    local int lp1;
    local array<Vector> vLocationList;
    ...
    {
        wEnemys = Level.Game.GetEveryEnemyList(byte(Pawn.PlayerReplicationInfo.Team.TeamIndex));        
    }
    else
    {
        wEnemys = Level.Game.GetPersonalEnemyList(Pawn);
    }
    HitLocation = Pawn.Location + Pawn.EyePosition();
    // End:0x183
    if(wEnemys.Length >= 2)
    {
        vLocationList.Length = wEnemys.Length;
        lp1 = 0;
        J0xE1:
        ...
        {
            vLocationList[lp1] = wEnemys[lp1].Location;
            ++lp1;
            // [Loop Continue]
            goto J0xE1;
        }
...
function ServerCallAirStrike(optional Controller Caller)
{
    ...
    Caller.PlayerReplicationInfo.bySupply_AirStrike = 0;
    HitLocation = CalcAirStrikeLocation();
    // End:0xB8
...
function DoAirStrikeFire(Vector HitLocation)
{
    ...
    A = Spawn(Class<wProjectile>(DynamicLoadObject("WWeapons.wAirStrikeProj", Class'Core.Class')),,, HitLocation);
