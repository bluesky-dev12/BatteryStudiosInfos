...
replication
{
    ...
    reliable if(int(Role) == int(ROLE_Authority))
        CurrentOrders, LeaderPRI, 
        SquadObjective, bFreelance;
}
...
{
    return ((FRand() < 0.7000000) && VSize(B.Enemy.Location - B.FocalPoint) < float(350)) && (Level.TimeSeconds - B.LastSeenTime) < float(4);
    //return;    
}
...
function Rotator GetFacingRotation()
{
    ...
    {
        // End:0x5A
        if(int(SquadObjective.DefenderTeamIndex) == Team.TeamIndex)
        {
            Rot.Yaw = Rand(65536);            
        }
        else
...
function bool LostEnemy(Bot B)
{
    ...
    {
        // End:0x110
        if(((M != B) && M.Enemy == Lost) && !M.LostContact(5.0000000))
        {
            bFound = true;
            // [Explicit Break]
            goto J0x127;
    ...
    if(((Team != none) && Killed.PlayerReplicationInfo != none) && Killed.PlayerReplicationInfo.Team == Team)
    {
        ...
        if(IsOnSquad(Killed))
        {
            ...
            if(B != none)
            {
                ...
                {
                    B.SendMessage(none, 'Other', B.GetMessageIndex('MANDOWN'), 10.0000000, 'Team');
                    // [Explicit Break]
                    goto J0xE7;
                }
                B = B.NextSquadMember;
...
function bool FindNewEnemyFor(Bot B, bool bSeeEnemy)
{
    ...
    if(BestEnemy != none)
    {
        ...
        {
            // End:0xF6
            if((ModifyThreat(0.0000000, BestEnemy, bSeeEnemy, B)) > float(5))
            {
                return false;
            }
...
function float AssessThreat(Bot B, Pawn NewThreat, bool bThreatVisible)
{
    ...
    ThreatValue = FClamp(NewStrength, 0.0000000, 1.0000000);
    ...
    if((NewThreat != B.Enemy) && B.Enemy != none)
    {
        ...
        // End:0x1AE
        if(dist > (0.7000000 * VSize(B.Enemy.Location - B.Pawn.Location)))
        {
            ThreatValue -= 0.2500000;
        }
        ThreatValue -= 0.2000000;
        // End:0x223
        if((B.IsHunting() && NewStrength < 0.2000000) && (Level.TimeSeconds - FMax(B.LastSeenTime, B.AcquireTime)) < 2.5000000)
        {
            ThreatValue -= 0.3000000;
        }
    }
...
function bool FindPathToObjective(Bot B, Actor o)
{
    ...
    if(B.bReachedGatherPoint || B.Pawn.ReachedDestination(AlternatePath))
    {
        ...
        if((B.Enemy != none) && B.EnemyVisible())
        {
            GatherWaitTime = 3.0000000;            
        }
        else
        {
            GatherWaitTime = 8.0000000;
        }
        // End:0x5F4
...
function SetDefenseScriptFor(Bot B)
{
    ...
    if(((B.GoalScript != none) && SquadObjective.OwnsDefenseScript(B.GoalScript)) && !B.GoalScript.bNotInVehicle || Vehicle(B.Pawn) == none)
    {
        ...
        // End:0x171
        if(!B.GoalScript.bRoamingScript && (B.bEnemyEngaged || B.GoalScript.bDontChangeScripts) || (FRand() < 0.8500000) && B.Pawn.ReachedDestination(B.GoalScript.GetMoveTarget()))
        {
            return;
        }
    }
...
function SetFreelanceScriptFor(Bot B)
{
    ...
    // End:0x95
    if(((B.GoalScript != none) && !B.GoalScript.bRoamingScript) && B.GoalScript.bDontChangeScripts || (FRand() < 0.8000000) && B.Pawn.ReachedDestination(B.GoalScript.GetMoveTarget()))
    {
        return;
    }
    // End:0xD2
    ...
    {
        // End:0x122
        foreach AllActors(Class'UnrealGame_Decompressed.UnrealScriptedSequence', S)
        {
            // End:0x121
...
function SetObjective(GameObjective o, bool bForceUpdate)
{
    ...
    if(SquadObjective == o)
    {
        ...
        // End:0x65
        if((int(SquadObjective.DefenderTeamIndex) == Team.TeamIndex) && SquadObjective.DefenseSquad == none)
        {
    ...
    {
        // End:0xCB
        if(((SquadObjective != none) && int(SquadObjective.DefenderTeamIndex) == Team.TeamIndex) && SquadObjective.DefenseSquad == self)
        {
...
function Retask(Bot B)
{
    ...
    else
    {
        ...
        if(B.InLatentExecution(B.503))
        {
            ...
            {
                // End:0x157
                if((int(B.Pawn.Physics) == int(2)) && JumpSpot(B.MoveTarget) != none)
                {
                    return;                    
                }
                else
                {
                    // End:0x1A9
                    if((B.MoveTimer > 0.3000000) && Vehicle(B.Pawn) == none)
                    {
                        B.MoveTimer = 0.0500000 + (0.1500000 * FRand());
                    }
                }
            }            
        }
        ...
        {
            B.ReTaskTime = (Level.TimeSeconds + 0.0500000) + (0.1500000 * FRand());
            GotoState('Retasking');
        }
    }
    //return;    
...
function bool TellBotToFollow(Bot B, Controller C)
{
    ...
    if(CloseToLeader(B.Pawn))
    {
        ...
        if(!B.bInitLifeMessage)
        {
            ...
            B.SendMessage(SquadLeader.PlayerReplicationInfo, 'Other', B.GetMessageIndex('GOTYOURBACK'), 10.0000000, 'Team');
        }
        // End:0x2D8
        if(B.Enemy == none)
        {
            ...
            {
                // End:0x209
                if((!o.bDisabled && DestroyableObjective(o) != none) && (Best == none) || int(Best.DefensePriority) < int(o.DefensePriority))
                {
            ...
            {
                // End:0x26A
                if(int(Best.DefenderTeamIndex) != Team.TeamIndex)
                {
                    // End:0x267
            ...
            // End:0x2C3
            if(B.FindInventoryGoal(0.0004000))
            {
                B.SetAttractionState();
                return true;
            }
...
function BotEnteredVehicle(Bot B)
{
    ...
    if(PlayerController(SquadLeader) != none)
    {
        ...
        {
            B.SendMessage(none, 'Other', B.GetMessageIndex('INPOSITION'), 10.0000000, 'Team');
        }        
    }
    else
    {
...
function bool CheckVehicle(Bot B)
{
    ...
    {
        // End:0xDF
        if(VSize(B.Pawn.Location - B.RouteGoal.Location) < (B.Pawn.CollisionRadius + (Vehicle(B.RouteGoal).EntryRadius * 1.5000000)))
        {
    ...
    if(Vehicle(B.Pawn) != none)
    {
        ...
        {
            // End:0x5C5
            if(int(Vehicle(B.Pawn).StuckCount) > 3)
            {
            ...
            {
                // End:0x6A4
                if(((float(B.Pawn.Health) < (B.Pawn.HealthMax * 0.1250000)) && !B.Pawn.bStationary) && (B.Skill + B.Tactics) > (float(4) + (float(7) * FRand())))
                {
        ...
        // End:0x7BA
        if(!B.Pawn.bStationary && int(PriorityObjective(B)) == 0)
        {
    ...
    if(SquadVehicle == none)
    {
        ...
        if(V != none)
        {
            ...
            if(newdist < BestDist)
            {
                ...
                {
                    NewRating += ((BestDist / newdist) * 0.0100000);
                    // End:0xD40
                    if((NewRating > BestRating) && (V.bTeamLocked || V.StronglyRecommended(self, Team.TeamIndex, SquadObjective)) || V.FastTrace(V.Location, B.Pawn.Location + (B.Pawn.CollisionHeight * vect(0.0000000, 0.0000000, 1.0000000))))
                    {
                        SquadVehicle = V;
...
function float VehicleDesireability(Vehicle V, Bot B)
{
    ...
    // End:0xB9
    if(((float(V.Health) < (V.HealthMax * 0.1250000)) && B.Enemy != none) && B.EnemyVisible())
    {
        return 0.0000000;
    }
...
function bool CheckSpecialVehicleObjectives(Bot B)
{
    ...
    if((Best == none) && (Vehicle(B.Pawn) == none) || !Vehicle(B.Pawn).bKeyVehicle)
    {
        ...
        {
            // End:0x2FD
            if((o.IsAccessibleTo(B.Pawn) && ((Team == none) || Team.TeamIndex >= 4) || o.TeamOwner[Team.TeamIndex] == none) && (Best == none) || FRand() < 0.5000000)
            {
                Best = o;
            }
...
function bool OverrideFollowPlayer(Bot B)
{
    ...
    {
        // End:0x98
        if(int(PickedObjective.DefenderTeamIndex) == Team.TeamIndex)
        {
            // End:0x93
...
function bool CheckSquadObjectives(Bot B)
{
    ...
    {
        // End:0x293
        if(int(PriorityObjective(B)) > 0)
        {
            SuperDist = 800.0000000;            
        }
        else
        {
            // End:0x2B1
            ...
            else
            {
                ...
                {
                    SuperDist = 1200.0000000;                    
                }
                else
                {
                    SuperDist = 3200.0000000;
                }
            }
        }
        bMovingToSuperPickup = (((((InventorySpot(B.RouteGoal) != none) && InventorySpot(B.RouteGoal).bSuperPickup) && B.RouteDist < (1.1000000 * SuperDist)) && InventorySpot(B.RouteGoal).markedItem != none) && InventorySpot(B.RouteGoal).markedItem.ReadyToPickup(2.0000000)) && B.Desireability(InventorySpot(B.RouteGoal).markedItem) > float(0);
        // End:0x528
...
function float BotSuitability(Bot B)
{
    ...
    if((GetOrders()) == 'Defend')
    {
        return 1.0000000 - Class<UnrealPawn>(B.PawnClass).default.AttackSuitability;
    }
...
defaultproperties
{
    ...
    MaxSquadSize=2
    bRoamingSquad=true
    RestingFormationClass=Class'UnrealGame_Decompressed.RestingFormation'
