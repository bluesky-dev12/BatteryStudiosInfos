...
function bool ReceiveACL_OnClick(GUIComponent Sender)
{
    ...
    if((TabIndex == 0) && Data.DataPerColumn[1].IntValue == 5)
    {
        ...
        BTWindowRequestClanHK(Controller.TopPage()).SetData(Data.DataPerColumn[1].strValue, Data.DataPerColumn[6].strValue, Data.DataPerColumn[5].strValue);
    ...
    else
    {
        ...
        if((TabIndex == 0) && Data.DataPerColumn[1].IntValue == 6)
        {
            ...
            BTWindowRequestApprovalClanHK(Controller.TopPage()).SetData(Data.DataPerColumn[1].strValue, Data.DataPerColumn[6].strValue);
...
function bool rfAckMemoList(array<int> MemoIdx, array<string> cnameFrom, array<string> dateRecv, array<string> Text, array<byte> Status, array<int> UntilExpire, array<BtrDouble> ItemUID, array<int> ItemID, array<byte> MsgType, array<int> ItemPartID)
{
    ...
    if(i < MemoIdx.Length)
    {
        ...
        Date[0] = Mid(dateRecv[i], 2, 2);
        Date[1] = Mid(dateRecv[i], 5, 2);
        Date[2] = Mid(dateRecv[i], 8, 2);
        Date[0] = (((Date[0] $ ".") $ Date[1]) $ ".") $ Date[2];
        TPMail.AddMail(RMode.RequestMailType, int(MsgType[i]), MemoIdx[i], Date[0], cnameFrom[i], Text[i], UntilExpire[i], int(Status[i]), ItemUID[i], ItemID[i], ItemPartID[i]);
        // End:0x189
