...
function bool CheckPrimaryWeaponDurability()
{
    ...
    if(i < GameMgr.InstanceItemLists.Length)
    {
        ...
        if(GameMgr.InstanceItemLists[i].SlotPosition != 13)
        {
            ...
            bCheckPCBang = (ItemInfo.Grade == 7) && MM.kPCBangType < 0;
            // End:0x157
            if(bCheckPCBang == true)
            {
                // End:0x11C
...
defaultproperties
{
    ...
    strBotModeWarn[0]="????????????(3) ?????????????????? 100% ???Bot Mode???????"
    strBotModeWarn[1]="??????????????????????????????????? Bot Mode ??????????????????????????????"
    strNoviceMode="???????????????? ????????????????AI (???????????)?????????????????????????????????? Minimap ????????"
    strBotMode="??? Bot Mode ???????????????? AI (???????????)????????????????????????????"
    TimeAutoStart=-1.0000000
