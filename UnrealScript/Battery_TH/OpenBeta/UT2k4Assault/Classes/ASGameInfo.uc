...
function UpdateObjectiveProgression(GameObjective DisabledObjective)
{
    ...
    {
        // End:0x213
        if(((int(ASGameReplicationInfo(GameReplicationInfo).ObjectiveProgress) == int(Go.ObjectivePriority)) && !Go.bDisabled) && !Go.bOptionalObjective)
        {
            CurrentObjective = Go;
            // End:0x1FD
            if(!DisabledObjective.bAnnounceNextObjective || ((int(CurrentProgressIndex) == int(BackupProgressIndex)) && Go.Announcer_ObjectiveInfo == DisabledObjective.Announcer_ObjectiveInfo) && !DisabledObjective.bOptionalObjective)
            {
                // [Explicit Break]
                goto J0x22A;
            }
...
function AnnounceNextObjective(GameObjective NewObjective)
{
    ...
    {
        QueueAnnouncerSound("", 1, byte(int(1) - int(NewObjective.DefenderTeamIndex)), 1, 200);        
    }
    else
    {
        QueueAnnouncerSound(NewObjective.Announcer_ObjectiveInfo, 1, byte(int(1) - int(NewObjective.DefenderTeamIndex)), 1, 200);
    }
    // End:0xB1
...
function ShowPathTo(PlayerController P, int TeamNum)
{
    ...
    if(Best != none)
    {
        ...
        {
            // End:0x1B7
            if(P.GetTeamNum() == (1 - int(Best.DefenderTeamIndex)))
            {
                // End:0x191
            ...
            {
                // End:0x230
                if(P.GetTeamNum() == int(Best.DefenderTeamIndex))
                {
                    // End:0x20A
...
function int ReduceDamage(int Damage, Pawn injured, Pawn instigatedBy, Vector HitLocation, out Vector Momentum, Class<DamageType> DamageType)
{
    ...
    return super(TeamGame).ReduceDamage(Damage, injured, instigatedBy, HitLocation, Momentum, DamageType);
    //return;    
}
