...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    txtReward = Class'Engine.BTCustomDrawHK'.static.MakeText(9, 2, "");
    txtReward.DrawColor = Class'GUIWarfareControls_Decompressed.BTUIColorPoolHK'.static.ChatSelf();
    txtLastReward = Class'Engine.BTCustomDrawHK'.static.MakeText(11, 2, "");
...
function Internal_OnRendered(Canvas C)
{
    ...
    else
    {
        ...
        t += 7;
        // End:0x260
        if(arrPrevQuest[1].Text != "")
        {
            Class'Engine.BTCustomDrawHK'.static.DrawTextArray(C, arrPrevQuest, 0, 0, float(L + 5), float(t), float(R), float(B));
            t += 24;
        }
        // End:0x2B9
        if(arrCurQuest[1].Text != "")
        {
            Class'Engine.BTCustomDrawHK'.static.DrawTextArray(C, arrCurQuest, 0, 0, float(L + 5), float(t), float(R), float(B));
            t += 20;
        }
        // End:0x305
        ...
        t = int((AWinPos.Y1 + float(73)) + float(24));
        // End:0x37A
        if(txtLastReward.Text != "")
        {
            Class'Engine.BTCustomDrawHK'.static.DrawText(C, txtLastReward, float(L + 5), float(t), float(R - 5), float(B));
            t -= 24;
        }
        // End:0x3D5
        if(txtReward.Text != "")
        {
            txtReward.FontSize = 10;
            Class'Engine.BTCustomDrawHK'.static.DrawText(C, txtReward, float(L + 5), float(t), float(R - 5), float(B));
        }
    }
    //return;    
}
