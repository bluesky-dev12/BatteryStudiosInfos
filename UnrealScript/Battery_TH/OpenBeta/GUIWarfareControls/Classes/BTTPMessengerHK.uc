...
function bool ButtonMainPCBang_Playing_OnClick(GUIComponent Sender)
{
    ...
    if(MM.kPCBangType == 0)
    {
        Class'GUIWarfareControls_Decompressed.BTWindowDefineARHK'.static.ShowInfo(Controller, 70);        
    }
    else
    {
        // End:0x5A
        if(MM.kPCBangType == 1)
        {
            Class'GUIWarfareControls_Decompressed.BTWindowDefineARHK'.static.ShowInfo(Controller, 71);            
        }
        else
        {
            // End:0x85
            if(MM.kPCBangType == 2)
            {
                Class'GUIWarfareControls_Decompressed.BTWindowDefineARHK'.static.ShowInfo(Controller, 72);
            }
        }
    }
    return true;
...
function Internal_OnVisiblePanelChanged(int Index)
{
    ...
    bValue = false;
    bValue = (TabControl.GetVisiblePanelIndex() == int(3)) && MM.kPCBangType >= 0;
    // End:0x101
    if(bValue == true)
    {
...
defaultproperties
{
    ...
    begin object name="mListBoundImage3" class=GUIWarfareControls_Decompressed.BTStretchedImageHK
        Image=Texture'Warfare_TH_UI_UI.Common.panel_2'
    end object
    ...
    strOffLine="???????"
    strLogout="?????????"
    strMainPCBang[0]="????????????????? ????????????? PC_gc223"
    strMainPCBang[1]="?? PC? ??_gc224"
    strMainPCBang[2]="????????????????? ??????????? PC_gc225"
    strMainPCBang[3]="????????????????????????? PC_gc226"
    bBoundToParent=true
    bScaleToParent=true
}