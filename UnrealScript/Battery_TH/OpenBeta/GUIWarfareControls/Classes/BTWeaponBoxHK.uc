...
function Init()
{
    ...
    bScope = true;
    bDotSight = false;
    WeaponName = Class'Engine.BTCustomDrawHK'.static.MakeText(9, 0, "CheyTac LRRS");
    WeaponName.DrawColor = Class'Engine.Canvas'.static.MakeColor(222, byte(255), 100, byte(255));
    //return;    
}
...
function bool Render(Canvas C, optional float Delta)
{
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawText(C, WeaponName, AWinPos.X1 + float(3), AWinPos.Y1 + float(3), AWinPos.X2, AWinPos.Y2);
    return true;
    //return;    
}
