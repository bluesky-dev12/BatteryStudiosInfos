...
function AnnounceScore(int ScoringTeam)
{
    ...
    QueueAnnouncerSound(Class'Engine.wGameManager'.default.VoicePackageName $ sndRoundWin, 1, byte(ScoringTeam));
    QueueAnnouncerSound(Class'Engine.wGameManager'.default.VoicePackageName $ sndRoundLose, 1, byTempTeam);
    wMSGameReplicationInfo(GameReplicationInfo).RoundOverTime = (wMSGameReplicationInfo(GameReplicationInfo).RoundTimeLimit - wMSGameReplicationInfo(GameReplicationInfo).RoundStartTime) + RemainingTime;
    //return;    
}
...
function RestartPlayer(Controller aPlayer)
{
    ...
    {
        // End:0x44A
        if(!aPlayer.PlayerReplicationInfo.IsSendGameStartTime)
        {
        ...
        // End:0x50A
        if(!aPlayer.PlayerReplicationInfo.IsSendRoundStartTime)
        {
...
function CheckLastMan(int TeamIndex)
{
    ...
    if(survivorCount == 1)
    {
        PlayerController(survivorC).QueueAnnouncement(Class'Engine.wGameManager'.default.VoicePackageName $ sndLastMan, 1);
    }
    //return;    
}
