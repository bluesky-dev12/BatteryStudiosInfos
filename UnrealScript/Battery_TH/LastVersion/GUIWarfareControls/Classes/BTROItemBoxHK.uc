...
function Update(optional Canvas C, optional float Delta)
{
    ...
    tempSkillImage = Texture(DynamicLoadObject("Warfare_TH_UI_UI.Store.icon_rem_skil", Class'Engine.Texture'));
    // End:0x53
    if(IsEmpty())
    {
        ...
        if((ItemInfo != none) && ItemInfo.ItemName != "")
        {
            ...
            else
            {
                ...
                if((ItemInfo.CostType == 1) || ItemInfo.CostType == 4)
                {
                    ...
                    // End:0xA03
                    if(int(InvenState) == int(15))
                    {
                        UnderTextForm.FontDrawType = 1;
                        Class'Engine.BTCustomDrawHK'.static.DrawTextRatio(C, UnderTextForm, AWinPos.X2 - float(157), AWinPos.Y2 - float(15), AWinPos.X2 + float(13), AWinPos.Y2 - float(2));                        
                    }
                    else
                    {
                        // End:0xA76
                        if(int(InvenState) == int(22))
                        {
                            UnderTextForm.FontDrawType = 2;
                            Class'Engine.BTCustomDrawHK'.static.DrawText(C, UnderTextForm, AWinPos.X1, AWinPos.Y2 - float(15), AWinPos.X2 - float(3), AWinPos.Y2 - float(2));                            
                        }
                        else
                        {
                            UnderTextForm.FontDrawType = 2;
...
function bool CanGift()
{
    ...
    if((instanceInfo != none) && instanceInfo.cashItemInfo != none)
    {
        return instanceInfo.cashItemInfo.CanGift();
    }
    return false;
    //return;    
}
...
defaultproperties
{
    ...
    strActiveSkillToolTip[0]="Kill Combo Skill ???????????????????????????????"
    strActiveSkillToolTip[1]="Accumulated kill ???????????????????????"
    strActiveSkillToolTip[2]="Assist Skill ????????????? Assist"
    strActiveSkillToolTip[3]="Death Skill ?????????????????????????????"
}