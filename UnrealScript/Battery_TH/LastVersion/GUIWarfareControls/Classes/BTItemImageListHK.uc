...
function ShowEquipItemAll()
{
    ...
    if(i < DataPool.Length)
    {
        Ro = DataPool[i];
        // End:0x127
        if(((((Ro.ItemInfo.ItemType <= 12) && Ro.ItemInfo.ItemType >= 8) || Ro.ItemInfo.ItemType == 14) || Ro.ItemInfo.ItemType == 34) || Ro.ItemInfo.ItemType == 36)
        {
            // End:0xEE
...
function ShowAllETC()
{
    ...
    if(i < DataPool.Length)
    {
        Ro = DataPool[i];
        // End:0xB4
        if((int(byte(Ro.ItemInfo.ItemType)) >= int(27)) && int(byte(Ro.ItemInfo.ItemType)) <= int(42))
        {
            // End:0xB1
            if((bShowOnlyInventory == false) || Ro.instanceInfo.SlotPosition == int(13))
            {
