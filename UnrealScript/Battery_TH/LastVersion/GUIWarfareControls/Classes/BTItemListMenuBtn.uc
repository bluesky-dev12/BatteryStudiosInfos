...
function RenderButton(Canvas C, optional float Delta)
{
    ...
    if(enableType != int(10))
    {
        ...
        if(disableBtn)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, disableImg, AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
            ...
            {
                Class'Engine.BTCustomDrawHK'.static.DrawTextRatio(C, MenuText[enableType], AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
            }            
        }
        else
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, normalImg, AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
            // End:0x263
            if(bMouseOn)
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, onImg, AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
            }
            // End:0x2AD
            if(bFocused)
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, focusImg, AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
            }
            // End:0x2F7
            if(bMouseClick)
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, clickImg, AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
            }
            // End:0x38C
            if(MenuText[enableType].Text != "")
            {
                Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, MenuText[enableType].Text, MenuText[enableType].FontDrawType, float(MenuText[enableType].FontSize), AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2, MenuText[enableType].DrawShadowColor);
            }
        }
    }
    //return;    
