...
function SaveData()
{
    ...
    if(CurrentInfo.bMusic)
    {
        ...
        {
            PC.ConsoleCommand("setdata ALAudio.FModAudioSubsystem MusicVolume" @ string(float(CurrentInfo.MusicVolume) * 0.0100000));            
        }
        else
        {
    ...
    if(CurrentInfo.bSound)
    {
        ...
        if(!bool(PC.ConsoleCommand("GETCONFIG_BOOL sound UseAL")))
        {
            PC.ConsoleCommand("setdata ALAudio.FModAudioSubsystem SoundVolume" @ string(float(CurrentInfo.SoundVolume) * 0.0100000));            
        }
        else
        {
