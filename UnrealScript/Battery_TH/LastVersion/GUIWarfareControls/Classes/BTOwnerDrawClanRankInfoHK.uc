...
function Internal_OnRendered(Canvas C)
{
    ...
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, BackgroundImage, float(X_1), AWinPos.Y1 + float(2), float(X_1 + 179), AWinPos.Y1 + float(30));
    }
    ...
    if(BackgroundImage.Image != none)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, BackgroundImage, float(X_1), AWinPos.Y1 + float(2), float(X_1 + 221), AWinPos.Y1 + float(30));
    }
