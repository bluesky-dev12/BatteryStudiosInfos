class BTEditBox2HK extends GUIEditBox
    editinlinenew
    instanced;
...
function bool OwnerDraw(Canvas C)
{
    ...
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        if(tempString != "")
        {
            C.BtrTextSize(tempString, FontSize[int(self.MenuState)], W, HT);            
        }
        else
        {
            C.BtrTextSize("|", FontSize[int(self.MenuState)], W, HT);
            W = 0.0000000;
        }
        // End:0x43D
        if(bHudMode == false)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, cursorImg, RWinPos.X1 + W, RWinPos.Y1, (RWinPos.X1 + W) + float(3), RWinPos.Y1 + HT);
        }
        // End:0x627
        if(bSelectionMode)
        {
...
defaultproperties
{
    ...
    bCaptureMouse=true
    OnDraw=BTEditBox2HK.OwnerDraw
    OnActivate=BTEditBox2HK.InternalActivate
    OnDeActivate=BTEditBox2HK.InternalDeactivate
    OnMousePressed=BTEditBox2HK.OnMousePressed
    OnMouseRelease=BTEditBox2HK.OnMouseRelease
    OnChange=BTEditBox2HK.Internal_OnChange
    OnKeyType=BTEditBox2HK.InternalOnKeyType
    OnKeyEvent=BTEditBox2HK.InternalOnKeyEvent
}