...
function ShowItemBuyWindow(BTROItemBoxHK Ro, int buyType, byte cashType)
{
    ...
    BTWindowItemBuyHK(Controller.TopPage()).ButtonRechargeCash.SetVisibility(false);
    //return;    
}
...
defaultproperties
{
    ...
    strOpenBlackBox="????????"
    strLabelHaveKey="?????????"
    strLabelHaveKeyNum="%0 ???"
    strLabelKeyName="????????? Black Box"
    strLabelKeyDesc[0]="??????????????????????????????????? Black Box"
    strLabelKeyDesc[1]="?????1????????????? Black Box ???1 ?????"
    strBlackBoxListItem="%0 (?? %1 ???)"
    strLabelIncludeHeader[0]="Black Box"
    strLabelIncludeHeader[1]="???????????"
}