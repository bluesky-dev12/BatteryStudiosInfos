class BTTPOptionBGQuality extends BTTabPanelHK
    editinlinenew
    instanced;

var export editinline ButtonGroups BGQualityGroup;
var export editinline BTOwnerDrawCaptionButtonHK BGQualityButton[3];
var localized string strBGQuality[3];

function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    local int i;
    local FloatBox fb;

    super(GUIPanel).InitComponent(MyController, myOwner);
    BGQualityGroup = ButtonGroups(NewComponent(new Class'GUIWarfareControls_Decompressed.ButtonGroups', Class'Engine.BTCustomDrawHK'.static.MakeEmptyBox()));
    i = 0;
    J0x40:

    // End:0x165 [Loop If]
    if(i < 3)
    {
        fb = Class'Engine.BTCustomDrawHK'.static.MakeFloatBox(AWinPos.X1 + float(i * 70), AWinPos.Y1, (AWinPos.X1 + float(i * 70)) + float(68), AWinPos.Y1 + float(28));
        BGQualityButton[i] = NewButtonComponent(fb);
        BGQualityButton[i].SetGraphicUpDownBtnImage();
        BGQualityButton[i].SetDefaultFontColor();
        BGQualityButton[i].SetFontSizeAll(9);
        BGQualityButton[i].ButtonID = i;
        BGQualityButton[i].Caption = strBGQuality[i];
        BGQualityGroup.AddButton(BGQualityButton[i]);
        i++;
        // [Loop Continue]
        goto J0x40;
    }
    //return;    
}

function ChangeButtonImage()
{
    local int i;

    i = 0;
    J0x07:

    // End:0x32 [Loop If]
    if(i < 3)
    {
        BGQualityButton[i].SetGraphicUpDownBtnImage();
        i++;
        // [Loop Continue]
        goto J0x07;
    }
    BGQualityButton[BGQualityGroup.GetSelectIndex()].buttonImage[0] = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.Button_Graphic_Opt_all_1_on;
    //return;    
}

function ApplyAWinPos()
{
    local int i;
    local FloatBox fb;

    super(GUIComponent).ApplyAWinPos();
    i = 0;
    J0x0D:

    // End:0xB5 [Loop If]
    if(i < 3)
    {
        fb = Class'Engine.BTCustomDrawHK'.static.MakeFloatBox(AWinPos.X1 + float(i * 70), AWinPos.Y1, (AWinPos.X1 + float(i * 70)) + float(68), AWinPos.Y1 + float(28));
        BGQualityButton[i].AWinPos = fb;
        BGQualityButton[i].ApplyAWinPos();
        i++;
        // [Loop Continue]
        goto J0x0D;
    }
    //return;    
}

function bool DisableAllButton()
{
    local int i;

    i = 0;
    J0x07:

    // End:0x32 [Loop If]
    if(i < 3)
    {
        BGQualityButton[i].DisableMe();
        i++;
        // [Loop Continue]
        goto J0x07;
    }
    return true;
    //return;    
}

function bool EnableAllButton()
{
    local int i;

    i = 0;
    J0x07:

    // End:0x32 [Loop If]
    if(i < 3)
    {
        BGQualityButton[i].EnableMe();
        i++;
        // [Loop Continue]
        goto J0x07;
    }
    return true;
    //return;    
}

defaultproperties
{
    strBGQuality[0]="???"
    strBGQuality[1]="????"
    strBGQuality[2]="???"
}