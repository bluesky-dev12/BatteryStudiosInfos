...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    nSelGameMode = byte(Class'Engine.wGameSettings'.static.GetModeIndex_TeamDeath());
...
function InitializeCombos()
{
    ...
    TeamBlanceRadioBtn.SetData(true);
    // End:0x141
    if(MatchMaker.kClanMatch_InChannel)
    {
        TeamBlanceRadioBtn.DisableMe();
...
function SetGameModeByExtraDataIndex(int idx)
{
    ...
    SetGameModeIndex(int(nSelGameMode));
    //return;    
}
...
function FillMapList()
{
    ...
    RespawnType = Class'Engine.wGameSettings'.static.GetRespawnType(mInfo, gmi.ModeIndex);
...
function FillGameModeList(optional bool bSet, optional int Index)
{
    ...
    if(lp1 < GameModeDispOrder.Length)
    {
        ...
        // End:0x112
        if(((GameModeIndex == Class'Engine.wGameSettings'.static.GetModeIndex_BotTeamDeathBeginner()) || GameModeIndex == Class'Engine.wGameSettings'.static.GetModeIndex_BotDomination()) || GameModeIndex == Class'Engine.wGameSettings'.static.GetModeIndex_Sabotage())
        {
            // [Explicit Continue]
            goto J0x43F;
        }
...
{
    local int lp1, lp2, iGameMode, iEnableLevel;
    local wDefenceLevelInfoParam defenceParam;
    ...
    if(lp1 < iEnableLevel)
    {
        ...
        // End:0x106 [Loop If]
        if(lp2 < PlayerOwner().Level.GameMgr.DefenceLevelInfoParams.Length)
        {
            defenceParam = PlayerOwner().Level.GameMgr.DefenceLevelInfoParams[lp2];
            // End:0xFC
            if((int(defenceParam.byDifficulty) == (lp1 + 1)) && defenceParam.bEnable)
            {
...
function ComboGameMode_OnChange(GUIComponent Sender)
{
    ...
    {
        // End:0x156
        if(Class'Engine.wGameSettings'.static.IsNotRestriction(GetGameModeIndex()))
        {
            ComboWeaponRestriction.DisableMe();
        }
        // End:0x192
...
function ComboMap_OnChange(GUIComponent Sender)
{
    ...
    local int iGameModeIndex, RespawnType;

    mInfo = MatchMaker.MapSettings.GetMapInfo(GetMapName());
    RespawnType = Class'Engine.wGameSettings'.static.GetRespawnType(mInfo, int(nSelGameMode));
    SetRadioRespawnType(RespawnType);
...
defaultproperties
{
    ...
    strLabeDF_Difficult[0]="Easy"
    strLabeDF_Difficult[1]="Normal"
    strLabeDF_Difficult[2]="Hard"
    strLabeDF_Difficult[3]="Very Hard"
