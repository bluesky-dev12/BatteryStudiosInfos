...
{
    return float(maxFontSize * 3) * (float(Controller.ResY) / 768.0000000);
    //return;    
}
...
function MultiColumnList_OnDrawItem(Canvas C, int Item, float X, float Y, float W, float HT, bool bSelected, bool bPending)
{
    ...
    if(i < ACLHeaderList.Length)
    {
        ...
        if(self.ACLRowList[Item].bSpliter)
        {
            ...
            if(i == 0)
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImage(C, self.ACLRowList[Item].BackgroundImage, X, Y, X + W, Y + columnHeight);
                C.DrawColor = Data.Text[0].DrawColor;
                Class'Engine.BTCustomDrawHK'.static.DrawStringPaddingOffset(C, Data.strValue, Data.Text[0].FontDrawType, float(Data.Text[0].FontSize), X, Y, X + W, Y + columnHeight, 0, 2, 0, 0, Data.Text[0].DrawShadowColor);
            }
            return;
        }
        // End:0x589
        if(i == 0)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImage(C, self.ACLRowList[Item].BackgroundImage, Left, Y, X + W, Y + columnHeight);
            // End:0x3C8
            if((bSelected && MultiColumnList.bMousePress == false) && MultiColumnList.LastSelected != Item)
            {
                ...
                SelectIndexX = int(X);
                SelectIndexY = int(Y);
                Class'Engine.BTCustomDrawHK'.static.DrawImage(C, self.SelectImage, Left, Y, X + W, Y + columnHeight);                
            }
            else
            {
                // End:0x467
                if((bSelected && MultiColumnList.bMousePress) && MultiColumnList.LastSelected != Item)
                {
                    ...
                    SelectIndexX = int(X);
                    SelectIndexY = int(Y);
                    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, self.ClickImage, Left, Y, X + W, Y + columnHeight);                    
                }
                else
                {
                    // End:0x4D5
                    if(MultiColumnList.LastSelected == Item)
                    {
                        ...
                        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, self.LastSelectedImage, X, Y, X + W, Y + columnHeight);
                    }
                }
            }
            // End:0x575
            if(ACLRowList[Item].DataPerColumn.Length >= 4)
            {
                ...
                {
                    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, self.MarkMe, X, Y, X + W, Y + columnHeight);
                }
            }
            // End:0x589
            if(bSelected)
        ...
        if(int(ACLHeaderList[i].ColumnType) <= int(4))
        {
            ...
            Class'Engine.BTCustomDrawHK'.static.DrawEllipsisString(C, DrawText, Data.Text[rowState].FontDrawType, float(Data.Text[rowState].FontSize), 1.0000000, Left, Y, Left + width, Y + columnHeight, Data.Text[rowState].DrawShadowColor);
            // [Explicit Continue]
            goto J0x8D3;
        }
        // End:0x865
