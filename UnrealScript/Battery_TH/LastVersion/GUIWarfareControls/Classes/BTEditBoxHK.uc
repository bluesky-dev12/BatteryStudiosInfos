...
var bool bCursorBlink;
var float BlinkTimer;
var float OldBlinkTimer;
var bool useCaps;
...
//var delegate<OnMaxTextStrNum> __OnMaxTextStrNum__Delegate;
...
delegate OnMaxTextStrNum(GUIComponent Sender)
{
    //return;    
}
...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    SetDefaultFontColor();
    SetFontSizeAll(10);
    BlinkTimer = Controller.ViewportOwner.Actor.Level.TimeSeconds;
    OldBlinkTimer = BlinkTimer - 0.7000000;
    //return;    
}
...
function bool OwnerDraw(Canvas C)
{
    ...
    BlinkTimer = Controller.ViewportOwner.Actor.Level.TimeSeconds;
    // End:0x7B
    if((BlinkTimer - OldBlinkTimer) > 0.6000000)
    {
    ...
    if(bHudMode == false)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, bgImg, RWinPos.X1 - float(6), RWinPos.Y1 - float(3), RWinPos.X2 + float(6), RWinPos.Y2 + float(3));
    }
    // End:0x161
    ...
    if(bHudMode == false)
    {
        ...
        Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, self.textStrVis, 3, float(FontSize[int(self.MenuState)]), AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2, self.FontShadowColor[int(self.MenuState)], 0, true);
    }
    // End:0x714
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        if(tempString != "")
        {
            C.BtrTextSizeOLD(tempString, FontSize[int(self.MenuState)], cursorW, HT);            
        }
        else
        {
            C.BtrTextSizeOLD("|", FontSize[int(self.MenuState)], cursorW, HT);
            cursorW = 0.0000000;
        }
        // End:0x4EF
        if((bHudMode == false) && bCursorBlink)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, cursorImg, (RWinPos.X1 + cursorW) + float(1), ((RWinPos.Y2 + RWinPos.Y1) - HT) / float(2), (RWinPos.X1 + cursorW) + float(4), ((RWinPos.Y2 + RWinPos.Y1) + HT) / float(2));
        }
        // End:0x704
        if(bSelectionMode)
        {
            ...
            if(bHudMode == false)
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, selectionImg, RWinPos.X1 + X, ((RWinPos.Y2 + RWinPos.Y1) - HT) / float(2), RWinPos.X1 + X2, ((RWinPos.Y2 + RWinPos.Y1) + HT) / float(2));
            }
        }
...
function DrawIMEInfo(Canvas C, float CursorWidth)
{
    ...
    if(IMEText != "")
    {
        ...
        if(IMESelPosEnd == 0)
        {
            ...
            Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, selectionImg, (RWinPos.X1 + CursorWidth) - width, RWinPos.Y1, RWinPos.X1 + CursorWidth, RWinPos.Y1 + Height);            
        }
        else
        {
            // End:0x1A2
            ...
            if(SelPosStart >= 0)
            {
                ...
                Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, selectionImg, (RWinPos.X1 + CursorWidth) - width, RWinPos.Y1, (RWinPos.X1 + CursorWidth) - SelectWidth, RWinPos.Y1 + Height);
            }
        }
        ...
        Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.panel_1_1, fbDraw.X1, fbDraw.Y1, fbDraw.X2, fbDraw.Y2);
        C.DrawColor = FontColor[int(MenuState)];
        Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, IMEText, 4, float(FontSize[int(MenuState)]), fbDraw.X1, fbDraw.Y1, fbDraw.X2, fbDraw.Y2, FontShadowColor[int(MenuState)], 0, true);
    }
    //return;    
}
...
function int MousePosToCaretPos(float mx, float my)
{
    ...
    // End:0xB4
    if((self.CacheTextWidth.Length > 0) && self.CacheTextWidth[0] == float(0))
    {
    ...
    // End:0xFE
    if(((self.CacheTextWidth.Length > 0) && mx >= self.CacheTextWidth[0]) || my >= CacheTextHeight)
    {
...
function bool CheckPastedTextCondition(string Text)
{
    ...
    if(i < i2)
    {
        ch = __NFUN_241__(Text, i);
        st = Chr(ch);
        // End:0x55
        if(true == (CheckKeyInputCondition(st)))
        {
            return true;
        }
        ++i;
...
function bool InternalOnKeyType(out byte key, optional string Unicode)
{
    ...
    if((int(key) < 32) && Controller.CtrlPressed)
    {
        ...
        if(bHudMode == false)
        {
            switch(key)
            {
                ...
                case 22:
                    ...
                    // End:0x129
                    if(true == (CheckPastedTextCondition(temp)))
                    {
                        // End:0x127
                        if((int(key) != 8) && int(key) != 27)
                        {
                            Class'GUIWarfareControls_Decompressed.BTWindowDefineInfoHK'.static.ShowInfo(Controller, 199);
                        }
                        return false;
                    }
                    // End:0x1D6
        ...
        {
            return true;
        }/* !MISMATCHING REMOVE, tried If got Type:Else Position:0x2DE! */
        // End:0x300
        if(bAllSelected)
        {
            TextStr = "";
        ...
        if(((AllowedCharSet == "") || (bIncludeSign && (st == "-") || st == "+") && TextStr == "") || InStr(AllowedCharSet, st) >= 0)
        {
            ...
            if((MaxWidth == 0) || Len(TextStr) < MaxWidth)
            {
                ...
                if((CheckKeyInputCondition(st)) == true)
                {
                    ...
                    if((int(key) != 8) && int(key) != 27)
                    {
                        Class'GUIWarfareControls_Decompressed.BTWindowDefineInfoHK'.static.ShowInfo(Controller, 198);
                    }
                    return false;
                }
                // End:0x40A
