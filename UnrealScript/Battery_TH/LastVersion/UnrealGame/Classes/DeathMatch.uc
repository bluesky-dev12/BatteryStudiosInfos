...
struct AddBotInfo
{
    ...
    var Controller callerController;
    var int Team;
    var int Grade;
    var int NPCID;
    var int iRegenLocationIndex;
    var bool bUseDynamicRegenLocation;
    var Vector v3DynamicRegenLocation;
    var float fDynamicRegenPitch;
};
...
var string sndModeName;
var string sndModeExplain;
...
{
    Proccess_AddDefaultInventory(PlayerPawn);
    //return;    
}

private function Proccess_AddDefaultInventory(Pawn PlayerPawn)
{
    // End:0x33
...
function AnnounceMatchInfo()
{
    ...
    if(C != none)
    {
        ...
        if(C.IsA('PlayerController'))
        {
            ...
            PlayerController(C).QueueAnnouncement(sndModeExplain, 1);
        }
        C = C.nextController;
        // [Loop Continue]
        goto J0x14;
...
function AnnounceTimesUp()
{
    ...
    if(P != none)
    {
        ...
        {
            PlayerController(P).QueueAnnouncement(string(VoiceTimesUp), 1);
        }
        P = P.nextController;
        // [Loop Continue]
        goto J0x14;
...
function ProcessDefaultInv()
{
    ...
    if((Level.TimeSeconds - InventoryListLastTime) > 0.1000000)
    {
        ...
        if(InventoryList.Length > 0)
        {
            ...
            // End:0x62
            if(P.IsA('wSentryGunPawn'))
            {
                Proccess_AddDefaultInventory(P);                
            }
            else
            {
...
function EnqueueAddBot(Class<Controller> BotClass, Controller callerController, int Team, int Grade, int NPCID, int iRegenLocationIndex, bool bUseDynamicRegenLocation, Vector v3DynamicRegenLocation, float fDynamicRegenPitch)
{
    local AddBotInfo i;

    i.BotClass = BotClass;
    i.callerController = callerController;
    i.Team = Team;
    i.Grade = Grade;
    i.NPCID = NPCID;
    i.iRegenLocationIndex = iRegenLocationIndex;
    i.bUseDynamicRegenLocation = bUseDynamicRegenLocation;
    i.v3DynamicRegenLocation = v3DynamicRegenLocation;
    i.fDynamicRegenPitch = fDynamicRegenPitch;
...
{
    local int i;
    local AddBotInfo botInfo;
    local Controller botController;
    local UnrealTeamInfo BotTeam;
    local Rotator rotTempRotation;

    dblog((("[DeathMatch::HandleGetBotID] AddBotQueue=" $ string(AddBotQueue.Length)) $ " BotID=") $ string(BotID.Length));
    i = AddBotQueue.Length - 1;
    J0x5F:

    // End:0x2BA [Loop If]
    if((i >= 0) && BotID.Length > 0)
    {
        botInfo = AddBotQueue[i];
        // End:0x2B0
        if(Team == botInfo.Team)
        {
            // End:0x11C
            if(botInfo.bUseDynamicRegenLocation)
            {
                rotTempRotation = botInfo.callerController.Pawn.Rotation;
                rotTempRotation.Pitch = int(botInfo.fDynamicRegenPitch);
                botController = Spawn(botInfo.BotClass,,, botInfo.v3DynamicRegenLocation, rotTempRotation);
                botController.bUseDynamicRegenLocation = true;                
            }
            else
            {
                botController = Spawn(botInfo.BotClass);
            }
            // End:0x158
            if(none != botInfo.callerController)
            {
                botController.SetCaller(botInfo.callerController);
            }
            // End:0x2B0
            if(botController != none)
            {
                // End:0x181
                if(false == botController.IsA('wAISentryGunController'))
                {
                    NumBots++;
                }
                botController.PlayerReplicationInfo.PlayerID = CurrentID++;
                botController.PlayerReplicationInfo.NPCID = botInfo.NPCID;
                botController.PlayerReplicationInfo.UID = BotID[BotID.Length - 1];
                botController.RegenLoc = botInfo.iRegenLocationIndex;
                BotID.Remove(BotID.Length - 1, 1);
                AddBotQueue.Remove(i, 1);
                BotTeam = GetBotTeam2(true, Team);
                BotTeam.AddToTeam(botController);
                botController.InitBot(botInfo.Grade);
                botController.GotoState('Dead');
                botController.PlayerReplicationInfo.Team.TeamIndex = Team;
                botController.PlayerReplicationInfo.TeamID = Team;
            }
        }
        i--;
        // [Loop Continue]
...
function AddwAIBot(string strNum, string strTeam, string aiClass, optional string strGrade, optional string strNPCID, optional string strRegenLocationIndex, optional Controller callerController, optional bool bUseDynamicRegenLocation, optional Vector v3DynamicRegenLocation, optional float fDynamicRegenPitch, optional bool bUseFakeID)
{
    local Class<Controller> BotClass;
    local int lp1, Num, Team, NPCID, Grade, iRegenLocationIndex;

    local bool forceTeam, bCreateFakeID;
    local array<int> BotID;
    ...
    // End:0x93
    if(GiveBotsFakeUID || bUseFakeID)
    {
        bCreateFakeID = true;
    }
    NPCID = int(strNPCID);
    iRegenLocationIndex = int(strRegenLocationIndex);
    ...
    if(lp1 < Num)
    {
        EnqueueAddBot(BotClass, callerController, Team, Grade, NPCID, iRegenLocationIndex, bUseDynamicRegenLocation, v3DynamicRegenLocation, fDynamicRegenPitch);
        // End:0x1CD
        if(bCreateFakeID)
        {
            BotID[0] = NPCID;
...
function AnnounceSentryGun_Spawn(Controller Caller)
{
    ...
    if(C != none)
    {
        ...
        else
        {
            ...
            if(PC == Caller)
            {
                PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_MySentryGun);                
            }
            else
            {
                // End:0xDF
                if(C.IsA('PlayerController') && C.Pawn.TeamIndex == Caller.Pawn.TeamIndex)
                {
                    PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_AllySentryGun);                    
                }
                else
                {
                    PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_EnemySentryGun);
                }
            }
        }
        C = C.nextController;
...
function AnnounceSentryGun_Destory(int iTeamIndex)
{
    ...
    if(C != none)
    {
        ...
        {
            // End:0x83
            if(PC.Pawn.TeamIndex == iTeamIndex)
            {
                PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_AllyDestroySentryGun);                
            }
            else
            {
                PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_EnemyDestroySentryGun);
            }
        }
        C = C.nextController;
        // [Loop Continue]
    ...
    function Timer()
    {
        ...
        if((bReady && !bReviewingJumpSpots) && !bChangedHost)
        {
            ...
            if(kMM.isGMRoom)
            {
                ...
                {
                    StartupStage = byte(int(10) + CountDown);
                }                
            }
            else
            {
...
state MatchOver
{
    ...
    function bool ChangeTeam(Controller Other, int iTeamID, bool bNewTeam)
    {
        return false;
        //return;        
    }
