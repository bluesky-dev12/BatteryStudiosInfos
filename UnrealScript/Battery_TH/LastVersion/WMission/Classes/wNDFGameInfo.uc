...
function int GetDefenceBotType(Controller C)
{
    ...
    {
        iNpcID = wAIDefencePawn(C.Pawn).iNpcID;
        return GetBotType(iNpcID);
    }
    // End:0xA8
    if((C != none) && wFlyingDefenceBot(C.Pawn) != none)
    {
        iNpcID = wFlyingDefenceBot(C.Pawn).iNpcID;
        return GetBotType(iNpcID);
    }
    // End:0xD1
...
function int GetDefenceBotTypeByInstigatorID(Controller C)
{
    local int iInstigatorID, iBotType;

    // End:0x50
    if((none != C) && none != C.Pawn)
    {
        iInstigatorID = C.Pawn.GetDelayedDamageInstigatorID();
        iBotType = GetBotType(iInstigatorID);
    }
    return iBotType;
    //return;    
}

function int GetBotType(int iNpcID)
{
    local int iBotType;
...
function int GetOptionalDeathMessageInfo(Controller Killer, Controller Killed, int IsHeadShot, int IsEnableAim)
{
    ...
    // End:0xFD
    if((BotType <= 0) && none != Killed)
    {
        BotType = GetDefenceBotTypeByInstigatorID(Killed);
    }
...
function float GetAddScoreRatio(Controller Owner)
{
    local float fAddRatio;

    fAddRatio = 0.0000000;
    // End:0x3B
    if(wDefenceController(Owner).bScoreUp)
    {
        fAddRatio = wDefenceController(Owner).fAddScore;
    }
    return fAddRatio;
    //return;    
}
...
{
    local float fGainScore;

    // End:0x9D
    if((Other.IsA('wAIDefenceBot') || Other.IsA('wFlyingDefenceBotController')) || Other.IsA('wDefenceBossController'))
    {
        fGainScore = float(AIController(Other).NPCParam.RewardPoint);
        // End:0x94
        if(wDefenceController(Killer).bScoreUp)
        {
            fGainScore += (fGainScore * (GetAddScoreRatio(Killer)));
        }
        return fGainScore;        
    }
    else
...
function SendMessageAddScore(Controller ReceiveController, Controller Killer, Controller Other, int KillWeapon, optional int iConKillorHeadShot)
{
    ...
    else
    {
        ...
        if(Killer.IsA('PlayerController') && addScore != 0)
        {
            ...
            {
                // End:0xC8
                if(wDefenceController(Killer).bPointUp)
                {
                    fAddPoint = wDefenceController(Killer).fAddPoint;
                }
                Killer.PlayerReplicationInfo.PointIns += (float(AIController(Other).NPCParam.IngamePoint) * (float(1) + FClamp(fAddPoint, 0.0000000, 1.0000000)));
            }
            // End:0x143
...
function SetAssistPoint(Controller Killer, Controller Other)
{
    ...
    local wMatchMaker MM;
    local int lp1, iMessageCode, iBonusPoint;
    ...
    {
        Log((("[wNDFGameInfo::SetAssistPoint]: /BonusPoint:" $ string(Killed.NPCParam.BonusPoint)) $ "/NPCID:") $ string(Killed.NPCParam.NPCID));
        C = Level.ControllerList;
        J0xE1:
        ...
        if(C != none)
        {
            ...
            if(((PC != none) && PC.bIsPlayer) && PC != PlayerController(Killer))
            {
                ...
                iBonusPoint = int(float(Killed.NPCParam.BonusPoint) * (1.0000000 + (GetAddScoreRatio(PC))));
                PC.PlayerReplicationInfo.Score += float(iBonusPoint);
                ...
                PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Score', iMessageCode,,,, iBonusPoint);
            }
            C = C.nextController;
            // [Loop Continue]
            goto J0xE1;
...
function wDefenceLevelInfoParam GetDefenceLevelInfoParams(byte byDifficulty)
{
    ...
    // End:0x7A [Loop If]
    if(i < GameMgr.DefenceLevelInfoParams.Length)
    {
        Param = GameMgr.DefenceLevelInfoParams[i];
        // End:0x70
        if(int(Param.byDifficulty) == int(byDifficulty))
        {
            return Param;
        }
        i++;
...
function float GetAddDamageRatio()
{
    ...
    Param = GetDefenceLevelInfoParams(byte(Level.GetMatchMaker().BotDifficulty));
    // End:0x48
    if(Param != none)
    {
        fRatio = Param.fAddNpcDamage;        
    }
    else
    {
        fRatio = 1.0000000;
