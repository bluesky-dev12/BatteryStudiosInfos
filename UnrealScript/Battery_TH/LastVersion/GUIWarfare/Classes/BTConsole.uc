...
function bool KeyEvent(Interactions.EInputKey key, Interactions.EInputAction Action, float Delta)
{
    ...
    if(bChatEnabled && !IsInState('Typing'))
    {
        ...
        if(int(Action) == int(3))
        {
            ...
            {
            }/* !MISMATCHING REMOVE, tried If got Type:Else Position:0x9D5! */
        }
        return super.KeyEvent(key, Action, Delta);
        //return;        
...
function SendMessage(MessageItem msgItem)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // End:0x243
                if(int(wAlienPlayerReplicationInfo(PRI).eObjType) == int(6))
                {
                    TeamName = "";                    
                }
                else
...
defaultproperties
{
    strRecordRetCode[0]="?????????????????? [Unknown Error]"
    strRecordRetCode[1]="???????????????????"
    strRecordRetCode[2]="?????????????????????"
    strRecordRetCode[3]="???????????????????????"
    ...
    MessageType_Color[4]=(System[623240703]=none,SetNewMailArrived=658.0000000,X2=55.0000000,Y2=678.0000000)
