...
function bool BTWindowCreateRoom_OnOK(GUIComponent Sender)
{
    ...
    // End:0x5E
    if(((int(MM.GMLevelFlag) & 1) == 1) || WindowCreateRoom == none)
    {
        return true;
    }
    // End:0xAA
    ...
    else
    {
        ...
        // End:0x2CD
        if(((int(NetRoomInfo.WeaponLimit) == 2) || int(NetRoomInfo.WeaponLimit) == 3) || (int(NetRoomInfo.WeaponLimit) >= 8) || int(NetRoomInfo.WeaponLimit) <= 10)
        {
        ...
        if(Class'Engine.wGameSettings'.static.IsBotModeIndex(int(NetRoomInfo.GameModeNum)))
        {
            ...
            // End:0x3AB
            if(int(NetRoomInfo.GameModeNum) == Class'Engine.wGameSettings'.static.GetModeIndex_Defence())
            {
                NetRoomInfo.BotModeDifficulty = byte(WindowCreateRoom.GetBotDifficultLevel());                
            }
            else
            {
        ...
        // End:0x752
        if(WindowCreateRoom.GetGameModeIndex() == Class'Engine.wGameSettings'.static.GetModeIndex_Defence())
        {
            NetRoomInfo.TeamCnt = 2;
...
{
    // End:0x2F
    if(Class'GUIWarfareControls.BTWindowQuickEnterRoomHK'.default.Map != 255)
    {
        MapIndex = Class'GUIWarfareControls.BTWindowQuickEnterRoomHK'.default.Map - 1;        
    }
    else
    {
        MapIndex = Class'GUIWarfareControls.BTWindowQuickEnterRoomHK'.default.Map;
    }
    GameMode = Class'GUIWarfareControls.BTWindowQuickEnterRoomHK'.default.GameMode;
    WRestrict = Class'GUIWarfareControls.BTWindowQuickEnterRoomHK'.default.WeaponRestriction;
    // End:0xA0
...
function bool ProcessQuickEnter(int MapIndex, int GameMode, int WRestrict, int botLevel)
{
    ...
    MapID = Class'GUIWarfareControls.BTWindowQuickEnterRoomHK'.default.MapID;
    // End:0xCC
    if(MapIndex < 0)
    {
        // End:0xB8
...
function bool OnBottomButton_Click(GUIComponent Sender)
{
    ...
    switch(btn.ButtonID)
    {
        ...
        case 0:
            ...
            GetWindowCreateRoom().SetGameModeByExtraDataIndex(int(GetWindowCreateRoom().nSelGameMode));
            // End:0x199
            break;
        // End:0xB0
        case 1:
...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    page_Main.TcpChannel.sfReqAttendanceList(0, 0, 0, EmptyBtrDouble());
    page_Main.TcpChannel.sfReqPuzzleList(0, 0, EmptyBtrDouble());
    //return;    
}
...
{
    TPMessenger.ACLMsg[int(TPMessenger.0)].MultiColumnList.MyScrollBar.MyGripButton.MenuState = 0;
    // End:0xB1
...
function OnCompleteOpened()
{
    ...
    if(page_Main.rMM.kCharacterCreation)
    {
        ...
        // End:0x1A6
        if(PlayerOwner().Level.bNoGoreVersion == false)
        {
            PlayerOwner().Level.GameMgr.bUseGore = true;
...
function UpdateFriendList(wMatchMaker MM)
{
    ...
    TPMessenger.UpdateFriendlyUser(int(TPMessenger.1));
    //return;    
}
...
function UpdateClanList(wMatchMaker MM)
{
    ...
    TPMessenger.UpdateFriendlyUser(int(TPMessenger.2));
    //return;    
}
...
function UpdatePCBangList(wMatchMaker MM)
{
    ...
    TPMessenger.UpdateFriendlyUser(int(TPMessenger.3));
    //return;    
}
...
function bool rfAckChannelUserList(int TotalCount, int Count, int StartIndex, array<int> UID, array<string> UserID, array<int> UserLevels, array<string> ClanName, array<int> ClanPattern, array<int> ClanBG, array<int> ClanBL, array<int> ClanGrade, array<int> ClanWin, array<int> ClanLose, array<byte> IsLookForClan, int Result, string ErrMsg, array<int> ClanLevel, array<int> LevelMarkID, array<int> TitleMarkID, array<int> Reserved1, array<int> Reserved2, array<int> Reserved3)
{
    ...
    if(Result == 0)
    {
        ...
        if(i < UID.Length)
        {
            ...
            // End:0x1E7
            if(TPMessenger.ACLMsg[int(TPMessenger.0)].ACLRowList.Length > (i + StartIndex))
            {
        ...
        // End:0x343
        if(TPMessenger.ACLMsg[int(TPMessenger.0)].ACLRowList.Length > TotalCount)
        {
            TPMessenger.ACLMsg[int(TPMessenger.0)].RemoveRowArray(TotalCount, TPMessenger.ACLMsg[int(TPMessenger.0)].ACLRowList.Length - TotalCount);
        }        
    }
    else
    {
...
function bool rfAckNotifyEndTutorial(int UserID, string CharName, int Level, string ClanName, int ClanMarkPattern, int ClanMarkBG, int ClanMarkBL, int ClanGrade, int ClanWin, int ClanLose, byte IsLookFor, int Result, int ClanLevel, int LevelMarkID, int TitleMarkID, int Reserved1, int Reserved2, int Reserved3)
{
    ...
    // End:0x1ED
    if(MM.bAutoMoveNoviceChannel)
    {
        page_Main.TutorialChannelAutoMove();
        MM.bAutoMoveNoviceChannel = false;
        MM.TutorialGrade = 0.0000000;
    }
    //return;    
}
...
function bool rfNotifyNewNickName(int Community_Type, string Old_NickName, string New_NickName)
{
    ...
    // End:0x97
    if(bool(Community_Type & (1 << 1)) && TPMessenger.TabControl.CurrentTabIndex == int(TPMessenger.3))
    {
        TPMessenger.ReplaceUserNickName(1, Old_NickName, New_NickName);
    }
    // End:0xEE
    if(bool(Community_Type & (1 << 2)) && TPMessenger.TabControl.CurrentTabIndex == int(TPMessenger.2))
    {
        TPMessenger.ReplaceUserNickName(2, Old_NickName, New_NickName);
    }
    // End:0x145
    if(bool(Community_Type & (1 << 3)) && TPMessenger.TabControl.CurrentTabIndex == int(TPMessenger.1))
    {
        TPMessenger.ReplaceUserNickName(3, Old_NickName, New_NickName);
    }
    // End:0x19C
    if(bool(Community_Type & (1 << 5)) && TPMessenger.TabControl.CurrentTabIndex == int(TPMessenger.0))
    {
...
{
    strEnterRoomMessage="??????????????????????????"
    strConnectingLogin="??????????????????????????? login server"
    ...
    // Reference: BTACLLobbyHKCN'GUIWarfare_Decompressed.BTPageLobby.mACLLobby'
    begin object name="mACLLobby" class=GUIWarfareControls.BTACLLobbyHKCN
        bUseAWinPos=true
        AWinPos=(X1=12.0000000,Y1=114.0000000,X2=728.0000000,Y2=450.0000000)
        RenderWeight=0.8000000
        OnRendered=mACLLobby.OnRendered
    end object
    ACLLobby=mACLLobby
    // Reference: BTACLLobbyHKCN'GUIWarfare_Decompressed.BTPageLobby.mACLLobbyEnterable'
    begin object name="mACLLobbyEnterable" class=GUIWarfareControls.BTACLLobbyHKCN
        bUseAWinPos=true
    ...
    bAllRoomList=true
    // Reference: BTTPChatHKCN'GUIWarfare_Decompressed.BTPageLobby.mTPChat'
    begin object name="mTPChat" class=GUIWarfareControls.BTTPChatHKCN
        OnPostDraw=mTPChat.Internal_OnPostDraw
        bUseAWinPos=true
        AWinPos=(X1=10.0000000,Y1=518.0000000,X2=460.0000000,Y2=736.0000000)
        RenderWeight=0.7000000
        OnPreDraw=mTPChat.Internal_OnPreDraw
        OnDeActivate=mTPChat.Internal_DeActivate
    end object
    TPChat=mTPChat
    // Reference: BTTPMessengerHKCN'GUIWarfare_Decompressed.BTPageLobby.mTPMessenger'
    begin object name="mTPMessenger" class=GUIWarfareControls.BTTPMessengerHKCN
        ...
        bUseAWinPos=true
        AWinPos=(X1=500.0000000,Y1=518.0000000,X2=746.0000000,Y2=733.0000000)
        RenderWeight=0.9000000
    end object
    TPMessenger=mTPMessenger
    // Reference: BTTPMyInfoHKCN'GUIWarfare_Decompressed.BTPageLobby.mTPMyInfo'
    begin object name="mTPMyInfo" class=GUIWarfareControls.BTTPMyInfoHKCN
        bUseAWinPos=true
        AWinPos=(X1=772.0000000,Y1=525.0000000,X2=1014.0000000,Y2=735.0000000)
        RenderWeight=0.6000000
        OnRendered=mTPMyInfo.OnRendered
    end object
    TPMyInfo=mTPMyInfo
    strDisconnectedChannel="??????????????????????? Chanel???. ???????????????????."
    strListTop[0]="NO"
    strListTop[1]="???"
    strListTop[2]="????????"
    strListTop[3]="Map"
    strListTop[4]="Game Mode"
    ...
    strMakeClanWarRoom="??????????? ClanMode"
    strRegularClanWar="??????????????? ClanMode"
    strFriendlyClanWar="????? Friendly room ?? Clan Mode"
    strButtonRegularClanWar="?????????????????????? ClanMode"
    strButtonFriendlyClanWar="????? Friendly room ?? ClanMode"
    ...
    CloseLastMenu=false
    fbAdvertise=(X1=757.0000000,Y1=92.0000000,X2=1024.0000000,Y2=175.0000000)
