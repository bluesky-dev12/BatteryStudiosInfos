...
function bool ImageListBox_OnClick(GUIComponent Sender)
{
    ...
    selectRO = BTROItemBoxHK(ItemList.DataList[ItemList.SelectItemIndex]);
    // End:0xBC
    if(selectRO.ItemInfo.EquipLevel > myLevel)
    {
        ItemList.SelectItemIndex = -1;
        return false;
    }
    iItemType = GM.GetItemType(selectRO.ItemInfo, selectRO.instanceInfo);
    // End:0x120
    if(bool(iItemType & GM.FlagBuyShop) == false)
    {
        ItemList.SelectItemIndex = -1;
    ...
    if(selectRO.bSelect)
    {
        ...
        if(bool(iItemType & GM.FlagCash) == true)
        {
            RepairSumCash -= selectRO.instanceInfo.cashItemInfo.WZCashPrice[0];            
        }
        else
        {
            RepairSumCost -= selectRO.ItemInfo.PointPrice[0];
        }
        UpdateNEditBox();        
    }
    else
...
function UpdateNEditBox()
{
    ...
    NEditBox[6].SetData(RepairSumCCoin);
    NEditBox[7].SetData(MyCCoin);
    NEditBox[8].SetData(MyCCoin - RepairSumCCoin);
    // End:0x149
...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    TPItemList.ItemList.SetItemSize(158, int(float(87) * (float(Controller.ResY) / float(768))));
...
defaultproperties
{
    ...
    fbNEditBox[0]=(X1=340.0000000,Y1=493.0000000,X2=547.0000000,Y2=507.0000000)
    fbNEditBox[1]=(X1=340.0000000,Y1=527.0000000,X2=547.0000000,Y2=541.0000000)
    fbNEditBox[2]=(X1=340.0000000,Y1=561.0000000,X2=547.0000000,Y2=575.0000000)
    fbNEditBox[3]=(X1=456.0000000,Y1=493.0000000,X2=679.0000000,Y2=507.0000000)
    fbNEditBox[4]=(X1=456.0000000,Y1=527.0000000,X2=679.0000000,Y2=541.0000000)
    fbNEditBox[5]=(X1=456.0000000,Y1=561.0000000,X2=679.0000000,Y2=575.0000000)
