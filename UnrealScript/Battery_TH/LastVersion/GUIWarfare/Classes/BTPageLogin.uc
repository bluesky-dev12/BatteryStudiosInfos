...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    if(Controller.ViewportOwner.Actor.Level.GetIsServiceBuild() == false)
    {
        ...
        ButtonBattery.bTabStop = false;
        ButtonBattery.Caption = "(THAILAND)Battery Login";
    }
    // End:0x3AC
    if(Controller.ViewportOwner.Actor.Level.GetIsServiceBuild() == false)
    {
...
{
    Log((((("[BTPageLogin::rfAckLoginWithTencentAuth] Result=" $ string(Result)) $ " wzp_AccountType=") $ string(tc_AccountType)) $ " wzp_IsBlock=") $ string(tc_IsBlock));
    __NFUN_270__((((("[BTPageLogin::rfAckLoginWithTencentAuth] Result=" $ string(Result)) $ " wzp_AccountType=") $ string(tc_AccountType)) $ " wzp_IsBlock=") $ string(tc_IsBlock));
    return page_Main.TcpLogin_rfAckLoginWithTencentAuth_Test(Result, tc_AccountType, tc_IsBlock);
    //return;    
}
...
{
    Log((((((((("[BTPageLogin::rfAckLoginWithWZAuth] Result=" $ string(Result)) $ " wzp_AccountType=") $ string(wzp_AccountType)) $ " wzp_IsBlock=") $ string(wzp_IsBlock)) $ " wzp_AuthKeySuccess=") $ string(wzp_AuthKeySuccess)) $ " ChannelType = ") $ string(ChannelType));
    __NFUN_270__((((((((("[BTPageLogin::rfAckLoginWithWZAuth] Result=" $ string(Result)) $ " wzp_AccountType=") $ string(wzp_AccountType)) $ " wzp_IsBlock=") $ string(wzp_IsBlock)) $ " wzp_AuthKeySuccess=") $ string(wzp_AuthKeySuccess)) $ " ChannelType = ") $ string(ChannelType));
    return page_Main.TcpLogin_rfAckLoginWithWZAuth_Test(Result, wzp_AccountType, wzp_IsBlock, wzp_AuthKeySuccess, ChannelType);
    //return;    
}

function bool rfAckLoginWithThailand(int Result)
{
    Log("[BTPageLogin::rfAckLoginWithThailand] Result=" $ string(Result));
    __NFUN_270__("[BTPageLogin::rfAckLoginWithThailand] Result=" $ string(Result));
    return page_Main.TcpLogin_rfAckLoginWithThailand_Test(Result);
    //return;    
}

function bool rfAckLoginWithGlobal(int Result)
{
    Log("[BTPageLogin::rfAckLoginWithGlobal] Result=" $ string(Result));
    __NFUN_270__("[BTPageLogin::rfAckLoginWithGlobal] Result=" $ string(Result));
    return page_Main.TcpLogin_rfAckLoginWithGlobal_Test(Result);
    //return;    
}
