...
simulated function DrawCSReviw(Canvas C)
{
    ...
    if(i < Level.CSReviewNameTags.Length)
    {
        ...
        ScreenPos.Z = 0.0000000;
        // End:0x8C
        if(C.VSize(ScreenPos) > 1000.0000000)
        {
            // [Explicit Continue]
            goto J0x186;
        }
...
simulated function LocalizedMessage2(Class<LocalMessage> Message, optional int Switch, optional bool bOption_Push, optional string CriticalString)
{
    ...
    {
        // End:0x36D
        if((Level.TimeSeconds - LocalMessages[iPrevIdx].EndOfLife) > 0.0100000)
        {
            LocalMessages[iPrevIdx].Drawn = false;
            LocalMessages[i].EndOfLife = Message.static.GetLifeTime(Switch) + Level.TimeSeconds;
            LocalMessages[i].StartTime = Level.TimeSeconds;            
        }
        else
        {
            LocalMessages[i].StartTime = LocalMessages[iPrevIdx].EndOfLife + 0.1500000;
            LocalMessages[i].EndOfLife = Message.static.GetLifeTime(Switch) + LocalMessages[i].StartTime;
        }        
    }
    else
    {
        LocalMessages[iPrevIdx].Drawn = false;
        LocalMessages[iPrevIdx].EndOfLife = Level.TimeSeconds;
        LocalMessages[i].EndOfLife = Message.static.GetLifeTime(Switch) + Level.TimeSeconds;
        LocalMessages[i].StartTime = Level.TimeSeconds;
    }
    LocalMessages[i].StringMessage = CriticalString;
...
simulated function RemoveLocalizedMessage(Class<LocalMessage> Message, optional int Switch, optional PlayerReplicationInfo RelatedPRI_1, optional PlayerReplicationInfo RelatedPRI_2, optional Object OptionalObject, optional string CriticalString)
{
    local int i, k;
    local PlayerReplicationInfo HUDPRI;
    ...
    if(i < 8)
    {
        ...
        // End:0xFD
        if(LocalMessages[i].Switch != Switch)
        {
            // [Explicit Continue]
            goto J0x160;
        }
        ...
        if(k < (8 - 1))
        {
            LocalMessages[k] = LocalMessages[k + 1];
            ++k;
            // [Loop Continue]
            goto J0x119;
        }
        ClearMessage(LocalMessages[k]);
        // [Explicit Break]
        goto J0x16A;
        J0x160:
...
simulated function DisplayLocalMessages(Canvas C)
{
    ...
    if(i > -1)
    {
        ...
        if(LocalMessages[i].Message == Class'Engine.wSpecialKillMessage')
        {
            ...
            fRemainTime = LocalMessages[i].EndOfLife - Level.TimeSeconds;
            // End:0x48C
            if(fRemainTime < (float(Class'Engine.wSpecialKillMessage'.default.Lifetime) - Class'Engine.wSpecialKillMessage'.default.fTotalEffectRunTime))
            {
                bDeleteMessage = true;
            }
        }
    ...
    if(i < 8)
    {
        ...
        {
            // End:0x54E
            if(Level.TimeSeconds < LocalMessages[i].StartTime)
            {
                // [Explicit Continue]
                goto J0x74A;
            }
    ...
    if(PawnOwnerPRI != none)
    {
        ...
        if(lp1 < Level.PawnList.Length)
        {
            ...
            if(curPawn.PlayerReplicationInfo != none)
            {
                ...
                UpdateNameTagInfo(C, curPawn, camLoc, camRot);
                // End:0x5C9
...
function DrawNameTag(Canvas C, wPawn curPawn)
{
    ...
    // End:0x80
    if((curPawn.PlayerReplicationInfo != none) && curPawn.PlayerReplicationInfo.IsInvulnerable)
    {
    ...
    if((nametag == KillerName) && KillerMarkTimeToRemove > Level.TimeSeconds)
    {
        ...
        C.DrawTile(KillerSprite.WidgetTexture, 63.0000000, 63.0000000, float(KillerSprite.TextureCoords.X1), float(KillerSprite.TextureCoords.Y1), float(KillerSprite.TextureCoords.X2), float(KillerSprite.TextureCoords.Y2));
    }
    //return;    
}

function DrawCustomNameTag(Canvas C, Vector vNameTagPos, wPawn curPawn, string strName, optional float fAdditionalX, optional float fAdditionalY)
{
    local float X1, Y1, X2, Y2, width, Height;

    local Color nametagColor;
    local float additionalx, additionaly;

    additionalx = fAdditionalX * _rX;
    additionaly = fAdditionalY * _rY;
    C.BtrTextSize(strName, NametagFontSize + 1, width, Height);
    width += additionalx;
    X1 = (vNameTagPos.X - (width / float(2))) + additionalx;
    Y1 = (vNameTagPos.Y - (Height / float(2))) + additionaly;
    C.DrawColor = C.MakeColor(byte(255), byte(255), byte(255), byte(255));
    C.SetPos(X1, Y1);
    X2 = X1 + width;
    Y2 = Y1 + Height;
    nametagColor = GetNametagColor(curPawn.PlayerReplicationInfo);
    C.DrawColor = NametagShadowColor;
    C.BtrDrawTextJustified(strName, 1, X1 + float(NametagShadowX), Y1 + float(NametagShadowY), X2 + float(NametagShadowX), Y2 + float(NametagShadowY), NametagFontSize);
    C.DrawColor = nametagColor;
    C.BtrDrawTextJustified(strName, 1, X1, Y1, X2, Y2, NametagFontSize);
    //return;    
}
...
function Color GetNametagColor(PlayerReplicationInfo Info)
{
    ...
    {
        // End:0xEE
        if((PlayerOwner.PlayerReplicationInfo.Team != none) && PlayerOwner.PlayerReplicationInfo.Team.TeamIndex == Info.Team.TeamIndex)
        {
            // End:0xE0
...
static function PlayHUDSound(PlayerController PC, HudBase.eHUDSOUND soundType)
{
    ...
    switch(soundType)
    {
        // End:0x38
        case 0:
            strSound = "Warfare_Sound_UI/Com/qslot_change";
            // End:0xF4
            break;
        // End:0x68
        case 1:
            strSound = "Warfare_Sound_UI/etc/qslotchange";
            // End:0xF4
            break;
        // End:0x94
        case 2:
            strSound = "Warfare_Sound_UI/etc/minimap";
            // End:0xF4
            break;
        // End:0xC2
        case 5:
            strSound = "Warfare_Sound_UI/etc/friendson";
            // End:0xF4
            break;
        // End:0xEE
        case 6:
            strSound = "Warfare_Sound_UI/etc/whisper";
            // End:0xF4
            break;
        // End:0xFFFF
        default:
    ...
    {
        PC.FmodClientPlaySound(strSound);
    }
    //return;    
}
...
simulated event WorldSpaceOverlays()
{
    ...
    {
        DrawDirectionalArrow(Level.CSReviewArrows[i].vPos, Level.CSReviewArrows[i].rRayDir, Level.CSReviewArrows[i].ArrowColor, 1.0000000);
        ++i;
        // [Loop Continue]
        goto J0x88;
    }
    ...
    {
        DrawSphere(Level.CSReviewPoints[i].vPos, Level.CSReviewPoints[i].PointColor, 6.0000000, 8);
        ++i;
        // [Loop Continue]
        goto J0x108;
    }
    ...
    if(i < Level.CSReviewSpheres.Length)
    {
        ...
        vCenterPos.Z += (Level.CSReviewSpheres[i].fInRadius * 0.5000000);
        DrawCircle(vCenterPos, MakeVector(1.0000000, 0.0000000, 0.0000000), MakeVector(0.0000000, 1.0000000, 0.0000000), Level.CSReviewSpheres[i].kColor, Level.CSReviewSpheres[i].fInRadius, 20);
        ++i;
        // [Loop Continue]
        goto J0x171;
    }
...
exec function userzone(string szPath, string szCommand)
{
    ...
    Level.kUserzoneInfo.CalcMapSize(4.0000000);
    bResult = false;
    // End:0x7E
    if((szPath == "del") || szPath == "DEL")
    {
...
defaultproperties
{
    ...
    IconDefendBombHolder=(WidgetTexture=Texture'Warfare_TH_UI_HUD_ETC.DisplayIcon.keep_c4',RenderStyle=5,TextureCoords=(X1=18,Y1=0,X2=110,Y2=128),TextureScale=0.3700000,DrawPivot=5,PosX=0.0000000,PosY=0.0000000,OffsetX=0,OffsetY=0,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
    IconInvulnerable=(WidgetTexture=Texture'Warfare_TH_UI_HUD_ETC.DisplayIcon.invincible_mark',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=64,Y2=64),TextureScale=0.5000000,DrawPivot=5,PosX=0.0000000,PosY=0.0000000,OffsetX=0,OffsetY=0,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
    ...
    DefaultCrosshair=Texture'Warfare_TH_UI_HUD_ETC.Crosshairs.cross_hair_throw'
    TargetCrosshair=Texture'Warfare_TH_UI_HUD_ETC.Crosshairs.cross_hair_throw_target'
    WAttackedBG=(WidgetTexture=Texture'Warfare_HUD.HUD.hud_damage',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=512,Y2=512),TextureScale=0.6500000,DrawPivot=8,PosX=0.0000000,PosY=0.0000000,OffsetX=0,OffsetY=0,ScaleMode=0,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
    ...
    BaseTeamColor[0]=none
    BaseTeamColor[1]=(Y2[3]=// Unable to decompile FixedArray data.)
    BaseTeamColor[2]=(FontArrayNames[4]="UT2003Fonts.FontEurostile17",FontArrayNames[5]="UT2003Fonts.FontEurostile14",FontArrayNames[6]="UT2003Fonts.FontEurostile12",FontArrayNames[7]="UT2003Fonts.FontEurostile9",FontArrayNames[8]="UT2003Fonts.FontSmallText",FontScreenWidthMedium=2048,FontScreenWidthMedium[1]=1600,FontScreenWidthMedium[2]=1280,FontScreenWidthMedium[3]=1024,FontScreenWidthMedium[4]=800,FontScreenWidthMedium[5]=640,FontScreenWidthMedium[6]=512,FontScreenWidthMedium[7]=400,FontScreenWidthMedium[8]=320,FontScreenWidthSmall=4096,FontScreenWidthSmall[1]=3200,FontScreenWidthSmall[2]=2560,FontScreenWidthSmall[3]=2048,FontScreenWidthSmall[4]=1600,FontScreenWidthSmall[5]=1280,FontScreenWidthSmall[6]=1024,FontScreenWidthSmall[7]=800,FontScreenWidthSmall[8]=640)
    FriendlyColor=(R=67,G=246,B=55,A=255)
    MyselfColor=(R=255,G=153,B=0,A=255)
    KillerSprite=(WidgetTexture=Texture'Warfare_TH_UI_HUD_ETC.DisplayIcon.our_sol_killer',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=63,Y2=63),TextureScale=0.6500000,DrawPivot=5,PosX=0.0000000,PosY=0.0000000,OffsetX=0,OffsetY=0,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
