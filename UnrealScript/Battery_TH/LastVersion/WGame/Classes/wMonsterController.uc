class wMonsterController extends ScriptedController;

const MAXSTAKEOUTDIST = 8000;
const ENEMYLOCATIONFUZZ = 4500;
const TACTICALHEIGHTADVANTAGE = 1220;
const MINSTRAFEDIST = 300;
const MINVIEWDIST = 300;
...
{
    return wMonster(Pawn).CanAttack(Other);
    //return;    
}

function StopFiring()
{
    wMonster(Pawn).StopFiring();
    bCanFire = false;
    bFire = 0;
    bAltFire = 0;
    //return;    
...
function bool FindNewEnemy()
{
    ...
    {
        // End:0x1B8
        if(((C.bIsPlayer && C.Pawn != none) && C.Pawn.PlayerReplicationInfo != none) && !C.Pawn.PlayerReplicationInfo.IsDead)
        {
            // End:0x102
            if(BestEnemy == none)
            {
...
function bool SetEnemy(Pawn NewEnemy, optional bool bHateMonster)
{
    ...
    bNewMonsterEnemy = ((bHateMonster && Level.Game.NumPlayers < 4) && !wMonster(Pawn).SameSpeciesAs(NewEnemy)) && !NewEnemy.Controller.bIsPlayer;
    // End:0xE2
    ...
    // End:0x172
    if((!bHateMonster && wMonster(Enemy) != none) && NewEnemy.Controller.bIsPlayer)
    {
        return false;
    }
...
function bool ClearShot(Vector TargetLoc, bool bImmediateFire)
{
    ...
    // End:0x25
    if(VSize(Enemy.Location - TargetLoc) > float(8000))
    {
        return false;
    }
    ...
    return false;
    // End:0x1BF
    if((wMonster(Pawn).SplashDamage() && VSize(Pawn.Location - TargetLoc) < wMonster(Pawn).GetDamageRadius()) || !FastTrace(TargetLoc + (vect(0.0000000, 0.0000000, 0.9000000) * Enemy.CollisionHeight), Pawn.Location))
    {
        StopFiring();
        return false;
    }
...
function CheckIfShouldCrouch(Vector StartPosition, Vector TargetPosition, float Probability)
{
    ...
    // End:0x78
    if(((!Pawn.bCanCrouch || !Pawn.bIsCrouched && FRand() > Probability) || Skill < (float(3) * FRand())) || wMonster(Pawn).RecommendSplashDamage())
    {
        Pawn.bWantsToCrouch = false;
        return;
    }
    GetAxes(Rotation, X, Y, Z);
    projStart = wMonster(Pawn).GetFireStart(X, Y, Z);
...
function EnemyChanged(bool bNewEnemyVisible)
{
    ...
    wMonster(Pawn).PlayChallengeSound();
    //return;    
}
...
function WhatToDoNext(byte CallingByte)
{
    ...
    // End:0x17D
    if(((wMonster(Pawn).bTryToWalk && int(Pawn.Physics) == int(4)) && ChoosingAttackLevel == 0) && ChooseAttackCounter == 0)
    {
        TryToWalk();
    }
    ChoosingAttackLevel++;
...
function ExecuteWhatToDoNext()
{
    ...
    // End:0xAC
    if(bPreparingMove && wMonster(Pawn).bShotAnim)
    {
...
function FightEnemy(bool bCanCharge)
{
    ...
    if((Enemy == FailedHuntEnemy) && Level.TimeSeconds == FailedHuntTime)
    {
        ...
        if(Enemy == FailedHuntEnemy)
        {
            ...
            {
                // End:0x1B5
                if(((LastRespawnTime != Level.TimeSeconds) && (LastSeenTime == float(0)) || (Level.TimeSeconds - LastSeenTime) > float(15)) && !Pawn.PlayerCanSeeMe())
                {
                    LastRespawnTime = Level.TimeSeconds;
                    EnemyVisibilityTime = 0.0000000;
...
function ChooseAttackMode()
{
    ...
    if((Enemy == none) || Pawn == none)
    {
        Log((("HERE 1 Enemy " $ string(Enemy)) $ " pawn ") $ string(Pawn));
    }
...
function bool TestDirection(Vector Dir, out Vector pick)
{
    ...
    pick = Dir * (float(300) + (float(2 * 300) * FRand()));
...
function Rotator AdjustAim(FireProperties FiredAmmunition, Vector projStart, int AimError)
{
    ...
    if((!bClean && Target == Enemy) && bEnemyInfoValid)
    {
        ...
        if(HitActor != none)
        {
            ...
            // End:0x7C9
            if(wMonster(Pawn).SplashDamage() && Skill >= float(4))
            {
                HitActor = Trace(HitLocation, HitNormal, FireSpot, projStart, false);
                // End:0x7C9
                if(HitActor != none)
...
function ReceiveWarning(Pawn shooter, float projSpeed, Vector FireDir)
{
    ...
    DodgeSkill = Skill + wMonster(Pawn).DodgeSkillAdjust;
    // End:0xC0
    if(((((((Pawn.Health <= 0) || DodgeSkill < float(4)) || Enemy == none) || !wMonster(Pawn).bCanDodge) || int(Pawn.Physics) == int(2)) || int(Pawn.Physics) == int(3)) || FRand() > ((0.2000000 * DodgeSkill) - 0.4000000))
    {
        return;
    }
...
function bool ShouldStrafeTo(Actor WayPoint)
{
    ...
    // End:0x19
    if(wMonster(Pawn).bAlwaysStrafe)
    {
        return true;
    }
    // End:0x30
...
{
    ignores MonitoredPawnAlert, EndState, BeginState, PickDestination, Timer, Formation, 
	    CancelCampFor;
...
state TacticalMove
{
    ...
    {
        Destination = Pawn.Location - (float(300) * Normal(Pawn.Velocity));
        //return;        
    }
    ...
    function bool EngageDirection(Vector StrafeDir, bool bForced)
    {
        ...
        MinDest = Pawn.Location + (float(300) * StrafeDir);
        // End:0x16C
        if(!bForced)
        {
        ...
        Destination = MinDest + (StrafeDir * ((0.5000000 * float(300)) + FMin(VSize(Enemy.Location - Pawn.Location), 300.0000000 * (FRand() + FRand()))));
        return true;
        //return;        
    }
...
DoStrafeMove:
    ...
    // End:0x18E
    if(((((Enemy == none) || EnemyVisible()) || !FastTrace(Enemy.Location, LastSeeingPos)) || wMonster(Pawn).PreferMelee()) || !Pawn.bCanStrafe)
    {
        goto 'FinishedStrafe';
    }
RecoverEnemy:
...
state StakeOut
{
    ...
    function FindNewStakeOutDir()
    {
        ...
        if(N != none)
        {
            ...
            dist = VSize(Dir);
            // End:0x109
            if((dist < float(8000)) && dist > float(300))
            {
...
Begin:
    ...
    FinishRotation();
    // End:0xCA
    if((((wMonster(Pawn).HasRangedAttack() && FRand() < 0.5000000) && VSize(Enemy.Location - FocalPoint) < float(150)) && (Level.TimeSeconds - LastSeenTime) < float(4)) && ClearShot(FocalPoint, true))
    {
        FireWeaponAt(Enemy);        
    }
    else
...
Begin:
    ...
    Sleep(0.1000000);
    // End:0x110
    if(((wMonster(Pawn).PreferMelee() || Target == none) || Target != Enemy) || wMonster(Pawn).bBoss)
    {
        WhatToDoNext(35);
    }
    // End:0x142
    ...
    Focus = Target;
    Sleep(FMax(wMonster(Pawn).RangedAttackTime(), 0.2000000 + (((0.5000000 + (0.5000000 * FRand())) * 0.4000000) * (float(7) - Skill))));
    WhatToDoNext(36);
    // End:0x1C0
    if(bSoaking)
    {
