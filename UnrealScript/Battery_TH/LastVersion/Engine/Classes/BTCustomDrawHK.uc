...
{
    return Class'Engine.BTCustomDrawHK'.static.MakeTextColor(fntSize, fntDrawType, txt, Class'Engine.Canvas'.static.MakeColor(byte(255), byte(255), byte(255), byte(255)), Class'Engine.Canvas'.static.MakeColor(0, 0, 0, 100));
    //return;    
}
...
{
    return MakeImageColor(width, Height, dt, img, Class'Engine.Canvas'.static.MakeColor(byte(255), byte(255), byte(255), byte(255)), Class'Engine.Canvas'.static.MakeColor(0, 0, 0, 100));
    //return;    
}
...
static function Color MakeColorRGB(byte R, byte G, byte B)
{
    local Color C;
...
static function DrawTextArrayPadding(Canvas C, array<Text> textArray, BTCustomDrawHK.DrawType dt, BTCustomDrawHK.TextArrayDrawType tadt, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB)
{
    ...
    tW = tR - tL;
    tH = tB - tT;
    Class'Engine.BTCustomDrawHK'.static.DrawTextArray(C, textArray, dt, tadt, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH));
    //return;    
}

static function DrawTextPaddingOffset(Canvas C, Text Text, float tL, float tT, float tR, float tB, int pL, int pT, int pR, int pB)
{
    Class'Engine.BTCustomDrawHK'.static.DrawText(C, Text, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB));
    //return;    
}
...
static function DrawText(Canvas C, Text Text, float tL, float tT, float tR, float tB, optional int PresetIndex, optional bool bUseD3DFont)
{
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawString(C, Text.Text, Text.FontDrawType, float(Text.FontSize), tL, tT, tR, tB, Text.DrawShadowColor, PresetIndex, bUseD3DFont);
    //return;    
}
...
{
    C.DrawColor = Text.DrawColor;
    Class'Engine.BTCustomDrawHK'.static.DrawString_UseModulateColor(C, Text.Text, Text.FontDrawType, float(Text.FontSize), tL, tT, tR, tB, Text.DrawShadowColor, PresetIndex, bUseD3DFont, bUseModulateColor, mColor);
    //return;    
}
...
{
    C.DrawColor = Text.DrawColor;
    Class'Engine.BTCustomDrawHK'.static.DrawStringCache(C, Text.Text, Text.FontDrawType, float(Text.FontSize), tL, tT, tR, tB, Text.DrawShadowColor, Level);
    //return;    
}
...
static function float DrawTextArray(Canvas C, array<Text> textArray, BTCustomDrawHK.DrawType dt, BTCustomDrawHK.TextArrayDrawType tadt, float tL, float tT, float tR, float tB, optional int Level)
{
    ...
    if(i < textArray.Length)
    {
        ...
        {
            Class'Engine.BTCustomDrawHK'.static.DrawEllipsisString(C, textArray[i].Text, 0, float(textArray[i].FontSize + addFontSize), 1.0000000, rX + rSaveW, rY + rSaveH, tR, (rY + rSaveH) + rOneH, textArray[i].DrawShadowColor, Level);
            // [Explicit Break]
            goto J0x587;            
        }
        else
        {
            Class'Engine.BTCustomDrawHK'.static.DrawEllipsisString(C, textArray[i].Text, 0, float(textArray[i].FontSize + addFontSize), 1.0000000, rX + rSaveW, rY + rSaveH, (rX + rSaveW) + rOneW, (rY + rSaveH) + rOneH, textArray[i].DrawShadowColor, Level);
        }
        rSaveW += rOneW;
        i++;
        // [Loop Continue]
...
static function DrawStringPadding(Canvas C, string Text, BTCustomDrawHK.DrawType dt, float FontSize, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB, optional Color BackShadowColor, optional int Level)
{
    ...
    tW = tR - tL;
    tH = tB - tT;
    Class'Engine.BTCustomDrawHK'.static.DrawString(C, Text, dt, FontSize, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH), BackShadowColor, Level);
    //return;    
}
...
{
    Class'Engine.BTCustomDrawHK'.static.DrawString(C, Text, dt, FontSize, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB), BackShadowColor, PresetIndex, bUseD3DFont);
    //return;    
}
...
static function DrawStringModulateAlpha(Canvas C, string Text, BTCustomDrawHK.DrawType dt, float fntSize, float tL, float tT, float tR, float tB, optional Color BackShadowColor, optional int PresetIndex, optional bool bUseD3DFont)
{
    ...
    C.DrawBackColor = Class'Engine.Canvas'.static.MakeColor(0, 0, 0, 200);
    Class'Engine.BTCustomDrawHK'.static.DrawString(C, Text, dt, fntSize, tL, tT, tR, tB, C.DrawBackColor, PresetIndex, bUseD3DFont);
...
static function FloatBox GetDrawStringFloatBox(Canvas C, string Text, BTCustomDrawHK.DrawType dt, float fntSize, float tL, float tT, float tR, float tB, optional Color BackShadowColor)
{
    ...
    switch(dt)
    {
        ...
        default:
            C.FontScaleX = OldScaleX;
            C.FontScaleY = OldScaleY;
            return Class'Engine.BTCustomDrawHK'.static.MakeFloatBox(0.0000000, 0.0000000, 0.0000000, 0.0000000);
            break;
    }
    fb = Class'Engine.BTCustomDrawHK'.static.MakeFloatBox(rX, rY, rX + rW, rY + rH);
    C.FontScaleX = OldScaleX;
    C.FontScaleY = OldScaleY;
    return fb;
    //return;    
...
static function DrawCullingString(Canvas C, string Text, BTCustomDrawHK.DrawType dt, float fntSize, float widthPerc, float tL, float tT, float tR, float tB, bool cullRight, optional Color BackShadowColor, optional int Level)
{
    local float X, Y, tW, tH, rX, rY,
	    rW, rH, textWidthLimit, ellipsisWidth, findWidth,
	    tempH;
...
static function DrawEllipsisStringPadding(Canvas C, string Text, BTCustomDrawHK.DrawType dt, float FontSize, float widthPerc, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB, optional Color BackShadowColor, optional int Level)
{
    ...
    tW = tR - tL;
    tH = tB - tT;
    Class'Engine.BTCustomDrawHK'.static.DrawEllipsisString(C, Text, dt, FontSize, widthPerc, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH), BackShadowColor, Level);
    //return;    
}
...
{
    Class'Engine.BTCustomDrawHK'.static.DrawEllipsisString(C, Text, dt, FontSize, widthPerc, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB), BackShadowColor, Level);
    //return;    
}
...
{
    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, img, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB));
    //return;    
}
...
static function DrawImagePadding(Canvas C, out Image img, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB)
{
    ...
    tW = tR - tL;
    tH = tB - tT;
    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, img, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH));
    //return;    
}
...
static function DrawImagePaddingCache(Canvas C, out Image img, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB, optional int Level)
{
    ...
    tW = tR - tL;
    tH = tB - tT;
    Class'Engine.BTCustomDrawHK'.static.DrawImageCache(C, img, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH), Level);
    //return;    
}
...
static function DrawImage(Canvas C, out Image img, float tL, float tT, float tR, float tB)
{
    ...
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImageOriginal(C, img, tL, tT, tR, tB);        
    }
    else
    {
        // End:0xB5
        ...
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageStretched(C, img, tL, tT, tR, tB);            
        }
        else
        {
            // End:0xFA
            ...
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImageScale(C, img, tL, tT, tR, tB);                
            }
            else
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImageScaleWithClipArea(C, img, tL, tT, tR, tB, img.X, img.Y, img.X + img.width, img.Y + img.Height);
            }
        }
    }
    //return;    
...
static function DrawImageCache(Canvas C, out Image img, float tL, float tT, float tR, float tB, optional int Level)
{
    ...
    if(int(img.DrawType) <= int(8))
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImageOriginalCache(C, img, tL, tT, tR, tB, Level);        
    }
    else
    {
        // End:0xBF
        if(int(img.DrawType) <= int(15))
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageStretchedCache(C, img, tL, tT, tR, tB, Level);            
        }
        else
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageScaleCache(C, img, tL, tT, tR, tB, Level);
        }
    }
    //return;    
}
...
static function string FormatString(string original, optional string arg0, optional string arg1, optional string arg2, optional string arg3, optional string arg4, optional string arg5, optional string arg6, optional string arg7, optional string arg8)
{
    ...
    args[7] = arg7;
    args[8] = arg8;
    return Class'Engine.BTCustomDrawHK'.static.FormatStringArray(original, args);
    //return;    
}
...
static function DrawImageRatio(Canvas C, out Image img, float tL, float tT, float tR, float tB)
{
    // End:0x12
    if(img.Image == none)
    {
    ...
    tT = tT * C.GetClientHeightRatio();
    tB = tB * C.GetClientHeightRatio();
    C.DrawColor = img.DrawColor;
    // End:0xA8
    if(int(img.DrawType) <= int(8))
    {
...
static function DrawTextRatio(Canvas C, Text Text, float tL, float tT, float tR, float tB, optional int PresetIndex, optional bool bUseD3DFont)
{
    C.DrawColor = Text.DrawColor;
    Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, Text.Text, Text.FontDrawType, float(Text.FontSize), tL, tT, tR, tB, Text.DrawShadowColor, PresetIndex, bUseD3DFont);
    //return;    
}

static function DrawImagePaddingRatio(Canvas C, out Image img, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB)
{
    local float tW, tH;

    tW = tR - tL;
    tH = tB - tT;
    Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, img, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH));
    //return;    
}

static function DrawImagePaddingOffsetRatio(Canvas C, out Image img, float tL, float tT, float tR, float tB, int pL, int pT, int pR, int pB)
{
    Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, img, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB));
    //return;    
}

static function DrawImageCacheRatio(Canvas C, out Image img, float tL, float tT, float tR, float tB, optional int Level)
{
    // End:0x12
    if(img.Image == none)
    {
...
static function DrawImagePaddingCacheRatio(Canvas C, out Image img, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB, optional int Level)
{
    local float tW, tH;

    tW = tR - tL;
    tH = tB - tT;
    Class'Engine.BTCustomDrawHK'.static.DrawImageCacheRatio(C, img, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH), Level);
    //return;    
}

static function DrawTextCacheRatio(Canvas C, Text Text, float tL, float tT, float tR, float tB, optional int Level)
{
    tT = tT * C.GetClientHeightRatio();
    tB = tB * C.GetClientHeightRatio();
    C.DrawColor = Text.DrawColor;
...
static function DrawStringRatio(Canvas C, string Text, BTCustomDrawHK.DrawType dt, float fntSize, float tL, float tT, float tR, float tB, optional Color BackShadowColor, optional int PresetIndex, optional bool bUseD3DFont)
{
    local float X, Y, tW, tH, rX, rY,
	    rW, rH;
...
static function DrawStringPaddingOffsetRatio(Canvas C, string Text, BTCustomDrawHK.DrawType dt, float FontSize, float tL, float tT, float tR, float tB, int pL, int pT, int pR, int pB, optional Color BackShadowColor, optional int PresetIndex, optional bool bUseD3DFont)
{
    Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, Text, dt, FontSize, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB), BackShadowColor, PresetIndex, bUseD3DFont);
    //return;    
}

static function float DrawTextArrayRatio(Canvas C, array<Text> textArray, BTCustomDrawHK.DrawType dt, BTCustomDrawHK.TextArrayDrawType tadt, float tL, float tT, float tR, float tB, optional int Level)
{
    local int i;
    local float X, Y, tW, tH, rX, rY,
	    rW, rH, rOneW, rOneH, rSaveW,
...
static function DrawTextPaddingOffsetRatio(Canvas C, Text Text, float tL, float tT, float tR, float tB, int pL, int pT, int pR, int pB)
{
    Class'Engine.BTCustomDrawHK'.static.DrawTextRatio(C, Text, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB));
    //return;    
}
