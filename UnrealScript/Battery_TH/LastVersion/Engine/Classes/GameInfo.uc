...
function PreBeginPlay()
{
    ...
    InitLogging();
    SC = new Class'Engine.GameInfoScriptPart';
...
function InitializeEveryPlayerStarts()
{
    ...
    if(Level.GetMatchMaker().BotTutorial)
    {
        ...
        // End:0xEC
        foreach AllActors(Class'Engine.PlayerStart_Defence', f)
        {
...
event InitGame(string Options, out string Error)
{
    ...
    {
        CurrentGameProfile = LoadDataObject(Class'Engine.GameProfile', "GameProfile", InOpt);
        // End:0xCD
...
function InitSavedLevel()
{
    ...
    {
        Level.ObjectPool = new (XLevel) Class'Engine.ObjectPool';
    }
    //return;    
}
...
{
    return Class'Engine.WaitViewNoTeam';
    //return;    
}
...
function WaitView GetWaitView(Class<WaitView> classWaitView)
{
    ...
    {
        // End:0x2F
        foreach AllActors(Class'Engine.WaitView', wv)
        {
...
event PlayerController Login(string Portal, string Options, out string Error)
{
    ...
    if(HasOption(Options, "Load"))
    {
        ...
        bIsSaveGame = true;
        // End:0x315
        foreach DynamicActors(Class'Engine.PlayerController', TestPlayer)
        {
            // End:0x314
    ...
    if(P != none)
    {
        ...
        if(P.IsA('PlayerController') && P.Pawn == none)
        {
            ...
            else
            {
                ...
                PlayerController(P).ReceiveLocalizedMessage(Class'Engine.wMessage_BGM', Class'Engine.wMessage_BGM'.default.Code_Start);
                // End:0x247
    ...
    if(C != none)
    {
        ...
        if((((PC != none) && !bGameEnded) && !IsInState('MatchOver')) && kMM.InGamePlaying)
        {
            ...
            if((C.GetTeamNum() == Killer.GetTeamNum()) && C.PlayerReplicationInfo != Killer.PlayerReplicationInfo)
            {
                ...
                // End:0x237
                if(fDist < Class'Engine.wMessage_Game'.default.MessageRange)
                {
                    Log(string(self) $ "ProcessKillMessage() iCode_Message [ class'wMessage_Game'.default.Code_AllyKillEnemy] ");
                    iCode_Message = Class'Engine.wMessage_Game'.default.Code_AllyKillEnemy;
                    PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game', iCode_Message, Killer.PlayerReplicationInfo, Killed.PlayerReplicationInfo);
                    // [Explicit Break]
                    goto J0x38C;
                }                
            }
            else
            {
                ...
                if((C.GetTeamNum() == Killed.GetTeamNum()) && C.PlayerReplicationInfo != Killed.PlayerReplicationInfo)
                {
                    ...
                    if(fDist < Class'Engine.wMessage_Game'.default.MessageRange)
                    {
                        ...
                        iCode_Message = Class'Engine.wMessage_Game'.default.Code_AllyDie;
                        PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game', iCode_Message, Killer.PlayerReplicationInfo, Killed.PlayerReplicationInfo);
                        // [Explicit Break]
                        goto J0x38C;
                    }
        ...
        if((((PC != none) && !bGameEnded) && !IsInState('MatchOver')) && kMM.InGamePlaying)
        {
            ...
            if((Killer.PlayerReplicationInfo == C.PlayerReplicationInfo) || Killer.Pawn == Pawn(PC.ViewTarget))
            {
                ...
                if((int(MyCollisionPart) == int(1)) && int(MyWeaponType) != int(13))
                {
                    ...
                    {
                        iCode_Message = Class'Engine.wSpecialKillMessage'.default.Code_MessageScore_Headshot_Aim;                        
                    }
                    else
                    {
                        iCode_Message = Class'Engine.wSpecialKillMessage'.default.Code_MessageScore_Headshot;
                    }
                    PC.ReceiveLocalizedMessage(Class'Engine.wSpecialKillMessage', iCode_Message);
                    iConKillorHeadShot = iConKillorHeadShot | 2;
                }
                // End:0x6D0
                if(bWallShot)
                {
                    iCode_Message = Class'Engine.wSpecialKillMessage'.default.Code_MessageScore_WallShot;
                    PC.ReceiveLocalizedMessage(Class'Engine.wSpecialKillMessage', iCode_Message);
                ...
                if((int(MyCollisionPart) == int(1)) && int(MyWeaponType) != int(13))
                {
                    iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_HeadShotKill;
                    // End:0x778
                    if((PlayerController(Killer) != none) && Killer.PlayerReplicationInfo == C.PlayerReplicationInfo)
                    {
                        PlayerController(Killer).ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', iCode_Message, SC.KillWeaponType, SC.KDLog.KillWeapon);
                    }
                }
                // End:0x843
                if(((Level.GetMatchMaker() != none) && Level.GetMatchMaker().BotTutorial) && int(MyWeaponType) == int(5))
                {
                    iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_KillGrenade;
                    // End:0x843
                    if((PlayerController(Killer) != none) && Killer.PlayerReplicationInfo == C.PlayerReplicationInfo)
                    {
                        PlayerController(Killer).ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', iCode_Message, SC.KillWeaponType, SC.KDLog.KillWeapon);
            ...
            else
            {
                ...
                if(Killed.PlayerReplicationInfo == C.PlayerReplicationInfo)
                {
                    iCode_Message = Class'Engine.wMessage_Game'.default.Code_AllyDie;
                    PC.ReceiveLocalizedMessage(Class'Engine.wMessage_Game', iCode_Message, Killer.PlayerReplicationInfo, Killed.PlayerReplicationInfo);
                    iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_Death;
                    // End:0x909
                    if(PC != none)
                    {
                        PC.ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', iCode_Message, SC.KillWeaponType, SC.KDLog.KillWeapon);
                    }
                }
            }
        }
...
function int ProcessKillMessageMulti(int iArray, Controller ReceiveController, Controller Killer, Controller Other, int KillWeapon)
{
    ...
    PlayerController(ReceiveController).ReceiveLocalizedMessage(Class'Engine.wMultiKillMessage', iArray, Killer.PlayerReplicationInfo);
...
function SendQuest_KillSuccession(Controller Killer, byte byKillSuccession)
{
    ...
    switch(byKillSuccession)
    {
        // End:0xA0
        case 2:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_2Kill;
            // End:0x1AD
            break;
        // End:0xBC
        case 3:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_3Kill;
            // End:0x1AD
            break;
        // End:0xD8
        case 4:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_4Kill;
            // End:0x1AD
            break;
        // End:0xF4
        case 5:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_5Kill;
            // End:0x1AD
            break;
        // End:0x110
        case 6:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_6Kill;
            // End:0x1AD
            break;
        // End:0x12C
        case 7:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_7Kill;
            // End:0x1AD
            break;
        // End:0x148
        case 8:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_8Kill;
            // End:0x1AD
            break;
        // End:0x164
        case 9:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_9Kill;
            // End:0x1AD
            break;
        // End:0x180
        case 10:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_10Kill;
            // End:0x1AD
            break;
        // End:0x19C
        case 15:
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_15Kill;
            // End:0x1AD
            break;
        // End:0xFFFF
        default:
...
{
    return DamageType != Class'Engine.Suicided';
    //return;    
}
...
function Killed(Controller Killer, Controller Killed, Pawn KilledPawn, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
{
    ...
    {
        // End:0x143
        if(((DamageType == Class'Engine.wDamageAirStrike') || strDamageType ~= "WWeapons_Res.wWeaponRes_Artillery") || strDamageType ~= "WWeapons_Res.wWeaponRes_PinpointBomb")
        {            
        }
        else
        {
            // End:0x1AD
            if(((DamageType == Class'Engine.wDamageHelicopter') || strDamageType ~= "WWeapons_Res.wWeaponRes_HeliGun") || strDamageType ~= "WWeapons_Res.wWeaponRes_HeliRPG7")
            {
            }
        }
        updateScore = true;
    ...
    // End:0x2D8
    if((Killed.Pawn != none) && DamageType != Class'Engine.wDamageChangedHost')
    {
...
function bool IsSetKDLogData(Controller Killer, Controller Killed, Pawn KilledPawn, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
{
    ...
    if(bSetKDLog)
    {
        ...
        else
        {
            ...
            {
                // End:0x3D6
                if((DamageType == Class'Engine.wDamageFlameThrower') || strDamageType ~= "WWeapons_Res.wWeaponRes_M2B")
                {
                ...
                else
                {
                    ...
                    {
                        // End:0x5FC
                        if(DamageType == Class'Engine.wDamageHelicopter')
                        {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // End:0x74D
                                    if(((DamageType == Class'Engine.wDamageRPG') || strDamageType ~= "WWeapons_Res.wWeaponRes_RPG7") || strDamageType ~= "WWeapons_Res.wWeaponRes_ATR")
                                    {
                                        // End:0x731
    ...
    if(bHeli || bSentry)
    {
        ...
        if((Killer != none) && kMM != none)
        {
            ...
            if(bHeli)
            {
                ...
                {
                    AllReceiveLocalizedMessage(Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_KilledFlyingDefeceBot, Killer.PlayerReplicationInfo, Killer.PlayerReplicationInfo, DamageType);                    
                }
                else
                {
                    AllReceiveLocalizedMessage(Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_KilledHelicopter, Killer.PlayerReplicationInfo, Killer.PlayerReplicationInfo, DamageType);
                }                
            }
            else
            {
                ...
                if(bSentry)
                {
                    AllReceiveLocalizedMessage(Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_KilledSentryGun, Killer.PlayerReplicationInfo, Killer.PlayerReplicationInfo, DamageType);
                }
            }
            // End:0x4B7
            if(iUid != -1)
...
function Class<DamageType> GetWDamageType(Class<DamageType> DamageType, int CollisionPart, int WeaponType, out int IsHeadShot, out int IsEnableAim)
{
    ...
    switch(MyWeaponType)
    {
        // End:0x8C
        case 11:
            return Class'Engine.wDamageAirStrike';
        // End:0x97
        case 20:
            return Class'Engine.wDamagePinpointBomb';
        // End:0xA2
        case 13:
            return Class'Engine.wDamageHelicopter';
        // End:0xFFFF
        default:
            return DamageType;
            break;
...
{
    BroadcastLocalizedMessage(Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_SuddenDeath);
...
function bool IsAnyPawnWithinRadius(NavigationPoint N)
{
    ...
    PAWN_RADIUS = Class'Engine.Pawn'.default.CollisionRadius;
    lp1 = 0;
    J0x1B:
...
{
    // End:0x1D
    if(N.Class == Class'Engine.PlayerStart')
    {
        return true;        
    }
    else
...
function NavigationPoint GetSafetestRespawnPoint(byte Team, array<NavigationPoint> starts, optional bool skipAppropriateCheck)
{
    ...
    {
        Level.GetLocalPlayerController().myHUD.AddTextMessage("GetSafetestRespawnPoint", Class'Engine.GameMessage', Level.GetLocalPlayerController().PlayerReplicationInfo);
    }
    MAX_TRY = 3;
    SHUFFLE_COUNT = 25;
...
function Pawn PickAlly(byte Team)
{
    ...
    if(bLeaveRespawnLogInChat)
    {
        Level.GetLocalPlayerController().myHUD.AddTextMessage("Picked Ally=" $ pickedAlly.OwnerName, Class'Engine.GameMessage', Level.GetLocalPlayerController().PlayerReplicationInfo);
    }
    return pickedAlly;
    //return;    
}
...
function bool IsSafeFromEnemySight(NavigationPoint N, byte Team)
{
    ...
    Foot = N.Location;
    crouch = Foot;
    crouch.Z += Class'Engine.Pawn'.default.CrouchHeight;
    stand = Foot;
    stand.Z += Class'Engine.Pawn'.default.EyeHeight;
    lp1 = 0;
    J0x125:
    ...
    if(lp1 < enemies.Length)
    {
        ...
        enemyCrouch.Z += Class'Engine.Pawn'.default.CrouchHeight;
        enemyStand = enemyFoot;
        enemyStand.Z += Class'Engine.Pawn'.default.EyeHeight;
        // End:0x259
    ...
    if(lpCandidate < respawnCandidates.Length)
    {
        ...
        if(int(respawnCandidateEnabled[lpCandidate]) == 1)
        {
            ...
            if(IsSafeFromEnemySight(respawnCandidates[lpCandidate], Team))
            {
                ...
                if(bLeaveRespawnLogInChat)
                {
                    Level.GetLocalPlayerController().myHUD.AddTextMessage("Score=" $ string(respawnCandidateScores[lpCandidate]), Class'Engine.GameMessage', Level.GetLocalPlayerController().PlayerReplicationInfo);
                }
...
function LogFinalRespawnPoint(NavigationPoint respawnPoint, byte Team)
{
    ...
    if(bLeaveRespawnLogInChat)
    {
        Level.GetLocalPlayerController().myHUD.AddTextMessage((("Closeset Enemy=" $ Pawn.OwnerName) @ "at dist=") $ string(minDist), Class'Engine.GameMessage', Level.GetLocalPlayerController().PlayerReplicationInfo);
    }
...
function ScoreKill(Controller Killer, Controller Other)
{
    ...
    else
    {
        ...
        if(Killer.PlayerReplicationInfo != none)
        {
            ...
            Other.PlayerReplicationInfo.fExp = FMax(0.0000000, Other.PlayerReplicationInfo.fExp - GameDecExp);
            Other.PlayerReplicationInfo.Point = FMax(0.0000000, Other.PlayerReplicationInfo.Point - GameDecPoint);
            PlayerController(Killer).ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', Class'Engine.wMessage_Quest'.default.Code_Quest_Kill, SC.KillWeaponType, SC.KDLog.KillWeapon);
            // End:0x325
            if(Level.GetMatchMaker().PlayWithBots)
            {
                PlayerController(Killer).ReceiveMessage_QuestType3(Class'Engine.wMessage_Quest', Class'Engine.wMessage_Quest'.default.Code_Quest_KillInBotMode, SC.KillWeaponType, SC.KDLog.KillWeapon);
            }
            // End:0x38D
            if(SC.KDLog.KillWeapon == 99999)
            {
                PlayerController(Killer).ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', Class'Engine.wMessage_Quest'.default.Code_Quest_MeeleAttackKill, SC.KillWeaponType, SC.KDLog.KillWeapon);                
            }
            else
            {
                // End:0x3E7
                ...
                {
                    PlayerController(Killer).ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', Class'Engine.wMessage_Quest'.default.Code_Quest_AimedKill, SC.KillWeaponType, SC.KDLog.KillWeapon);
                }
            }
...
function int GetAssistScore(out byte byAssistType, int iMessageCode)
{
    ...
    {
        // End:0x2E
        case Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist:
            byAssistType = 3;
            fAssistScore = GameAssistScore;
            // End:0x82
            break;
        // End:0x55
        case Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist4:
            byAssistType = 4;
            fAssistScore = 4.0000000;
            // End:0x82
            break;
        // End:0x7C
        case Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist5:
            byAssistType = 5;
...
function SetAssistPoint(Controller Killer, Controller Other)
{
    ...
    if(Other.IsA('wHelicopterController'))
    {
        ...
        iMessageCode = Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist_Special;        
    }
    else
    {
        fAssistScore = GameAssistScore;
        iMessageCode = Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist;
    }
    iTemp = 0;
    J0x9C:
    ...
    if(iTemp < AssistList.Length)
    {
        ...
        if(kMM.IsFixedAssistPoint && Other.Pawn != none)
        {
            ...
            // End:0x180
            if(iMessageCode == Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist)
            {
        ...
        {
            PlayerController(AssistList[iTemp]).ReceiveLocalizedMessage(Class'Engine.wMessage_Score', iMessageCode);
        }
        AssistList[iTemp].ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', Class'Engine.wMessage_Quest'.default.Code_Quest_Assist, SC.KillWeaponType, SC.KDLog.KillWeapon);
        // End:0x33B
...
function SendMessageAddScore(Controller ReceiveController, Controller Killer, Controller Other, int KillWeapon, optional int iConKillorHeadShot)
{
    ...
    else
    {
        ...
        if(Killer.IsA('PlayerController') && addScore != 0)
        {
            ...
            if(Other.IsA('wHelicopterController'))
            {
                PlayerController(ReceiveController).ReceiveLocalizedMessage(Class'Engine.wMessage_Score', Class'Engine.wMessage_Score'.default.Code_MessageScore_Heli_Killed);
                return;
            }
            // End:0xD2
            if(Other.IsA('wAISentryGunController'))
            {
                PlayerController(ReceiveController).ReceiveLocalizedMessage(Class'Engine.wMessage_Score', Class'Engine.wMessage_Score'.default.Code_MessageScore_Sentry);
                return;
            }
            // End:0x107
            if((iConKillorHeadShot & 1) == 0)
            {
                PlayerController(ReceiveController).ReceiveLocalizedMessage(Class'Engine.wMessage_Score', Class'Engine.wMessage_Score'.default.Code_MessageScore_Normal);
            }
            // End:0x131
            ...
            switch(KillWeapon)
            {
                ...
                // End:0x380
                case 5021:
                    iCode_Message = Class'Engine.wSpecialKillMessage'.default.Code_MessageScore_Sentry;
                    PlayerController(ReceiveController).ReceiveLocalizedMessage(Class'Engine.wSpecialKillMessage', iCode_Message);
                    // End:0x3EF
                    break;
                // End:0xFFFF
...
static function FillPlayInfo(PlayInfo PlayInfo)
{
    ...
    {
        Class'Engine.BroadcastHandler'.static.FillPlayInfo(PlayInfo);
    }
    PlayInfo.PopClass();
    // End:0x488
    if(Class'Engine.GameInfo'.default.VotingHandlerClass != none)
    {
        Class'Engine.GameInfo'.default.VotingHandlerClass.FillPlayInfo(PlayInfo);
        PlayInfo.PopClass();        
    }
    else
    {
...
function SendRawMatchResult()
{
    ...
    // End:0x4DA
    foreach kMM.playingLevelInfo.DynamicActors(Class'Engine.PlayerReplicationInfo', PRI)
    {
        // End:0x15A
        if(!PRI.IsConnected)
        {
    ...
    if(C != none)
    {
        ...
        if(P != none)
        {
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_MatchEnd;
            P.ReceiveMessage_QuestType1(Class'Engine.wMessage_Quest', iCode_Message);
            iCode_Message = Class'Engine.wMessage_Quest'.default.Code_Quest_MatchEndInBotMode;
            // End:0x58C
            if(Level.GetMatchMaker().PlayWithBots)
            {
                P.ReceiveMessage_QuestType3(Class'Engine.wMessage_Quest', iCode_Message);
            }
        }
        C = C.nextController;
        // [Loop Continue]
...
defaultproperties
{
    ...
    GIPropDescText[9]="Enable this option to send game statistics to the Battery global stats server"
