...
{
    Gun = wGun(Weapon);
    TexZoom = Texture(DynamicLoadObject(szTexZoomName, Class'Engine.Texture'));
    //return;    
}
...
simulated function RenderScope(Canvas Canv)
{
    ...
    Canv.DrawTile(Texture'Engine.BlackTexture', barW, barH, 0.0000000, 0.0000000, float(Texture'Engine.BlackTexture'.USize), float(Texture'Engine.BlackTexture'.VSize));
    ...
    if(fBlendCurr > float(0))
    {
        ...
        Canv.SetPos(0.0000000, 0.0000000);
        Canv.DrawTile(Texture'Engine.BlackTexture', float(Canv.SizeX), float(Canv.SizeY), 0.0000000, 0.0000000, float(Texture'Engine.BlackTexture'.USize), float(Texture'Engine.BlackTexture'.VSize));
    }
    //return;    
}
