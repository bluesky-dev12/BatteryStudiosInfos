...
const SPECIFIC_BONE_ROTATOR_COUNT = 3;
...
var float StandHeight;
var float StandRadius;
var byte byUseKey;
var bool bCrawler;
...
var bool bNoCoronas;
var bool bEnableSentryGunInstall;
var bool bThumped;
...
var name SentryGunPack_IdleAnim;
var name SentryGunPack_MovementAnims[4];
var name SentryGunPack_PutDown;
var name SentryGunPack_Select_Anim;
var name SentryGunPack_Plant;
var name SentryGunPack_PlantStop;
var name SentryGunPack_TakeoffStillAnim;
var name SentryGunPack_AirStillAnim;
var name SentryGunPack_TakeoffAnims[4];
var name SentryGunPack_AirAnims[4];
var name SentryGunPack_LandAnims[5];
var name SentryGunPack_Weapon_Select_Anim;
var name SentryGunPack_Weapon_Plant_Anim;
var name SentryGunPack_Weapon_Idle_Anim;
var name TurretIdle;
...
var int iDelayedDamageInstigatorID;
var bool bReloading;
...
var float fMinStaminaRatio;
var name naSpecificBoneName[3];
var Rotator rotSpecificBoneRotator[3];
var bool bApplyInverseBoneRotation;
...
replication
{
    ...
    reliable if(bNetDirty && int(Role) == int(ROLE_Authority))
        ...
        bIsCrouched, bIsDefencing, 
        bIsProned, bIsSprinting, 
        bSimulateGravity, bSpecialHUD, 
        byUseKey, naSpecificBoneName;
    ...
    // Pos:0x110
    reliable if(int(Role) < int(ROLE_Authority))
        ServerFmodPlaySound, ServerStopAllSoundByActor, 
...
function ServerFmodPlaySound(string SoundName, optional Actor.ESoundSlot Slot, optional float Pitch, optional bool bNoOverride, optional bool Attenuate, optional bool Loop, optional int SellectTeam, optional bool bExceptOwner)
{
    FmodPlaySound(SoundName, Slot, Pitch, bNoOverride, Attenuate, Loop, SellectTeam, bExceptOwner);
    //return;    
}
...
// Export UPawn::execEnablePositionSentryGunInstall(FFrame&, void* const)
native simulated function bool EnablePositionSentryGunInstall(out Vector v3Normal, out int iUpside)
{
    //native.v3Normal;
    //native.iUpside;        
}
...
function NotifyEquipItemChanged()
{
    //return;    
}
...
function bool CanGrabLadder()
{
    ...
    {
        // End:0x38
        if(int(19) == int(PendingWeapon.WeaponType))
        {
            return false;
        }
    }
    // End:0x46
    if(false == bCanClimbLadders)
    {
        return false;
    }
    // End:0x53
    ...
    // End:0x65
    if(int(11) == int(Physics))
    {
        return false;
    }
    // End:0x77
    if(int(2) != int(Physics))
    {
        return true;
    }
    // End:0x8F
    if(Abs(Velocity.Z) <= JumpZ)
    {
        return true;
    }
    return false;
...
event FellOutOfWorld(Actor.eKillZType KillType)
{
    ...
    {
        Died(none, Class'Engine.FellLava', Location);        
    }
    else
    {
        // End:0x8F
        ...
        {
            Died(none, Class'Engine.fell', Location);            
        }
        else
        {
            // End:0xA4
...
function KilledBy(Pawn EventInstigator)
{
    ...
    Died(Killer, Class'Engine.Suicided', Location);
    //return;    
}
    ...
    if(Velocity.Z < (-0.5000000 * MaxFallSpeed))
    {
        ...
        if(int(Role) == int(ROLE_Authority))
        {
            ...
            if(Velocity.Z < (float(-1) * MaxFallSpeed))
            {
                ...
                {
                    TakeDamage(int(FClamp((-100.0000000 * (EffectiveSpeed + MaxFallSpeed)) / MaxFallSpeed, 0.0000000, 20.0000000)), none, Location, vect(0.0000000, 0.0000000, 0.0000000), Class'Engine.fell');
                }
            }
        }
        // End:0x125
...
function CheckTouchWeaponPickup()
{
    ...
    // End:0x42
    foreach TouchingActors(Class'Engine.wWeaponPickup', WPick)
    {
        // End:0x41
...
simulated function bool SwitchWeapon(byte f, optional bool bQuickGr)
{
    ...
    {
        // End:0x7B
        if((int(Weapon.ClientState) == int(Weapon.5)) || int(Weapon.ClientState) == int(Weapon.6))
        {
            return false;
        }
    }
    ...
    if(int(kMM.eWeaponLimit) != int(kMM.0))
    {
        ...
        if(!Controller.IsPermitSpecialMode(true, int(newWeapon.InventoryGroup), newWeapon.WeaponType, int(f)))
        {
            ...
            {
                PlayerController(Controller).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_HoldWeapon);
            }
            return false;
        }
    }
    ...
    else
    {
        ...
        {
            // End:0x374
            if(int(newWeapon.WeaponType) == int(19))
            {
                // End:0x332
...
simulated function SwitchWeaponWClass(name ClassName, optional string sReason)
{
    ...
    if(sReason ~= "ThrowsTheWeapon")
    {
        ...
        if(newWeapon != none)
        {
            newWeapon.ActionThrowsWeapon(1);
        }
    }
    // End:0x119
    if(Weapon == none)
...
simulated function bool SwitchWeaponByWeaponID(int iItemID, optional bool bQuickGr)
{
    local wWeapon newWeapon;
    local wMatchMaker kMM;
    ...
    if((Weapon != none) && Weapon.Inventory != none)
    {
        newWeapon = Weapon.Inventory.WeaponChangeWID(iItemID);
        // End:0xB2
        if(((int(newWeapon.InventoryGroup) == 2) || int(newWeapon.InventoryGroup) == 3) || int(newWeapon.InventoryGroup) == 4)
        {
            return false;
        }        
    }
    ...
    Log((("1.Bom Name: " $ string(newWeapon.WeaponType)) $ " iItemID : ") $ string(iItemID));
    // End:0x11B
    if(newWeapon == none)
    {
        newWeapon = Inventory.WeaponChangeWID(iItemID);
    }
    Log((("2. Bom type: " $ string(newWeapon.WeaponType)) $ " iItemID : ") $ string(iItemID));
    kMM = Level.GetMatchMaker();
    // End:0x22D
    if(int(kMM.eWeaponLimit) != int(kMM.0))
    {
        // End:0x22D
        if(!Controller.IsPermitSpecialMode(true, int(newWeapon.InventoryGroup), newWeapon.WeaponType, int(newWeapon.InventoryGroup)))
        {
            // End:0x22B
            if((PlayerController(Controller) != none) && int(kMM.eWeaponLimit) == int(kMM.5))
            {
...
function DeleteInventory(Inventory Item)
{
    ...
    // End:0x207
    if(wWeapon(Item) != none)
    {
        // End:0x1D6
        if(int(wWeapon(Item).InventoryGroup) == 5)
        {
        ...
        {
            // End:0x207
            if(int(wWeapon(Item).InventoryGroup) == 6)
            {
...
{
    local PlayerController PC;
    local bool needRefreshSlot5;
    local wWeapon slot6Weapon;
    ...
    if((Controller != none) && Weapon != none)
    {
        ...
        {
            // End:0x28D
            if(int(OldWeapon.InventoryGroup) == 5)
            {
                needRefreshSlot5 = true;                
            }
            else
            {
                // End:0x2D4
                if(int(OldWeapon.InventoryGroup) == 6)
                {
                    slot6Weapon = PC.GetWeaponByInventoryGroup(6);
                    PlayerController(Controller).ClientChangeSlot6(slot6Weapon);
                }
            }
        }
        // End:0x2F3
        ...
        else
        {
            ...
            if(int(Physics) == int(1))
            {
                ...
                else
                {
                    ...
                    {
                        PlayAnim(IdleAnim, 1.0000000, 0.0000000);
                    }
                }
                // End:0x13C
                if(bDoTorsoTwist)
...
singular event BaseChange()
{
    ...
    else
    {
        ...
        {
            footdamage = 100;
            Base.TakeDamage(footdamage, self, Location, 0.5000000 * Velocity, Class'Engine.wDamageStepOn');
            // End:0xA3
        ...
        else
        {
            ...
            if((Decoration(Base) != none) && Velocity.Z < float(-400))
            {
                ...
                Base.TakeDamage(int((((float(-2) * Mass) / decorMass) * Velocity.Z) / float(400)), self, Location, 0.5000000 * Velocity, Class'Engine.Crushed');
            }
        }
    }
    //return;    
...
function TakeHeadRoundBullet(Vector HitLocation, int WeaponType)
{
    ...
    FmodPlaySoundLocal("Warfare_Sound_Char/whizby/char_whizby", 0, 1.0000000,, true,,, false, HitLocation);
    //return;    
}
...
function Controller GetDamagedByKiller(Class<DamageType> DamageType, Pawn instigatedBy)
{
    ...
    {
        // End:0x151
        if(Killer.IsA('wHelicopterController') || Killer.IsA('wAISentryGunController'))
        {
            Killer = Killer.GetCaller();
        }
    }
    return Killer;
    //return;    
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
{
    ...
    if((Health - ActualDamage) <= 0)
    {
        ...
        UpdateDied(ActualDamage, Killer, DamageType, HitLocation, CollisionPart, WeaponType, bWallShot);        
    }
    else
    {
...
function int GetAssistScore(int UID)
{
    ...
    {
        return Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist;
    }
    // End:0x153
    ...
    else
    {
        ...
        {
            return Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist4;            
        }
        else
        {
            return Class'Engine.wMessage_Score'.default.Code_MessageScore_Assist5;
        }
    }
    return 0;
    //return;    
...
function SetDelayedDamageInstigatorID(int iInstigatorID)
{
    iDelayedDamageInstigatorID = iInstigatorID;
    //return;    
}

function int GetDelayedDamageInstigatorID()
{
    return iDelayedDamageInstigatorID;
    //return;    
}
...
function Process_TossWeapon_When_Dying(LevelInfo levenInfo, Pawn Pawn, wWeapon Weapon)
{
    ...
    levenInfo.Game.SC.KDLog.DeathWeapon = Weapon.BaseParams.iWeaponID;
    // End:0xAB
    if(Weapon.Parts.Length > 0)
    {
        levenInfo.Game.SC.KDLog.DeathWeaponPart = Weapon.Parts[0].PartsGroupID;
    }
    // End:0xB8
    if(IsA('wBotVehicle'))
    {
    ...
    // End:0xDA
    if(none != DrivenVehicle)
    {
        // End:0xDA
        if(false == DrivenVehicle.bAllowWeaponToss)
        {
            return;
        }
    }
    ...
    {
        Pawn.Controller.LastPawnWeapon = Weapon.Class;
    }
    Weapon.HolderDied();
    // End:0x18E
    if(IsTossWeapon(levenInfo, Pawn, Weapon))
    {
...
function bool IsTossWeapon(LevelInfo levenInfo, Pawn Pawn, wWeapon Weapon)
{
    ...
    bSkillWeapon = false;
    // End:0x10E
    if(5 == int(Weapon.InventoryGroup))
    {
        ItemParam = levenInfo.GameMgr.GetItemParam(Weapon.BaseParams.iWeaponID);
        // End:0xA9
        ...
        {
            // End:0x10B
            if((int(Pawn.TypeOfWeapon) == int(Weapon.7)) || int(Pawn.TypeOfWeapon) == int(Weapon.8))
            {
                bTossWeapon = true;
            }
        }        
    ...
    // End:0x151
    if(1 != int(Weapon.InventoryGroup))
    {
        // End:0x151
        if(2 != int(Weapon.InventoryGroup))
        {
            // End:0x151
            if(false == bTossWeapon)
            {
                return false;
            }
        }
...
simulated function SetAimMode(bool bToggle)
{
    ...
    if(bIsIdle == true)
    {
        ...
        if(bIsCrouched == true)
        {
            ...
            AnimAction = IdleAnim;
            AnimBlendParams(1, 0.0000000);
            PlayAnim(IdleAnim, 1.0000000, 1.0000000, 0);            
        }
        else
        {
            // End:0x1B5
...
simulated event Tick(float Delta)
{
    ...
    UpdateSpecificBoneRotation(Delta);
    //return;    
}

protected simulated function UpdateSpecificBoneRotation(float fElapsedTime)
{
    local int i;
    ...
    {
        // End:0x44
        if('None' != naSpecificBoneName[i])
        {
            SetBoneRotation(naSpecificBoneName[i], rotSpecificBoneRotator[i]);
        }
        ++i;
        // [Loop Continue]
        goto J0x07;
...
simulated function bool AddSpecificBoneRotator(name nameBone, int iAddIndex)
{
    // End:0x0E
    if(3 <= iAddIndex)
    {
    ...
    naSpecificBoneName[iAddIndex] = nameBone;
    return false;
    //return;    
}

simulated function bool RemoveSpecificBoneRotator(name nameBone, int iAddIndex)
{
    // End:0x0E
    if(3 <= iAddIndex)
    {
    ...
    naSpecificBoneName[iAddIndex] = 'None';
    return false;
    //return;    
}
...
function wWeapon AddWeapon(BtrDouble iUniqueID, int iWeaponID, int iAddPartsID, optional bool bPickupChange, optional int iPainting_Item_ID)
{
    ...
    switch(WBP.eType)
    {
        ...
        // End:0x32F
        case 18:
            W = wWeapon(CreateInventory("WWeapons.wWeapon_SentryGun"));
            // End:0x36C
            break;
        // End:0x369
        case 19:
            W = wWeapon(CreateInventory("WWeapons.wTelegraphPack_Install"));
            // End:0x36C
            break;
        // End:0xFFFF
        default:
...
function GetBoneNameByCollisionID(int CollisionID, out name BoneName)
{
    switch(byte(CollisionID))
    {
        ...
        // End:0x100
        case 18:
            BoneName = 'Bone_SentryG01_Axis';
            // End:0x15D
            break;
        // End:0x113
        case 19:
            BoneName = 'Bone_SentryG01_LF';
            // End:0x15D
            break;
        // End:0x126
        case 20:
            BoneName = 'Bone_SentryG01_RF';
            // End:0x15D
            break;
        // End:0x139
        case 21:
            BoneName = 'Bone_SentryG01_B';
            // End:0x15D
            break;
        // End:0x14C
        case 22:
            BoneName = 'Bone_SentryG01_Weapon';
            // End:0x15D
            break;
        // End:0xFFFF
        default:
...
Begin:
    ...
    if(((false && PlayerReplicationInfo != none) && PlayerReplicationInfo.IsIntervented) && PlayerReplicationInfo.NumLives <= 1)
    {
        switch(Level.GetMatchMaker().kGame_GameMode)
        {
            ...
            // End:0x79
            case Class'Engine.wGameSettings'.static.GetModeIndex_DOA():
            // End:0xF5
...
defaultproperties
{
    ...
    StandHeight=78.0000000
    StandRadius=34.0000000
    bJumpCapable=true
    bCanJump=true
    bCanWalk=true
    bCanDoubleJump=true
