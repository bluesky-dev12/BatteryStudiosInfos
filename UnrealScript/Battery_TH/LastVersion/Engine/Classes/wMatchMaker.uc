...
{
    var int iUserID;
    var int iClearRewardID;
    var int iScoreRewardID;
    var byte byClearRewardTYPE;
    var byte byClearRewardItemCount;
    var byte byScoreRewardTYPE;
    var byte byScoreRewardItemCount;
    var int iReserved1;
    var int iReserved2;
};
...
struct native stATTEND_INFO
{
    var array<byte> StateList;
    var byte IsComplete;
    var array<byte> UpdatedNum;
    var BtrDouble ItemIdx;
    var int RewardItemID;
    var int RewardItemCount;
};

struct native stDAILY_REWARD_ITEM_INFO
{
    var array<byte> Type;
    var array<byte> Number;
    var array<int> ItemID;
    var array<int> ItemCount;
};

var TencentLoginInfo LoginInfo;
var array<string> SelectedServerIPs;
var array<int> SelectedServerPorts;
...
var float TutorialGrade;
var bool bAutoMoveNoviceChannel;
var bool kNotifyGrenadeLimited;
...
var byte kCommunityOptions[4];
var byte kTempCommunityOptions[4];
...
var array<RewardInfo> RewardInfos;
...
var int lastBuyItemID;
var string ERServerIP;
var int ERServerPort;
var int MinimapMode;
var array<int> ChannelRestrictionMaps;
var array<int> ChannelRestrictionModes;
var stATTEND_INFO stAttendInfo;
var stATTEND_INFO stPuzzleInfo;
var stDAILY_REWARD_ITEM_INFO stDailyRewardItemInfo;
var array<string> SpecialQuestRankList;
...
function PI_EnterChannel(int ServerGroupID, string ServerGroupName, int ServerID, string ServerName, int ChannelID, string ChannelName, optional bool bClanWarChannel, optional bool IsDedicatedServer, optional bool IsInNovcieChannel)
{
    ...
    kRoomID = -1;
    kTeamID = -1;
    kIsInNovcieChannel = IsInNovcieChannel;
    ...
    Log("[wMatchMaker::PI_EnterChannel()] kIsInNovcieChannel=" $ string(IsInNovcieChannel));
    //return;    
}
...
function UpdateQuest(int MissionType, int MissionOption1, int MissionOption2, int AddCount)
{
    ...
    if(i < MissionList.Length)
    {
        ...
        {
            // End:0xE4
            if(((MissionList[i].MissionBotMode == 0) && InGamePlaying) && Class'Engine.wGameSettings'.static.IsBotModeIndex(kGame_GameMode))
            {
                // [Explicit Continue]
                goto J0x3D7;
            }
            // End:0x124
            if(((MissionList[i].QuestType != 8) && InGamePlaying) && kGame_GameMode == Class'Engine.wGameSettings'.static.GetModeIndex_BotTutorial())
            {
                // [Explicit Continue]
                goto J0x3D7;
            }
...
// Export UwMatchMaker::execGetParamsFileName(FFrame&, void* const)
native final function string GetParamsFileName();

// Export UwMatchMaker::execGetParamsExcelPathFileName(FFrame&, void* const)
native final function string GetParamsExcelPathFileName();
...
event AddServerIP(string ServerIP)
{
    SelectedServerIPs[SelectedServerIPs.Length] = ServerIP;
    //return;    
}
...
event BTPingCheckerLinker CreatePingChecker()
{
    ...
    if(PingChecker == none)
    {
        PingChecker = new Class'Engine.BTPingCheckerLinker';
...
function bool StartNetworkChecking(int ClientsCount, bool bConnectGame)
{
    ...
    if(bPendingStarted == true)
    {
        ...
        __NFUN_270__("LoadingInfo - bPendingStarted ==          true");
        return false;
    }
    // End:0x104
    if(bIsOwner == true)
    ...
    if(kIsDSHost)
    {
        ...
        if((CreateLoadingStater(kUserName)) == false)
        {
            ...
            __NFUN_270__("LoadingInfo - Failed create loading stater");
            return false;
        }        
    }
    else
...
event EndMatch(optional bool bForceEndMatch)
{
    ...
    if(InGamePlaying == true)
    {
        ...
        // End:0x113
        if(self.IsNewDefenceMode())
        {
            // End:0x113
            if(self.kNeedUpdateItemList)
            {
                self.kTcpChannel.sfReqItemList(self.kUID);
            }
        }        
    }
    else
...
{
    MapSettings = new Class'Engine.wMapSettings';
...
event Material GetLoadingImage()
{
    ...
    resourceName = "Warfare_TH_UI_Map.Loadingimage.Loading_BasicTraining";
    // End:0x5B
    if(BotTutorial)
    {
        return Material(DynamicLoadObject(resourceName, Class'Engine.Material'));
    }
    return Material(DynamicLoadObject(MapSettings.GetMapInfo(szMapName).ResourceLoadingImage, Class'Engine.Material'));
    //return;    
}
...
event Material GetMiniMapInfoImage()
{
    ...
    {
        // End:0x19
        case Class'Engine.wGameSettings'.static.GetModeIndex_TeamDeath():
        // End:0x2B
        case Class'Engine.wGameSettings'.static.GetModeIndex_DeathMatch():
        // End:0x3D
        case Class'Engine.wGameSettings'.static.GetModeIndex_BotTeamDeath():
        // End:0x4F
        case Class'Engine.wGameSettings'.static.GetModeIndex_BotDeathMatch():
        // End:0x85
        case Class'Engine.wGameSettings'.static.GetModeIndex_BotTeamDeathBeginner():
        ...
        case Class'Engine.wGameSettings'.static.GetModeIndex_SD():
            ...
            break;
        // End:0xB3
        case Class'Engine.wGameSettings'.static.GetModeIndex_Domination():
        // End:0xF3
        case Class'Engine.wGameSettings'.static.GetModeIndex_BotDomination():
            switch(kGame_WeaponLimit)
            {
                // End:0xD3
                case int(1):
        ...
        // End:0x117
        case Class'Engine.wGameSettings'.static.GetModeIndex_BotTutorial():
        // End:0x12B
        case Class'Engine.wGameSettings'.static.GetModeIndex_Defence():
            return none;
        // End:0xFFFF
        default:
            break;
    }
    return Material(DynamicLoadObject(MapSettings.GetMapInfo(szMapName).ResourceMiniMapInfoImage[ImageIndex], Class'Engine.Material'));
    //return;    
}
...
{
    return kPCBangType >= 0;
    //return;    
}

function bool IsPlayingPCBangPlus()
{
    return kPCBangType >= 1;
    //return;    
}
...
function SetLastBuyItemInfo(string ItemName, int ItemType, int ItemGrade, int ItemID)
{
    lastBuyItemName = ItemName;
    lastBuyItemType = ItemType;
    lastBuyItemGrade = ItemGrade;
    lastBuyItemID = ItemID;
    //return;    
}
...
function string GetDefenceGameInfo()
{
    local string sGameinfo;

    sGameinfo = "WMission.wNDFGameInfo";
    return sGameinfo;
    //return;    
}

function bool IsNewDefenceMode()
{
    return kGame_GameMode == Class'Engine.wGameSettings'.static.GetModeIndex_Defence();
    //return;    
}

function bool IsEnableUsingItem()
{
    local bool bEnable;

    bEnable = kGame_GameMode == Class'Engine.wGameSettings'.static.GetModeIndex_Defence();
    return bEnable;
    //return;    
}
