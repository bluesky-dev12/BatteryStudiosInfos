...
{
    BuyExpiredItemList.Insert(0, 1);
    BuyExpiredItemList[0] = ItemID;
    BuyExpiredSlotPosList.Insert(0, 1);
    BuyExpiredSlotPosList[0] = SlotPos;
    BuyExpiredUniqueList.Insert(0, 1);
    BuyExpiredUniqueList[0] = UniqueID;
    BuyExpiredCashInfoList[0].CashElements.Insert(0, 1);
    BuyExpiredCashInfoList[0].CashElements[0] = CategoryID;
    BuyExpiredCashInfoList[1].CashElements.Insert(0, 1);
    BuyExpiredCashInfoList[1].CashElements[0] = PackageID;
    BuyExpiredCashInfoList[2].CashElements.Insert(0, 1);
    BuyExpiredCashInfoList[2].CashElements[0] = PriceID;
    BuyExpiredCashInfoList[3].CashElements.Insert(0, 1);
    BuyExpiredCashInfoList[3].CashElements[0] = cashType;
    //return;    
}
...
function SetExpiredData(array<BTExpiredItem> inputData)
{
    local int i;

    ExpiredItems = inputData;
    i = 0;
    J0x12:

    // End:0x85 [Loop If]
    if(i < ExpiredItems.Length)
    {
        AddItem(ExpiredItems[i].ItemTID, ExpiredItems[i].ItemUID, ExpiredItems[i].SkillUID, 0, int(ExpiredItems[i].SlotPos));
        ++i;
        // [Loop Continue]
        goto J0x12;
    }
...
{
    local int i, iSlotPosition, ReservedItemID;
    ...
    if(i < TPItemList.ItemList.DataPool.Length)
    {
        iSlotPosition = TPItemList.ItemList.DataPool[i].instanceInfo.SlotPosition;
        // End:0x126
        ...
        if(TPItemList.ItemList.DataPool[i].bSelect)
        {
            ...
            {
                PushExpiredItemID(TPItemList.ItemList.DataPool[i].ItemInfo.ItemID, ReservedUniqueID, TPItemList.ItemList.DataPool[i].instanceInfo.cashItemInfo.WZCategorySeq, TPItemList.ItemList.DataPool[i].instanceInfo.cashItemInfo.WZPackageSeq, TPItemList.ItemList.DataPool[i].instanceInfo.cashItemInfo.WZPriceSeq[0], iSlotPosition, selectCashType[i]);                
            }
            else
            {
                PushExpiredItemID(TPItemList.ItemList.DataPool[i].ItemInfo.ItemID, ReservedUniqueID, -1, -1, -1, iSlotPosition, selectCashType[i]);
            }
            // [Explicit Continue]
            goto J0x440;
        }
        ...
        if(TPItemList.ItemList.DataPool[i].instanceInfo.UniqueSkillID == 0)
        {
            ...
            GameMgr.RemoveInstanceItem(TPItemList.ItemList.DataPool[i].instanceInfo.UniqueID);
            // [Explicit Continue]
            goto J0x440;
        }
        GameMgr.RemoveQuickSlotBoxSkillItem(TPItemList.ItemList.DataPool[i].instanceInfo.UniqueSkillID);
        GameMgr.RemoveInstanceItemByUniqueSkillID(TPItemList.ItemList.DataPool[i].instanceInfo.UniqueSkillID);
        J0x440:
...
function AddItem(int ItemID, BtrDouble UniqueItemID, int UniqueSkillID, int PartID, int SlotPosition)
{
    ...
    GameMgr = Controller.ViewportOwner.Actor.Level.GameMgr;
    // End:0x79
    if((TPItemList.ItemList.FindPoolItem(UniqueItemID) != none) || TPItemList.ItemList.FindPoolSkillItem(UniqueSkillID) != none)
    {
        return;
    }
...
{
    MainMenu.TcpChannel.sfReqExpandSlotConfirm();
