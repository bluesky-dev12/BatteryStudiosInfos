...
var FloatBox fbAdvertise[5];
var string AdvertiseURL[5];
...
function Internal_GetFocus()
{
    ...
    {
        // End:0xC0
        if((AdvertiseURL[i] != "") && (fbAdvertise[i].X2 - fbAdvertise[i].X1) != float(0))
        {
            PlayerOwner().OpenWeb(AdvertiseURL[i], int(fbAdvertise[i].X1), int(fbAdvertise[i].Y1), int(fbAdvertise[i].X2), int(fbAdvertise[i].Y2));
        }
        i++;
        // [Loop Continue]
        goto J0x16;
