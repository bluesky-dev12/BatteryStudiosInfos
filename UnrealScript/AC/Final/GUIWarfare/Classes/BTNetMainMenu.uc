...
struct SendQuickSlotInfo
{
    var array<byte> iQuickSlotOrders;
    var array<int> iItemIDs;
    var array<BtrDouble> dwUniqueIDs;
    var array<byte> iSlotPosKeys;
    var array<byte> iSlotPosValues;
};

struct UpdateQuickSlotInfo
{
    var byte iQuickSlotIndex;
    var int iItemID;
    var BtrDouble dwUniqueID;
    var Object.SLOT_POS eNewSlotPos;
    var bool bEquipped;
    var bool bSendServer;
    var bool bIsSkill;
};
...
var array<int> NoviceChannelIndex;
...
var export editinline BTPageRoomLobby RoomLobby;
var export editinline BTPageRoomLobby RoomLobbyDefence;
var export editinline BTPageMatchResult page_MatchResult2;
var export editinline BTPageMatchResult MatchResult2;
var export editinline BTPageMatchResult MatchResult2_Defence;
var string pagename_RoomLobby;
var string pagename_RoomLobby_Defence;
var string pagename_MatchResult2;
var string pagename_MatchResult2_Defence;
...
var export editinline BTPageLucky page_Lucky;
var string pagename_Lucky;
...
var array<LoginServerStruct> LoginServers;
var byte iTryLoginCount;
var byte iMaxTryLoginCount;
...
var byte UserInfoWindowShowTab;
...
function AddExpiredItem(bool bSkill, byte DisplayOrder, byte SlotPos, int ItemTID, BtrDouble ItemUID)
{
    ExpiredItems.Length = ExpiredItems.Length + 1;
    ExpiredItems[ExpiredItems.Length - 1] = new Class'GUIWarfareControls.BTExpiredItem';
    ExpiredItems[ExpiredItems.Length - 1].DisplayOrder = DisplayOrder;
    ExpiredItems[ExpiredItems.Length - 1].SlotPos = SlotPos;
    ExpiredItems[ExpiredItems.Length - 1].ItemTID = ItemTID;
    // End:0xB0
    if(bSkill)
    {
        ExpiredItems[ExpiredItems.Length - 1].SkillUID = __NFUN_923__(ItemUID);        
    }
    else
    {
        ExpiredItems[ExpiredItems.Length - 1].ItemUID = ItemUID;
    }
    //return;    
}
...
function bool CheckExpiredItemList()
{
    ...
    if(ExpiredItems.Length > 0)
    {
        ...
        BTWindow.SetExpiredData(ExpiredItems);
...
function AddServerChannelList(int ServerID, int ChannelID, string ServerIP, int serverPort, int ServerDataPort, string ServerName, string ServerShortName, string ChannelName, int ServerGroupID, string ServerGroupName, int IsClanChannel, string ChannelNickName, bool IsDedicatedServer, float ChannelOption[19], optional int nCurUser, optional int nMaxUser)
{
    local int AddIdx, i;
    ...
    if(i < 19)
    {
        SCIList[AddIdx].ChannelOption[i] = ChannelOption[i];
        i++;
        // [Loop Continue]
        goto J0x143;
    }
    // End:0x1C0
    if((SCIList[AddIdx].ChannelOption[17] != float(0)) && SCIList[AddIdx].IsClanChannel == 0)
    {
        NoviceChannelIndex[NoviceChannelIndex.Length] = AddIdx;
    }
...
function bool TutorialChannelAutoMove()
{
    local int i, j;
    local bool bContinue;

    // End:0xBA
    if(rMM.TutorialGrade <= 1.5000000)
    {
        // End:0x2C
        if(rMM.kIsInNovcieChannel)
        {
            return true;
        }
        i = 0;
        ...
        // End:0xB7 [Loop If]
        if(i < NoviceChannelIndex.Length)
        {
            // End:0xAD
            if(SCIList[NoviceChannelIndex[i]].nMaxUser > SCIList[NoviceChannelIndex[i]].nCurUser)
            {
                StartAutoMove(SCIList[NoviceChannelIndex[i]].ServerID, SCIList[NoviceChannelIndex[i]].ChannelID, -1);
                return true;
            }
            i++;
            // [Loop Continue]
    ...
    else
    {
        ...
        if(i < SCIList.Length)
        {
            ...
            // End:0x13D [Loop If]
            if(j < NoviceChannelIndex.Length)
            {
                // End:0x133
                if((i == NoviceChannelIndex[j]) || SCIList[i].IsClanChannel != 0)
                {
                    bContinue = true;
                }
                j++;
            ...
            // End:0x1A3
            if(SCIList[i].nMaxUser > SCIList[i].nCurUser)
            {
                StartAutoMove(SCIList[i].ServerID, SCIList[i].ChannelID, -1);
                return true;
            }
            J0x1A3:
...
function StartAutoMove(int Server, int Channel, int gameroom, optional string Password, optional int ClanMatch_EnterState)
{
    ...
    // End:0x30A
    if((((page_Lobby2 != none) && page_curr == page_Lobby2) || (page_RoomLobby != none) && page_curr == page_RoomLobby) || (page_MatchResult2 != none) && page_curr == page_MatchResult2)
    {
...
function KeepMoving()
{
    ...
    if(AutoMoveDirectionOut)
    {
        ...
        {
            // End:0x50C
            if(((page_RoomLobby != none) && page_RoomLobby == page_curr) || (page_MatchResult2 != none) && page_curr == page_MatchResult2)
            {
                __NFUN_270__((("AutoMoveDirectionOut=" $ string(AutoMoveDirectionOut)) $ ", AutoMoveState=") $ string(AutoMoveState));
...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    {
        bFind = LoadLoginServerList_Ini();
        // End:0x2CB
        if(bFind == false)
        {
            LoginServers.Length = 1;
            // End:0x186
            if("NA" == Controller.ViewportOwner.Actor.Level.AuthLogin_RegionKey)
            {
                LoginServers[0].szIP = "acuslogin01.webzen.com";                
            }
            else
            {
                // End:0x1E0
                ...
                {
                    LoginServers[0].szIP = "aceulogin01.webzen.com";                    
                }
                else
                {
                    // End:0x23A
                    ...
                    {
                        LoginServers[0].szIP = "acsalogin01.webzen.com";                        
                    }
                    else
                    {
                        LoginServers[0].szIP = "";
            ...
            Log((((string(self) $ " AuthLogin_RegionKey :") $ Controller.ViewportOwner.Actor.Level.AuthLogin_RegionKey) $ "/ szLoginServerIP : ") $ LoginServers[0].szIP);
            LoginServers[0].iPort = 56000;
        }
        UpdateBestLoginServer();        
    }
    else
    {
        bFind = LoadLoginServerList_Ini();
        ...
        if(bFind == false)
        {
            ...
            {
                LoginServers[0].szIP = "218.234.76.43";                
            }
            else
            {
                LoginServers[0].szIP = "202.31.212.65";
            }
            LoginServers[0].iPort = 55000;
        }
        UpdateBestLoginServer();
    }
    Log(((("page_Main.TcpLogin.Connect( " $ szLoginServerIP) $ ", ") $ string(szLoginServerPort)) $ " ) Start");
    rMM.bServerConnected = page_Main.TcpLogin.ConnectDomain(szLoginServerIP, szLoginServerPort);
...
function bool LoadLoginServerList_Ini()
{
    local string strTemp;
    local int lp1;
    local bool haveIP;
    ...
    haveIP = false;
    LoginServers.Length = 0;
    strTemp = Controller.ViewportOwner.Actor.ConsoleCommand("GETTEST_STRING TEST SERVER_COUNT");
    // End:0x197
    if(strTemp != "")
    {
        LoginServers.Length = int(strTemp);
        // End:0x81
        ...
        // End:0x192 [Loop If]
        if(lp1 < LoginServers.Length)
        {
            strTemp = ("GETTEST_STRING TEST SERVER_IP[" $ string(lp1)) $ "]";
            strTemp = Controller.ViewportOwner.Actor.ConsoleCommand(strTemp);
            LoginServers[lp1].szIP = strTemp;
            strTemp = ("GETTEST_STRING TEST SERVER_PORT[" $ string(lp1)) $ "]";
            strTemp = Controller.ViewportOwner.Actor.ConsoleCommand(strTemp);
            LoginServers[lp1].iPort = int(strTemp);
            ++lp1;
            // [Loop Continue]
            goto J0x88;
        }
    ...
    else
    {
        ...
        if((strTemp != "") && haveIP)
        {
            LoginServers[0].iPort = int(strTemp);
            return true;
        }
    }
    return false;
...
function InitializeTcpLogin(string IP, int Port)
{
    ...
    TcpLogin.ConnectDomain(IP, Port);
    //return;    
}
...
function HaltNetwork()
{
    ...
    if(resultChannel)
    {
        page_curr.TcpChannel = none;
    }
    // End:0x94
    if(resultLogin)
    {
        page_curr.TcpLogin = none;
    }
...
function SetPage(Object.EPage PageType, bool CloseLastMenu)
{
    ...
    switch(PageType)
    {
        ...
        case 10:
            // End:0x2F0
            if(rMM.szGameClass ~= rMM.GetDefenceGameInfo())
            {
                Controller.OpenMenu(pagename_RoomLobby_Defence);
                // End:0x2E2
                if(RoomLobby == none)
                {
                    RoomLobby = BTPageRoomLobby(Controller.FindPersistentMenuByName(pagename_RoomLobby_Defence));
                }
                page_RoomLobby = RoomLobby;                
            }
            else
            {
                Controller.OpenMenu(pagename_RoomLobby);
                // End:0x32E
                if(RoomLobbyDefence == none)
                {
                    RoomLobbyDefence = BTPageRoomLobby(Controller.FindPersistentMenuByName(pagename_RoomLobby));
                }
                page_RoomLobby = RoomLobbyDefence;
            }
            // End:0x6AF
            break;
        // End:0x3FB
        case 12:
            ...
            if(rMM.szGameClass ~= rMM.GetDefenceGameInfo())
            {
                Controller.OpenMenu(pagename_MatchResult2_Defence);
                // End:0x3A1
                if(MatchResult2 == none)
                {
                    MatchResult2 = BTPageMatchResult(Controller.FindPersistentMenuByName(pagename_MatchResult2_Defence));
                }
                page_MatchResult2 = MatchResult2;                
            }
            else
            {
                Controller.OpenMenu(pagename_MatchResult2);
                // End:0x3ED
                if(MatchResult2_Defence == none)
                {
                    MatchResult2_Defence = BTPageMatchResult(Controller.FindPersistentMenuByName(pagename_MatchResult2));
                }
                page_MatchResult2 = MatchResult2_Defence;
            }
            // End:0x6AF
            break;
        // End:0x441
        ...
        // End:0x6AC
        case 27:
            Controller.OpenMenu(pagename_Lucky);
            page_Lucky = BTPageLucky(Controller.FindMenuByClass(Class'GUIWarfare_Decompressed.BTPageLucky'));
            // End:0x6AF
            break;
        // End:0xFFFF
        default:
...
function TcpLogin_OnConnect(int ErrCode)
{
    ...
    {
        // End:0x12B
        if(int(iTryLoginCount) >= int(iMaxTryLoginCount))
        {
...
function TcpLogin_OnClose(int ErrCode)
{
    ...
    Exit_LoginServer(ErrCode);
    //return;    
}

function TcpLogin_OnError(int ErrCode)
{
    Log("[BTPageLogin::TcpLogin_OnError] ErrCode=" $ string(ErrCode));
    Class'GUIWarfareControls.BTWindowDefineStateHK'.static.CloseWindowAllByName(Controller, Class'GUIWarfareControls.BTWindowDefineStateHK');
    // End:0x111
    if(ErrCode == 0)
    {
...
function TryConnect_Login()
{
    ...
    __NFUN_270__((("[FAILED: TryConnect]  IP-" $ szLoginServerIP) $ ", Port-") $ string(szLoginServerPort));
    rMM.bServerConnected = TcpLogin.Connect(szLoginServerIP, szLoginServerPort);
    Class'GUIWarfareControls.BTWindowDefineStateHK'.static.ShowState(Controller, 7);
    //return;    
}

function Exit_LoginServer(int ErrCode)
{
    Class'GUIWarfareControls.BTWindowDefineInfoHK'.static.ShowInfo(Controller, 7, string(ErrCode));
    BTWindowDefineInfoHK(Controller.TopPage()).__OnOK__Delegate = Exit_OnOK;
...
function SortLoginServerList()
{
    local int lp1, lp2;
    local LoginServerStruct temp;

    lp1 = 0;
    J0x07:

    // End:0x223 [Loop If]
    if(lp1 < (LoginServers.Length - 1))
    {
        lp2 = lp1 + 1;
        J0x28:

        // End:0x219 [Loop If]
        if(lp2 < LoginServers.Length)
        {
            // End:0x20F
            if((int(LoginServers[lp1].iFaild) > int(LoginServers[lp2].iFaild)) || (int(LoginServers[lp1].iFaild) == int(LoginServers[lp2].iFaild)) && LoginServers[lp1].fTimeStamp > LoginServers[lp2].fTimeStamp)
            {
                temp.iFaild = LoginServers[lp1].iFaild;
                temp.iPort = LoginServers[lp1].iPort;
                temp.szIP = LoginServers[lp1].szIP;
                temp.fTimeStamp = LoginServers[lp1].fTimeStamp;
                LoginServers[lp1].iFaild = LoginServers[lp2].iFaild;
                LoginServers[lp1].iPort = LoginServers[lp2].iPort;
                LoginServers[lp1].szIP = LoginServers[lp2].szIP;
                LoginServers[lp1].fTimeStamp = LoginServers[lp2].fTimeStamp;
                LoginServers[lp2].iFaild = temp.iFaild;
                LoginServers[lp2].iPort = temp.iPort;
                LoginServers[lp2].szIP = temp.szIP;
                LoginServers[lp2].fTimeStamp = temp.fTimeStamp;
            }
            ++lp2;
            // [Loop Continue]
            goto J0x28;
...
function SortLoginServerList2(int fromIdx)
{
    local LoginServerStruct temp;

    temp.iFaild = LoginServers[0].iFaild;
    temp.iPort = LoginServers[0].iPort;
    temp.szIP = LoginServers[0].szIP;
    temp.fTimeStamp = LoginServers[0].fTimeStamp;
    LoginServers[0].iFaild = LoginServers[fromIdx].iFaild;
    LoginServers[0].iPort = LoginServers[fromIdx].iPort;
    LoginServers[0].szIP = LoginServers[fromIdx].szIP;
    LoginServers[0].fTimeStamp = LoginServers[fromIdx].fTimeStamp;
    LoginServers[fromIdx].iFaild = temp.iFaild;
    LoginServers[fromIdx].iPort = temp.iPort;
    LoginServers[fromIdx].szIP = temp.szIP;
    LoginServers[fromIdx].fTimeStamp = temp.fTimeStamp;
    //return;    
}

function UpdateBestLoginServer()
{
    local int lp1;

    // End:0x3F
    if((szLoginServerIP != "") && LoginServers.Length >= 1)
    {
        ++LoginServers[0].iFaild;
        SortLoginServerList();
        lp1 = 0;
        ++iTryLoginCount;        
    }
    ...
    {
        lp1 = Rand(LoginServers.Length);
        // End:0x63
        if(lp1 != 0)
        {
            SortLoginServerList2(lp1);
        }
    }
    szLoginServerIP = LoginServers[0].szIP;
    szLoginServerPort = LoginServers[0].iPort;
    LoginServers[0].fTimeStamp = Controller.ViewportOwner.Actor.Level.TimeSeconds;
    Log((("[BTNetMainMenu::UpdateBestLoginServer] IP : " $ szLoginServerIP) $ " Port : ") $ string(szLoginServerPort));
    //return;    
}
...
function CheckWebzenShopItemList()
{
    ...
    {
        // End:0x1DE
        if((((GameMgr.kWebzenShopVer_Year == -1) && GameMgr.kWebzenShopVer_YearID == -1) && GameMgr.kWebzenShopVer_SalesZoneCode == -1) || GameMgr.kWZPackageList.Length == 0)
        {
            Log("[BTNetMainMenu:::CheckWebzenShopItemList] sfReqWebzenShopScriptVersion()");
            __NFUN_270__("[BTNetMainMenu:::CheckWebzenShopItemList] sfReqWebzenShopScriptVersion()");
...
function bool EquipQuickSlot(int iQuickSlotIdx, Object.SLOT_POS eSlotPos, int iItemID, BtrDouble dwUniqueID, bool bIsSkill)
{
    local bool bRet;

    Log((("BTNetMainMenu::EquipQuickSlot] iQuickSlotIdx : " $ string(iQuickSlotIdx)) $ " iSlotPosition : ") $ string(int(eSlotPos)));
    bRet = AddEquippedItemInfo(iQuickSlotIdx, iItemID, eSlotPos, dwUniqueID, true, bIsSkill);
    return bRet;
    //return;    
}

function bool UnEquipQuickSlot(int iQuickSlotIdx, Object.SLOT_POS eSlotPos, bool bIsSkill)
{
    local int iItemID;
    ...
    Log((("BTNetMainMenu::UnEquipQuickSlot] iQuickSlotIdx : " $ string(iQuickSlotIdx)) $ " iSlotPosition : ") $ string(int(eSlotPos)));
    GameMgr = Controller.ViewportOwner.Actor.Level.GameMgr;
    GameMgr.FindQuickSlotBoxBySlotPosition(iQuickSlotIdx, eSlotPos, iItemID, dwUniqueID);
    // End:0x118
    if(iQuickSlotIdx == 0)
    {
        // End:0xEB
        if((int(eSlotPos) == int(0)) && iItemID == GameMgr.GetMainWeaponID())
        {
            return false;            
        }
        else
        {
            // End:0x118
            if((int(eSlotPos) == int(1)) && iItemID == GameMgr.GetSubWeaponID())
            {
                return false;
            }
        }
    ...
    // End:0x145
    if((int(eSlotPos) == int(4)) && iItemID == GameMgr.GetDefaultMeleeWeaponID())
    {
        return false;
    }
    bRet = AddEquippedItemInfo(iQuickSlotIdx, iItemID, eSlotPos, dwUniqueID, false, bIsSkill);
    return bRet;
    //return;    
}

function bool AddEquippedItemInfo(int iQuickSlotIdx, int iItemID, Object.SLOT_POS eSlotPos, BtrDouble dwUniqueID, bool bEquipped, bool bIsSkill)
{
    local UpdateQuickSlotInfo kUpdateQuickSlotInfo;

    // End:0xA5
    if((iItemID != 0) && int(eSlotPos) != int(13))
    {
        kUpdateQuickSlotInfo.bEquipped = bEquipped;
        kUpdateQuickSlotInfo.eNewSlotPos = eSlotPos;
        kUpdateQuickSlotInfo.iItemID = iItemID;
        kUpdateQuickSlotInfo.dwUniqueID = dwUniqueID;
        kUpdateQuickSlotInfo.iQuickSlotIndex = byte(iQuickSlotIdx);
        kUpdateQuickSlotInfo.bSendServer = false;
        kUpdateQuickSlotInfo.bIsSkill = bIsSkill;
        UpdateQuickSlotItemList[UpdateQuickSlotItemList.Length] = kUpdateQuickSlotInfo;        
    }
    else
    {
        Log("BTNetMainMenu::AddEquippedItemInfo] QuickSlot Request Fail!");
        return false;
    }
    return true;
    //return;    
...
function bool EquipOnlyModeItem_QS(int iItemID, Object.SLOT_POS eSlotPos, BtrDouble dwUniqueID, bool bIsSkill)
{
    local bool bRet;

    Log((("BTNetMainMenu::EquipOnlyModeItem] iItemID : " $ string(iItemID)) $ " SlotPos : ") $ string(eSlotPos));
    bRet = AddEquippedItemInfo(255, iItemID, eSlotPos, dwUniqueID, true, bIsSkill);
    return bRet;
    //return;    
}

function bool UnequipOnlyModeItem_QS(int iItemID, Object.SLOT_POS eSlotPos, BtrDouble dwUniqueID, bool bIsSkill)
{
    local bool bRet;

    Log("BTNetMainMenu::UnequipOnlyModeItem] slotpos : " $ string(eSlotPos));
    bRet = AddEquippedItemInfo(255, iItemID, eSlotPos, dwUniqueID, false, bIsSkill);
    return bRet;
    //return;    
}

function bool EquipItemSlot(int iItemID, Object.SLOT_POS eSlotPos, BtrDouble dwUniqueID)
{
    TcpChannel.sfReqEquipItem(iItemID, dwUniqueID, int(eSlotPos), 1);
    //return;    
}

function bool UnequipItemSlot(int iItemID, Object.SLOT_POS eSlotPos, BtrDouble dwUniqueID)
{
    TcpChannel.sfReqEquipItem(iItemID, dwUniqueID, int(eSlotPos), 0);
    //return;    
}

function UpdateReqQuickSlot()
{
    local SendQuickSlotInfo SendQuickSlot;
    local int lp1, iQSlotIndex;
    local wGameManager GameMgr;

    Log("BTNetMainMenu::UpdateReqQuickSlot");
    GameMgr = Controller.ViewportOwner.Actor.Level.GameMgr;
    lp1 = 0;
    J0x5B:

    // End:0x27C [Loop If]
    if(lp1 < UpdateQuickSlotItemList.Length)
    {
        // End:0x85
        if(UpdateQuickSlotItemList[lp1].bSendServer == true)
        {
            // [Explicit Continue]
            goto J0x272;
        }
        iQSlotIndex = int(UpdateQuickSlotItemList[lp1].iQuickSlotIndex);
        // End:0x120
        if(UpdateQuickSlotItemList[lp1].bEquipped && iQSlotIndex < 4)
        {
            // End:0xDB
            if(!GameMgr.IsEnableQuickSlot(iQSlotIndex))
            {
                // [Explicit Continue]
                goto J0x272;
            }
            // End:0x120
            if(UpdateQuickSlotItemList[lp1].bIsSkill && !GameMgr.IsEnableSkillSlot(int(UpdateQuickSlotItemList[lp1].eNewSlotPos) - int(14)))
            {
                // [Explicit Continue]
                goto J0x272;
            }
        }
        Log((((((("QuickSlotIdx :" @ string(iQSlotIndex)) @ "SlotPoses :") @ string(UpdateQuickSlotItemList[lp1].eNewSlotPos)) @ "ItemID :") @ string(UpdateQuickSlotItemList[lp1].iItemID)) @ "Equip :") @ string(int(UpdateQuickSlotItemList[lp1].bEquipped)));
        SendQuickSlot.iQuickSlotOrders[SendQuickSlot.iQuickSlotOrders.Length] = byte(iQSlotIndex);
        SendQuickSlot.iItemIDs[SendQuickSlot.iItemIDs.Length] = UpdateQuickSlotItemList[lp1].iItemID;
        SendQuickSlot.dwUniqueIDs[SendQuickSlot.dwUniqueIDs.Length] = UpdateQuickSlotItemList[lp1].dwUniqueID;
        SendQuickSlot.iSlotPosKeys[SendQuickSlot.iSlotPosKeys.Length] = UpdateQuickSlotItemList[lp1].eNewSlotPos;
        SendQuickSlot.iSlotPosValues[SendQuickSlot.iSlotPosValues.Length] = byte(int(UpdateQuickSlotItemList[lp1].bEquipped));
        UpdateQuickSlotItemList[lp1].bSendServer = true;
        J0x272:
    ...
    // End:0x2CE
    if(SendQuickSlot.iItemIDs.Length > 0)
    {
        TcpChannel.sfReqUpdateQSlot_2(SendQuickSlot.iQuickSlotOrders, SendQuickSlot.iItemIDs, SendQuickSlot.dwUniqueIDs, SendQuickSlot.iSlotPosKeys, SendQuickSlot.iSlotPosValues);
    }
    UpdateQuickSlotItemList.Remove(0, UpdateQuickSlotItemList.Length);
    //return;    
}

function CalcQuickSlotItemCount(out int iStartIndex, out int iUpdateCount)
{
    iStartIndex = 0;
    J0x07:

    // End:0x3B [Loop If]
    if(iStartIndex < UpdateQuickSlotItemList.Length)
    {
        // End:0x31
        if(UpdateQuickSlotItemList[iStartIndex].bSendServer == true)
        {
            // [Explicit Break]
            goto J0x3B;
        }
    ...
    iUpdateCount = UpdateQuickSlotItemList.Length - iStartIndex;
    //return;    
}

function UpdateAckQuickSlot_2(array<byte> iQuickSlotIdxes, array<int> iItemIDs, array<BtrDouble> dwUniqueIDs, array<byte> bySlotPoses, array<byte> bySlotValues)
{
    local int lp1;
    local wItemBoxHK ItemInfo;
    local wItemBoxInstanceHK instanceInfo;
    ...
    Log("BTNetMainMenu::UpdateAckQuickSlot_2");
    GameMgr = Controller.ViewportOwner.Actor.Level.GameMgr;
    lp1 = 0;
    J0x5D:

    // End:0x50E [Loop If]
    if(lp1 < iQuickSlotIdxes.Length)
    {
        Log((((((("QuickSlotIdx :" @ string(iQuickSlotIdxes[lp1])) @ "SlotPoses :") @ string(bySlotPoses[lp1])) @ "ItemID :") @ string(iItemIDs[lp1])) @ "SlotValue :") @ string(bySlotValues[lp1]));
        ItemInfo = GameMgr.FindUIItem(iItemIDs[lp1]);
        // End:0x113
        if(ItemInfo == none)
        {
            // [Explicit Continue]
        ...
        // End:0x336
        if(int(bySlotValues[lp1]) == 1)
        {
            // End:0x1EA
            if(GameMgr.CheckSkillSlot(bySlotPoses[lp1]))
            {
                // End:0x1B0
                if(GameMgr.CheckOnlyModeItem(ItemInfo.byModeItem))
                {
                    instanceInfo = GameMgr.FindInstanceSkillItem(__NFUN_923__(dwUniqueIDs[lp1]));
                    // End:0x1AD
                    if(instanceInfo != none)
                    {
                        instanceInfo.SlotPosition = int(bySlotPoses[lp1]);
                    }                    
                }
                else
                {
                    GameMgr.SetInstance_SkillSlotPos(int(iQuickSlotIdxes[lp1]), __NFUN_923__(dwUniqueIDs[lp1]), int(bySlotPoses[lp1]));
                }                
            }
            else
            {
                // End:0x224
                if(__NFUN_912__(EmptyBtrDouble(), dwUniqueIDs[lp1]))
                {
                    instanceInfo = GameMgr.FindFirstInstanceItemByItemID(iItemIDs[lp1]);                    
                }
                else
                {
                    instanceInfo = GameMgr.FindInstanceItem(dwUniqueIDs[lp1]);
                }
                // End:0x2AC
                if(instanceInfo == none)
                {
                    Log("[BTNetMainMenu::UpdateAckQuickSlot_2] Not Found InstanceItem, ItemID : " $ string(iItemIDs[lp1]));
                    // [Explicit Continue]
                    goto J0x504;
                }
                dwUniqueIDs[lp1] = instanceInfo.UniqueID;
                // End:0x305
                if(GameMgr.CheckOnlyModeItem(ItemInfo.byModeItem))
                {
                    instanceInfo.SlotPosition = int(bySlotPoses[lp1]);                    
                ...
                {
                    GameMgr.SetInstance_QSlotItemSlotPos(int(iQuickSlotIdxes[lp1]), instanceInfo, int(bySlotPoses[lp1]));
                }
            }
            // [Explicit Continue]
            goto J0x504;
        ...
        if(GameMgr.CheckSkillSlot(bySlotPoses[lp1]))
        {
            ...
            {
                GameMgr.UnsetInstance_SkillSlotPos(int(iQuickSlotIdxes[lp1]), int(bySlotPoses[lp1]));
            }
            // [Explicit Continue]
            goto J0x504;
        }
        ...
        GameMgr.UnsetInstance_QSlotItemSlotPos(int(iQuickSlotIdxes[lp1]), int(bySlotPoses[lp1]));
        J0x504:
...
function RequestUpdateQuickSlot(int iQuickSlotIdx)
{
    ...
    local BtrDouble arMWItemID, arPistolItemID, arTWItemID1, arTWItemID2, arMeleeID;
    ...
    arMeleeID = GameMgr.QuickSlotBoxList[iQuickSlotIdx].UniqueID[4];
    ...
    TcpChannel.sfReqUpdateQSlot(arQslotID, arQslotName, arMWItemID, arPistolItemID, arTWItemID1, arTWItemID2, arSkill1, arSkill2, arSkill3, arSkill4, arMeleeID);
    //return;    
}
...
function bool TcpLogin_rfAckLoginWithIndonesia_Test(int Result)
{
    Log("[BTNetMainMenu::TcpLogin_rfAckLoginWithIndonesia_Test] Result=" $ string(Result));
    __NFUN_270__("[BTNetMainMenu::TcpLogin_rfAckLoginWithIndonesia_Test] Result=" $ string(Result));
    // End:0xE0
    if(Result != 0)
    {
        Class'GUIWarfareControls.BTWindowErrorDefineHK'.static.ShowError(Controller, Result);
    ...
    TcpLogin.sfReqLoginInHouseTest(szAccountName, "battery", AuthLogin_GUID);
    return true;
    //return;    
}

function bool TcpLogin_rfAckLoginWithIndonesia_Live(int Result)
{
    local int webLogFailCode;
    ...
    TcpLogin.sfReqLoginInHouseTest(AuthLogin_AccountID, szLoginPassword, AuthLogin_GUID);
    return true;
    //return;    
}
...
function SendLogin()
{
    ...
    if(Controller.ViewportOwner.Actor.Level.GetIsServiceBuild())
    {
        ...
        Log(((((((("sfReqLoginWithGlobal(" $ AuthLogin_AccountID) $ ", ") $ AuthLogin_GUID) $ ", ") $ AuthLogin_AuthKey) $ ", ") $ string(AuthLogin_ChannelingType)) $ ")");
...
function SendCharDetailInfoWithClan(string CharName, byte ShowTab)
{
    UserInfoWindowShowTab = ShowTab;
    TcpChannel.sfReqCharDetailInfoWithClan(CharName);
    //return;    
}
...
defaultproperties
{
    ...
    pagename_Lobby2="GUIWarfare.BTPageLobby"
    ...
    pagename_RoomLobby_Defence="GUIWarfare.BTPageRoomLobby_Defence"
    pagename_MatchResult2="GUIWarfare.BTPageMatchResult"
    pagename_MatchResult2_Defence="GUIWarfare.BTPageMatchResult_Defence"
    pagename_Auction="GUIWarfare.BTPageAuction"
    pagename_Help="GUIWarfare.BTPageHelp"
    pagename_Mail="GUIWarfare.BTPageMail"
    pagename_Store="GUIWarfare.BTPageStore"
    ...
    pagename_Lucky="GUIWarfare.BTPageLucky"
    PageOnOpen=2
    szAccountName="NG"
    iMaxTryLoginCount=3
    NatType=-1
