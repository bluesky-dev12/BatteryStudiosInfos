...
function bool IsHaveMainWeapon(int nIndex)
{
    ...
    iItemID = MyStatus.QuickSlotList[nIndex].iItemID[0];
    // End:0x68
    if(iItemID != 0)
    {
        return true;
...
function bool BlockChangeWeapon(int nIndex)
{
    ...
    {
        iItemID = MyStatus.QuickSlotList[nIndex].iItemID[1];
        // End:0x2F5
        if(iItemID == 0)
        {
            return false;
...
function SendMessage(MessageItem msgItem)
{
    ...
    // End:0x9F
    if((int(ViewportOwner.Actor.Level.GetMatchMaker().GMLevelFlag) & 7) > 0)
    {
        // End:0x9F
        if(MainMenu.TextToGMCommand(msgItem.Text))
        {
            return;
        }
    }
