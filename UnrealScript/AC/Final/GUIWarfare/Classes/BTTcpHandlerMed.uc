...
{
    return Owner.rfAckMyEquipedQSlotList(QSlotDispOrder, MWUniqueID, MWItemID, MWPartID, MWPaintID, PistolUniqueID, PistolItemID, PistolPartID, PistolPaintID, TW1UniqueID, TWItemID1, TW2UniqueID, TWItemID2, Skill1ID, Skill2ID, Skill3ID, Skill4ID, Result, KnifeUniqueID, KnifeItemID);
    //return;    
}
...
{
    return Owner.rfAckEntryQSlotIDList(UserNum, QSlotDispOrder, MWItemID, MWPartID, MWPaintID, PistolItemID, PistolPartID, PistolPaintID, TWItemID1, TWItemID2, Skill1ID, Skill2ID, Skill3ID, Skill4ID, KWItemID);
    //return;    
}
...
{
    return Owner.rfAckEntryQSlotUniqueIDList(UserNum, QSlotDispOrder, MWUniqueID, PistolUniqueID, TW1UniqueID, TW2UniqueID, KnifeUniqueID);
    //return;    
}
...
function bool rfReqNotifyShopPackageItemData(int PackageProductNo, array<int> ProductCode, array<int> ProductQuantity, array<int> ProductHour, string Description)
{
    ...
    Owner.rfReqNotifyShopPackageItemData(PackageProductNo, ProductCode, ProductQuantity, ProductHour, Description);
    return false;
    //return;    
}
...
{
    Owner.rfAckCharDetailInfoWithClan(ErrorCode, CharName, Exp, Level, LevelMarkID, PreMonthlyRanking, MonthlyRanking, Win, Lose, Kill, Death, Assist, KillCountByAimShooting, HeadShot2, TotPlayTime, DisCon, MainWeaponItemID, MainWeaponPartID, SubWeaponItemID, SubWeaponPartID, AFSpecialCostumeItemID, RSASpecialCostumeItemID, DispHeadItem, DispFaceItem, DispBackpack, DispPouch, DispCamouflage, ClanName, ClanLevel, CM_Pattern, CM_BG, CM_BL, ClanExp, ClanGrade, ClanActivity, MonthlyConnectingCount, UserClanWin, UserClanLose, UserClanDraw, ClanWin, ClanLose, ClanDraw, PreClanTotalRankCur, ClanTotalRankCur, PreClanMonthlyRankCur, ClanMonthlyRankCur, PreClanMonthlyConnectingRank, ClanMonthlyConnectingRank, ClanIntro);
    return true;
    //return;    
}
...
{
    Log("[BTTcpHandlerMed::rfAckPcBangItemList] Length : " $ string(ItemTID.Length));
    Owner.rfAckPcBangItemList(ItemTID, PartID);
    return true;
    //return;    
}
...
{
    Log("[BTTcpHandlerMed::rfAckUpdateQSlot_2] Result : " $ string(Result));
    Owner.rfAckUpdateQSlot_2(Result, ErrMsg, DisplayOrder, ItemTID, ItemUID, SlotPos_Key, SlotPos_Value);
    return true;
    //return;    
}
...
{
    Owner.rfAckExpandSlotConfirm(Result, ErrMsg, QSlotID, MWItemID, PistolItemID, TWItemID1, TWItemID2, Skill1, Skill2, Skill3, Skill4, KnifeItemID);
    return true;
    //return;    
}
...
{
    Owner.rfNotifyPcBangInfo(PcBang_Grade);
    return true;
    //return;    
}
...
{
    Owner.rfReqAddSpecialRank(NickName);
    return true;
    //return;    
}
