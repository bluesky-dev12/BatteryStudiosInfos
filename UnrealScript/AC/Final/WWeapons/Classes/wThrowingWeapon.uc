...
simulated function AnimEnd(int Channel)
{
    ...
    if(((bQuickThrow == true) && int(ClientState) == int(2)) && Anim != 'Fire')
    {
        ClientState = 4;
        // End:0xD9
        if(((int(InventoryGroup) == 4) && int(PlayerController(Instigator.Controller).bQuickGrenade) == 1) || (int(InventoryGroup) == 4) && int(PlayerController(Instigator.Controller).bQuickSubGrenade) == 1)
        {
...
simulated function bool IsFiring()
{
    ...
    return (int(ClientState) == int(4)) && ((((FireMode[0] != none) && FireMode[0].IsFiring()) || (FireMode[1] != none) && FireMode[1].IsFiring()) || (FireMode[2] != none) && FireMode[2].IsFiring()) || bTmFiring;
    //return;    
}
...
simulated function GetAmmoCount(out float MaxAmmoPrimary, out float CurAmmoPrimary)
{
    ...
    // End:0xE1
    if(bNoAmmoInstances && Instigator.Controller != none)
    {
        ...
        TempWeapon = PC.GetWeaponByInventoryGroup(InventoryGroup, self);
...
simulated function bool IsCanUseWeapon()
{
    ...
    fCurAmmoCount = 0.0000000;
    fMaxAmmoCount = 0.0000000;
    GetAmmoCount(fMaxAmmoCount, fCurAmmoCount);
    return fCurAmmoCount >= 1.0000000;
    //return;    
}

defaultproperties
{
    szWeaponAttachmentName="WWeapons.wThrowingAttachment"
