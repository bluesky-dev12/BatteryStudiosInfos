...
const BT_FALSE = 0;
const RUU = 65535;
const ESUPPLY_ITEM_COUNT = 5;
const INVALID_SUPPLY_ITEM_ID = 255;
const TEAM_ID_COUNT = 2;
const TEAM_ID_AF = 0;
const TEAM_ID_RSA = 1;
const TEAM_ID_ALIEN_MODE_HUMAN = 0;
const TEAM_ID_ALIEN_MODE_ALIEN = 1;
const TEAM_ID_DEFFENCE_MODE_HUMAN = 0;
const TEAM_ID_DEFFENCE_MODE_MOB = 1;
const COMMON_MODE_ITEM = 0;
const INVAILD_SLOT_INDEX = -1;
const INVAILD_ITEM_ID = 0;
const WEAPONID_BOMBING = 5006;
const WEAPONID_HELICOPTER = 5009;
const WEAPONID_UAV = 5010;
const WEAPONID_SENTRYGUN = 5017;
const WEAPONID_SENTRYGUN_PACK = 5018;
const WEAPONID_DEFENCE_SENTRYGUN = 5021;
const ITEMID_DEFAULT_BODY = 15000;
const ITEMID_DEFAULT_KNIFE = 9000;
const SUPPLY_ITEM_CULL_DISTANCE = 6500;
const SUPPLY_ITEM_STAY_TIME = 120;
const SUPPLY_ITEM_STAY_TIME_HEALTH_PACK = 10;
const NPCID_SENTRYGUN = 200;
const NPCID_DEFENCE_SENTRYGUN = 201;
const ALL_MAP = 255;
const ALL_GAMEMODE = 255;
const QUICK_SLOT_ORDER_ONLY_MODEITEM = 255;
const QUICKSLOT_EXTEND = 1;
...
enum SLOT_POS
{
    ...
    SLOT_POS_MELEE_WEAPON,          // 4
    DISABLE_SLOT_POS_QUICK_MAIN_WEAPON1,// 5
    DISABLE_SLOT_POS_QUICK_MAIN_WEAPON2,// 6
    DISABLE_SLOT_POS_QUICK_MAIN_WEAPON3,// 7
    DISABLE_SLOT_POS_QUICK_SUB_WEAPON1,// 8
    DISABLE_SLOT_POS_QUICK_SUB_WEAPON2,// 9
    DISABLE_SLOT_POS_QUICK_THROW_WEAPON1,// 10
    DISABLE_SLOT_POS_QUICK_THROW_WEAPON2,// 11
    DISABLE_SLOT_POS_QUICK_THROW_WEAPON3,// 12
    SLOT_POS_INVEN,                 // 13
    SLOT_POS_SKILL1,                // 14
    SLOT_POS_SKILL2,                // 15
    SLOT_POS_SKILL3,                // 16
    ...
    SLOT_POS_INVEN_USE_7,           // 26
    SLOT_POS_ACCESSORY,             // 27
    SLOT_POS_BODY,                  // 28
    SLOT_POS_INVEN_USE_8,           // 29
    SLOT_POS_INVEN_USE_PUBLIC,      // 30
    SLOT_POS_INVEN_UNKNOWN,         // 31
    SLOT_POS_SPECIAL_ALIEN,         // 32
    SLOT_POS_END                    // 33
};

enum ESUPPLY_ITEM_TYPE
{
...
// Export UObject::execSRand(FFrame&, void* const)
native static final function SRand(int Seed)
{
    //native.Seed;        
}

// Export UObject::execSignedRand(FFrame&, void* const)
native static final function int SignedRand(int iMin, int iMax)
{
    //native.iMin;
    //native.iMax;        
}
...
// Export UObject::execDegree2RUU(FFrame&, void* const)
native static final function int Degree2RUU(float fAngle)
{
    //native.fAngle;        
}

// Export UObject::execRUU2Degree(FFrame&, void* const)
native static final function float RUU2Degree(int iRUU)
{
    //native.iRUU;        
}
...
static simulated function GetDatefromString(out string in, string char, optional bool bFull)
{
    ...
    if(bFull == false)
    {
        temp[0] = Mid(temp[0], 2, 2);
        in = (((temp[2] $ det) $ temp[1]) $ det) $ temp[0];        
    }
    else
    {
        in = (((temp[2] $ det) $ temp[1]) $ det) $ temp[0];
...
function FloatBox AddFloatBox(int startX, int startY, FloatBox value1)
{
    local FloatBox retValue;

    retValue.X1 = float(startX) + value1.X1;
    retValue.X2 = float(startX) + value1.X2;
    retValue.Y1 = float(startY) + value1.Y1;
    retValue.Y2 = float(startY) + value1.Y2;
    return retValue;
    //return;    
}

function FloatBox FloatBoxOffsetX(FloatBox origBox, float offset)
{
    local FloatBox retValue;
...
function FloatBox FloatBoxOffsetY(FloatBox origBox, float offset)
{
    local FloatBox retValue;
...
function bool CheckInFloatBox(FloatBox checkFloatBox, float X, float Y, float ClientRatioX, float ClientRatioY)
{
    // End:0x72
    if(((checkFloatBox.X1 * ClientRatioX) <= X) && (X * ClientRatioX) <= checkFloatBox.X2)
    {
        // End:0x72
        if(((checkFloatBox.Y1 * ClientRatioY) <= Y) && Y <= (checkFloatBox.Y2 * ClientRatioY))
        {
            return true;
        }
    }
