...
function bool OptionBase_OnKeyEvent(out byte key, out byte Action, float Delta)
{
    ...
    local int i;
    local export editinline BTComboBoxHK comboHK;
    ...
    if(i < Content.Length)
    {
        comboHK = BTComboBoxHK(Content[i]);
        // End:0x73
        if((comboHK != none) && comboHK.ComboBox.List.OnKeyEvent(key, Action, Delta))
        {
            return true;
        }
        ++i;
    ...
    iAction = Action;
    // End:0xD2
    if((int(iAction) == int(3)) && int(iKey) == int(236))
    {
    ...
    {
        // End:0x10E
        if((int(iAction) == int(3)) && int(iKey) == int(237))
        {
