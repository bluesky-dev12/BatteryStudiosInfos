...
var int iGameMode;
var() automated FloatBox fbModeWeaponLimit;
...
function SetData(string RoomName, int GameState, string MapName, Material MapImg, array<Material> supplyImg, string roommaster, string GameMode, string Weapon, string HardCore, string gamerunning, optional bool bBotMode, optional string CurMaxUser, optional int GameModeNum)
{
    local int i;
...
function bool RenderFixed(Canvas C, optional float Delta)
{
    ...
    X2 = 1022.0000000;
    Y2 = 360.0000000;
    Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, MapImage, X, Y, X2, Y2);
    i = 0;
    J0x84:
    ...
    if(i < SupplyImage.Length)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, SupplyImage[i], X2 - ((SupplyImage[i].width + float(5)) * float(i + 1)), 210.0000000, X2 - (float(5) + (float(i) * SupplyImage[i].width)), 235.0000000);
        i++;
        // [Loop Continue]
        goto J0x84;
    }
    ...
    if(i < 6)
    {
        ...
        if((i == 2) && bBotMode)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, strHeader[5], 0, float(FontSize), fbHeader[i].X1, fbHeader[i].Y1, fbHeader[i].X2, fbHeader[i].Y2, FontShadowColor);            
        }
        else
        {
            Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, strHeader[i], 0, float(FontSize), fbHeader[i].X1, fbHeader[i].Y1, fbHeader[i].X2, fbHeader[i].Y2, FontShadowColor);
        }
        Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, strData[i], 0, float(FontSize), fbData[i].X1, fbData[i].Y1, fbData[i].X2, fbData[i].Y2, FontShadowColor);
        i++;
        // [Loop Continue]
        goto J0x131;
    }
    Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, strMapName, 0, float(FontMapNameSize), fbMapName.X1, fbMapName.Y1, fbMapName.X2, fbMapName.Y2, FontShadowColor);
    // End:0x392
    if(Class'Engine.wGameSettings'.static.GetModeIndex_Alien() == iGameMode)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.WaitRoom_RoomInof_SubBG, 766.0000000, 334.0000000, 1020.0000000, 357.0000000);
        Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, strAlienModeLimit, 0, float(FontMapNameSize), fbModeWeaponLimit.X1, fbModeWeaponLimit.Y1, fbModeWeaponLimit.X2, fbModeWeaponLimit.Y2, FontShadowColor);
    }
    C.DrawColor = FontTitleColor;
    Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, strTitle, 0, float(FontTitleSize), fbTitle.X1, fbTitle.Y1, fbTitle.X2, fbTitle.Y2, FontShadowColor);
    return true;
    //return;    
}
...
defaultproperties
{
    ...
    strHeader[0]="Host"
    strHeader[1]="Team Bal."
    strHeader[2]="Mode"
    strHeader[3]="Sub Mode"
    strHeader[4]="Players"
    strHeader[5]="Status"
    fbModeWeaponLimit=(X1=774.0000000,Y1=340.0000000,X2=1020.0000000,Y2=353.0000000)
    strAlienModeLimit="No Flash / Smoke"
}