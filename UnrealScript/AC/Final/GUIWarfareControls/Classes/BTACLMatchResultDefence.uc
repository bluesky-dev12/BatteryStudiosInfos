class BTACLMatchResultDefence extends BTAutoColumnListHK
    editinlinenew
    instanced;
...
var Image ItemReward[2];
var wItemBoxHK ItemInfo[2];
var int iScoreRewardCount;
var int iClearRewardCount;
var array<Text> tarray;
var BTScrollString ScoreRewardScrollString;
var BTScrollString ClearRewardScrollString;
...
function AddTeamMemeber(bool bMe, int UID, int ClanMark, int ClanBG, int ClanBL, int Level, string NickName, string ClanName, int Score, int Kill, int Assist, int Death, int Exp, int expup, int Point, int pointup, array<int> BonusIDs, int LookingFor, bool bFriendly, int LevelMarkID, int TitleMarkID, int Reserved1, int Reserved2, int Reserved3, int Reserved4)
{
    ...
    Data.DataPerColumn[6].IntValue = Death;
    ...
    AddRow(Data);
    ItemInfo[0] = GameMgr.FindUIItem(Reserved1);
    ItemInfo[1] = GameMgr.FindUIItem(Reserved2);
    iScoreRewardCount = Reserved3;
    iClearRewardCount = Reserved4;
    ItemReward[0] = Class'Engine.BTCustomDrawHK'.static.MakeImage(152, 64, 0, none);
    ItemReward[0].Image = ItemInfo[0].ItemImage;
    ItemReward[1] = Class'Engine.BTCustomDrawHK'.static.MakeImage(152, 64, 0, none);
    ItemReward[1].Image = ItemInfo[1].ItemImage;
    //return;    
}
...
function LC1_Draw(Canvas C, float L, float t, float R, float B, ACLData Data, int Column, int row)
{
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawString(C, strStatus[0], Data.Text[i].FontDrawType, float(Data.Text[i].FontSize), L + float(7), t + float(48), R + float(103), B + float(30), Data.Text[i].DrawShadowColor);
    Class'Engine.BTCustomDrawHK'.static.DrawString(C, strStatus[1], Data.Text[i].FontDrawType, float(Data.Text[i].FontSize), L + float(325), t + float(48), R + float(455), B + float(30), Data.Text[i].DrawShadowColor);
    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ItemRewardBG, L + float(138), t + float(43), R + float(307), B + float(129));
    // End:0x25C
    if(ItemReward[0].Image != none)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ItemReward[0], (L + float(138)) + float(2), (t + float(43)) + float(20), R + float(307), B + float(129));
    }
    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ItemRewardBG, L + float(456), t + float(43), R + float(625), B + float(129));
    // End:0x312
    if(ItemReward[1].Image != none)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ItemReward[1], (L + float(456)) + float(2), (t + float(43)) + float(20), R + float(625), B + float(129));
    }
    C.DrawColor = C.MakeColor(byte(255), 192, 0, byte(255));
    fbItemName = Class'Engine.BTCustomDrawHK'.static.MakeFloatBox(L + float(138), t + float(43), R + float(273), B + float(129));
    bMouseOn = CheckInFloatBox(fbItemName, Controller.MouseX, Controller.MouseY, 1.0000000, C.GetClientHeightRatio());
    // End:0x4F7
    if((ItemInfo[0] != none) && ItemInfo[0].ItemName != "")
    {
        ScoreRewardScrollString.ProcessScrollPoint(C, culledItemName, ItemInfo[0].ItemName, L + float(140), t + float(48), R + float(273), B + float(30), bMouseOn, float(Data.Text[i].FontSize), Controller.RenderDelta);
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, culledItemName, Data.Text[i].FontDrawType, float(Data.Text[i].FontSize), L + float(140), t + float(48), R + float(273), B + float(30), Data.Text[i].DrawShadowColor);        
    }
    else
    {
        C.DrawColor = C.MakeColor(byte(255), byte(255), byte(255), byte(255));
        ScoreRewardScrollString.ProcessScrollPoint(C, culledItemName, strStatus[2], L + float(137), t + float(86), R + float(273), B + float(65), bMouseOn, float(Data.Text[i].FontSize), Controller.RenderDelta);
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, culledItemName, 1, float(Data.Text[i].FontSize), L + float(137), t + float(86), R + float(273), B + float(65), Data.Text[i].DrawShadowColor);
    }
    fbItemName = Class'Engine.BTCustomDrawHK'.static.MakeFloatBox(L + float(458), t + float(43), R + float(595), B + float(129));
    bMouseOn = CheckInFloatBox(fbItemName, Controller.MouseX, Controller.MouseY, 1.0000000, C.GetClientHeightRatio());
    // End:0x7DA
    if((ItemInfo[1] != none) && ItemInfo[1].ItemName != "")
    {
        ClearRewardScrollString.ProcessScrollPoint(C, culledItemName, ItemInfo[1].ItemName, L + float(458), t + float(48), R + float(595), B + float(30), bMouseOn, float(Data.Text[i].FontSize), Controller.RenderDelta);
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, culledItemName, Data.Text[i].FontDrawType, float(Data.Text[i].FontSize), L + float(458), t + float(48), R + float(595), B + float(30), Data.Text[i].DrawShadowColor);        
    }
    else
    {
        C.DrawColor = C.MakeColor(byte(255), byte(255), byte(255), byte(255));
        ClearRewardScrollString.ProcessScrollPoint(C, culledItemName, strStatus[2], L + float(455), t + float(86), R + float(595), B + float(65), bMouseOn, float(Data.Text[i].FontSize), Controller.RenderDelta);
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, culledItemName, 1, float(Data.Text[i].FontSize), L + float(455), t + float(86), R + float(595), B + float(65), Data.Text[i].DrawShadowColor);
    }
    // End:0x99E
    if(iScoreRewardCount > 0)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, "x " $ string(iScoreRewardCount), Data.Text[i].FontDrawType, float(Data.Text[i].FontSize), L + float(277), t + float(65), R + float(307), B + float(95), Data.Text[i].DrawShadowColor);
    }
    // End:0xA3D
    if(iClearRewardCount > 0)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, "x " $ string(iClearRewardCount), Data.Text[i].FontDrawType, float(Data.Text[i].FontSize), L + float(595), t + float(65), R + float(625), B + float(95), Data.Text[i].DrawShadowColor);
    }
    //return;    
}
...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    self.ItemRewardBG = Class'Engine.BTCustomDrawHK'.static.MakeImage(169, 86, 0, Texture'Warfare_GP_UI_UI.Store.list_remodeling_remodelingitem');
    ...
    self.AddColumn(headerInfo);
    headerInfo = Class'GUIWarfareControls_Decompressed.BTAutoColumnListHeaderHK'.static.MakeHeader("score", 739.0000000 - float(666), 7);
    headerInfo.__CustomDraw__Delegate = LC3_Draw_Score;
    self.AddColumn(headerInfo);
    headerInfo = Class'GUIWarfareControls_Decompressed.BTAutoColumnListHeaderHK'.static.MakeHeader("kill", 794.0000000 - float(739), 7);
    ...
    tarray[1].DrawColor = Class'GUIWarfareControls_Decompressed.BTUIColorPoolHK'.static.SmallPerc();
    ScoreRewardScrollString = new Class'GUIWarfareControls_Decompressed.BTScrollString';
    ScoreRewardScrollString.InitValue();
    ClearRewardScrollString = new Class'GUIWarfareControls_Decompressed.BTScrollString';
    ClearRewardScrollString.InitValue();
    //return;    
}
...
defaultproperties
{
    strStatus[0]="Point reward earned"
    strStatus[1]="Clear rewards earned"
    strStatus[2]="No rewards."
}