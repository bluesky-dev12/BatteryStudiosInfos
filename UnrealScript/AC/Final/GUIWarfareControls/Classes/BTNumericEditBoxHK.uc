...
function Internal_OnRendered(Canvas C)
{
    ...
    C.Style = 5;
    Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, BackgroundImage, RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2);
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, buttonImage[SelectMenuState], RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2);
    ...
    {
        Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, DrawStr, CaptionDrawType, float(FontSize[SelectMenuState]), RWinPos.X1 + float(CaptionPadding[0]), RWinPos.Y1 + float(CaptionPadding[1]), (RWinPos.X2 - float(CaptionPadding[2])) - float(3), RWinPos.Y2 - float(CaptionPadding[3]), FontShadowColor[SelectMenuState]);
    }
    // End:0x2CA
    if((bReadOnly == false) && bActiveCursor)
    {
        ...
        Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, cursorImg, cursorPos.X2, cursorPos.Y1, cursorPos.X2 + float(3), cursorPos.Y2);
    }
    //return;    
}
