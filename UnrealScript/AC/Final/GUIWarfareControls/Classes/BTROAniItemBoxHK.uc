...
var BTROAniItemBoxHK.E_ANI_END_TYPE EndAnimationType;
...
//var delegate<OnNextRoll> __OnNextRoll__Delegate;
...
function RandEndAniType()
{
    ...
    switch(EndAnimationType)
    {
        ...
        // End:0xAD
        case 4:
            RollFinishCount = RollMaxCount + 1;
            // End:0xB0
            break;
        // End:0xFFFF
        default:
...
function bool Render(Canvas C, optional float Delta)
{
    ...
    if(bRenderInfo)
    {
        ...
        // End:0x656
        if((bBackgroundInfo && bShowGrade) && ItemName.Text != "")
        {
...
function bool RollOneRound(float deltaValue)
{
    local bool bOk;
    local int changeItemValue;

    bOk = false;
    // End:0x89
    if((RollStartX * float(RollDirection)) > RollEndValue)
    {
        bOk = true;
        // End:0x45
        if(bStartCheckCount)
        ...
        changeItemValue = SetNextRollInfo();
        // End:0x70
        if(changeItemValue == 1)
        {
    ...
    {
        RollStartX += (GetChangeValue(bSlowMove, deltaValue));
        // End:0xCA
        if(bCheckFinish)
        {
            // End:0xCA
...
function float GetChangeValue(bool bSlow, float deltaValue)
{
    local float changeValue;

    // End:0x45
    if(bSlow)
    {
        changeValue = (RollAniStep * deltaValue) * (((RollEndValue * float(RollDirection)) + (0.0200000 * float(RollDirection))) - RollStartX);        
    }
    else
    {
        changeValue = (RollAniStep * deltaValue) * float(RollDirection);
    }
    return changeValue;
    //return;    
}

function int SetNextRollInfo()
{
    local int ChangeItemInfoValue;

    ChangeItemInfoValue = 1;
    OnNextRoll();
    switch(EndAnimationType)
    {
        // End:0x6B
        case 0:
            ...
            {
                ChangeItemInfoValue = 2;
                bSlowMove = true;
                bCheckFinish = true;
                RollEndValue = 0.0100000;
            }
            // End:0x2E2
            break;
        // End:0x123
        case 1:
            ...
            {
                // End:0xC8
                if(RollCount < RollFinishCount)
                {
                    RollStartX = -1.0000000;
                    RollEndValue = -0.4000000;
                    bSlowMove = true;                    
                }
                else
                {
                    ...
                    if(RollCount == RollFinishCount)
                    {
                        ChangeItemInfoValue = 0;
                        RollDirection *= float(-1);
                        RollEndValue = 1.0000000;                        
                    }
                    else
                    {
    ...
    return ChangeItemInfoValue;
    //return;    
}

defaultproperties
{
    bStartCheckCount=true
    RollDirection=1
    RollStartX=-1.0000000
    RollEndValue=1.0000000
    bRenderInfo=true
