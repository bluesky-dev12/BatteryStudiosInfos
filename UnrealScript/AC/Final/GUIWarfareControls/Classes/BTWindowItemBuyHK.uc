...
var localized string strUnlimited;
...
function SetRenderObject(BTROItemBoxHK ro)
{
    ...
    else
    {
        ...
        if(i < cii.8)
        {
            ...
            // End:0x53D
            if(int(byte(ItemInfo.CostType)) == int(5))
            {
                Combo.ComboBox.AddItem((((strUnlimited @ "(") @ string(cii.WZCashPrice[i])) @ strCashType) @ ")");
                // [Explicit Break]
                goto J0x678;
                // [Explicit Continue]
                goto J0x66E;
