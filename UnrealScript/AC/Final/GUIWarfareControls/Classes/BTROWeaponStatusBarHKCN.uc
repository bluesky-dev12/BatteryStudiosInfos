...
function bool Render(Canvas C, optional float Delta)
{
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, Gauge, AWinPos.X1, AWinPos.Y1, AWinPos.X1 + float(gwidth), AWinPos.Y2);
    // End:0x173
    if(DataArray[1] > 0)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, Plus, AWinPos.X1 + float(gwidth), AWinPos.Y1, (AWinPos.X1 + float(gwidth)) + float(gmove), AWinPos.Y2);        
    }
    else
    {
        // End:0x1DC
        if(DataArray[1] < 0)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, Minus, (AWinPos.X1 + float(gwidth)) - float(gmove), AWinPos.Y1, AWinPos.X1 + float(gwidth), AWinPos.Y2);
        }
    }
    return true;
    //return;    
