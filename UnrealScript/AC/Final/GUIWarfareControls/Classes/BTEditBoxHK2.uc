...
function bool OwnerDraw(Canvas C)
{
    ...
    if(bHudMode == false)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, bgImg, RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2);
    }
    // End:0xEA
    ...
    if(bHudMode == false)
    {
        ...
        Class'Engine.BTCustomDrawHK'.static.DrawStringRatio(C, self.textStrVis, 0, float(FontSize[int(self.MenuState)]), RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2, self.FontShadowColor[int(self.MenuState)], 0, true);
    }
    // End:0x637
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        if(tempString != "")
        {
            C.BtrTextSize(tempString, FontSize[int(self.MenuState)], cursorW, HT);            
        }
        else
        {
            C.BtrTextSize("|", FontSize[int(self.MenuState)], cursorW, HT);
            cursorW = 0.0000000;
        }
        // End:0x43D
        if(bHudMode == false)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, cursorImg, RWinPos.X1 + cursorW, RWinPos.Y1, (RWinPos.X1 + cursorW) + float(3), RWinPos.Y1 + HT);
        }
        // End:0x627
        if(bSelectionMode)
        {
            ...
            else
            {
                ...
                C.BtrTextSize(tempString, FontSize[int(self.MenuState)], X, HT);
            }
            // End:0x576
            if(E > float(self.EndVis - self.FirstVis))
            {
                C.BtrTextSize(self.textStrVis, FontSize[int(self.MenuState)], X2, HT);                
            }
            else
            {
                tempString = self.Mid(self.textStrVis, 0, int(E));
                C.BtrTextSize(tempString, FontSize[int(self.MenuState)], X2, HT);
            }
            // End:0x627
            if(bHudMode == false)
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImageRatio(C, selectionImg, RWinPos.X1 + X, RWinPos.Y1, RWinPos.X1 + X2, RWinPos.Y1 + HT);
            }
        }
...
function CaculateVisibleText(bool bHoldFirst, int pos)
{
    ...
    if(bHoldFirst)
    {
        ...
        if(i >= pos)
        {
            ...
            saveC.BtrTextSize(temp, FontSize[int(self.MenuState)], scrollW, HT);
            // End:0xDC
...
function int MousePosToCaretPos(float mx, float my)
{
    ...
    {
        saveC.BtrTextSize(self.textStrVis, FontSize[int(self.MenuState)], W, HT);
...
function int CaretBinarySearch(int lCursor, int rCursor, float mx, int Parent)
{
    ...
    if(textLen == 1)
    {
        ...
        if(self.CacheTextWidth[Parent] == float(0))
        {
            ...
            saveC.BtrTextSize(Text, FontSize[int(self.MenuState)], W, HT);
...
function bool InternalOnKeyType(out byte key, optional string Unicode)
{
    ...
    if(int(key) < 32)
    {
        ...
        if(bHudMode == false)
        {
            switch(key)
            {
                ...
                case 22:
                    ...
                    if(bLimitDrawSize)
                    {
                        fb = Class'Engine.BTCustomDrawHK'.static.GetDrawStringFloatBox(saveC, temp, 0, float(FontSize[int(self.MenuState)]), 0.0000000, 0.0000000, 1024.0000000, 768.0000000);
                        W = fb.X2 - fb.X1;
                        // End:0x18A
        ...
        {
            return true;
        }/* !MISMATCHING REMOVE, tried If got Type:Else Position:0x299! */
        // End:0x2BB
        if(bAllSelected)
        {
            TextStr = "";
...
defaultproperties
{
    ...
    bCaptureMouse=true
    OnDraw=BTEditBoxHK2.OwnerDraw
    OnActivate=BTEditBoxHK2.InternalActivate
    OnDeActivate=BTEditBoxHK2.InternalDeactivate
    OnMousePressed=BTEditBoxHK2.OnMousePressed
    OnMouseRelease=BTEditBoxHK2.OnMouseRelease
    OnChange=BTEditBoxHK2.Internal_OnChange
    OnKeyType=BTEditBoxHK2.InternalOnKeyType
    OnKeyEvent=BTEditBoxHK2.InternalOnKeyEvent
}