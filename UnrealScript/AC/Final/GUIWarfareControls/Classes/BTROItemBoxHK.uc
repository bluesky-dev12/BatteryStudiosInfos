...
function FloatBox GetItemImageBox()
{
    ...
    // End:0x52
    if(((int(InvenState) == int(0)) || int(InvenState) == int(15)) || int(InvenState) == int(24))
    {
...
function GetExtraItemString(out string outputString, out Color outputColor)
{
    ...
    if((ItemInfo.CostType == 1) || ItemInfo.CostType == 4)
    {
        GetTimeString(outputString, outputColor);        
    }
    else
    {
        // End:0x85
...
function GetTimeString(out string outputString, out Color outputColor)
{
    // End:0x9A
...
function GetTimeStringOneStep(out string outputString, out Color outputColor, Color DefaultColor)
{
    // End:0x90
    if(instanceInfo.LeftTime.Day >= 1)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // End:0x150
                if(instanceInfo.LeftTime.Hour > 12)
                {
                    outputColor = DefaultColor;                    
                }
                else
                ...
                outputString = string(instanceInfo.LeftTime.Hour) $ strLeftTime[3];                
            }
            else
            {
                outputColor = Class'GUIWarfareControls_Decompressed.BTUIColorPoolHK'.static.LabelWarning();
