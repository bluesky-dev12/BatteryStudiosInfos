...
simulated function AnimEnd(int Channel)
{
    ...
    if(int(ClientState) == int(4))
    {
        ...
        if(((Anim == ReloadAnim) || Anim == ReloadEmptyAnim) && int(ReloadState) == int(1))
        {
            ...
            {
                fReloadAniSumRate = ReloadAnimRate;
                // End:0x251
            ...
            else
            {
                ...
                {
                    fReloadAniSumRate = ReloadEndAnimRate;
                    // End:0x2E5
                    if(bQuickReload == true)
                    {
                        fReloadAniSumRate *= 1.5000000;
                    ...
                    PlayAnim(ReloadEndAnim, fReloadAniSumRate, 0.0000000);                    
                }
                else
                {
                    SetActioned();
        ...
        else
        {
            ...
            if(Anim == ReloadBeginAnim)
            {
                ...
                else
                {
                    ...
                    {
                        fReloadAniSumRate = ReloadEmptyAnimRate;
                        // End:0x4E4
                        if(bQuickReload == true)
                        {
                            fReloadAniSumRate *= 1.5000000;
    ...
    if((int(ClientState) == int(4)) && int(ReloadState) == int(3))
    {
        ...
        if(Instigator.IsLocallyControlled())
        {
            ...
            if(Mesh != none)
            {
                ...
                {
                    fReloadAniSumRate = BaseParams.fReloadBeginRate;
                    // End:0x1D7
                    if(bQuickReload == true)
                    {
                        fReloadAniSumRate *= 1.5000000;
                    ...
                    PlayAnim(ReloadBeginAnim, fReloadAniSumRate, 0.0000000);                    
                }
                else
                {
                    // End:0x37A
...
function ServerReload()
{
    ...
    iCodeID = Class'Engine_Decompressed.wMessage_Game'.default.Code_AllyReload;
    Level.Game.TeamReceiveLocalizedMessageDistance(byte(iTeamNum), Class'Engine_Decompressed.wMessage_Game', iCodeID, Pawn(Owner).PlayerReplicationInfo,,, vLocation, fMessageRange, true);
    //return;    
}
...
simulated function SetBaseParams(wWeaponBaseParams wBase)
{
    ...
    {
        FireModeClass[1] = Class'Engine_Decompressed.wAimFire';        
    }
    else
    {
...
simulated function ScopeAttached(bool bEnabled)
{
    ...
    {
        // End:0x120
        if(FireModeClass[1] != Class'Engine_Decompressed.wZoomFire3D')
        {
            FireMode[1].DestroyEffects();
            DelFireMode(1);
            FireModeClass[1] = Class'Engine_Decompressed.wZoomFire3D';
...
simulated function MuzzleFlashModified(bool bEnabled, string strAddMuzClass)
{
    bModMuzFlash = bEnabled;
    strModMuzFlashClass = ResParams.default.szClass_Muzz_1st $ strAddMuzClass;
    //return;    
}
