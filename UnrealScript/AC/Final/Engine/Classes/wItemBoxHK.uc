...
function bool IsShowStore(wGameManager GameMgr)
{
    local bool isShow;

    // End:0xD1
    if((((((ItemType == int(GameMgr.37)) || ItemType == int(GameMgr.38)) || ItemType == int(GameMgr.39)) || ItemType == int(GameMgr.40)) || (ItemType >= int(GameMgr.20)) && ItemType <= int(GameMgr.26)) || (ItemType >= int(GameMgr.42)) && ItemType <= int(GameMgr.45))
    {
        isShow = false;        
    }
    else
...
function bool IsShowInventory(wGameManager GameMgr)
{
    local bool isShow;
...
function int GetPartIDValue(int Hour, int Quantity, int Durability)
{
    // End:0x22
    if((CostType == 1) || CostType == 4)
    {
        return Hour;        
    }
    else
