...
simulated function PostBeginPlay()
{
    ...
    {
        VolumeEffect = new (Level.XLevel) Class'Engine_Decompressed.EFFECT_WaterVolume';
    }
    //return;    
}
...
simulated event bool PawnEnteredVolume(Pawn Other)
{
...
function Trigger(Actor Other, Pawn EventInstigator)
{
    ...
    if(DamagePerSec != float(0))
    {
        ...
        if(bPainCausing)
        {
            ...
            if(PainTimer == none)
            {
                PainTimer = Spawn(Class'Engine_Decompressed.VolumeTimer', self);
            }
            // End:0x5B
