...
event ModeDoFire()
{
    ...
    if(int(ExpressType) == int(1))
    {
        ...
        if(Instigator.IsLocallyControlled())
        {
            ...
            if(MWeapon == none)
            {
                MWeapon = Instigator.Inventory.WeaponChangeWClass('wKnife');
            }
            // End:0xF4
            if(MWeapon != none)
            {
                Instigator.PendingWeapon = MWeapon;
                Instigator.PendingWeapon.bUseEkey = 1;
                Instigator.PendingWeapon.SetEKeyMeleeFire();
...
function DoTrace(Vector Start, Rotator Dir)
{
    ...
    if(bExistTarg)
    {
        ...
        if(int(HitCollisionPart) != 0)
        {
            ...
            {
                fDamageAmp = GetCollisionPartsDamage(int(HitCollisionPart));                
            }
            else
            {
        ...
        WeaponAttachment(Weapon.ThirdPersonActor).FlashCount = 0;
        WeaponAttachment(Weapon.ThirdPersonActor).ThirdPersonEffects();        
    }
    else
    {
        Instigator.FmodPlaySound(FmodSound_Miss, 0,,, false);
...
simulated function ReSetMeleeFireAnim(int nWeaponID)
{
    // End:0x44
    if(Weapon.HasAnim('MeleeAttack'))
    {
