...
function AddBlockedItem(BtrDouble D)
{
    ...
    FindQuickSlotBoxBySlotPosition(0, 0, iItemID, dwUniqueID);
    // End:0x61
    if(__NFUN_912__(dwUniqueID, D))
    {
        SetInstance_QSlotItemSlotPos(0, FindFirstInstanceItemByItemID(2000), int(0));        
    }
    else
    {
        FindQuickSlotBoxBySlotPosition(0, 1, iItemID, dwUniqueID);
        // End:0x9A
        if(__NFUN_912__(dwUniqueID, D))
        {
            SetInstance_QSlotItemSlotPos(0, FindFirstInstanceItemByItemID(1010), int(1));
        }
    }
...
function int IsUsablePartItem(int iItemID, int iPartsArrayIndex)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // End:0x1A1
                if((wcpp.strAddPartGroupID != wcip.strAddPartSilencerGroupID) && wcpp.strAddPartGroupID != wcip.strAddPartMuzzleGroupID)
                {
                    return 0;
                }                
            }
...
function LoadUILists(bool bIsClient)
{
        ...
        if((cItemParam.iItemID != -1) && cItemResParam.iItemResourceID != -1)
        {
            ...
            if(((((int(cItemParam.eType) == int(20)) || int(cItemParam.eType) == int(21)) || int(cItemParam.eType) == int(26)) || int(cItemParam.eType) == int(24)) || int(cItemParam.eType) == int(25))
            {
                ...
                if((wcpp.iItemID > 0) && wcpp.iItemID == cItemParam.iItemID)
                {
                    ...
                    // End:0x346
                    if((wcpp.strPartImageName != "-") && bIsClient == true)
                    {
                ...
                {
                    // End:0x39A
                    if((cItemResParam.strRes_Icon != "-") && bIsClient == true)
                    {
            ...
            // End:0x45B
            if((cItemResParam.Resource_UI_CapsuleIcon_File != "-") && bIsClient == true)
            {
                itemBox.CapsuleImage = Texture(DynamicLoadObject(cItemResParam.Resource_UI_CapsuleIcon_File, Class'Engine_Decompressed.Texture'));
            }
            ...
            if((wcip.iItemID > 0) && ItemID == wcip.iItemID)
            {
                ...
                if(WeaponCustomPartsParams.Length > 0)
                {
                    ...
                    if((IsEnableRemovePart(cItemParam, 20)) > 0)
                    {
                        ...
                        // End:0x787
                        if(((wcpp.strPartImageName != "-") && wcpp.strPartImageName != "") && bIsClient == true)
                        {
                ...
                if(i < WeaponCustomPartsParams.Length)
                {
                    ...
                    if((IsUsablePartItem(wcip.iItemID, i)) > 0)
                    {
                        ...
                        // End:0xC6D
                        if(((cPartResParam.strRes_Icon != "-") && cPartResParam.strRes_Icon != "") && bIsClient == true)
                        {
            ...
            itemBox.CostType = int(cItemParam.ECostType);
...
function GetQuickSlotIndexOfBoxSkillItem(int UniqueSkillID, out array<int> QuickSlotIdx, out array<int> SlotPosition)
{
    local int i, j;
    ...
    if(i < QuickSlotBoxList.Length)
    {
        ...
        if(j < 4)
        {
            ...
            if(QuickSlotBoxList[i].UniqueSkillID[j] == UniqueSkillID)
            {
                QuickSlotIdx[QuickSlotIdx.Length] = i;
                SlotPosition[SlotPosition.Length] = j;
            }
            j++;
            // [Loop Continue]
            goto J0x2B;
...
function bool FindQuickSlotItemByUnqiueID(BtrDouble dwUniqueID, out int iQuickSlotIndex, out int iSlotIndex)
{
    local int lp1, lp2;
    ...
    // End:0x7B [Loop If]
    if(lp1 < QuickSlotBoxList.Length)
    {
        lp2 = 0;
        J0x1E:
        ...
        {
            // End:0x67
            if(__NFUN_912__(QuickSlotBoxList[lp1].UniqueID[lp2], dwUniqueID))
            {
                iQuickSlotIndex = lp1;
                iSlotIndex = lp2;
                return true;
...
function bool FindQuickSlotSkillByUniqueID(int dwUniqueID, out int iQuickSlotIndex, out int iSlotIndex)
{
    local int lp1, lp2;
    ...
    if(lp1 < QuickSlotBoxList.Length)
    {
        ...
        {
            // End:0x66
            if(QuickSlotBoxList[lp1].UniqueSkillID[lp2] == dwUniqueID)
            {
                iQuickSlotIndex = lp1;
                iSlotIndex = lp2;
                return true;
...
function FindQuickSlotBoxBySlotPosition(int iQuickSlotIndex, Object.SLOT_POS eSlotPos, out int iItemID, out BtrDouble UniqueID)
{
    iItemID = 0;
    switch(eSlotPos)
    {
        // End:0x4E
        case 0:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].ItemID[0];
            UniqueID = QuickSlotBoxList[iQuickSlotIndex].UniqueID[0];
            // End:0x267
            break;
        // End:0x8E
        case 1:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].ItemID[1];
            UniqueID = QuickSlotBoxList[iQuickSlotIndex].UniqueID[1];
            // End:0x267
            break;
        // End:0xD0
        case 2:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].ItemID[2];
            UniqueID = QuickSlotBoxList[iQuickSlotIndex].UniqueID[2];
            // End:0x267
            break;
        // End:0x112
        case 3:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].ItemID[3];
            UniqueID = QuickSlotBoxList[iQuickSlotIndex].UniqueID[3];
            // End:0x267
            break;
        // End:0x154
        case 4:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].ItemID[4];
            UniqueID = QuickSlotBoxList[iQuickSlotIndex].UniqueID[4];
            // End:0x267
            break;
        // End:0x197
        case 14:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].SkillID[0];
            UniqueID = __NFUN_921__(QuickSlotBoxList[iQuickSlotIndex].UniqueSkillID[0]);
            // End:0x267
            break;
        // End:0x1DA
        case 15:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].SkillID[1];
            UniqueID = __NFUN_921__(QuickSlotBoxList[iQuickSlotIndex].UniqueSkillID[1]);
            // End:0x267
            break;
        // End:0x21F
        case 16:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].SkillID[2];
            UniqueID = __NFUN_921__(QuickSlotBoxList[iQuickSlotIndex].UniqueSkillID[2]);
            // End:0x267
            break;
        // End:0x264
        case 17:
            iItemID = QuickSlotBoxList[iQuickSlotIndex].SkillID[3];
            UniqueID = __NFUN_921__(QuickSlotBoxList[iQuickSlotIndex].UniqueSkillID[3]);
            // End:0x267
            break;
        // End:0xFFFF
        default:
...
function bool AddInstanceItemByItemID(int ItemID, int PartID)
{
    local wItemBoxHK ItemInfo;
    local wItemBoxInstanceHK instanceInfo;

    ItemInfo = self.FindUIItem(ItemID);
    // End:0x23
    if(ItemInfo == none)
    {
        return false;
    }
    instanceInfo = self.FindFirstInstanceItemByItemID(ItemID);
    // End:0x46
    if(instanceInfo != none)
    {
        return false;
    ...
    instanceInfo.ItemID = ItemID;
    instanceInfo.UniqueID = EmptyBtrDouble();
    instanceInfo.UniqueSkillID = 0;
    instanceInfo.PartID = PartID;
...
function bool AddCashItem(out wItemBoxHK ItemInfo, out wItemBoxInstanceHK instanceInfo, string strEndDate)
{
    ...
    else
    {
        ...
        instanceInfo.cashItemInfo.WZEndDate = Class'Engine_Decompressed.BTCustomDrawHK'.static.FormatString(strEndDate, string(wsp.EndDate_Month), string(wsp.EndDate_Day));
        // End:0x593
...
{
    local wItemBoxInstanceHK instanceInfo;
    local int iQuickIndex, iSlotPos;
    ...
    if(SlotPos == int(0))
    {
        ...
        // End:0xD5
        if(CheckSameQuickSlotItem(0, 0, instanceInfo.ItemID, instanceInfo.UniqueID))
        {
            return true;
        }        
    }
    else
    {
        ...
        if(SlotPos == int(1))
        {
            ...
            // End:0x1B0
            if(CheckSameQuickSlotItem(0, 1, instanceInfo.ItemID, instanceInfo.UniqueID))
            {
                return true;
            }            
        }
    ...
    // End:0x1E6
    if(FindQuickSlotItemByUnqiueID(instanceInfo.UniqueID, iQuickIndex, iSlotPos))
    {
        UnsetInstance_QSlotItemSlotPos(iQuickIndex, iSlotPos);
    }
    SetInstance_QSlotItemSlotPos(0, instanceInfo, SlotPos);
    return true;
    //return;    
}
...
function bool SetInstance_QSlotItemSlotPos(int iQuickSlotIndex, wItemBoxInstanceHK instanceInfo, int iSlotPos)
{
    local int iSlotIndex;

    // End:0x13F
    if(instanceInfo != none)
    {
        Log("[wGameManager::SetInstance_ItemSlotPos] CurSlotItem ItemID = " $ string(instanceInfo.ItemID));
        iSlotIndex = ConvertQuickSlotTypeToIndex(byte(iSlotPos));
        // End:0x13F
        if(iSlotIndex != -1)
        {
            // End:0xBD
            ...
            {
                AvoidDuplication_QSlotItemSlotPosByItemID(instanceInfo.ItemID, byte(iSlotPos), iSlotIndex);                
            }
            else
            {
                AvoidDuplication_QSlotItemSlotPos(instanceInfo.ItemID, instanceInfo.UniqueID, byte(iSlotPos), iSlotIndex);
            }
            QuickSlotBoxList[iQuickSlotIndex].ItemID[iSlotIndex] = instanceInfo.ItemID;
            QuickSlotBoxList[iQuickSlotIndex].UniqueID[iSlotIndex] = instanceInfo.UniqueID;
            return true;
        }
    }
    return false;
...
function UnsetInstance_QSlotItemSlotPos(int iQuickSlotIndex, int iSlotPos)
{
    local int iSlotIndex;
    ...
    if(iSlotIndex != -1)
    {
        QuickSlotBoxList[iQuickSlotIndex].ItemID[iSlotIndex] = 0;
        QuickSlotBoxList[iQuickSlotIndex].UniqueID[iSlotIndex] = EmptyBtrDouble();
    }
    //return;    
}

function AvoidDuplication_QSlotItemSlotPos(int iItemID, BtrDouble UniqueID, Object.SLOT_POS eSlotPos, int iSlotPos)
{
    local int lp1, lp2;

    // End:0xCC
    if((int(eSlotPos) == int(2)) || int(eSlotPos) == int(3))
    {
        lp1 = 0;
        J0x29:
        ...
        if(lp1 < QuickSlotBoxList.Length)
        {
            ...
            {
                // End:0xB5
                if(__NFUN_912__(QuickSlotBoxList[lp1].UniqueID[lp2], UniqueID))
                {
                    QuickSlotBoxList[lp1].ItemID[lp2] = 0;
                    QuickSlotBoxList[lp1].UniqueID[lp2] = EmptyBtrDouble();
                    return;
                }
                ++lp2;
                // [Loop Continue]
    ...
    else
    {
        ...
        {
            // End:0x11B
            if((iItemID == 9000) && QuickSlotBoxList[lp1].ItemID[iSlotPos] == 9000)
            {
                // [Explicit Continue]
                goto J0x17F;
            }
            // End:0x17F
            if(__NFUN_912__(QuickSlotBoxList[lp1].UniqueID[iSlotPos], UniqueID))
            {
                QuickSlotBoxList[lp1].ItemID[iSlotPos] = 0;
                QuickSlotBoxList[lp1].UniqueID[iSlotPos] = EmptyBtrDouble();
                return;
            }
            J0x17F:
...
function AvoidDuplication_QSlotItemSlotPosByItemID(int iItemID, Object.SLOT_POS eSlotPos, int iSlotPos)
{
    local int lp1, lp2;
    ...
    if((int(eSlotPos) == int(2)) || int(eSlotPos) == int(3))
    {
        ...
        if(lp1 < QuickSlotBoxList.Length)
        {
            ...
            {
                // End:0xB4
                if(QuickSlotBoxList[lp1].ItemID[lp2] == iItemID)
                {
                    QuickSlotBoxList[lp1].ItemID[lp2] = 0;
                    QuickSlotBoxList[lp1].UniqueID[lp2] = EmptyBtrDouble();
                    return;
    ...
    else
    {
        ...
        if(lp1 < QuickSlotBoxList.Length)
        {
            ...
            // End:0x17D
            if(QuickSlotBoxList[lp1].ItemID[iSlotPos] == iItemID)
            {
                QuickSlotBoxList[lp1].ItemID[iSlotPos] = 0;
                QuickSlotBoxList[lp1].UniqueID[iSlotPos] = EmptyBtrDouble();
                return;
...
function bool SetInstance_SkillSlotPos(int iQuickSlotIndex, int SkillUniqueID, int iSlotPos)
{
    local wItemBoxInstanceHK instanceInfo;
    local int iSlotIndex;
    ...
    if(instanceInfo != none)
    {
        ...
        if(iSlotIndex != -1)
        {
            QuickSlotBoxList[iQuickSlotIndex].SkillID[iSlotIndex] = instanceInfo.ItemID;
            QuickSlotBoxList[iQuickSlotIndex].UniqueSkillID[iSlotIndex] = instanceInfo.UniqueSkillID;
            return true;
        }
    }
    return false;
...
function UnsetInstance_SkillSlotPos(int iQuickSlotIndex, int iSlotPos)
{
    local int iSlotIndex;
    ...
    if(iSlotIndex != -1)
    {
        QuickSlotBoxList[iQuickSlotIndex].SkillID[iSlotIndex] = 0;
        QuickSlotBoxList[iQuickSlotIndex].UniqueSkillID[iSlotIndex] = 0;
    }
    //return;    
}
...
function int GetDefaultMeleeWeaponID()
{
    return 9000;
    //return;    
}
...
function BtrDouble GetDefaultMeleeWeaponUniqueID()
{
    local int i;
    ...
    {
        // End:0x4B
        if(InstanceItemLists[i].ItemID == (GetDefaultMeleeWeaponID()))
        {
            return InstanceItemLists[i].UniqueID;
        }
        i++;
...
protected function LoadWeaponCustomInfoParameters(string strExcelFileName)
{
    ...
    if(true)
    {
        ...
        strData = excel.getData("Weapon_CustomInfo", "addpart_muzzlegroup_id", iIndex);
        // End:0x2FB
        if(strData != "-")
        {
            wci.strAddPartMuzzleGroupID = strData;
        }
...
function int GetWeaponPartsIDByCustomInfo(int iItemID)
{
    local wWeaponCustomInfoParam WeaponCustomInfo;
    local int iSightPartID, iBarrelPartID, iSilencerPartID, iGunstockPartID, iPartsID;

    iPartsID = 0;
    WeaponCustomInfo = GetWeaponCustomInfoParam(iItemID);
    // End:0x93
    if(WeaponCustomInfo != none)
    {
        iSightPartID = WeaponCustomInfo.iDefaultSightUniID;
        iBarrelPartID = WeaponCustomInfo.iDefaultBarrelUniID;
        iSilencerPartID = WeaponCustomInfo.iDefaultBarrelUniID;
        iGunstockPartID = WeaponCustomInfo.iDefaultGunstockUniID;
        iPartsID = ((iSightPartID | iBarrelPartID) | iSilencerPartID) | iGunstockPartID;
    }
    return iPartsID;
    //return;    
}

event InitHK(bool bIsClient)
{
    // End:0x7D
    if(bIsClient == true)
    {
...
{
    return Material(DynamicLoadObject(str, Class'Engine_Decompressed.Material'));
    //return;    
}
...
function LoadNPC(string strExcelFileName)
{
    ...
    local string strID, strNameGroupID, strLv, strType, strMainWeapon, strPistol,
	    strKnife, strGrenade1, strGrenade2, strSprintRate, strActiveSkill,
	    strPassive1, strPassive2, strPassive3, strSpChar_AF, strSpChar_RSA,
	    strHead, strFace, strAccessory, strHP, ActionGroupID,
	    DamageRate, ExplosionDmageRate, MeleeDamageRate, ReactionTime, DetourRate,
	    DetourWeightMin, DetourWeightMax, SafeWeightMin, SafeWeightMax, HeadshotChance,
	    RotationgSpeed, MeleeActionRange, MeleeRange, EffectiveRange, CloseRange,
	    MeleeDamage, OuterRangeSpreadMin, OuterRangeSpreadMax, EffectiveRangeSpreadMin, EffectiveRangeSpreadMax,
	    CloseRangeSpreadMin, CloseRangeSpreadMax, ZeroRangeSpreadMin, ZeroRangeSpreadMax, RewardPoint,
	    IngamePoint, BonusPoint, HeadshotChanceMG, HeadshotSpreadRate, strDefaultRunSpeed;
    ...
    if(true)
    {
        ...
        strKnife = excel.getData("NPC", "knife_groupid", iIndex);
        ...
        Param.KnifeGroupID = int(strKnife);
...
function LoadDefenceShopParam()
{
    ...
    if(DefenceShopParams.Length == 0)
    {
        ...
        if(true)
        {
            ...
            Param = new Class'Engine_Decompressed.wDefenceShopParam';
...
function LoadDefenceRewardParam()
{
    ...
    if(DefenceRewardParams.Length == 0)
    {
        ...
        if(true)
        {
            ...
            Param = new Class'Engine_Decompressed.wDefenceRewardParam';
...
function LoadDefenceLevelInfoParam(string strExcelFileName)
{
    ...
    if(true)
    {
        ...
        Param = new Class'Engine_Decompressed.wDefenceLevelInfoParam';
...
function bool LoadAlienGameParam(string strExcelFileName)
{
    ...
    AlienGameParam = new Class'Engine_Decompressed.wAlienGameParam';
    // End:0xA8
    ...
    AlienGameParam.PlayerStart_SuperHero_Count[0] = byte(excel.getData("Mode_Alien", "ps_super_hero_count_1", 0));
    AlienGameParam.PlayerStart_SuperHero_Count[1] = byte(excel.getData("Mode_Alien", "ps_super_hero_count_2", 0));
    AlienGameParam.PlayerStart_SuperHero_Count[2] = byte(excel.getData("Mode_Alien", "ps_super_hero_count_3", 0));
    AlienGameParam.fSuperAlien_KnockBackRatio = float(excel.getData("Mode_Alien", "super_alien_knock_ratio", 0));
    AlienGameParam.fSuperAlien_SpeedRatio = float(excel.getData("Mode_Alien", "super_alien_speed_ratio", 0));
    AlienGameParam.fSuperAlien_RemainingTime = float(excel.getData("Mode_Alien", "super_alien_remainingtime", 0));
...
function LoadAlienGameCharParams(string strExcelFileName)
{
    ...
    if(true)
    {
        ...
        Param = new Class'Engine_Decompressed.wAlienCharParam';
        // End:0xF8
        if(Param == none)
        {            
        }
    ...
    // End:0x9D9 [Loop If]
    if(lp1 < (AlienCharParams.Length - 1))
    {
        lp2 = lp1;
        J0x94B:

        // End:0x9CF [Loop If]
        if(lp2 < AlienCharParams.Length)
        {
            // End:0x9C5
            if(int(AlienCharParams[lp1].SkinID) > int(AlienCharParams[lp2].SkinID))
            {
                Param = AlienCharParams[lp1];
                AlienCharParams[lp1] = AlienCharParams[lp2];
                AlienCharParams[lp2] = Param;
            }
            ++lp2;
            // [Loop Continue]
            goto J0x94B;
...
function int GetStartAlienHP(wGameManager.EAlienModeType eObjType, wGameManager.EAlienMode_Object_Type eSkinType, int iPlayerCount)
{
    ...
    return iHPValue[lp1] + AlienCharParams[int(eSkinType)].add_HP[lp1];
    //return;    
}
...
function int GetMaxSuperHeroCount(int iPlayerCount)
{
    local int SelIndex;
    ...
    return int(AlienGameParam.PlayerStart_SuperHero_Count[SelIndex]);
    //return;    
}
...
function wAlienCharParam GetAlienCharParam(wGameManager.EAlienMode_Object_Type eSkinType)
{
    local int lp1;
    ...
    {
        // End:0x45
        if(int(AlienCharParams[lp1].SkinID) == int(eSkinType))
        {
...
function wAlienCharParam GetAlienCharParamByShopId(int ItemID)
{
    ...
    {
        // End:0x41
        if(AlienCharParams[lp1].ShopID == ItemID)
        {
            return AlienCharParams[lp1];
        }
        ++lp1;
...
function AddDailyQuestRewardParams(BTNetGameData GameData)
{
    ...
    if(1 != 0)
    {
        ...
        cParam = new Class'Engine_Decompressed.wDailyQuestRewardParam';
        // End:0x98
...
function bool CheckSkillSlot(Object.SLOT_POS eSlotPos)
{
    // End:0x24
    if((int(14) <= int(eSlotPos)) && int(17) >= int(eSlotPos))
    {
        return true;
    }
    return false;
...
function bool CheckWeaponSlot(Object.SLOT_POS eSlotPos)
{
    // End:0x24
    if((int(0) <= int(eSlotPos)) && int(3) >= int(eSlotPos))
    {
        return true;
    }
    return false;
...
function bool CheckExtendQuickSlot(int iItemType)
{
    local wGameManager.EItemType eType;

    eType = byte(iItemType);
    // End:0x31
    if((int(eType) == int(42)) || int(eType) == int(43))
    {
        return true;
    }
    return false;
...
function bool CheckExtendSkillSlot(int iItemType)
{
    local wGameManager.EItemType eType;

    eType = byte(iItemType);
    // End:0x31
    if((int(eType) == int(44)) || int(eType) == int(45))
    {
        return true;
    }
    return false;
...
function bool CheckTotalEquipSlotItem(int iItemType)
{
    local wGameManager.EItemType eType;

    eType = byte(iItemType);
    // End:0x31
    if((CheckQuickSlotItem(int(eType))) || CheckEquipSlotItem(int(eType)))
    {
        return true;
    }
    return false;
...
function bool CheckEquipSlotItem(int iItemType)
{
    local wGameManager.EItemType eType;
...
function bool CheckMeleeWeapon(int iItemType)
{
    return (int(byte(iItemType)) == int(18)) || int(byte(iItemType)) == int(7);
    //return;    
}
...
function bool CheckSameQuickSlotItem(int iQuickSlotIndex, Object.SLOT_POS eSlotPos, int iItemID, BtrDouble dwUniqueID)
{
    local int iSlotIndex;

    iSlotIndex = ConvertQuickSlotTypeToIndex(eSlotPos);
    // End:0x22
    if(iSlotIndex == -1)
    {
        return false;
    ...
    // End:0x6F
    if((QuickSlotBoxList[iQuickSlotIndex].ItemID[iSlotIndex] == iItemID) && __NFUN_912__(QuickSlotBoxList[iQuickSlotIndex].UniqueID[iSlotIndex], dwUniqueID))
    {
        return true;
    }
    return false;
...
function int GetItemSlotByItemType(wItemBoxHK ItemInfo, optional int iQuickSlotIndex, optional byte byTeamIndex)
{
    local int lp1;

    // End:0x17
    if(false == (IsEnableQuickSlot(iQuickSlotIndex)))
    {
        return -1;
    }
    // End:0x36
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // End:0xCA
                if(iQuickSlotIndex >= 0)
                {
                    // End:0x9C
                    if(QuickSlotBoxList[iQuickSlotIndex].ItemID[2] == 0)
                    {
                        return int(2);                        
                    }
                    else
                    {
                        // End:0xC1
                        if(QuickSlotBoxList[iQuickSlotIndex].ItemID[3] == 0)
                        {
                            return int(3);                            
                        }
                        else
            ...
            {
                // End:0xF1
                if(CheckMeleeWeapon(ItemInfo.ItemType))
                {
                    return int(4);                    
                }
                else
                {
                    ...
                    if(CheckSkillItem(ItemInfo.ItemType))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // End:0x162
                                if(CheckOnlyModeItem(ItemInfo.byModeItem))
                                {
                                    return int(15);
                                }
                                lp1 = 1;
                                ...
                                {
                                    // End:0x1A8
                                    if(QuickSlotBoxList[iQuickSlotIndex].SkillID[lp1] == ItemInfo.ItemID)
                                    {
                                        return -1;
                                    }
                                    ++lp1;
                                ...
                                {
                                    // End:0x202
                                    if(QuickSlotBoxList[iQuickSlotIndex].SkillID[lp1] == 0)
                                    {
                                        // End:0x202
                                        if(true == (IsEnableSkillSlot(lp1)))
                                        {
                                            return int(14) + lp1;
                                        }
                                    }
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                // End:0x2E6
                                                if((CheckOnlyModeItem(ItemInfo.byModeItem)) == true)
                                                {
                                                    return int(32);
                                                }
                                                // End:0x2FB
...
function bool IsEnableQuickSlot(int iQuickSlotIndex)
{
    local wItemBoxInstanceHK slotInstance;

    // End:0x0E
    if(iQuickSlotIndex < 2)
    {
        return true;
    }
    // End:0x39
    if(1 == 1)
    {
        slotInstance = FindFirstInstanceItemByItemType((int(42) + iQuickSlotIndex) - 2);
        return slotInstance != none;
    }
    return true;
    //return;    
}

function bool IsEnableSkillSlot(int iSkillSlotIndex)
{
    local wItemBoxInstanceHK slotInstance;

    // End:0x0E
    if(iSkillSlotIndex < 2)
    {
        return true;
    }
    // End:0x39
    if(1 == 1)
    {
        slotInstance = FindFirstInstanceItemByItemType((int(44) + iSkillSlotIndex) - 2);
        return slotInstance != none;
    }
    return false;
    //return;    
...
function int GetItemSlotByUniqueID(BtrDouble dwUniqueID)
{
    local int lp1, lp2;
...
function int GetItemSlotByUniqueSkillID(int UniqueSkillID)
{
    local int lp1, lp2;
    ...
    if(lp1 < QuickSlotBoxList.Length)
    {
        ...
        {
            // End:0x83
            if(QuickSlotBoxList[lp1].UniqueSkillID[lp2] == UniqueSkillID)
            {
                switch(lp2)
                {
                    // End:0x5E
...
function int ConvertSlotPosToUIIndex(Object.SLOT_POS eSlotPos)
{
    switch(eSlotPos)
    {
        // End:0x0E
...
function Object.SLOT_POS ConvertUIIndexToSlotPos(int iUIIndex)
{
    switch(iUIIndex)
    {
        // End:0x0E
...
function Object.SLOT_POS ConvertIndexToQuickSlotType(int iQuickSlotType, bool bIsSkill)
{
    // End:0x3F
    if(bIsSkill == false)
    {
        switch(iQuickSlotType)
        {
            // End:0x1A
            case 0:
                return 0;
...
function int ConvertQuickSlotTypeToIndex(Object.SLOT_POS eSlotPos)
{
    switch(eSlotPos)
    {
        // End:0x0E
...
function bool IsSameQuickSlotItem(int iQuickSlotIndex, Object.SLOT_POS eSlotPos, wItemBoxHK ItemInfo, wItemBoxInstanceHK instanceInfo)
{
    local int iItemID[3];
    local BtrDouble dwUniqueID[3];
    local int lp1;
    ...
    if(CheckSkillItem(ItemInfo.ItemType))
    {
        ...
        if(int(byte(ItemInfo.SkillUseType)) == int(0))
        {
            FindQuickSlotBoxBySlotPosition(iQuickSlotIndex, 14, iItemID[0], dwUniqueID[0]);            
        }
        else
        {
            // End:0xBE
            if(int(byte(ItemInfo.SkillUseType)) == int(1))
            {
                FindQuickSlotBoxBySlotPosition(iQuickSlotIndex, 15, iItemID[0], dwUniqueID[0]);
                FindQuickSlotBoxBySlotPosition(iQuickSlotIndex, 16, iItemID[1], dwUniqueID[1]);
                FindQuickSlotBoxBySlotPosition(iQuickSlotIndex, 17, iItemID[2], dwUniqueID[2]);
            }
        }        
    }
    else
    {
        ...
        if((int(eSlotPos) == int(2)) || int(eSlotPos) == int(3))
        {
            FindQuickSlotBoxBySlotPosition(iQuickSlotIndex, 2, iItemID[0], dwUniqueID[0]);
            FindQuickSlotBoxBySlotPosition(iQuickSlotIndex, 3, iItemID[1], dwUniqueID[1]);            
        }
        else
        {
            FindQuickSlotBoxBySlotPosition(iQuickSlotIndex, eSlotPos, iItemID[0], dwUniqueID[0]);
        }
    }
    lp1 = 0;
    J0x141:
    ...
    if(lp1 < 3)
    {
        ...
        // End:0x1DE
        if(iItemID[lp1] == ItemInfo.ItemID)
        {
            // End:0x1BD
            if(CheckSkillItem(ItemInfo.ItemType))
            {
                // End:0x1BA
                if(__NFUN_912__(__NFUN_921__(instanceInfo.UniqueSkillID), dwUniqueID[lp1]))
                {
                    return true;
                }
                // [Explicit Continue]
            ...
            // End:0x1DE
            if(__NFUN_912__(instanceInfo.UniqueID, dwUniqueID[lp1]))
            {
                return true;
            }
        }
...
function int GetAlienModeAttackBuff(int iWeaponType, int Damage, float AttackLevel)
{
    ...
    iFinalDamage = int(float(int(float(Damage) + (float(Damage) * AttackLevel))) * fAddDamageRatio);
    return iFinalDamage;
    //return;    
}
...
function bool IsUseSkill_InGame(int iGameMode, int iItemID, out byte byModeItem)
{
    ...
    if(IsUseItem_InGame(iGameMode, iItemID, byModeItem))
    {
        ...
        if(SkillBaseParam != none)
        {
            ...
            // End:0x7B
            if(iGameMode == Class'Engine_Decompressed.wGameSettings'.static.GetModeIndex_Alien())
            {
                // End:0x7B
...
static function bool IsSuperHeroType(wGameManager.EAlienModeType eObjType)
{
    // End:0x12
...
{
    // End:0x36
    if(((int(eObjType) == int(0)) || int(eObjType) == int(1)) || int(eObjType) == int(2))
    {
        return true;
    }
    return false;
...
{
    // End:0x36
    if(((int(eObjType) == int(3)) || int(eObjType) == int(4)) || int(eObjType) == int(5))
    {
        return true;
    }
    return false;
...
function AddCapsuleRewardParams(BTNetGameData GameData)
{
    ...
    if(1 != 0)
    {
        ...
        cParam = new Class'Engine_Decompressed.wCapsuleRewardParam';
