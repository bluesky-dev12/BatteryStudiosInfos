...
replication
{
    ...
    reliable if(int(Role) == int(ROLE_Authority))
        ...
        ReSetMeleeFireAnim_Client, SetHands_Client, 
        wLoadOut_Client;
    ...
    reliable if(int(Role) < int(ROLE_Authority))
        ...
        ServerThrowingWeapon, SetEKeyMeleeFire;
}
...
simulated function ClientWeaponSet(bool bPossiblySwitch)
{
    ...
    else
    {
        ...
        {
            // End:0x480
            if((Level.GetMatchMaker().kGame_GameMode == Class'Engine_Decompressed.wGameSettings'.static.GetModeIndex_DOA()) || int(Level.GetMatchMaker().1) == int(Level.GetMatchMaker().eWeaponLimit))
            {
                nStartWeaponIdx = 3;
            }
        }
    ...
    // End:0x4E6
    if(((Instigator.Weapon == none) && BaseParams.iInventoryGroup == nStartWeaponIdx) || Instigator.IsA('wSentryGunPawn'))
    {
        Instigator.PendingWeapon = self;
        Instigator.ChangedWeapon();        
    }
...
simulated function BringUp(optional wWeapon PrevWeapon)
{
    ...
    // End:0x5E5
    if((none != OldWeapon) && OldWeapon.IsA('wDemopack'))
    {
...
simulated function ProcessMoveAni()
{
    ...
    else
    {
        ...
        {
            // End:0x102
            if(int(1) == int(Instigator.Physics))
            {
                LoopAnim(RunAnim, 1.0000000, 0.0000000);
            }
        }
    }
    //return;    
...
simulated function SetEKeyMeleeFire()
{
    bUseEkey = 1;
    //return;    
}
...
{
    // End:0x24
    if((int(4) != int(ClientState)) && int(5) != int(ClientState))
    {
        return false;
    }
    // End:0x47
...
simulated function Material Get1stCamouflageMt(int iItemID, byte byTeam)
{
    ...
    if(int(byTeam) == 0)
    {
        ...
        {
            mt = Material(GM.DynamicLoadObject(str1stCamouflage, Class'Engine_Decompressed.Material'));
        }        
    }
    else
    {
...
function ReSetMeleeFireAnim_Server(int nWeaponID)
{
    wMeleeFire(FireMode[2]).ReSetMeleeFireAnim(nWeaponID);
    ReSetMeleeFireAnim_Client(nWeaponID);
    //return;    
}

simulated function ReSetMeleeFireAnim_Client(int nWeaponID)
{
    // End:0x2C
    if(int(Role) != int(ROLE_Authority))
    {
    ...
    if(iSilencerPartID > (1 << 17))
    {
        ...
        if(wcpp.iItemID > 0)
        {
            ...
            if(ItemResParam.iItemResourceID != -1)
            {
                ...
                {
                    AddParts(7, ItemResParam.strRes_3rd_RSA_Backpack, "", "", vect(0.0000000, 0.0000000, 0.0000000), wcpp.iItemID, iAddPartsID);                    
                }
                else
                {
...
simulated function AddParts(wWeaponAcce.EAcceType Type, string szName, string szSkinName, string strHaveDotName, Vector AimDiff, int iItemID, int iPartsGroupID, optional bool bSightSub)
{
    ...
    if(Parts.Length == i)
    {
        ...
        if(Len(szSkinName) > 1)
        {
            Parts[i].Skins[0] = Material(DynamicLoadObject(szSkinName, Class'Engine_Decompressed.Material'));
        }
        // End:0x284
