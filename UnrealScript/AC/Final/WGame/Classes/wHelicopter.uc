...
var string CONFIG_FinalExplosionSoundExplosion;
...
simulated function HeliExplosionWhenDestroyed()
{
    ...
    if(!bHidden)
    {
        FmodPlaySound(CONFIG_FinalExplosionSoundExplosion, 10);
    }
    //return;    
}
...
simulated function ClientPlayMoveSound()
{
    ...
    if(!bHidden)
    {
        ...
        {
            FmodPlaySoundUseID(MoveSound, 0, 1.0000000, false, true, true);            
        }
        else
        {
            FmodPlaySoundUseID(MoveDamagedSound, 0, 1.0000000, false, true, true);
        }
    }
    //return;    
}
...
simulated function ClientPlayStaySound()
{
    ...
    if(!bHidden)
    {
        ...
        if(float(Health) > (float(default.Health) * 0.3000000))
        {
            FmodPlaySoundUseID(StaySound, 0, 1.0000000, false, true, true);            
        }
        else
        {
            FmodPlaySoundUseID(MoveDamagedSound, 0, 1.0000000, false, true, true);
...
static function bool SpawnHeli(LevelInfo Level, wHelicopter lastHeli, Controller Caller)
{
    ...
    Caller.PlayerReplicationInfo.abySupplyItemFlag[int(2)] = 0;
    // End:0x202
    if(wHeli == none)
    {
    ...
    wHeli.Controller.SetCaller(Caller);
...
defaultproperties
{
    ...
    CONFIG_FinalExplosionSoundExplosion="Warfare_Sound_Vehicle/bombing/veh_bombing_burst_near"
    CONFIG_FinalExlosionSoundRadius=3000.0000000
    bBombExplosion=true
    MinimapIconAlly=(WidgetTexture=TexRotator'Warfare_GP_UI_HUD_ETC.minimapicon.icon_wh_heli_rot',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=127,Y2=127),TextureScale=0.6500000,DrawPivot=8,PosX=0.0000000,PosY=0.0000000,OffsetX=86,OffsetY=104,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
    MinimapIconEnemy=(WidgetTexture=TexRotator'Warfare_GP_UI_HUD_ETC.minimapicon.icon_r_heli_rot',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=127,Y2=127),TextureScale=0.6500000,DrawPivot=8,PosX=0.0000000,PosY=0.0000000,OffsetX=86,OffsetY=104,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
    bMeleeFighter=false
    bCanDodge=false
    DodgeSkillAdjust=4.0000000
    ProjectileClass=Class'Engine.wRocket'
    ScoringValue=5
    StaySound="Warfare_Sound_Vehicle/hind/veh_hind_stop"
    MoveSound="Warfare_Sound_Vehicle/hind/veh_hind_move"
    MoveDamagedSound="Warfare_Sound_Vehicle/hind/veh_hind_damaged"
    SpindownSound="Warfare_Sound_Vehicle/hind/veh_hind_spindown"
    CONFIG_SPINDOWN_SOUND_RADIUS=2000.0000000
    sndScream=""
    sndDie=""
