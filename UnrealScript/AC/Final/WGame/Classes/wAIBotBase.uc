...
function InitBot(int Grade)
{
    ...
    // End:0xE0
    if(IsA('wAISentryGunController'))
    {
        // End:0xC6
        if("WMission.wNDFGameInfo" == Level.GRI.GameClass)
        {
            NPCParam = AILevel.GetNPCParamByNPCID(201);            
        }
        else
        {
            NPCParam = AILevel.GetNPCParamByNPCID(200);
        }        
    }
    else
    {
...
function SetupWeapons(Pawn aPawn)
{
    ...
    items[items.Length] = AILevel.GetItemParamByGroupID(NPCParam.KnifeGroupID);
...
function Rotator AdjustAim(FireProperties FiredAmmunition, Vector projStart, int AimError)
{
    ...
    if(pawnTarget != none)
    {
        ...
        // End:0x1BC
        if(int(Pawn.TypeOfWeapon) == int(18))
        {
            // End:0xCF
            if(dist <= NPCParam.CloseRange)
            {
                distRatio = dist / NPCParam.CloseRange;
                distFactorMax = distRatio * NPCParam.CloseRangeSpreadMax;                
            }
            else
            {
                // End:0x140
                if(dist <= NPCParam.EffectiveRange)
                {
                    ...
                    distFactorMax = distRatio * NPCParam.EffectiveRangeSpreadMax;                    
                }
                else
                {
                    distRatio = (dist - NPCParam.EffectiveRange) / (outerRange - NPCParam.EffectiveRange);
                    distFactorMax = distRatio * NPCParam.OuterRangeSpreadMax;
                }
            }
            aimErrorX = float(SignedRand(0, int(distFactorMax)));
            aimErrorY = float(SignedRand(0, int(distFactorMax)));            
        }
        else
        {
            // End:0x268
    ...
    {
        // End:0x677
        if(int(18) == int(Pawn.TypeOfWeapon))
        {
            lookCoords = WeaponAttachment(Pawn.Weapon.ThirdPersonActor).GetBoneCoords('Bone_Flash');
            LookDir = Rotator(lookCoords.Origin - projStart);            
        }
        else
        {
