...
function ServerRequestWeapon(int iWeaponID)
{
    ...
    if(Pawn != none)
    {
        ...
        if(kUser != none)
        {
            ...
            {
                // End:0x1C1
                if(kUser.EquippedQSlots[0].ItemID[i] == iWeaponID)
                {
                    W = Pawn.AddWeapon(kUser.EquippedQSlots[0].ItemUniqueID[i], kUser.EquippedQSlots[0].ItemID[i], kUser.EquippedQSlots[0].PartsID[i], false);
                    added = true;
                    // [Explicit Break]
                    goto J0x1CB;
                }
...
function ServerCallSentryGunInstall(Controller callingController)
{
    ...
    Level.Game.AnnounceSentryGun_Spawn(callingController);
    //return;    
}
...
function bool IsActiveMyTeamSentryGun()
{
    ...
    // End:0x65 [Loop If]
    if(i < Level.SentryGuns.Length)
    {
        // End:0x5B
        if(Level.SentryGuns[i].TeamIndex == PlayerReplicationInfo.Team.TeamIndex)
        {
            return true;
        }
        ++i;
...
simulated function bool ActivateSupplyItem(Controller callingController, Object.ESUPPLY_ITEM_TYPE eSupplyItemType)
{
    // End:0x19
