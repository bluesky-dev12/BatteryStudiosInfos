...
replication
{
    ...
    reliable if(int(Role) == int(ROLE_Authority))
        ...
        bIsTurreting, bReloading, 
        bWaitingPreAnim, bWasReloading;
    ...
    // Pos:0x0A8
    reliable if((bNetDirty && bNetInitial) && int(Role) == int(ROLE_Authority))
        HealthMax;
...
function TakeHeadRoundBullet(Vector HitLocation, int WeaponType)
{
    ...
    PlaySoundHK2(Sound(DynamicLoadObject("Warfare_Sound_Char.whizby.char_whizby", Class'Engine.Sound')), 0, 1.0000000,, 50.0000000,, true,,, false, HitLocation);
    //return;    
}
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
{
    ...
    // End:0x4A0
    if(Killer.IsA('wHelicopterController'))
    {
