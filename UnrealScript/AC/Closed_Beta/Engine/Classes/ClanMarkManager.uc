...
{
    GameMgr = gm;
    ex_clanmark = Class'Engine.BTCustomDrawHK'.static.MakeImage(32, 32, 28, Material(DynamicLoadObject("Warfare_UI_Clan.Clan_Mark.ex_clanmark", Class'Engine.Material')));
    ex_clanmark_01 = Class'Engine.BTCustomDrawHK'.static.MakeImage(32, 32, 28, Material(DynamicLoadObject("Warfare_UI_Clan.Clan_Mark.ex_clanmark_01_GP", Class'Engine.Material')));
    ex_clanmark_02 = Class'Engine.BTCustomDrawHK'.static.MakeImage(32, 32, 28, Material(DynamicLoadObject("Warfare_UI_Clan.Clan_Mark.ex_clanmark_02_GP", Class'Engine.Material')));
    //return;    
}
...
function bool AddClanMark(string ClanName, int Pattern, int BG, int BL, optional bool bSetClanState, optional bool bClanLF)
{
    ...
    if(Index >= 0)
    {
        ...
        cnam = new Class'Engine.ClanNameAndMark';
        ...
        if(ClanMarkList[Index].bClanLF)
        {
            switch(Phase)
            {
                ...
                case 0.0000000:
                    ...
                    {
                        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ex_clanmark, float(X1), float(Y1), float(X2), float(Y2));
                        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ClanMarkList[Index].ClanLevel_Img, float(X1), float(Y1), float(X2), float(Y2));                        
                    }
                    else
                    {
                        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ClanMarkList[Index].BL_Img, float(X1), float(Y1), float(X2), float(Y2));
                        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ClanMarkList[Index].BG_Img, float(X1), float(Y1), float(X2), float(Y2));
                        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ClanMarkList[Index].Pattern_Img, float(X1), float(Y1), float(X2), float(Y2));
                        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ClanMarkList[Index].ClanLevel_Img, float(X1), float(Y1), float(X2), float(Y2));
                    }
                    // End:0x654
                    break;
                // End:0x41F
                case 1.0000000:
                    Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ex_clanmark_01, float(X1), float(Y1), float(X2), float(Y2), AccumDelta);
                    // End:0x2DC
                    if(ClanMarkList[Index].Pattern == 0)
                    {
                        Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ex_clanmark, float(X1), float(Y1), float(X2), float(Y2), 1.0000000 - AccumDelta);
                        Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ClanMarkList[Index].ClanLevel_Img, float(X1), float(Y1), float(X2), float(Y2), 1.0000000 - AccumDelta);                        
                    }
                    else
                    {
                        Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ClanMarkList[Index].BL_Img, float(X1), float(Y1), float(X2), float(Y2), 1.0000000 - AccumDelta);
                        Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ClanMarkList[Index].BG_Img, float(X1), float(Y1), float(X2), float(Y2), 1.0000000 - AccumDelta);
                        Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ClanMarkList[Index].Pattern_Img, float(X1), float(Y1), float(X2), float(Y2), 1.0000000 - AccumDelta);
                        Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ClanMarkList[Index].ClanLevel_Img, float(X1), float(Y1), float(X2), float(Y2), 1.0000000 - AccumDelta);
                    }
                    // End:0x654
                    break;
                // End:0x45F
                case 2.0000000:
                    Class'Engine.BTCustomDrawHK'.static.DrawImage(C, ex_clanmark_01, float(X1), float(Y1), float(X2), float(Y2));
                    // End:0x654
                    break;
                // End:0x651
                case 3.0000000:
                    ...
                    Class'Engine.BTCustomDrawHK'.static.DrawImage_Alpha(C, ex_clanmark_01, float(X1), float(Y1), float(X2), float(Y2), 1.0000000 - AccumDelta);
                    // End:0x654
                    break;
                // End:0xFFFF
                default:
