...
event PlayerController Login(string Portal, string Options, out string Error)
{
    ...
    NewPlayer.PlayerReplicationInfo.SideID = int(PickTeam(iSellectTeam, none));
    // End:0x9C4
    if((int(Level.GetMatchMaker().GMLevelFlag) & 2) == 2)
    {
    ...
    if(C != none)
    {
        ...
        if((((PC != none) && !bGameEnded) && !IsInState('MatchOver')) && kMM.InGamePlaying)
        {
            ...
            if((Killer.PlayerReplicationInfo == C.PlayerReplicationInfo) || Killer.Pawn == Pawn(PC.ViewTarget))
            {
                ...
                if(-1 != iArray)
                {
                    PC.ReceiveLocalizedMessage(Class'Engine.wMultiKillMessage', iArray);
...
{
    local int TempSwitch, IsHeadShot, IsEnableAim, IsAimShot, IsWallShot, IsPinPointKill;
...
function NavigationPoint FindPlayerStart(Controller Player, optional byte inTeam, optional string incomingName)
{
    ...
    // End:0x167
    if((((Player != none) && Player.StartSpot != none) && int(Level.NetMode) == int(NM_Standalone)) && bWaitingToStartMatch || (Player.PlayerReplicationInfo != none) && Player.PlayerReplicationInfo.bWaitingPlayer)
    {
...
defaultproperties
{
    ...
    GameAddHeliExp=15.0000000
    GameAddHeliPoint=5.0000000
