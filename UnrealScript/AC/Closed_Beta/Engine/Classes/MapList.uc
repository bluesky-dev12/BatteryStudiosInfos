...
event PreBeginPlay()
{
    ...
    Class'Engine.CacheManager'.static.GetMapList(CachedMaps);
    // End:0x4A
    if(HasInvalidMaps())
    {
        MapNum = 0;
...
function string GetNextMap()
{
    ...
    else
    {
        ...
        if(i == Maps.Length)
        {
            Class'Engine.MaplistRecord'.static.CreateMapItem(CurrentMap, Item);
            Class'Engine.MaplistRecord'.static.CreateMapItemList(Maps, ArItem);
            i = 0;
            J0xA6:
            ...
            {
                // End:0xDB
                if(Class'Engine.MaplistRecord'.static.CompareItems(Item, ArItem[i]))
                {
                    // [Explicit Break]
                    goto J0xE5;
                }
            ...
            if(i == ArItem.Length)
            {
                ...
                {
                    // End:0x131
                    if(Class'Engine.MaplistRecord'.static.CompareItemsSlow(Item, ArItem[i]))
                    {
                        // [Explicit Break]
                        goto J0x13B;
                    }
...
function int FindCacheIndex(string MapName)
{
    ...
    tmp = Class'Engine.MaplistRecord'.static.GetBaseMapName(MapName);
    i = 0;
    J0x21:
...
static function bool StaticHasInvalidMaps(optional bool bReadOnly)
{
    ...
    Class'Engine.CacheManager'.static.GetMapList(Recs);
    ...
    if(i >= 0)
    {
        ...
        URL = default.Maps[i];
        MapName = Class'Engine.MaplistRecord'.static.GetBaseMapName(URL);
        j = 0;
        J0x91:
