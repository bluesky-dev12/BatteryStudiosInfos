...
replication
{
    ...
    reliable if(int(Role) < int(ROLE_Authority))
        ...
        ServerSetVel, ServerUAV;

    // Pos:0x0AC
    reliable if(int(Role) == int(ROLE_Authority))
        ClientOnBeginIntervented, ClientOnBeginRound, 
        ClientOnBeginWaitingForStart, ClientOnEndIntervented, 
        ClientOnEndWaitingForStart, ClientTestAllWeapons, 
        ...
        if(fSprintTime < Pawn.wMyParam.Stm_MaxTime)
        {
            ...
            if((int(Pawn.Physics) == int(1)) || int(Pawn.Physics) == int(11))
            {
                ...
                else
                {
                    ...
                    {
                        fSprintTime += (DeltaTime * 1.4000000);
                    }
                    // End:0x7AD
                    if(fSprintTime > Pawn.wMyParam.Stm_MaxTime)
                    {
