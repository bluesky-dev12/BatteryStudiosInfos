...
function ShakeView()
{
    ...
    if(P != none)
    {
        ...
        if(Weapon.IsA('wGun') && int(wGun(Weapon).AimState) == int(2))
        {
            ...
            if(bUseWT)
            {
                P.wShakeView(Range.Pitch, Range.Yaw, BaseParams.fViewKickAds_Accel, BaseParams.fViewKickAds_Decel, BaseParams.fRateOfFire + BaseParams.fViewKickAds_StayTime, BaseParams.fViewKickAds_DecaySpeed, fDecayFastBeginTime);                
            }
            else
            {
        ...
        else
        {
            ...
            if(bUseWT)
            {
                P.wShakeView(Range.Pitch, Range.Yaw, BaseParams.fViewKickHip_Accel, BaseParams.fViewKickHip_Decel, BaseParams.fRateOfFire + BaseParams.fViewKickHip_StayTime, BaseParams.fViewKickHip_DecaySpeed, fDecayFastBeginTime);                
            }
            else
            {
...
function PlaySoundTeam()
{
    ...
    if(int(Weapon.WeaponType) == int(5))
    {
        ...
        strSound = Class'Engine.wGameManager'.default.VoicePackageName $ ResParams.default.str_sound_Grenade;
        ...
        Weapon.PlayOwnedSound(Sound(DynamicLoadObject(strSound, Class'Engine.Sound')), 7, 1.0000000,, fRadius,,,, Instigator.GetTeamNum());
        Level.Game.TeamReceiveLocalizedMessageEx(byte(Instigator.GetTeamNum()), Class'Engine.wMessage_Game', iCodeID, Instigator.PlayerReplicationInfo,,, vLocation, fMessageRange, true);
    }
    //return;    
}
