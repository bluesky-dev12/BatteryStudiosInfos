...
event Sound GetSound(string AName)
{
    ...
    {
        // End:0x77
        if(AName == Class'Engine.AnnouncerVoice'.default.CachedSounds[i].CacheName)
        {
            return Class'Engine.AnnouncerVoice'.default.CachedSounds[i].CacheSound;
        }
        i++;
        // [Loop Continue]
        goto J0x21;
    }
    NewSound = Class'Engine.AnnouncerVoice'.static.PrecacheSound(strSoundName, AName);
    return NewSound;
    //return;    
}
...
static function Sound PrecacheSound(string AName, optional string strFullName)
{
    ...
    // End:0x28
    if(Class'Engine.AnnouncerVoice'.default.CachedSounds.Length > 500)
    {
        Class'Engine.AnnouncerVoice'.static.ResetPrecacheSound();
    }
    NewSound = PrecacheFallbackPackage(Class'Engine.AnnouncerVoice'.default.AlternateFallbackSoundPackage, AName, strFullName);
    // End:0xDA
    if(NewSound == none)
    {
        Warn((((((("Could not find " $ AName) $ " in ") $ Class'Engine.AnnouncerVoice'.default.SoundPackage) $ " nor in fallback package ") $ Class'Engine.AnnouncerVoice'.default.FallbackSoundPackage) $ "nor in Alternate") $ Class'Engine.AnnouncerVoice'.default.AlternateFallbackSoundPackage);
    }
    return NewSound;
    //return;    
}
