    ...
    if(int(iAction) == int(1))
    {
        ...
        if(Controller.ViewportOwner.Actor.Level.GetIsServiceBuild())
        {
            ...
            if(page_Main.bConnectedLoginServer)
            {
                ...
                Log(((((("page_Main.TcpLogin.sfReqLoginWithGlobal(" $ page_Main.AuthLogin_AccountID) $ ", ") $ page_Main.AuthLogin_GUID) $ ", ") $ page_Main.AuthLogin_AuthKey) $ ")");
        ...
        if((bSaveConnectLoginServer == false) && page_Main.bConnectedLoginServer == true)
        {
            ...
            else
            {
                ...
                if(page_Main.bConnectedLoginServer)
                {
                    ...
                    page_Main.AuthLogin_AuthKey = Controller.ViewportOwner.Actor.Level.AuthLogin_AuthKey;
                    page_Main.AuthLogin_RegionKey = Controller.ViewportOwner.Actor.Level.AuthLogin_RegionKey;
                    Log(((((((("page_Main.TcpLogin.sfReqLoginWithGlobal(" $ page_Main.AuthLogin_AccountID) $ ", ") $ page_Main.AuthLogin_GUID) $ ", ") $ page_Main.AuthLogin_AuthKey) $ ",") $ page_Main.AuthLogin_RegionKey) $ ")");
                    page_Main.TcpLogin.sfReqLoginWithGlobal(page_Main.AuthLogin_AccountID, page_Main.AuthLogin_GUID, page_Main.AuthLogin_AuthKey);                    
                }
                else
                {
...
function bool rfAckClientChecksum(int Result)
{
    ...
    if(Result == 0)
    {
        ...
        if(page_Main.bConnectedLoginServer)
        {
            ...
            page_Main.AuthLogin_AuthKey = Controller.ViewportOwner.Actor.Level.AuthLogin_AuthKey;
            page_Main.AuthLogin_RegionKey = Controller.ViewportOwner.Actor.Level.AuthLogin_RegionKey;
            Log(((("page_Main.TcpLogin.sfReqLoginWithGlobal(" $ page_Main.AuthLogin_AccountID) $ ", ") $ page_Main.AuthLogin_GUID) $ " )");
            page_Main.TcpLogin.sfReqLoginWithGlobal(page_Main.AuthLogin_AccountID, page_Main.AuthLogin_GUID, page_Main.AuthLogin_AuthKey);            
        }
        else
        {
