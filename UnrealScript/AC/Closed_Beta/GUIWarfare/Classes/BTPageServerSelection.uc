...
var export editinline BTOwnerDrawCaptionButtonHK ChannelAD;
...
function bool rfAckNotifyExpiredSkillIDs(int UserID, array<BtrDouble> ExpiredSkillUniqueIDs, array<int> ExpiredSkillIDs, int Result, string ErrMsg, array<byte> ExpiredSkillSlotPositions)
{
    ...
    {
        page_Main.AddExpiredSkillItem(ExpiredSkillIDs[i], int(ExpiredSkillSlotPositions[i]));
        GameMgr.RemoveInstanceItemByUniqueSkillID(__NFUN_923__(ExpiredSkillUniqueIDs[i]));
        GameMgr.RemoveQuickSlotBoxSkillItem(__NFUN_923__(ExpiredSkillUniqueIDs[i]));
        i++;
        // [Loop Continue]
        goto J0x88;
    }
...
function bool rfAckBuyShopItem(int ItemCount, array<int> Result, array<int> BoughtItemTypeID, array<BtrDouble> BoughtItemUniqueID, array<int> BoughtPartID, array<BtrDouble> BoughtUntilTIme, array<int> BoughtStackCnt, int PointRemain, int CashRemain)
{
    ...
    if(BTWindow != none)
    {
        ...
        {
            BTWindow.PopExpiredCashInfo(ReservedCategoryID, ReservedPackageID, ReservedPriceID);
            // End:0x450
            if(ReservedCategoryID == -1)
            {
                Log("sfReqBuyShopItem(" $ string(ReservedItemID));
                page_Main.TcpChannel.sfReqBuyShopItem(ReservedItemID, __NFUN_921__(0), 1, 1);                
            }
            else
            {
                temp.Length = 1;
                ...
                page_Main.TcpChannel.sfReqBuyItemByWebzenCash(ReservedPackageID, ReservedCategoryID, ReservedPriceID, temp, __NFUN_921__(0), 1, 1);
            }
        }        
    }
    else
...
function bool rfAckCharInfoFromLoginServer(int Result, string CharName, int Level, int Exp)
{
    local wMyPlayerStatus MyStatus;
...
{
    Log("[BTPageServerSelection::rfAckPopUpMessage()] MessageType=" $ string(MessageType));
