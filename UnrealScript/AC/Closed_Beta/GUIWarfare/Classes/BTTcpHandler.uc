...
function bool rfAckFriendPosition(int Result, string TargetCharName, string fServername, string fServerShortName, string fServerIP, int fServerPort, byte CurPos, byte CurState, int ChannelNum, string ChannelNickName, int GameRoomNum)
{
    ...
    // End:0xB1 [Loop If]
    if((i > 0) && !MainMenu.Pages[--i].TcpChannel.rfAckFriendPosition(Result, TargetCharName, fServername, fServerShortName, fServerIP, fServerPort, CurPos, CurState, ChannelNum, ChannelNickName, GameRoomNum))
    {
        // [Loop Continue]
        goto J0x3C;
    }
