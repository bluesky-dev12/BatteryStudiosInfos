...
function CallEvent(optional bool ItemCheck, optional int iReservation1, optional int iReservation2)
{
    ...
    switch(iReservation1)
    {
        ...
        case 100:
            // End:0xCE
            if(float(iReservation2) > 0.0000000)
            {
                fDrawEffectTime2 = float(iReservation2) / 10.0000000;
            }
            fBeginDrawItem = Level.TimeSeconds;
            fEndOfLife = ((((Level.TimeSeconds + fDrawItemTime) + fFadeOutTime) + fDrawEffectTime1) + fDrawEffectTime2) + fDrawEffectTime3;
            bDrawItem = true;
            iDrawEffectState = 1;
            iCurrentQuestID = 9001;
        // End:0xFFFF
        default:
            // End:0x139
            break;
...
function DrawGiftItem(Canvas C)
{
    ...
    WGiftItemBG.WidgetTexture = Material(DynamicLoadObject("Warfare_GP_UI_UI.list_item_store_n", Class'Engine.Material'));
...
function DrawGiftItemT(Canvas C)
{
    ...
    local bool bGiftIsPoint;
    local string sItemDisplayName, sRes_Icon;
    local float fPassingTime, fStartValue, fCurrentValue, fDestValue, fFinalValue, fDeltaValue,
	    fFadeValue;
...
defaultproperties
{
    ...
    lsCMoveTrainingField2="Follow the arrows to the RPG range."
    lsQPickUpRocketLauncher="Pick up the RPG."
    lsCPickUpRocketLauncher="Approach the RPG and press [F]."
    lsQDoFireRocketLauncher="Shoot the target with the RPG."
    lsCDoFireRocketLauncher="The explosion caused by RPGs can also damage you. Use caution when firing at nearby targets."
    lsQDoFireRocketLauncherEnd="RPG training complete."
    ...
    WQuestKeyRightMouse=(WidgetTexture=Texture'Warfare_GP_UI_HUD.Common.img_keyboard_2',RenderStyle=5,TextureCoords=(X1=376,Y1=160,X2=440,Y2=224),TextureScale=1.0000000,DrawPivot=0,PosX=0.0000000,PosY=0.0000000,OffsetX=768,OffsetY=289,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
}