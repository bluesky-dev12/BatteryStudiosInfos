...
function InitializeLeftPanel()
{
    ...
    ButtonWeapon[4].DefaultSlotImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.new_img_slot_hlmt;
    ButtonWeapon[5].DefaultSlotImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.new_img_slot_face;
    ButtonWeapon[6].DefaultSlotImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.new_img_slot_gogl;
    ButtonWeapon[7].DefaultSlotImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.new_img_slot_clth_af;
    ButtonWeapon[8].DefaultSlotImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.new_img_slot_clth_rsa;
    ButtonWeapon[9].DefaultSlotImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.new_img_slot_class;
    ButtonWeapon[10].DefaultSlotImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.new_img_slot_clth_af;
        ...
        if((TPItemList.ItemList.bShowOnlyInventory == false) || ro.instanceInfo.SlotPosition == 13)
        {
            ...
            if(Count < 3)
            {
                ...
                if(bool(iItemType & gm.FlagPCBang) == true)
                {
                    temp.bDisDelay = true;
                    temp.bDisModi = true;
                    temp.bDisRepair = true;
                    temp.bDisSell = true;                    
                }
                else
                {
                    temp.bDelay = bool(iItemType & gm.FlagRenew);
                    temp.bDisDelay = !bool(iItemType & gm.FlagRenew);
                    temp.bSell = true;
                    temp.bDisSell = !bool(iItemType & gm.FlagSellShop);
                    temp.bModify = bool(iItemType & gm.FlagModify);
                    // End:0x2DD
                    if(ro.ItemInfo.ItemType <= 4)
                    {
                        // End:0x2A3
                ...
                temp.bDisModi = true;
                temp.dbUniqID = ro.instanceInfo.UniqueID;
                temp.nSkillID = ro.instanceInfo.UniqueSkillID;
                temp.Init();
...
function bool Internal_OnPreDraw(Canvas C)
{
    ...
    if(i < TPItemList.ItemList.SubMenuBtn.Length)
    {
        ...
        if(temp.ClickBtnIndex == 4)
        {
            ...
            {
                // End:0x5BF
                if(TPItemList.ItemList.DataPool[nIndex].ItemInfo.ItemType == 22)
                {
                    // End:0x5BC
                ...
                if((BTROItemBoxHK(ro).ItemInfo.CostType == 4) || BTROItemBoxHK(ro).ItemInfo.CostType == 1)
                {
                    ...
                    BTWindowItemRenew(Controller.TopPage()).SetRenderObject(BTROItemBoxHK(ro), BTROItemBoxHK(ro).ItemInfo, BTROItemBoxHK(ro).instanceInfo, BTROItemBoxHK(ro).instanceInfo.CashItemInfo);
                    BTWindowItemRenew(Controller.TopPage()).SetMoney(MM.kPoint, MM.kCash);
...
{
    strSlot="Slot"
    // Reference: ButtonGroups'GUIWarfareControls_Decompressed.BTTPInventoryWeaponHKCN.mgroups'
    begin object name="mgroups" class=GUIWarfareControls_Decompressed.ButtonGroups
    end object
    InvenGroup=mgroups
    // Reference: BTStretchedImageHK'GUIWarfareControls_Decompressed.BTTPInventoryWeaponHKCN.mBackgroundImage'
    ...
    // Referen[0]ce: TabControlMocker'GUIWarfareControls_Decompressed.BTTPInventoryWeaponHKCN.mTabControl'
    begin object name="mTabControl" class=GUIWarfareControls_Decompressed.TabControlMocker
    end object
    TabControl=mTabControl
    // Reference: TabControlMocker'GUIWarfareControls_Decompressed.BTTPInventoryWeaponHKCN.mTabControl2'
    ...
    OnPreDraw=BTTPInventoryWeaponHKCN.Internal_OnPreDraw
    OnRendered=BTTPInventoryWeaponHKCN.Internal_OnRendered
}