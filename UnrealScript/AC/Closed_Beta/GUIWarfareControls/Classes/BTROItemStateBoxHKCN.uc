...
function bool Render(Canvas C, optional float Delta)
{
    ...
    Class'Engine.BTCustomDrawHK'.static.DrawText(C, textForm, AWinPos.X1 + float(X[0]), AWinPos.Y1 + float(i), AWinPos.X1 + float(X[1]), (AWinPos.Y1 + float(i)) + float(14));
    Class'Engine.BTCustomDrawHK'.static.DrawTextArray(C, textAmmosize, 3, 0, AWinPos.X1 + float(X[1]), AWinPos.Y1 + float(i), AWinPos.X1 + float(X[2]), (AWinPos.Y1 + float(i)) + float(14));
    Y = 3;
    i = 0;
    J0x411:
    ...
    if(i < 5)
    {
        ...
        WeaponState[i].AWinPos = Class'Engine.BTCustomDrawHK'.static.MakeFloatBox(AWinPos.X1 + float(X[1]), (AWinPos.Y1 + float(Y)) + float(j), AWinPos.X2, ((AWinPos.Y1 + float(Y)) + float(7)) + float(j));
