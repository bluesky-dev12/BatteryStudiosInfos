...
function bool Internal_OnDraw(Canvas C)
{
    ...
    if(itemBox.ItemInfo == none)
    {
        ...
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, DefaultSlotImage, RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2);
        C.ColorModulate = CanvasModulateValue;
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, DefaultSlotName[0], 0, 9.0000000, (RWinPos.X1 + float(3)) + float(2), (RWinPos.Y1 + float(3)) + float(2), RWinPos.X2, RWinPos.Y2);
