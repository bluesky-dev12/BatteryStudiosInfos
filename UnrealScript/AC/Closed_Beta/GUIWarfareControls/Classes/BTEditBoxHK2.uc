...
function bool OwnerDraw(Canvas C)
{
    ...
    if(bHudMode == false)
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, bgImg, RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2);
    }
    // End:0xEA
    ...
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        if(bHudMode == false)
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImage(C, cursorImg, RWinPos.X1 + cursorW, RWinPos.Y1, (RWinPos.X1 + cursorW) + float(3), RWinPos.Y1 + HT);
        }
        // End:0x627
        if(bSelectionMode)
        {
