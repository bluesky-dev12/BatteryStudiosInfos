...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    if(i < 5)
    {
        ...
        if(i >= 2)
        {
            LockImage[i] = new Class'GUIWarfareControls_Decompressed.BTOwnerDrawImageHK';
            LockImage[i].BackgroundImage = Class'GUIWarfareControls_Decompressed.BTUIResourcePoolHK'.default.Inven_Skill_Lock;
            LockImage[i].bUseAWinPos = true;
            LockImage[i].AWinPos.X1 = fbStartItemBtns.X1 + float(10);
            LockImage[i].AWinPos.Y1 = fbStartItemBtns.Y1 + float(10);
            LockImage[i].AWinPos.X2 = fbStartItemBtns.X2 - float(10);
            LockImage[i].AWinPos.Y2 = fbStartItemBtns.Y2 - float(10);
            LockImage[i].ApplyAWinPos();
            LockImage[i].CaptionDrawType = 4;
            AppendComponent(LockImage[i]);
        }
        ...
        btnRemoveOrRollback[i].CaptionPadding[4] = 2;
        switch(i)
        {
            // End:0x5FF
            case 0:
...
function Refresh()
{
    local int iSightPartID, iBarrelPartID, iSilencerPartID, iGunstockPartID, i;
...
function SetData(wItemBoxHK ib, wItemBoxInstanceHK ibi)
{
    ...
    local wItemBoxInstanceHK instanceInfo;
    local int iSightPartIndex, iSilencerPartIndex, iBarrelPartIndex, iGunstockPartIndex, iPaintingPartIndex, iSightPartID,
	    iSilencerPartID, iBarrelPartID, iGunstockPartID, PartIndex;
