...
function InitializeContent()
{
    ...
    BTTPOptionRadioButtonHK(Content[1]).ButtonRadio.ButtonOn.DisableMe();
    BTTPOptionRadioButtonHK(Content[1]).ButtonRadio.ButtonOff.DisableMe();
    ...
    BTTPOptionRadioButtonHK(Content[2]).ButtonRadio.ButtonOn.Caption = strUsageRecord;
    BTTPOptionRadioButtonHK(Content[2]).ButtonRadio.__OnSelect__Delegate = OnSelect_RecordOption;
    Content[3] = NewComponent(new Class'GUIWarfareControls_Decompressed.BTComboBoxHK', Class'Engine.BTCustomDrawHK'.static.MakeEmptyBox(), 0.8000000);
    BTComboBoxHK(Content[3]).__OnChange__Delegate = OnChange_RecordResoultion;
    //return;    
}
...
function LoadData()
{
    ...
    BTTPOptionRadioButtonHK(Content[2]).ButtonRadio.SetData(CurrentInfo.bRecordEnable);
    //return;    
}
...
{
    CurrentInfo.iRecordResolution = int(iRecordStartOffset) + BTComboBoxHK(Content[3]).ComboBox.GetIndex();
...
{
    Label[3].SetVisibility(bVisible);
    BTComboBoxHK(Content[3]).SetVisibility(bVisible);
    BTComboBoxHK(Content[3]).SetEnabled(bVisible);
    //return;    
}
...
function bool Internal_OnPreDraw(Canvas C)
{
    ...
    // End:0x43
    if(BTComboBoxHK(Content[3]).bVisible != CurrentInfo.bRecordEnable)
    {
        ShowRecordMenu(CurrentInfo.bRecordEnable);
    }
    //return;    
...
function OnChangeScreen(int iScreenWidth, int iScreenHeight)
{
    ...
    iRatioIndex = BTComboBoxHK(Content[3]).ComboBox.ItemCount();
    iPrevSelIndex = BTComboBoxHK(Content[3]).ComboBox.GetIndex();
    BTComboBoxHK(Content[3]).ComboBox.__OnChange__Delegate = None;
    BTComboBoxHK(Content[3]).ComboBox.Clear();
    ...
    if(i < 3)
    {
        ...
        BTComboBoxHK(Content[3]).ComboBox.AddItem(strRecordResolution[j + i] $ strRecordQuality[i]);
        J0x181:
    ...
    BTComboBoxHK(Content[3]).ComboBox.__OnChange__Delegate = OnChange_RecordResoultion;
    // End:0x233
    ...
    else
    {
        ...
        {
            iRatioIndex = Max(0, iPrevSelIndex + (BTComboBoxHK(Content[3]).ComboBox.ItemCount() - iRatioIndex));
        }
        BTComboBoxHK(Content[3]).ComboBox.SetIndex(iRatioIndex);
    }
    //return;    
}
...
{
    strLabel[0]="Voice"
    strLabel[1]="Auto Repair"
    strLabel[2]="Recording Video"
    strLabel[3]="Resolution"
