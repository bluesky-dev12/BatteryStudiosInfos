...
function bool OnDraw(Canvas C)
{
    ...
    {
        Class'Engine.BTCustomDrawHK'.static.DrawImage(C, GaugeBGImage, StateBox.AWinPos.X1 + float(BGOffsetX), StateBox.AWinPos.Y1, StateBox.AWinPos.X2 + float(BGOffsetX), StateBox.AWinPos.Y2);
    }
    StateBox.Render(C);
    return true;
    //return;    
