/*******************************************************************************
 * Decompiled by UE Explorer, an application developed by Eliot van Uytfanghe!
 * Path Engine\Classes\JumpDest.uc
 * Package Imports:
 *	Engine
 *	Core
 *
 * Stats:
 *	Properties:6
 *	Functions:4
 *
 *******************************************************************************/
class JumpDest extends NavigationPoint
    hidecategories(Lighting,LightColor,Karma,Force)
    native
    notplaceable;

var bool bOptionalJumpDest;
var() bool bForceDoubleJump;
var int NumUpstreamPaths;
var ReachSpec UpstreamPaths[8];
var Vector NeededJump[8];
var float CalculatedGravityZ[8];

function int GetPathIndex(ReachSpec Path)
{
    local int i;

    // End:0x0d
    if(Path == none)
    {
        return 0;
    }
    i = 0;
    J0x14:
    // End:0x45 [While If]
    if(i < 4)
    {
        // End:0x3b
        if(UpstreamPaths[i] == Path)
        {
            return i;
        }
        ++ i;
        // This is an implied JumpToken; Continue!
        goto J0x14;
    }
    return 0;
}

event int SpecialCost(Pawn Other, ReachSpec Path)
{
    local int Num;

    // End:0x1c
    if(Other.Physics == 4)
    {
        return 100;
    }
    // End:0x32
    if(Vehicle(Other) != none)
    {
        return 10000000;
    }
    Num = GetPathIndex(Path);
    // End:0x98
    if(Abs(Other.JumpZ / Other.PhysicsVolume.Gravity.Z) >= Abs(NeededJump[Num].Z / CalculatedGravityZ[Num]))
    {
        return 100;
    }
    return 10000000;
}

function DoJump(Pawn Other)
{
    Other.bWantsToCrouch = false;
    Other.bWantsToProne = false;
    Other.Controller.MoveTarget = self;
    Other.Controller.Destination = Location;
    Other.bNoJumpAdjust = true;
    // End:0xac
    if(bForceDoubleJump)
    {
        Other.Velocity = Other.Controller.EAdjustJump(0.0, Other.default.GroundSpeed);
    }
    // End:0xe3
    else
    {
        Other.Velocity = Other.Controller.EAdjustJump(0.0, Other.GroundSpeed);
    }
    Other.Acceleration = vect(0.0, 0.0, 0.0);
    Other.SetPhysics(2);
    Other.Controller.SetFall();
    Other.DestinationOffset = CollisionRadius;
}

event bool SuggestMovePreparation(Pawn Other)
{
    local int Num;

    // End:0x16
    if(Other.Controller == none)
    {
        return false;
    }
    Num = GetPathIndex(Other.Controller.CurrentPath);
    // End:0x8d
    if(Abs(Other.JumpZ / Other.PhysicsVolume.Gravity.Z) < Abs(NeededJump[Num].Z / CalculatedGravityZ[Num]))
    {
        return false;
    }
    DoJump(Other);
    return false;
}

defaultproperties
{
    bSpecialForced=true
}