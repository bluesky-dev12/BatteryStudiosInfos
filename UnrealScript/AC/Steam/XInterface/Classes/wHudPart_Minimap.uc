 ...
 * Path XInterface\Classes\wHudPart_Minimap.uc
 * Package Imports:
 *	XInterface
 *	Engine
 *	Core
...
function DrawMinimap_Airstrike(Canvas C)
{
    ...
    if(true == Level.IsAirStriking || Level.fAirStrikeFadeTime > 0.0)
    {
        ...
        // End:0xde
        if(VSize(v1 - v2) > HudOwner.PlayerOwner.fAirStrikeRadius + 6000.0)
        {
            return;
        }
        ...
        tx1 = float(Sprite.TextureCoords.X2) * tx1 / AirStrikeDist;
        ty1 = float(Sprite.TextureCoords.Y2) * ty1 / AirStrikeDist;
        tx2 = float(Sprite.TextureCoords.X2) * tx2 / AirStrikeDist;
        ty2 = float(Sprite.TextureCoords.Y2) * ty2 / AirStrikeDist;
        AirStrikeDist = float(C.DrawColor.A);
        C.DrawColor.A = byte(255.0 * Level.fAirStrikeFadeTime / 2.0);
        C.SetPos(AirStrikeX1 * _rX, AirStrikeY1 * _rY);
        C.DrawTile(Sprite.WidgetTexture, AirStrikeX2 - AirStrikeX1 * _rX, AirStrikeY2 - AirStrikeY1 * _rY, tx1, ty1, tx2 - tx1, ty2 - ty1);
...
{
    fFadeEffectTime=1.0
    MinimapBGMap=(WidgetTexture=TexRotator'Warfare_UI.HUD_MinimapBG.repair_rot',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=511,Y2=511),TextureScale=0.650,DrawPivot=0,PosX=0.0,PosY=0.0,OffsetX=11,OffsetY=11,ScaleMode=4,Scale=1.0,Tints=(R=255,G=255,B=255,A=192),Tints[1]=(R=255,G=255,B=255,A=192))
    MinimapMyself=(WidgetTexture=TexRotator'Warfare_UI.HUD.minimap_myself2_rot',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=63,Y2=63),TextureScale=0.650,DrawPivot=8,PosX=0.0,PosY=0.0,OffsetX=104,OffsetY=104,ScaleMode=4,Scale=1.0,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
}