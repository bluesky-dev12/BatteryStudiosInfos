 ...
 * Path UnrealGame\Classes\DMSquad.uc
 * Package Imports:
 *	UnrealGame
 *	Engine
 *	Core
...
{
    return SquadMembers.Skill >= float(4) && FRand() < 0.650 - 0.150 * float(Level.Game.NumBots);
}
...
function bool AssignSquadResponsibility(Bot B)
{
    ...
    {
        // End:0x298
        if(B.Skill > float(5) && Level.Game.NumBots == 1 && B.Pawn.Weapon.AIRating > 0.70 || B.Pawn.Weapon.bSniping)
        {
