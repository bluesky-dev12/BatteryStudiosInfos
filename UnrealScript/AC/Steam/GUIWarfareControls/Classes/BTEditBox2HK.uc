 ...
 * Path GUIWarfareControls\Classes\BTEditBox2HK.uc
 * Package Imports:
 *	GUIWarfareControls
 *	XInterface
 *	Core
...
function bool OwnerDraw(Canvas C)
{
    ...
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        if(tempString != "")
        {
            C.BtrTextSize(tempString, FontSize[self.MenuState], W, HT);
        }
        // End:0x3d5
        else
        {
            C.BtrTextSize("|", FontSize[self.MenuState], W, HT);
            W = 0.0;
        }
        // End:0x43d
        if(bHudMode == false)
        {
            class'BTCustomDrawHK'.static.DrawImage(C, cursorImg, RWinPos.X1 + W, RWinPos.Y1, RWinPos.X1 + W + float(3), RWinPos.Y1 + HT);
        }
        // End:0x627
        if(bSelectionMode)
        {
