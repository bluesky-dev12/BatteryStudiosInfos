 ...
 * Path GUIWarfareControls\Classes\BTOptionKeyButtonHK.uc
 * Package Imports:
 *	GUIWarfareControls
 *	XInterface
 *	Core
...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    self.SetControlKeyBtnImage();
    self.SetDefaultFontColor();
    self.SetFontSizeAll(9);
}
...
function bool Internal_OnClick(GUIComponent Sender)
{
    ...
    self.SetFontColorAll(class'BTUIColorPoolHK'.static.LabelWarning());
    SaveKey = BindName;
    SaveCaption = Caption;
    Caption = strNewKey;
    return true;
...
{
    // End:0x20
    if(48 <= NewKey && NewKey <= 57)
    {
        return false;
    }
    // End:0x40
    if(NewKey == 84 || NewKey == 112)
    {
        return false;
    }
    return true;
...
defaultproperties
{
    strNewKey="Enter a new key. Press ESC to cancel."
}