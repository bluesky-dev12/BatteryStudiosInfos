 ...
 * Path GUIWarfareControls\Classes\BTEditBoxHK.uc
 * Package Imports:
 *	GUIWarfareControls
 *	XInterface
 *	Core
...
function bool OwnerDraw(Canvas C)
{
    ...
    BlinkTimer = Controller.ViewportOwner.Actor.Level.TimeSeconds;
    // End:0x7b
    if(BlinkTimer - OldBlinkTimer > 0.60)
    {
    ...
    if(bHudMode == false)
    {
        ...
        class'BTCustomDrawHK'.static.DrawString(C, self.textStrVis, 3, float(FontSize[self.MenuState]), RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2, self.FontShadowColor[self.MenuState], 0, true);
    }
    // End:0x714
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        if(tempString != "")
        {
            C.BtrTextSizeOLD(tempString, FontSize[self.MenuState], cursorW, HT);
        }
        // End:0x44c
        else
        {
            C.BtrTextSizeOLD("|", FontSize[self.MenuState], cursorW, HT);
            cursorW = 0.0;
        }
        // End:0x4ef
        if(bHudMode == false && bCursorBlink)
        {
            class'BTCustomDrawHK'.static.DrawImage(C, cursorImg, RWinPos.X1 + cursorW + float(1), RWinPos.Y2 + RWinPos.Y1 - HT / float(2), RWinPos.X1 + cursorW + float(4), RWinPos.Y2 + RWinPos.Y1 + HT / float(2));
        }
        // End:0x704
        if(bSelectionMode)
        {
            ...
            else
            {
                ...
                C.BtrTextSizeOLD(tempString, FontSize[self.MenuState], X, HT);
            }
            // End:0x628
            if(E > float(self.EndVis - self.FirstVis))
            {
                C.BtrTextSizeOLD(self.textStrVis, FontSize[self.MenuState], X2, HT);
            }
            // End:0x677
            else
            {
                tempString = self.Mid(self.textStrVis, 0, int(E));
                C.BtrTextSizeOLD(tempString, FontSize[self.MenuState], X2, HT);
            }
            // End:0x704
            if(bHudMode == false)
            {
                class'BTCustomDrawHK'.static.DrawImage(C, selectionImg, RWinPos.X1 + X, RWinPos.Y2 + RWinPos.Y1 - HT / float(2), RWinPos.X1 + X2, RWinPos.Y2 + RWinPos.Y1 + HT / float(2));
            }
        }
...
function CaculateVisibleText(bool bHoldFirst, int pos)
{
    ...
    if(bHoldFirst)
    {
        ...
        if(i >= pos)
        {
            ...
            saveC.BtrTextSizeOLD(temp, FontSize[self.MenuState], scrollW, HT);
            // End:0xdc
...
function int MousePosToCaretPos(float mx, float my)
{
    ...
    {
        saveC.BtrTextSizeOLD(self.textStrVis, FontSize[self.MenuState], W, HT);
...
function int CaretBinarySearch(int lCursor, int rCursor, float mx, int Parent)
{
    ...
    if(textLen == 1)
    {
        ...
        if(self.CacheTextWidth[Parent] == float(0))
        {
            ...
            saveC.BtrTextSizeOLD(Text, FontSize[self.MenuState], W, HT);
...
function bool CheckPastedTextCondition(string Text)
{
    ...
    if(i < i2)
    {
        ch = UnresolvedNativeFunction_241(Text, i);
        st = Chr(ch);
        // End:0x55
        ...
        if(bHudMode == false)
        {
            switch(key)
            {
                ...
                case 22:
                    ...
                    if(bLimitDrawSize)
                    {
                        fb = class'BTCustomDrawHK'.static.GetDrawStringFloatBoxOLD(saveC, temp, 0, float(FontSize[self.MenuState]), 0.0, 0.0, 1024.0, 768.0);
                        W = fb.X2 - fb.X1;
                        // End:0x18a
