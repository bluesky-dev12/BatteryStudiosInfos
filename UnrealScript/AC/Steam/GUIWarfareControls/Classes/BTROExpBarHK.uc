 ...
 * Path GUIWarfareControls\Classes\BTROExpBarHK.uc
 * Package Imports:
 *	GUIWarfareControls
 *	Core
 *
...
function Init()
{
    ...
    back = class'BTUIResourcePoolHK'.default.gau_exp_back;
    Gauge = class'BTUIResourcePoolHK'.default.gau_exp_gauge;
...
function bool Render(Canvas C, optional float Delta)
{
    ...
    {
        class'BTCustomDrawHK'.static.DrawImagePadding(C, Gauge, AWinPos.X1 + float(LineWidth), AWinPos.Y1 + float(LineWidth), AWinPos.X2 - float(LineWidth), AWinPos.Y2 - float(LineWidth), 0.0, 0.0, DataPerc, 0.0);
    }
