 ...
 * Path GUIWarfareControls\Classes\BTROClanMarkHK.uc
 * Package Imports:
 *	GUIWarfareControls
 *	Core
 *
...
function SetData(wGameManager GameMgr, int ClanPattern, int ClanBG, int ClanBL, optional bool bSize128, optional int ClanLevel)
{
    ...
    ClanLevel_Img = class'BTCustomDrawHK'.static.MakeImage(16, 16, 0, GameMgr.ClanLevelTextures[ClanLevel]);
}
...
function bool Render(Canvas C, optional float Delta)
{
    ...
    {
        class'BTCustomDrawHK'.static.DrawImage(C, ClanMark[2], AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
        bExistClanMark = true;
    }
    // End:0x11b
    if(ClanMark[1].Image != none)
    {
        class'BTCustomDrawHK'.static.DrawImage(C, ClanMark[1], AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
        bExistClanMark = true;
    }
    // End:0x178
    if(ClanMark[0].Image != none)
    {
        class'BTCustomDrawHK'.static.DrawImage(C, ClanMark[0], AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
        bExistClanMark = true;
    }
    // End:0x1ce
    if(bExistClanMark == false)
    {
        class'BTCustomDrawHK'.static.DrawImage(C, class'BTUIResourcePoolHK'.default.ex_clanmark, AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
    }
    // End:0x21f
    if(ClanLevel_Img.Image != none)
    {
        class'BTCustomDrawHK'.static.DrawImage(C, ClanLevel_Img, AWinPos.X1, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
    }
    return true;
}
