 ...
 * Path GUIWarfareControls\Classes\BTPointBoxHK.uc
 * Package Imports:
 *	GUIWarfareControls
 *	XInterface
 *	Core
...
function SetData(BtrDouble Point)
{
    ...
    SavePoint = class'BTCustomDrawHK'.static.MakeText(12, 3, " " $ class'BTCustomDrawHK'.static.ToWonString(strBDPoint) $ strPoint);
    SavePoint.DrawColor = class'BTUIColorPoolHK'.static.Point();
...
function Internal_OnRendered(Canvas C)
{
    ...
    class'BTCustomDrawHK'.static.DrawImage(C, class'BTUIResourcePoolHK'.default.img_point_b, AWinPos.X1 + save_X, AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
    class'BTCustomDrawHK'.static.DrawText(C, SavePoint, AWinPos.X1 + save_X + float(imgW), AWinPos.Y1, AWinPos.X2, AWinPos.Y2);
}
