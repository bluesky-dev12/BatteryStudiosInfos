 ...
 * Path GUIWarfareControls\Classes\BTSliderHK.uc
 * Package Imports:
 *	GUIWarfareControls
 *	XInterface
 *	Core
...
class BTSliderHK extends BTTabPanelHK
    ...
    dependson(BTSliderGripButtonHK)
    editinlinenew
    instanced;
...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    fbEmpty = class'BTCustomDrawHK'.static.MakeFloatBox(0.0, 0.0, 0.0, 0.0);
    ...
    ButtonSliderGrip = BTSliderGripButtonHK(NewComponent(new class'BTSliderGripButtonHK', fbEmpty));
    ...
    LabelSliderBack = NewLabelComponent(fbEmpty, class'BTUIResourcePoolHK'.default.slider_new_back);
    self.MinValue = 0.0;
    self.MaxValue = 100.0;
    bAcceptsInput = true;
    bCaptureMouse = true;
...
function Internal_OnRendered(Canvas C)
{
    ...
    ResX = 1024.0 / float(Controller.ResX);
    ...
    if(bGrip)
    {
        ...
        else
        {
            ...
            // End:0x383
            if(tempValue < 1.50 || tempValue > 0.50)
            {
                ChangeValue = Ceil(ChangeValue - 0.50);
                // End:0x2bd
                if(CanChangeValue(int(CurrentValue), int(ChangeValue)) == false)
                {
                    return;
...
defaultproperties
{
    ...
    OnActivate=Internal_OnActivate
    OnDeActivate=Internal_OnDeactivate
}