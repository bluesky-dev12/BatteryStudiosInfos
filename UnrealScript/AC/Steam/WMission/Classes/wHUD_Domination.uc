 ...
 * Path WMission\Classes\wHUD_Domination.uc
 * Package Imports:
 *	WMission
 *	XInterface
 *	Engine
...
simulated function DrawDominationFlagObjective(Canvas C, wFlagObjective Go, Vector ScreenPos, Rotator camRot)
{
    ...
    {
        // End:0xbc
        foreach PawnOwner.TouchingActors(class'wFlagObjective', TGO)
        {
            // End:0xbb
            if(TGO == Go)
            {
...
function DrawDebug(Canvas C, int X, out int Y, int dy)
{
    ...
    foreach DynamicActors(class'wFlagObjective', f)
    {
        ...
        Y += dy;
        C.BtrDrawTextJustified("    TeamA=" $ string(f.LinkedTeam_A.Length) @ " TeamB=" $ string(f.LinkedTeam_B.Length), 0, float(X), float(Y), 640.0, float(Y + dy), 9);
        Y += dy;
        C.BtrDrawTextJustified("    iActiveTeam=" $ string(f.iActiveTeam) @ " fActiveStartTime=" $ string(f.fActiveStartTime), 0, float(X), float(Y), 640.0, float(Y + dy), 9);
        Y += dy;                
    }
}
