...
function bool SameTeam(Controller A, Controller B)
{
    ...
    // End:0x7C
    if(A.IsA('wAISentryGunController'))
    {
        // End:0x77
        if(none != B.PlayerReplicationInfo)
        {
            // End:0x77
            if(A.Pawn.OwnerName == B.PlayerReplicationInfo.PlayerName)
            {
                return true;
            }
        }
    ...
    {
        // End:0xDB
        if(B.IsA('wAISentryGunController'))
        {
            // End:0xD9
            if(none != A.PlayerReplicationInfo)
            {
                // End:0xD9
                if(B.Pawn.OwnerName == A.PlayerReplicationInfo.PlayerName)
                {
                    return true;
                }
            }
...
function CalcResultPoints()
{
    ...
    if(C != none)
    {
        ...
        PRI.fExp += ((float(GameReplicationInfo.ElapsedTime) / 12.0000000) * (GetResultPointRatio_Intervention(PRI)));
...
{
    Other.PlayerReplicationInfo.TeamID = iTeamID;
    return true;
    //return;    
}
