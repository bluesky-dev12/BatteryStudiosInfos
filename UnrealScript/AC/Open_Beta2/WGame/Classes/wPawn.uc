...
replication
{
    ...
    reliable if(int(Role) == int(ROLE_Authority))
        ClientSkillDeadPlayer, ClientSwitchToLastWeapon, 
        ClientWeaponReset;
...
event PlayLandedSound(int SurfType)
{
    ...
    if(((Health > 0) && !bHidden) && (Level.TimeSeconds - SplashTime) > 0.2500000)
    {
        ...
        FmodPlaySoundLocal(SoundGroupClass.static.GetLandedSound2D(SurfType), 0);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if(int(TypeOfWeapon) == int(19))
                            {
                                IdleWeaponAnim = SentryGunPack_IdleAnim;                                
                            }
                            else
                            {
...
simulated function Setup(PlayerRecord Rec, optional bool bLoadNow)
{
    ...
    {
        // End:0x383
        if(none != PlayerReplicationInfo.Team)
        {
            // End:0x380
            if(PlayerReplicationInfo.Team.TeamIndex == Level.GetMatchMaker().My_iTeam)
            {
                AddAllyEffect();
            }            
        }
        ...
        {
            // End:0x3B4
            if(PlayerReplicationInfo.TeamID == Level.GetMatchMaker().My_iTeam)
            {
                AddAllyEffect();
            }
        }        
...
simulated function SetCamouflage(int iItemID, byte byTeam)
{
    ...
    if(int(byTeam) == 0)
    {
        ...
        {
            mt = Material(GM.DynamicLoadObject(cResParam.strRes_3rd, Class'Engine.Material'));
        }        
    }
    else
    {
        ...
        {
            mt = Material(GM.DynamicLoadObject(cResParam.strRes_3rd_RSA, Class'Engine.Material'));
        }
    }
    Skins[0] = mt;
    //return;    
...
function AddSkillByInventory(wMatchUserInfo kUser)
{
    ...
    if(i < kUser.EquippedItems.Length)
    {
        ...
        // End:0x1F1
        if(kUser.EquippedItems[i].SlotPosition == int(26))
        {
            // End:0x1F1
...
function SetSelectedQuickSlotSkills(wMatchUserInfo kUser)
{
    ...
    else
    {
        ...
        if(i < 4)
        {
            AddedSkillList[i] = GetInvenSkillIDbyPos(kUser, int(14) + i);
            i++;
            // [Loop Continue]
            goto J0x127;
        }
...
function int GetInvenSkillIDbyPos(wMatchUserInfo kUser, int iPos)
{
    ...
    if(i < kUser.EquippedSkills.Length)
    {
        ...
        {
            // End:0xA8
            if((int(kMM.1) == int(kMM.eWeaponLimit)) && kUser.EquippedSkills[i].SlotPosition == int(14))
            {
                iResult = 0;                
            }
            else
    ...
    if(kMM.UserInfos.Length > 0)
    {
        ...
        if(kUser.EquippedItems.Length > 0)
        {
            ...
            else
            {
                ...
                {
                    // End:0x187
                    if(((kUser.EquippedItems[i].SlotPosition == int(3)) && Level.GRI.GameClass == "WGame.wTeamGame") && kMM.IsSDRespawn())
                    {
                        // [Explicit Continue]
                        goto J0x70D;
                    }
                    ...
                    {
                        // End:0x234
                        if(kUser.EquippedItems[i].SlotPosition == int(0))
                        {
                    ...
                    {
                        // End:0x3C4
                        if((kUser.EquippedItems[i].SlotPosition == int(2)) || kUser.EquippedItems[i].SlotPosition == int(3))
                        {                            
                        }
                        else
                        {
                    ...
                    {
                        SetPistolInvenByWeaponLimited(i, kMM, kUser, false);
                        // [Explicit Continue]
                        goto J0x70D;
                    }
                    // End:0x55F
                    ...
                    {
                        SetShotGunlInvenByWeaponLimited(i, kMM, kUser, false);
                        // [Explicit Continue]
                        goto J0x70D;
                    }
                    // End:0x59A
                    ...
                    {
                        SetMGlInvenByWeaponLimited(i, kMM, kUser, false);
                        // [Explicit Continue]
                        goto J0x70D;
                    }
                    // End:0x5D5
                    ...
                    {
                        SetFlameThrowerlInvenByWeaponLimited(i, kMM, kUser, false);
                        // [Explicit Continue]
                        goto J0x70D;
                    }
                    // End:0x610
                    ...
                    {
                        SetGrenadlInvenByWeaponLimited(i, kMM, kUser, false);
                        // [Explicit Continue]
                        goto J0x70D;
                    }
                    AddWeapon(kUser.EquippedItems[i].UniqueID, kUser.EquippedItems[i].ItemID, kUser.EquippedItems[i].AddPartID, false, kUser.EquippedItems[i].Painting_Item_ID);
...
function SetPistolInvenByWeaponLimited(int i, wMatchMaker kMM, wMatchUserInfo kUser, bool IsQuick)
{
    ...
    {
        // End:0x79
        if(((kUser.EquippedItems[i].SlotPosition == int(0)) || kUser.EquippedItems[i].SlotPosition == int(2)) || kUser.EquippedItems[i].SlotPosition == int(3))
        {            
        }
        else
        {
...
function SetSelectedQuickSlotInventory(wMatchUserInfo kUser, bool bQuickslotChange)
{
    ...
    if(i < 4)
    {
        ...
        else
        {
            ...
            {
                // End:0x1C5
                if(kUser.EquippedItems[j].SlotPosition == (int(0) + i))
                {
...
simulated function AddAllyEffect()
{
    // End:0x0D
    if(none != allyEffect)
    {
    ...
    if(IsA('wSentryGunPawn'))
    {
        ...
        AttachToBone(allyEffect, 'Bone_Body');        
    }
    else
    {
