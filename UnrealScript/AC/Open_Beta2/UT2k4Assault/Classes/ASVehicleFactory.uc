...
simulated function PostBeginPlay()
{
    ...
    if(int(Role) == int(ROLE_Authority))
    {
        ...
        {
            // End:0x6C
            foreach AllActors(Class'UT2k4Assault_Decompressed.ASVehicleFactory', NextFactory, NextFactoryTag)
            {
                // End:0x6C
                break;                
            }            
...
event Trigger(Actor Other, Pawn EventInstigator)
{
    ...
    {
        // End:0x15B
        if((bEnabled && int(TriggeredFunction) == int(2)) && NextFactory != none)
        {
            // End:0x13B
            if(Child != none)
            {
...
function SpawnVehicle()
{
    ...
    if(BlockCount < MaxSpawnBlockCount)
    {
        ...
        foreach CollidingActors(Class'Engine.Pawn', P, VehicleClass.default.CollisionRadius * 1.5000000)
        {
            ...
            {
                PlayerController(P.Controller).ReceiveLocalizedMessage(Class'UT2k4Assault_Decompressed.Message_ASKillMessages', 2 + BlockCount);
            }            
        }        
        // End:0xD8
        if(bBlocked)
...
defaultproperties
{
    ...
    VehicleClass=Class'UT2k4Assault_Decompressed.ASTurret'
    bMovable=false
}