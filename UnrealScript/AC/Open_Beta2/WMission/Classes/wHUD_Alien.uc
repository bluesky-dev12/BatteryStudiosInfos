...
var SpriteWidget WHeroIconDisplay;
var Vector vHeroLocation;
var bool bHeroLocationMark;
...
function UpdateNameTagInfo(Canvas C, wPawn curPawn, Vector camLoc, Rotator camRot)
{
    ...
    if(Class'Engine.wGameManager'.static.IsHeroType(PRI.eObjType))
    {
        ...
        {
            vHeroLocation = curPawn.Location;
            bHeroLocationMark = !curPawn.NametagVisInFront;
        }
    }
    //return;    
}
...
{
    local float X1, Y1, fWidth, fHeight;
    local wAlienPlayerReplicationInfo PRI;
    ...
    if(Class'Engine.wGameManager'.static.IsHeroType(PRI.eObjType))
    {
        WHeroIconDisplay.PosX = curPawn.NametagScreenPos.X / C.ClipX;
        WHeroIconDisplay.PosY = (curPawn.NametagScreenPos.Y - float(7)) / C.ClipY;
        DrawSpriteWidget(C, WHeroIconDisplay);
    }
    //return;    
}
...
simulated function DrawObjectives(Canvas C)
{
    ...
    // End:0x2A
    if(bHeroLocationMark)
    {
        DrawObjectSprite3(C, WHeroIconDisplay, vHeroLocation, true);
    }
    //return;    
}
...
function ChangeHudType(byte Type)
{
    switch(Type)
    {
        ...
        case 1:
            ...
            SndHeartBeatDanagerSound = Class'WMission_Decompressed.wHUD_Alien'.default.SndHeartBeatDanagerSound;
            SndFallDanagerSound = Class'WMission_Decompressed.wHUD_Alien'.default.SndFallDanagerSound;
            SndHeartBeatEmergencySound = Class'WMission_Decompressed.wHUD_Alien'.default.SndHeartBeatEmergencySound;
            // End:0x13F
            break;
        // End:0xF8
        case 0:
    ...
    if(PlayerOwner != none)
    {
        ...
        if(lp1 < PlayerOwner.SkillBase.Skills.Length)
        {
            ...
            if(SkillObject.IsA('wSkill_NoTransferAlien'))
            {
                ...
                wAlienPlayerReplicationInfo(PawnOwner.PlayerReplicationInfo).bNoTransferAlien = SkillObject.bUpdateProcess;
            }
            // End:0x116
            if((lp2 + 1) > 2)
            {
...
function UpdateModeEquippedItem()
{
    ...
    AlienObjectTypes[AlienObjectTypes.Length] = 1;
    lp1 = 0;
    J0x54:
    ...
    if(lp1 < GameMgr.InstanceItemLists.Length)
    {
        ...
        // End:0x147
        if((instanceInfo != none) && instanceInfo.SlotPosition == int(21))
        {
...
function CallEvent(optional bool InvenCheck, optional int iReservation1, optional int iReservation2)
{
    ...
    if(iReservation1 == 14001)
    {
        ...
        if(iReservation2 <= (AlienObjectTypes.Length - 1))
        {
            ...
            {
                // End:0xAB
                if(int(PRI.bySkinID) != int(AlienObjectTypes[int(SelectedAlienObjectType)]))
                {
                    PRI.bySkinID = AlienObjectTypes[int(SelectedAlienObjectType)];
                    wAlienController(PlayerOwner).ServerChangePawn(AlienObjectTypes[int(SelectedAlienObjectType)]);
                }
                bSelectAlienType = false;
                __DrawDisplaySelectMenu__Delegate = None;
            }
    ...
    if(Message == Class'Engine.wMessage_Game_Alien')
    {
        switch(Switch)
        {
            ...
            case Class'Engine.wMessage_Game_Alien'.default.Code_AMDefenceToAlien:
                ...
                wAlienPlayerReplicationInfo(PlayerOwner.PlayerReplicationInfo).bNoTransferAlien = false;
...
defaultproperties
{
    ...
    WAlienSelectBut_Cli=(WidgetTexture=Texture'Warfare_GP_UI_UI.Store.list_remodeling_remodelingitem',RenderStyle=5,TextureCoords=(X1=169,Y1=0,X2=338,Y2=86),TextureScale=0.3700000,DrawPivot=5,PosX=0.0000000,PosY=0.0000000,OffsetX=0,OffsetY=0,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
    WAlienSelectBut_Nor=(WidgetTexture=Texture'Warfare_GP_UI_UI.Store.list_remodeling_remodelingitem',RenderStyle=5,TextureCoords=(X1=0,Y1=0,X2=169,Y2=86),TextureScale=0.3700000,DrawPivot=5,PosX=0.0000000,PosY=0.0000000,OffsetX=0,OffsetY=0,ScaleMode=4,Scale=1.0000000,Tints=(R=255,G=255,B=255,A=255),Tints[1]=(R=255,G=255,B=255,A=255))
    ...
    SndHeartBeatDanagerSound="Warfare_Sound_Alien_Mode/Character/char_alien_heartbeat_01"
    SndHeartBeatEmergencySound="Warfare_Sound_Alien_Mode/Character/char_alien_breath_emergency_01"
    SndFallDanagerSound="Warfare_Sound_Alien_Mode/Character/char_alien_fall_01"
