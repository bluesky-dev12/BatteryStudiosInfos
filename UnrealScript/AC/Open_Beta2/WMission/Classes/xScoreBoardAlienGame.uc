...
function DrawContentDescriptionTeam(Canvas C, float _OffSetX, float _OffSetY)
{
    ...
    C.BtrDrawTextJustifiedWithVolumeLine(Score, 1, C.ClipX * fScoreX1, C.ClipY * fContentBeginY, C.ClipX * fScoreX2, C.ClipY * fContentEndY, int(ContentFontSize * C.ClipY), DefaultFontShadowColor);
...
function DrawTeamListSection_C(Canvas C, int lpTeam, PlayerReplicationInfo currPRI, int memberCount, string HostName, float beginY, float markBeginY, float markPingY)
{
    ...
    WPingBack.OffsetY = 307 + (memberCount * 49);
    ...
    WPing.OffsetX = 1100;
    WPing.OffsetY = 311 + (memberCount * 49);
