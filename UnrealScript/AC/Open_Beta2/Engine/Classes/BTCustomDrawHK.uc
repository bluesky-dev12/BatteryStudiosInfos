...
{
    C.DrawColor = Text.DrawColor;
    Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawStringRatio(C, Text.Text, Text.FontDrawType, float(Text.FontSize), tL, tT, tR, tB, Text.DrawShadowColor, PresetIndex, bUseD3DFont);
    //return;    
}
...
static function DrawImagePaddingRatio(Canvas C, out Image img, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB)
{
    ...
    tW = tR - tL;
    tH = tB - tT;
    Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawImageRatio(C, img, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH));
    //return;    
}
...
{
    Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawImageRatio(C, img, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB));
    //return;    
}
...
static function DrawImagePaddingCacheRatio(Canvas C, out Image img, float tL, float tT, float tR, float tB, float pL, float pT, float pR, float pB, optional int Level)
{
    ...
    tW = tR - tL;
    tH = tB - tT;
    Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawImageCacheRatio(C, img, tL + (pL * tW), tT + (pT * tH), tR - (pR * tW), tB - (pB * tH), Level);
    //return;    
}
...
{
    Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawStringRatio(C, Text, dt, FontSize, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB), BackShadowColor, PresetIndex, bUseD3DFont);
    //return;    
}
...
{
    Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawTextRatio(C, Text, tL + float(pL), tT + float(pT), tR - float(pR), tB - float(pB));
    //return;    
}
