...
function PreBeginPlay()
{
    ...
    // End:0x77
    if(((((int(TriggerType) == int(0)) || int(TriggerType) == int(1)) || int(TriggerType) == int(5)) || int(TriggerType) == int(6)) || (int(TriggerType) == int(2)) && ClassIsChildOf(ClassProximityType, Class'Engine_Decompressed.Pawn'))
    {
...
function bool IsRelevant(Actor Other)
{
    ...
    switch(TriggerType)
    {
        ...
        // End:0x138
        case 2:
            return ClassIsChildOf(Other.Class, ClassProximityType) && !ClassIsChildOf(ClassProximityType, Class'Engine_Decompressed.Vehicle') || Pawn(Inst).IsPlayerPawn();
        // End:0x13F
        case 3:
            return true;
        // End:0x171
...
state() OtherTriggerDamageGR
{
    function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
    {
        ...
        if(((bInitiallyActive && int(TriggerType) == int(4)) && float(Damage) >= DamageThreshold) && instigatedBy != none)
        {
            ...
            // End:0x8B
            if((DamageType == Class'Engine_Decompressed.wDamageGR') || strDamageType ~= "WWeapons_Res.wWeaponRes_FragGrenade")
            {                
            }
            else
            {
...
state() OtherTriggerDamageRocket
{
    function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
    {
        ...
        if(((bInitiallyActive && int(TriggerType) == int(4)) && float(Damage) >= DamageThreshold) && instigatedBy != none)
        {
            strDamageType = string(DamageType);
            // End:0x83
            if((DamageType == Class'Engine_Decompressed.wDamageRPG') || strDamageType ~= "WWeapons_Res.wWeaponRes_ATR")
            {                
            }
            else
            {
...
defaultproperties
{
    ...
    Texture=Texture'Engine_Decompressed.S_Trigger'
}