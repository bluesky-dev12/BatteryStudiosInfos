...
function Landed(Vector HitNormal)
{
    ...
    {
        TakeDamage(100, Pawn(Owner), HitNormal, HitNormal * float(10000), Class'Engine_Decompressed.Crushed');
    }
...
function Trigger(Actor Other, Pawn EventInstigator)
{
    ...
    TakeDamage(1000, Instigator, Location, vect(0.0000000, 0.0000000, 1.0000000) * float(900), Class'Engine_Decompressed.Crushed');
    //return;    
}
...
singular function BaseChange()
{
    ...
    else
    {
        ...
        if(Pawn(Base) != none)
        {
            Base.TakeDamage(int((float(int(float(1) - (Velocity.Z / float(400)))) * Mass) / Base.Mass), Instigator, Location, 0.5000000 * Velocity, Class'Engine_Decompressed.Crushed');
        ...
        else
        {
            ...
            {
                Base.TakeDamage(int(float(1) - (((Mass / Base.Mass) * Velocity.Z) / float(30))), Instigator, Location, 0.2000000 * Velocity, Class'Engine_Decompressed.Crushed');
                Velocity.Z = 100.0000000;
                // End:0x1EF
                if(FRand() < 0.5000000)
                {
