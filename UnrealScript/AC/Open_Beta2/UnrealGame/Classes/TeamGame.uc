...
function Class<WaitView> GetWaitViewClass(byte Team)
{
    ...
    if(int(Team) == 0)
    {
        return Class'Engine.WaitViewAF';        
    }
    else
    {
        return Class'Engine.WaitViewRSA';
    }
    //return;    
}
...
function int GetTeamID_by_Balancing(int iTeamID, Controller C)
{
    ...
    {
        // End:0x2E3
        if((bPlayersBalanceTeams && int(Level.NetMode) != int(NM_Standalone)) && PlayerController(C) != none)
        {
            // End:0x12E
            if(SmallTeam.Size < BigTeam.Size)
            {
...
function bool ChangeTeam(Controller Other, int iTeamID, bool bNewTeam)
{
    ...
    {
        // End:0x4A
        if(!CurrentGameProfile.CanChangeTeam(Other, iTeamID))
        {
            return false;
        }
    }
    ...
    NewTeam = Teams[GetTeamID_by_Balancing(iTeamID, Other)];
    // End:0xD5
    ...
    if(NewTeam.AddToTeam(Other))
    {
        ...
        {
            GameEvent("TeamChange", "" $ string(iTeamID), Other.PlayerReplicationInfo);
        }
    }
    return true;
    //return;    
...
function AnnounceWinningTeam()
{
    ...
    else
    {
        ...
        // End:0xC6
        if(int(byWinningTeam) != iCurWinningTeam)
        {
            TeamReceiveLocalizedMessage(byWinningTeam, Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_TakingLead);
            TeamReceiveLocalizedMessage(byte(int(1) - int(byWinningTeam)), Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_FallingBehind);
            iCurWinningTeam = int(byWinningTeam);
        }
    }
    //return;    
}
...
function AnnounceEndingWinningTeam()
{
    ...
    else
    {
        ...
        TeamReceiveLocalizedMessage(byWinningTeam, Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_EndingWinning);
        TeamReceiveLocalizedMessage(byte(int(1) - int(byWinningTeam)), Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_EndingLosing);
    }
    //return;    
}
...
function AnnounceAlmostWinningTeam()
{
    ...
    if(Teams[0].Score == Teams[1].Score)
    {
        AllReceiveLocalizedMessage(Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_EndingLosing);
        AllReceiveLocalizedMessage(Class'Engine.wMessage_BGM', Class'Engine.wMessage_BGM'.default.Code_AlmostOverFallBehind);        
    }
    else
    {
        // End:0x8A
        ...
        TeamReceiveLocalizedMessage(byWinningTeam, Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_EndingWinning);
        TeamReceiveLocalizedMessage(byte(int(1) - int(byWinningTeam)), Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_EndingLosing);
        TeamReceiveLocalizedMessage(byWinningTeam, Class'Engine.wMessage_BGM', Class'Engine.wMessage_BGM'.default.Code_AlmostOverLead);
        TeamReceiveLocalizedMessage(byte(int(1) - int(byWinningTeam)), Class'Engine.wMessage_BGM', Class'Engine.wMessage_BGM'.default.Code_AlmostOverFallBehind);
    }
    //return;    
}
...
function ScoreKill(Controller Killer, Controller Other)
{
    ...
    if(true)
    {
        ...
        else
        {
            ...
            if(Killer == Other)
            {
                Other.PlayerReplicationInfo.Team.Score = FMax(0.0000000, Other.PlayerReplicationInfo.Team.Score - GameDecScore);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(FriendlyFireScale > float(0))
                    {
                        Killer.PlayerReplicationInfo.NetUpdateTime = Level.TimeSeconds - float(1);
                        Killer.PlayerReplicationInfo.Score -= GameDecScore;
                        Killer.PlayerReplicationInfo.Team.Score = FMax(0.0000000, Killer.PlayerReplicationInfo.Team.Score - GameDecScore);
                        Killer.PlayerReplicationInfo.Team.NetUpdateTime = Level.TimeSeconds - float(1);
                        TeamScoreEvent(Killer.PlayerReplicationInfo.Team.TeamIndex, GameDecScore, "team_frag");
                    }
                }
...
function bool SameTeam(Controller A, Controller B)
{
    ...
    // End:0x63
    if(A.IsA('wHelicopterController'))
    {
        return A.GetTeamNum() == B.PlayerReplicationInfo.Team.TeamIndex;        
    }
    else
    {
        // End:0xA9
        if(B.IsA('wHelicopterController'))
        {
            return B.GetTeamNum() == A.PlayerReplicationInfo.Team.TeamIndex;
        }
    }
...
function PlayEndOfMatchMessage()
{
    ...
    TeamReceiveLocalizedMessage(byWinnerTeam, Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_Win);
    TeamReceiveLocalizedMessage(byte(int(1) - int(byWinnerTeam)), Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_Lose);
    //return;    
}
...
defaultproperties
{
    ...
    TeamAIType[0]=Class'UnrealGame_Decompressed.TeamAI'
    TeamAIType[1]=Class'UnrealGame_Decompressed.TeamAI'
    ...
    TGPropDescText[3]="??? ????? ?? ??? ???? ??? ? ???? ??? ????."
    TGPropDescText[4]="? ? ????? ????? ?????."
    TGHints[0]="???? ???? ?? ??? ??, %INGAMECHAT%? ??? ????? ?? ?? ??? ? ? ????."
    TGHints[1]="??? ????? ???? ???? ?? ??? ??? ? ????. ????? ??, ???? ??? ?????."
    TGHints[2]="%THROWWEAPON%? ???? ???? ??? ??? ?? ????."
    TGHints[3]="???? ???? ?? ??? ??? ??? ??????? ????? ???? ????."
    TGHints[4]="%VOICETALK%? ?? ????? ????? ? ? ????."
    TGHints[5]="%TEAMTALK%? ?? ??? ???? ?? ? ???? ?? ? ????"
    TGHints[6]="?? ??? ?? ??? ??? ?? ???? ???? ??? ?? ??? ?? ???? ??? ? ????."
    ...
    BeaconName="Team"
    GoalScore=60
    VoiceReplicationInfoClass=Class'UnrealGame_Decompressed.TeamVoiceReplicationInfo'
    Description="? ????: ???? ??? ???? ?? ? ?? ???? ????. ?? ?? ??? ??? ?? ????."
}