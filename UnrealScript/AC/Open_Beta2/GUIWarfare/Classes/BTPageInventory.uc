...
function bool ImageListBox_OnlClick(GUIComponent Sender)
{
    ...
    if(nCurQuickSlot == 0)
    {
        ...
        if(GameMgr.CheckSkillItem(ItemInfo.ItemType))
        {
            TPInventory.TPWeapon.EquipSkill();            
        }
        else
        {
            // End:0x2A3
...
function bool TPQSClick(GUIComponent Sender)
{
    ...
    if(i < 2)
    {
        iItemType = GameMgr.GetItemType(TPInventory.TPQuickSlot.SaveQS[btn.ButtonID].ItemInfo[i], TPInventory.TPQuickSlot.SaveQS[btn.ButtonID].instanceInfo[i]);
        TPInventory.TPQuickSlot.btnModify[i].bVisible = true;
        TPInventory.TPQuickSlot.btnModify[i].bDisable = false;
        // End:0x1ED
        if((TPInventory.TPQuickSlot.SaveQS[btn.ButtonID].ItemInfo[i] == none) || bool(iItemType & GameMgr.FlagModify) == false)
        {
            TPInventory.TPQuickSlot.btnModify[i].bDisable = true;
        }
        i++;
        // [Loop Continue]
        goto J0x99;
    ...
    if(i < 8)
    {
        TPInventory.TPQuickSlot.ButtonQuickSlot[i].SetData(TPInventory.TPQuickSlot.SaveQS[btn.ButtonID].ItemInfo[i], TPInventory.TPQuickSlot.SaveQS[btn.ButtonID].instanceInfo[i]);
        i++;
        // [Loop Continue]
        goto J0x1FE;
    }
    ...
    self.TPInventory.TPQuickSlot.ShowSkillTransparent(TPInventory.TPQuickSlot.ButtonQuickSlot[4].itemBox, TPInventory.TPQuickSlot.ButtonQuickSlot[5].itemBox, TPInventory.TPQuickSlot.ButtonQuickSlot[6].itemBox, TPInventory.TPQuickSlot.ButtonQuickSlot[7].itemBox);
    return false;
    //return;    
}
...
function InternalOnOpen()
{
    ...
    nCurQuickSlot = 0;
    self.TPInventory.TPEquipItem.CharacterModel.ChangeWeapon(self.TPInventory.TPQuickSlot.SaveQS[0].instanceInfo[0].ItemID, self.TPInventory.TPQuickSlot.SaveQS[0].instanceInfo[0].PartID, self.TPInventory.TPQuickSlot.SaveQS[0].instanceInfo[0].PaintID);
    i = 0;
    J0x23B:
    ...
    if(i < 8)
    {
        TPInventory.TPQuickSlot.ButtonQuickSlot[i].SetData(TPInventory.TPQuickSlot.SaveQS[nCurQuickSlot].ItemInfo[i], TPInventory.TPQuickSlot.SaveQS[nCurQuickSlot].instanceInfo[i]);
        i++;
        // [Loop Continue]
        goto J0x23B;
    }
