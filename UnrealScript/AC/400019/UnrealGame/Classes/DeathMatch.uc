...
function EnqueueAddBot(Class<Controller> BotClass, Controller callerController, int Team, int Grade, int NPCID, int iRegenLocationIndex, bool bUseDynamicRegenLocation, Vector v3DynamicRegenLocation)
{
    local AddBotInfo i;
...
function HandleGetBotID(int Team, array<int> BotID)
{
    ...
    if((i >= 0) && BotID.Length > 0)
    {
        ...
        if(Team == botInfo.Team)
        {
            ...
            {
                botController = Spawn(botInfo.BotClass,,, botInfo.v3DynamicRegenLocation, botInfo.callerController.Pawn.Rotation);
...
function AddwAIBot(string strNum, string strTeam, string aiClass, optional string strGrade, optional string strNPCID, optional string strRegenLocationIndex, optional Controller callerController, optional bool bUseDynamicRegenLocation, optional Vector v3DynamicRegenLocation, optional bool bUseFakeID)
{
    ...
    if(lp1 < Num)
    {
        EnqueueAddBot(BotClass, callerController, Team, Grade, NPCID, iRegenLocationIndex, bUseDynamicRegenLocation, v3DynamicRegenLocation);
        // End:0x1C8
        if(bCreateFakeID)
        {
            BotID[0] = NPCID;
...
function AnnounceSentryGun(Controller Caller)
{
    ...
    if(C != none)
    {
        ...
        if(C == Caller)
        {
            PlayerController(C).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_MySentryGun);            
        }
        else
        {
            // End:0xBB
            if(C.IsA('PlayerController') && C.GetTeamNum() == Caller.GetTeamNum())
            {
                PlayerController(C).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_AllySentryGun);                
            }
            else
            {
                PlayerController(C).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_EnemySentryGun);
            }
        }
        C = C.nextController;
        // [Loop Continue]
