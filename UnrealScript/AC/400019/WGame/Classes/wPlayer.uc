...
const fServerReqLockTime = 0.2f;
const fServerReqLockTimeRespawn = 1.1f;
...
var bool bReqAcitveRespawn;
var bool bActiveNowRespawn;
var float fReqLastTime;
...
replication
{
    ...
    reliable if(int(Role) < int(ROLE_Authority))
        ...
        ServerPlayWeaponNullSound, ServerReqNowRespawn, 
        ServerRequestMapList, ServerRequestPlayerInfo, 
        ServerRequestRules, ServerSetClassicTrans, 
...
function ProcessUseKey()
{
    ...
    if(int(bUseKey) == 1)
    {
        ...
        {
            // End:0x106
            if(((Pawn != none) && Pawn.Weapon != none) && int(Pawn.Weapon.ClientState) == int(Pawn.Weapon.4))
            {
                ProcessSetObjective();
            }
        }        
    ...
    else
    {
        ...
        if((int(bUseKey) == 0) && (int(byOldUseKey) == 1) || bHasDemoPack)
        {
            ...
            if(bHasDemoPack)
            {
                ...
                {
                    Pawn.FmodPlaySound(Pawn.Weapon.FmodNullSound, 3, 1.0000000,, false,,, true);
                }
                // End:0x1CB
...
function ProcessQuickGrenadeKey2()
{
    ...
    if(int(bOldQuickGrenade) != int(bQuickGrenade))
    {
        ...
        {
            // End:0xB9
            if(((Pawn.Weapon != none) && int(Pawn.Weapon.InventoryGroup) == 4) && (IsFragGrenade(Pawn.Weapon)) == true)
            {
                // End:0xB6
                if(Pawn.Weapon.IsFiring() == false)
                {
    ...
    if(int(bOldQuickSubGrenade) != int(bQuickSubGrenade))
    {
        ...
        {
            // End:0x21B
            if(((Pawn.Weapon != none) && int(Pawn.Weapon.InventoryGroup) == 4) && (IsSpecialGrenade(Pawn.Weapon)) == true)
            {
                // End:0x218
                if(Pawn.Weapon.IsFiring() == false)
                {
...
{
    Pawn.FmodPlaySound(Wpn.FmodNullSound, Slot, 1.0000000,, false,,, true);
    //return;    
}
...
simulated function SetPlayerEquipInfo()
{
    local int i, j;
    local wMatchMaker kMM;
    local wMatchUserInfo kUser;
    local int iBody[2], iHelmet, iAcce, iBackpack, iPouch, iCamouflage;
            ...
            if(kMM.UserInfos[i].UserName == PlayerReplicationInfo.PlayerName)
            {
                ...
                if(j < kUser.EquippedItems.Length)
                {
                    switch(kUser.EquippedItems[j].SlotPosition)
                    {
                        ...
                        // End:0x174
                        case 20:
                            iBackpack = kUser.EquippedItems[j].ItemID;
                            // End:0x27F
                            break;
                        // End:0x205
                        case 21:
                            // End:0x1E1
                            if(int(kUser.EquippedItems[j].byModeItem) != 0)
                            {
                                UpdateModeItem_SkinCharacter(iBody[0], kMM, kUser.EquippedItems[j].byModeItem, kUser.EquippedItems[j].ItemID);                                
                            }
                            else
                            {
                        ...
                        // End:0x255
                        case 24:
                            iPouch = kUser.EquippedItems[j].ItemID;
                            // End:0x27F
                            break;
                        // End:0x27C
                        case 25:
                            iCamouflage = kUser.EquippedItems[j].ItemID;
                            // End:0x27F
                            break;
                        // End:0xFFFF
                        default:
                ...
                PlayerReplicationInfo.NewSetCharacterEquipItem(iBody[0], iBody[1], iHelmet, iAcce, iBackpack, iPouch, iCamouflage);
                return;
            }
            i++;
            // [Loop Continue]
    ...
    PlayerReplicationInfo.NewSetCharacterEquipItem(15000, 15000, -1, -1, -1, -1, -1);
    //return;    
}

function UpdateModeItem_SkinCharacter(out int BodyItemID, wMatchMaker MM, byte byModeItem, int ItemID)
{
    // End:0x24
...
function ServerCallHelicopter(optional Pawn lastHeliPawn, optional Controller Caller)
{
    ...
    // End:0x35
    if(Level.GRI.GameClass == "WMission.wNDFGameInfo")
    {
        return;
    }
    // End:0x47
    ...
    Class'WGame_Decompressed.wHelicopter'.static.SpawnHeli(Level, lastHeli, Caller);
    //return;    
}
...
function ServerCallAirStrike(optional Controller Caller)
{
    ...
    Caller.PlayerReplicationInfo.abySupplyItemFlag[int(3)] = 2;
...
{
    callingController.PlayerReplicationInfo.abySupplyItemFlag[int(4)] = 0;
    Level.Game.AnnounceSentryGun(callingController);
    //return;    
}
...
exec function art()
{
    ...
    {
        // End:0x43
        if(Class'WGame_Decompressed.wBombingPack'.static.CanPickUpPack(P))
        {
...
{
    local wSentryGunPawn Pawn;

    // End:0x40
    foreach DynamicActors(Class'WGame_Decompressed.wSentryGunPawn', Pawn)
    {
        // End:0x3F
        if(Pawn.Controller.Caller.GetTeamNum() == (GetTeamNum()))
        {            
            return true;
        }        
    }    
...
function DoAirStrikeFire(Vector HitLocation)
{
    ...
    A = Spawn(Class<wProjectile>(DynamicLoadObject("WWeapons.wAirStrikeProjPack", Class'Core.Class')),,, HitLocation);
...
{
    // End:0x3B
    if(0 == int(PlayerReplicationInfo.abySupplyItemFlag[int(eSupplyItemType)]))
    {
        // End:0x39
        if(bShowMessage)
        {
    ...
    // End:0x4E
    if(int(1) == int(eSupplyItemType))
    {        
    }
    else
    {
        // End:0x88
        if(int(2) == int(eSupplyItemType))
        {
            // End:0x85
            if(IsActiveHelicopter())
            {
                // End:0x83
                if(bShowMessage)
                {
        ...
        else
        {
            ...
            {
                // End:0x129
                if(int(4) == int(eSupplyItemType))
                {
                    // End:0x129
                    if(IsActiveMyTeamSentryGun())
                    {
                        // End:0x127
                        if(bShowMessage)
                        {
                            myHUD.CallSkill(6, true, false);
                            PlayerController(callingController).ReceiveLocalizedMessage(Class'Engine.wMessage_Game_ImpSystem', Class'Engine.wMessage_Game_ImpSystem'.default.Code_AlreadyInstalledSentryGun);
                        }
                        return false;
                    }
                }
...
{
    // End:0x19
    if(false == (CanUseActivateSupplyItem(callingController, eSupplyItemType, true)))
    {
        return false;
    }
    // End:0x32
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(int(4) == int(eSupplyItemType))
                {
                    ServerCallSentryGunInstall(callingController);
                }
            }
        }
    }
...
exec function TTT()
{
    Pawn.AddWeapon(EmptyBtrDouble(), 5001, 0, false);
    //return;    
}

exec function tT(float fRate)
{
    Pawn.AddWeapon(EmptyBtrDouble(), 5003, 0, false);
    //return;    
}
...
{
    // End:0x1B
    if((GetSensitivityAim()) >= 0.1000000)
    {
        ChangeMouseSenseAim(-0.0500000);
    }
    //return;    
...
simulated function ChangeMouseSenseAim(float dt)
{
    ...
    ReceiveLocalizedMessage(Class'Engine.wMessage_Game', Class'Engine.wMessage_Game'.default.Code_MouseSenseChangeAim);
...
exec function MakeItemBox(string t)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(t == "bombing")
            {
                spawnedActor = Spawn(Class'WGame_Decompressed.wBombingPack',,, (Pawn.Location - (CollisionHeight * vect(0.0000000, 0.0000000, 1.0000000))) + vect(100.0000000, 100.0000000, 0.0000000));                
            }
            else
            {
                // End:0x15C
                ...
                else
                {
                    ...
                    if(t == "sentry")
                    {
                        spawnedActor = Spawn(Class'WGame_Decompressed.wSentryGunPack',,, (Pawn.Location - (CollisionHeight * vect(0.0000000, 0.0000000, 1.0000000))) + vect(100.0000000, 100.0000000, 0.0000000));
                    }
                }
            }
        }
...
function bool UsingItem_Slot8()
{
    ...
    bEnableRequir = PlayerReplicationInfo.IsDead && __NFUN_913__(__NFUN_921__(0), iUniqueID);
    bEnableRequir = bEnableRequir && self.GetItemCount(iItemID) > 0;
    Log((((string(self) $ " [ ReqUsingItem::UsingItem_Slot8 ] / bActiveNowRespawn :") $ string(bActiveNowRespawn)) $ " / bEnableRequir:") $ string(bEnableRequir));
    // End:0x10F
    if(!bActiveNowRespawn && bEnableRequir)
    {
        ServerReqNowRespawn(iUniqueID);
        fReqLastTime = Level.TimeSeconds;        
    }
    else
    {
    ...
    Log((((((string(self) $ " [ ReqUsingItem::UsingItem_Slot8 ] / iItemID:") $ string(iItemID)) $ " / iUniqueID: ") $ __NFUN_918__(iUniqueID)) $ " / bEnableRequir:") $ string(bEnableRequir));
    return true;
    //return;    
}

function ServerReqUsingItem(int iItemID, BtrDouble iUniqueID)
{
    //return;    
}

function ServerReqNowRespawn(BtrDouble iUniqueID)
{
    local wMatchMaker kMM;

    // End:0x41
    if((((IsInState('PlayerWalking') || IsInState('PlayerWaiting')) || IsInState('Spectating')) || IsInState('SpectatingGM')) || IsInState('ChangedHost'))
    {
        return;
    }
    // End:0x62
    if(!PlayerReplicationInfo.IsDead || bActiveNowRespawn)
    {
        return;
    }
    kMM = Level.GetMatchMaker();
    ...
    {
        Log(((((string(self) $ " Using Item During Game, PlayerName:") $ PlayerReplicationInfo.PlayerName) $ " /ItemID : 92006") $ " /iUniqueID :") $ __NFUN_918__(iUniqueID));
        kMM.kTcpChannel.sfReqUseItemDuringGame(PlayerReplicationInfo.PlayerName, 92006, iUniqueID, 1);
    }
    //return;    
}

function int GetItemCount(int iItemID)
{
    local int lp1, iItemCount;
    ...
    if(lp1 < sts.ItemList.Length)
    {
        ...
        {
            // End:0xD3
            if(iItemID == sts.ItemList[lp1].iItemID)
            {
                iItemCount = sts.ItemList[lp1].iOverlapCount;
                // [Explicit Break]
                goto J0xDD;
            }
        }
...
function SetItemCount(int iItemID, int RemainCount)
{
    local int lp1, iItemCount;
    local wCharacterManager chMgr;
    local wMyPlayerStatus sts;
    ...
    if(lp1 < sts.ItemList.Length)
    {
        ...
        if((sts.ItemList[lp1].iSlotPosition == int(26)) || sts.ItemList[lp1].iSlotPosition == int(29))
        {
            ...
            if(iItemID == sts.ItemList[lp1].iItemID)
            {
                sts.ItemList[lp1].iOverlapCount = RemainCount;
                // [Explicit Break]
                goto J0xDD;
            }
        }
...
function BtrDouble GetUniqueID(int iItemID)
{
    local int lp1;
    local wCharacterManager chMgr;
    local wMyPlayerStatus sts;
    ...
    {
        // End:0x8D
        if(sts.ItemList[lp1].iItemID == iItemID)
        {
            iUniqueID = sts.ItemList[lp1].iUniqueID;
            // [Explicit Break]
            goto J0x97;
        }
        lp1++;
