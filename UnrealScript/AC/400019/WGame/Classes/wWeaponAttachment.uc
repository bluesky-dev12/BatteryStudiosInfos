...
simulated function MakeMeleeHitEffect()
{
    local PlayerController PC;
    local Vector invlookvec, StartLocation;
    local float dotv, Alpha;

    stMBPF[0].mvEffLocation[0] = RepEffectLocation[0];
    StartLocation = Instigator.Location;
    GetHitInfo(0, 0);
    PC = Level.GetLocalPlayerController();
    // End:0x1F0
    if(((Instigator != none) && Instigator.Controller == PC) || VSize(PC.ViewTarget.Location - stMBPF[0].mvEffLocation[0]) < float(4000))
    {
        invlookvec = StartLocation - stMBPF[0].mvEffLocation[0];
        StartLocation = stMBPF[0].mvEffLocation[0];
        // End:0xF4
        ...
        dotv = invlookvec Dot stMBPF[0].mHitNormal[0];
        dotv = Abs(dotv);
        // End:0x13B
        if(dotv > 0.7000000)
        {            
        ...
        else
        {
            ...
            invlookvec = (invlookvec * Alpha) + (stMBPF[0].mHitNormal[0] * (float(1) - Alpha));
        }
        // End:0x1F0
        if(stMBPF[0].mHitMaterial[0] != none)
        {
            // End:0x1C8
            if(int(FiringMode) == 2)
            {
                Spawn(ResParams.default.effect_Melee_Attack1,,, stMBPF[0].mvEffLocation[0], Rotator(invlookvec));                
            }
            else
            {
                Spawn(ResParams.default.effect_Melee_Attack2,,, stMBPF[0].mvEffLocation[0], Rotator(invlookvec));
            }
        }
    }
    //return;    
