...
var int bestScore;
var bool DefenceObjectGod;
var int iNumBotsWave;
var int iProgressWave;
var bool bWave;
var float fWaveStartTime;
var float fWaveEndTime;
var float fWaveTime;
...
function Killed(Controller Killer, Controller Killed, Pawn KilledPawn, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
{
    ...
    KilledBots(Killer, Killed, KilledPawn, DamageType, CollisionPart, WeaponType);
    //return;    
}
...
function AnnounceMatchInfo()
{
    ...
    if(C != none)
    {
        ...
        {
            PlayerController(C).FmodClientPlaySound(Class'Engine.wGameManager'.default.VoicePackageName $ sndModeName);
        }
        C = C.nextController;
        // [Loop Continue]
        goto J0x14;
...
exec function DefencePass(optional int Round)
{
    //return;    
}

exec function nuclearGod()
{
    //return;    
}
...
function NuclearDamageable()
{
    ...
    DefenceObjectGod = !DefenceObjectGod;
    C = Level.ControllerList;
    J0x23:
    ...
    {
        PC = PlayerController(C);
        PC.Player.Console.AddMessageItem(PC.Player.Console.ConstructMessageItem(" DefenceObjectGod :" $ string(DefenceObjectGod), 6));
        C = C.nextController;
        // [Loop Continue]
        goto J0x23;
    }
