...
var() name Barricade;
var Decoration OwnerBarricade;
var int AttackerCnt;
var string DamageSound;
...
simulated event PostBeginPlay()
{
    ...
    // End:0x144
    foreach DynamicActors(Class'Engine.Decoration', OwnerBarricade, Barricade)
    {
        Health = float(OwnerBarricade.Health);
        Log((string(self) $ " DynamicActors DECO_ExplodingBarrel : ") $ string(Barricade));
        // End:0x144
        break;        
    }    
    //return;    
...
{
    FmodPlaySound(DamageSound, 0);
    // End:0x53
    if(OwnerBarricade != none)
    {
        OwnerBarricade.TakeDamage(Damage, instigatedBy, HitLocation, Momentum, DamageType, CollisionPart, WeaponType, bWallShot);        
    }
    else
    {
        Health -= float(Damage);
...
{
    DamageCapacity=100.0000000
    DamageSound="Warfare_Sound_Defense_Mode/Object/ombing"
