...
function StreamFinished(int Handle, Interactions.EStreamFinishReason Reason)
{
    ...
    Log((("StreamFinished() Handle:" $ string(Handle)) @ "Reason:") $ string(GetEnum(Enum'Engine.Interactions.EStreamFinishReason', int(Reason))), 'MusicPlayer');
    // End:0x10B
    if((Handle == CurrentSongHandle) && CurrentSongHandle != 0)
    {
        Log((("CurrentSongPosition:" $ Class'Engine.StreamBase'.static.FormatTimeDisplay(CurrentSongPosition)) @ "Total song time:") $ Class'Engine.StreamBase'.static.FormatTimeDisplay(GetStreamDuration()));
