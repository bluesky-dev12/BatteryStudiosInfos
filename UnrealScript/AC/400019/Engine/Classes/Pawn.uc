...
struct sSpecificBoneRotatorInfo
{
    var name nameBone;
    var Rotator boneRotator;
};
...
var array<sSpecificBoneRotatorInfo> m_aSpecificBoneRotateInfo;
...
replication
{
    ...
    reliable if(bNetDirty && int(Role) == int(ROLE_Authority))
        ...
        byUseKey, m_aSpecificBoneRotateInfo;
...
simulated function bool SwitchWeapon(byte f, optional bool bQuickGr)
{
    ...
    // End:0x1DD
    if(int(19) == int(newWeapon.WeaponType))
    {
        // End:0x14F
        if(bIsSprinting)
        {
        ...
        // End:0x1DD
        if(((int(Physics) != int(1)) && int(Physics) != int(11)) && int(Physics) != int(9))
        {
            return false;
        }
    }
...
protected function UpdateSpecificBoneRotation(float fElapsedTime)
{
    local int i;
    ...
    // End:0x47 [Loop If]
    if(i < m_aSpecificBoneRotateInfo.Length)
    {
        SetBoneRotation(m_aSpecificBoneRotateInfo[i].nameBone, m_aSpecificBoneRotateInfo[i].boneRotator);
        ++i;
        // [Loop Continue]
        goto J0x07;
    }
...
simulated function AddSpecificBoneRotator(name nameBone)
{
    local sSpecificBoneRotatorInfo boneRotatorInfo;

    boneRotatorInfo.nameBone = nameBone;
    m_aSpecificBoneRotateInfo[m_aSpecificBoneRotateInfo.Length] = boneRotatorInfo;
    //return;    
}
