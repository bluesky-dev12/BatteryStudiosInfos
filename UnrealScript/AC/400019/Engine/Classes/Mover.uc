...
var() float DelayTime;
var(MoverSounds) string FmodOpeningSound;
var(MoverSounds) string FmodOpenedSound;
var(MoverSounds) string FmodClosingSound;
var(MoverSounds) string FmodClosedSound;
var(MoverSounds) string FmodMoveAmbientSound;
var(MoverSounds) string FmodLoopSound;
...
function FinishedClosing()
{
    ...
    FmodPlaySound(FmodClosedSound, 0, float(SoundPitch) / 64.0000000, false);
...
{
    FmodPlaySound(FmodOpenedSound, 0, float(SoundPitch) / 64.0000000, false);
...
function DoOpen()
{
    ...
    MakeNoise(1.0000000);
    FmodPlaySound(FmodOpeningSound, 0, float(SoundPitch) / 64.0000000, false);
...
function DoClose()
{
    ...
    MakeNoise(1.0000000);
    FmodPlaySound(FmodClosingSound, 0, float(SoundPitch) / 64.0000000, false);
...
function bool EncroachingOn(Actor Other)
{
    ...
    {
        Other.TakeDamage(10000, none, Other.Location, vect(0.0000000, 0.0000000, 0.0000000), Class'Engine.Crushed');
        return false;
    }
    // End:0xD1
    if(Other.IsA('Pickup'))
    ...
    if(EncroachDamage != 0)
    {
        Other.TakeDamage(EncroachDamage, Instigator, Other.Location, vect(0.0000000, 0.0000000, 0.0000000), Class'Engine.Crushed');
    }
    P = Pawn(Other);
    // End:0x231
...
state() LoopMove
{
    ...
    event Trigger(Actor Other, Pawn EventInstigator)
    {
        ...
        bOpening = true;
        FmodPlaySound(FmodOpeningSound, 0);
...
state() TriggerToggleLauncher extends TriggerToggle
{
    function TakeDamage(int Damage, Pawn instigatedBy, Vector HitLocation, Vector Momentum, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional bool bWallShot)
    {
        ...
        {
            // End:0x3C
            if(DamageType == Class'Engine.wDamageRPG')
            {
                self.Trigger(self, instigatedBy);
            }
        }
...
state() TriggerAdvance
{
    ...
    function Trigger(Actor Other, Pawn EventInstigator)
    {
        ...
        SetPhysics(8);
        FmodAmbientSound = FmodMoveAmbientSound;
        // End:0x66
        if(!bOpening)
        {
