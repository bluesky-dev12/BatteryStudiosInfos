...
function Internal_OnRendered(Canvas C)
{
    ...
    if(Caption != "")
    {
        ...
        {
            Class'Engine.BTCustomDrawHK'.static.DrawCullingString(C, Caption, CaptionDrawType, float(FontSize[int(self.MenuState)]), 1.0000000, Bounds[0], Bounds[1], Bounds[2], Bounds[3], cullRight, FontShadowColor[int(self.MenuState)]);
        }
        // End:0x256
        if(bUseFontScaleValue)
        {
    ...
    if(((bDrawToolTips == true) && decoToolTips != none) && decoToolTips.OriginalString.Text != "")
    {
        ...
        // End:0x3AC
        if(decoToolTips.CheckIn(float(int(Controller.MouseX)), float(int(Controller.MouseY)), 1.0000000, float(Controller.ResY) / 768.0000000))
        {
