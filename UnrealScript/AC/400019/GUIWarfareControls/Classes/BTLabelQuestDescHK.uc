...
function InitComponent(GUIController MyController, GUIComponent myOwner)
{
    ...
    arrCurQuest[1] = Class'Engine.BTCustomDrawHK'.static.MakeText(11, 0, "");
    txtQuestExtDesc = Class'Engine.BTCustomDrawHK'.static.MakeText(9, 0, "");
    txtReward = Class'Engine.BTCustomDrawHK'.static.MakeText(9, 0, "");
    txtReward.DrawColor.R = byte(255);
    txtReward.DrawColor.G = 192;
    txtReward.DrawColor.B = 0;
    txtLastReward = Class'Engine.BTCustomDrawHK'.static.MakeText(9, 0, "");
    txtLastReward.DrawColor.R = byte(255);
    txtLastReward.DrawColor.G = 192;
    txtLastReward.DrawColor.B = 0;
    //return;    
}
...
function Internal_OnRendered(Canvas C)
{
    ...
    else
    {
        ...
        {
            Class'Engine.BTCustomDrawHK'.static.DrawText(C, txtQuestExtDesc, float(L), float(t), float(R), float(B), 0, true);
        }
        // End:0x2EE
        if(txtReward.Text != "")
        {
            t += 47;
            Class'Engine.BTCustomDrawHK'.static.DrawText(C, txtReward, float(L), float(t), float(R - 5), float(B));
        }
        // End:0x341
        if(txtLastReward.Text != "")
        {
            t += 16;
            Class'Engine.BTCustomDrawHK'.static.DrawText(C, txtLastReward, float(L), float(t), float(R - 5), float(B));
        }
    }
    //return;    
}
