...
event bool CreateBackGroundImage(wMatchMaker MatchMaker)
{
    ...
    else
    {
        ...
        ModeFB2.Length = 1;
        ModeFB2[0] = Class'Engine_Decompressed.BTCustomDrawHK'.static.MakeFloatBox(0.0000000, 128.0000000, 256.0000000, 192.0000000);
        ...
        Mat_img_SupplyIcon[0] = Material(DynamicLoadObject("Warfare_GP_UI_Map.LoadingImage.img_loading_supplyicon_helicopter", Class'Engine_Decompressed.Material'));
        Mat_img_SupplyIcon[1] = Material(DynamicLoadObject("Warfare_GP_UI_Map.LoadingImage.img_loading_supplyicon_bombing", Class'Engine_Decompressed.Material'));
        Mat_img_SupplyIcon[2] = Material(DynamicLoadObject("Warfare_GP_UI_Map.LoadingImage.img_loading_supplyicon_UAV", Class'Engine_Decompressed.Material'));
...
event DrawLoadingScreen(Canvas C)
{
    ...
    else
    {
        ...
        if((float(ModeIndex) % float(13)) < float(ModeFB.Length))
        {
            ...
            C.DrawColor = Class'Engine_Decompressed.Canvas'.static.MakeColor(byte(255), byte(255), byte(255), byte(255));
            Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawImageWithClipArea(C, img, fb.X1, fb.Y1, fb.X1 + (img.width * ratioX), fb.Y1 + (img.Height * ratioY), ModeFB2[int(float(ModeIndex) % float(13))].X1, ModeFB2[int(float(ModeIndex) % float(13))].Y1, ModeFB2[int(float(ModeIndex) % float(13))].X2, ModeFB2[int(float(ModeIndex) % float(13))].Y2);
            // End:0xE16
            if(MM.kGame_WeaponLimit != 0)
            {
                Class'Engine_Decompressed.BTCustomDrawHK'.static.DrawString(C, GameSubMode, 5, 14.0000000, (fb.X1 + ((img.width * float(2)) / float(3))) * ratioX, ((fb.Y1 + img.Height) + float(5)) * ratioY, ((fb.X1 - float(25)) + img.width) * ratioX, ((fb.Y1 + img.Height) + float(25)) * ratioY, kShadowColor, 0, false);
...
defaultproperties
{
    ...
    strModeHelp1[8]="Use the mounted located in certain locations on maps."
