...
function FillGameMode()
{
    ...
    // End:0x2C5
    if(Class'Engine.wGameSettings'.static.IsBotModeIndex(IntClass(ComboGameMode.ComboBox.List.GetObjectAtIndex(ComboGameMode.ComboBox.Index)).Index) || GameModeIndex == IntClass(ComboGameMode.ComboBox.List.GetObjectAtIndex(ComboGameMode.ComboBox.Index)).Index)
    {
...
function ComboMap_OnChange(GUIComponent Sender)
{
    ...
    // End:0x2D9
    if(modeInfo.ModeIndex == Class'Engine.wGameSettings'.static.GetModeIndex_DOA())
    {
        ComboWeaponRestriction.DisableMe();
    }
    //return;    
