...
function bool OwnerDraw(Canvas C)
{
    ...
    if(bHudMode == false)
    {
        ...
        Class'Engine.BTCustomDrawHK'.static.DrawString(C, self.textStrVis, 3, float(FontSize[int(self.MenuState)]), RWinPos.X1, RWinPos.Y1, RWinPos.X2, RWinPos.Y2, self.FontShadowColor[int(self.MenuState)], 0, true);
    }
    // End:0x714
    if(bActiveCursor && bReadOnly == false)
    {
        ...
        {
            Class'Engine.BTCustomDrawHK'.static.DrawImage(C, cursorImg, (RWinPos.X1 + cursorW) + float(1), ((RWinPos.Y2 + RWinPos.Y1) - HT) / float(2), (RWinPos.X1 + cursorW) + float(4), ((RWinPos.Y2 + RWinPos.Y1) + HT) / float(2));
        }
        // End:0x704
        if(bSelectionMode)
        {
            ...
            if(bHudMode == false)
            {
                Class'Engine.BTCustomDrawHK'.static.DrawImage(C, selectionImg, RWinPos.X1 + X, ((RWinPos.Y2 + RWinPos.Y1) - HT) / float(2), RWinPos.X1 + X2, ((RWinPos.Y2 + RWinPos.Y1) + HT) / float(2));
            }
        }
...
function DrawIMEInfo(Canvas C, float CursorWidth)
{
    ...
    if(IMEText != "")
    {
        ...
        if(IMESelPosEnd == 0)
        {
            ...
            Class'Engine.BTCustomDrawHK'.static.DrawImage(C, selectionImg, (RWinPos.X1 + CursorWidth) - width, RWinPos.Y1, RWinPos.X1 + CursorWidth, RWinPos.Y1 + Height);            
        }
        else
        {
            // End:0x1A2
            ...
            if(SelPosStart >= 0)
            {
                ...
                Class'Engine.BTCustomDrawHK'.static.DrawImage(C, selectionImg, (RWinPos.X1 + CursorWidth) - width, RWinPos.Y1, (RWinPos.X1 + CursorWidth) - SelectWidth, RWinPos.Y1 + Height);
            }
        }
