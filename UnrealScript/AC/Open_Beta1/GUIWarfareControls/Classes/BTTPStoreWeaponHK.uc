...
function bool BTWindowItemBuyHK_OnOK(GUIComponent Sender)
{
    ...
    {
        // End:0x1F2
        if(((int(BTWindow.cashType) == 0) && (MM.kCash - cii.WZCashPrice[BTWindow.OptionDay - 1]) < 0) || (int(BTWindow.cashType) == 1) && (MM.kCCoinCash - cii.WZCashPrice[BTWindow.OptionDay - 1]) < 0)
        {
    ...
    if(BTWindow.ItemInfo.CostType == 4)
    {
        ...
        if(gm.FindFirstInstanceItemByItemID(BTWindow.ItemID) != none)
        {
            ...
            BTWindowItemBuyTerm(Controller.TopPage()).SetCashType(int(BTWindow.cashType));
            return true;
        }
    }
    // End:0x474
    if(cii != none)
    {
        ...
        if(ProductNo != -1)
        {
            ...
            TcpChannel.sfReqBuyItemByWebzenCashGP20(ProductNo, BTWindow.cashType, __NFUN_921__(0));
        }        
    }
    else
    {
...
function bool BTWindowItemBuyTerm_OnOK(GUIComponent Sender)
{
    ...
    {
        GetCashItemInfo(temp, ib, cii);
        ProductNo = GameMgr.GetGP20ProductNo(BTWindow.ItemID, cii.WZCashPrice[int(BTWindow.OptionDay) - 1], byte(BTWindow.cashType));
        // End:0x1A9
        if(ProductNo != -1)
        {
            Log((("sfReqBuyItemByWebzenCashGP20(" $ string(ProductNo)) $ ",") $ string(BTWindow.cashType));
            TcpChannel.sfReqBuyItemByWebzenCashGP20(ProductNo, byte(BTWindow.cashType), __NFUN_921__(0));
        }        
    }
    else
    {
...
function fill(array<int> ShopItemList)
{
    ...
    if(i < GameMgr.kGP20ProductList.Length)
    {
        ...
        showItem = Class'Engine.wItemBoxCashHK'.static.IsShowStore(GP20Product);
        // End:0x369
        if(!showItem)
        {
            // [Explicit Continue]
        ...
        ro.instanceInfo.CashItemInfo.SetCashItemInfo(ro.ItemInfo, GP20Product, Class'GUIWarfareControls_Decompressed.BTWindowDefineInfoHK'.static.GetClientString(122));
        // End:0x481
