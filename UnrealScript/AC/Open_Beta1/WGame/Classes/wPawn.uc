...
simulated function FootStepping(int side)
{
    ...
    {
        PlaySound(SoundFootsteps[SurfaceNum], 0, FootstepVolume,, 80.0000000,, true,,, false);        
    }
    else
    {
        PlaySound(SoundFootsteps_3rd[SurfaceNum], 0, FootstepVolume,, 80.0000000,, true,,, false);
    }
    //return;    
}
...
function TakeDamageSound(int LeftHealth, int ActualDamage, Vector HitLocation, Class<DamageType> DamageType, optional int CollisionPart, optional int WeaponType, optional Controller Killer)
{
    ...
    if(LeftHealth <= 0)
    {
        ...
        if(Killer != none)
        {
            ...
            {
                PlayerController(Killer).ClientPlaySoundHK(sndDie, 0, 1.0000000, false, 1000.0000000, 1.0000000, true, false, -1, false, Location);                
            }
            else
            {
                PlayerController(Killer).ClientPlaySoundHK(sndDie, 0, 1.0000000, false, 1000.0000000, 1.0000000, true, false, -1, false, Location);
    ...
    else
    {
        ...
        if(DamageType != Class'Engine.fell')
        {
            ...
            if(Killer != none)
            {
                ...
                if(int(byte(CollisionPart)) == int(1))
                {
                    PlayerController(Killer).ClientPlaySoundHK(sndScream, 0, 1.0000000, false, 2000.0000000, 1.0000000, true, false, -1, false, Location);
                    PlayerController(Killer).ClientPlaySoundHK(SoundGroup'Warfare_Sound_Impact.char.impact_char_headshot', 0, 0.7000000, false, 2000.0000000, 1.0000000, true, false, -1, false, Location);                    
                }
                else
                {
                    PlayerController(Killer).ClientPlaySoundHK(sndScream, 0, 1.0000000, false, 2000.0000000, 1.0000000, true, false, -1, false, Location);
                    PlayerController(Killer).ClientPlaySoundHK(SoundGroup'Warfare_Sound_Impact.char.impact_char_nomal', 0, 0.7000000, false, 2000.0000000, 1.0000000, true, false, -1, false, Location);
                }
            }
            // End:0x249
            if(int(byte(CollisionPart)) == int(1))
