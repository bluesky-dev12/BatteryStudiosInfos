...
simulated event ThirdPersonEffects()
{
    ...
    if(int(Level.NetMode) != int(NM_DedicatedServer))
    {
        ...
        if(int(FiringMode) == 0)
        {
            ...
            Log(((("ThirdPersonEffects =" @ string(OldSpawnHitCount)) $ " ") $ "SpawnHitCount = ") @ string(SpawnHitCount));
            // End:0x3FC
            if(int(OldSpawnHitCount) != int(SpawnHitCount))
            {
                ...
                if(nIndex < nBulletCntPerFire)
                {
                    ...
                    if(i <= Max)
                    {
                        GetHitInfo(nIndex, i);
                        // End:0x3CD
                        if(((Instigator != none) && Instigator.Controller == PC) || VSize(PC.ViewTarget.Location - stMBPF[nIndex].mvEffLocation[i]) < float(4000))
                        {
